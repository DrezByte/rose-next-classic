 ...
 */
long
__fseek(FILE* fp, int lOff, int iOrg) {
    fseek(fp, lOff, iOrg);
    return ftell(fp);
}
...
__is_excext(const char* stFileName) {
    ...
    if ((sFileName2 = new char[strlen(stFileName) + 1])) {
        ...
        for (; iv != g_stFindFile.vecExcExt.end(); iv++) {
            ...
            if (strcmp(sFileName2, (*iv).c_str()) == 0) {
                return true;
            }
        }
        /// 스트링을 위해 할당한 메모리를 해제
...
__GetFileNames(const char* pBaseDir, const char* prefix) {
    ...
    if (strlen(pBaseDir) > 0) { /*BOOL bChange =*/
    ...
    while (!fFinished) {
        ...
        if (!(FileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
            *(ppFile[i] + nMaxName - 1) = 0; /// strncpy를 사용할 경우 주의
            strncpy(ppFile[i++],
                FileData.cFileName,
                nMaxName); /// 파일이름을 카피, FileData.cFileName은 char [260]
            nRetNum++; /// 갯수 증가
        }
 ...
 */
long
__vfseek(FILE* stream, long offset, int origin) {
...
bool
CFileMode::CheckMode(const char* FileName, int iChkMode, bool bForce) {
