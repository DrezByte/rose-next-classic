...
//-------------------------------------------------------------------------------------------------
bool
WSLSV_Socket::WndPROC(WPARAM wParam, LPARAM lParam) {
...
WS_lsvSOCKET::Init(HWND hWND,
    ...
    if (NULL == m_pReconnectTimer)
        m_pReconnectTimer =
            new CTimer(m_hMainWND, WS_TIMER_LSV, RECONNECT_TIME_TICK, (TIMERPROC)WS_TimerProc);
}

//-------------------------------------------------------------------------------------------------
bool
WS_lsvSOCKET::Connect() {
    return m_SockLSV.Connect(m_hMainWND, m_LoginServerIP.Get(), m_LoginServerPORT, m_uiSocketMSG);
}
void
WS_lsvSOCKET::Disconnect() {
...
//-------------------------------------------------------------------------------------------------
void
WS_lsvSOCKET::Send_srv_ACTIVE_MODE(bool bActive) {
...
//-------------------------------------------------------------------------------------------------
void
WS_lsvSOCKET::Send_zws_SERVER_INFO() {
...
//-------------------------------------------------------------------------------------------------
void
WS_lsvSOCKET::Send_wls_CHANNEL_LIST() {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return;
...
//-------------------------------------------------------------------------------------------------
void
WS_lsvSOCKET::Send_zws_CONFIRM_ACCOUNT_REQ(DWORD dwSocketID, t_PACKET* pPacket) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return;
    //	this->Lock ();
...
//-------------------------------------------------------------------------------------------------
void
WS_lsvSOCKET::Recv_lsv_CHECK_ALIVE() {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return;
    //	this->Lock ();
...
void
WS_lsvSOCKET::Recv_lsv_ANNOUNCE_CHAT() {
    /*
...
//-------------------------------------------------------------------------------------------------
bool
WS_lsvSOCKET::Proc_SocketMSG(WPARAM wParam, LPARAM lParam) {
        ...
        case FD_READ: {
            ...
            while (m_SockLSV.Peek_Packet(m_pRecvPket, true)) {
                ...
                switch (m_pRecvPket->m_HEADER.m_wType) {
                    ...
                    default:
                        g_LOG.CS_ODS(0xffff,
                            "Undefined Login server packet... Type[ 0x%x ], Size[ %d ]\n",
