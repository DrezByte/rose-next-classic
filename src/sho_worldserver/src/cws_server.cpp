...
void
CWS_Server::AddChannelCLIENT(DWORD dwGSClientIDX, CWS_Client* pClient) {
    m_iCurUserCNT++;
...
void
CWS_Server::SubChannelCLIENT(CWS_Client* pClient) {
    if (pClient->Get_GSID()) {
        m_iCurUserCNT--;
        m_nUserPERCENT = (int)(m_iCurUserCNT * 100.f / (1 + m_dwLimitUserCNT));
        if (abs(m_nBeforePERCENT - m_nUserPERCENT) >= DIFF_USER_PERCENT) {
...
//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Send_str_PACKET(WORD wType, char* szString) {
...
//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Send_wsv_CHEAT_REQ(t_PACKET* pPacket,
    DWORD dwReqGSID,
    DWORD dwReplyGSID,
    char* szCode) {
    classPACKET pCPacket = classPACKET();
...
//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Recv_zws_SERVER_INFO(t_PACKET* pPacket) {
...
//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Send_wls_CONFIRM_ACCOUNT_REPLY(BYTE btResult,
    DWORD dwWSID,
    DWORD dwGSID,
    CWS_Client* pCLIENT) // DWORD dwLoginTIME, DWORD dwRight, char *szAccount, char *szChar, bool
                         // bFirstZONE, DWORD dwLSID )
{
    classPACKET pCPacket = classPACKET();
...
//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Recv_zws_CONFIRM_ACCOUNT_REQ(t_PACKET* pPacket) {
    CWS_Client* pCLIENT =
        (CWS_Client*)g_pUserLIST->GetSOCKET(pPacket->m_zws_CONFIRM_ACCOUNT_REQ.m_dwServerID);
    ...
    for (short nI = 0; nI < 16; nI++)
        if (pCLIENT->password_buffer[nI]
            ...
            pCLIENT->CloseSocket();
            g_LOG.CS_ODS(0xffff,
                "??? Mismatch LS<->WS password, account: %s, char: %s \n",
                pCLIENT->Get_ACCOUNT(),
                pCLIENT->Get_NAME());
...
//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Recv_zws_SUB_ACCOUNT(t_PACKET* pPacket) {
...
//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Recv_gsv_CHANGE_CHAR(t_PACKET* pPacket) {
    CWS_Client* pUSER;
...
//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Recv_gsv_CHEAT_REQ(t_PACKET* pPacket) {
    ...
    if (pArg1) {
        if (!_strcmpi(pToken, "/w")) {
            ...
            if (pORDER) {
                pORDER->Send_gsv_WHISPER((char*)"]", &szCode[3]);
            }
        } else if (!_strcmpi(pToken, "/move")) {
            ...
            if (pUSER && pUSER->GetSERVER()) {
                pUSER->GetSERVER()->Send_wsv_CHEAT_REQ(pPacket,
                    pPacket->m_gsv_CHEAT_REQ.m_dwReqUSER,
                    pUSER->Get_GSID(),
                    pToken /* szCode */);
            }
        } else if (!_strcmpi(pToken, "/call")) {
            // cli->gsv->wsv->gsv->cli
            pArg2 = m_TmpStr.GetTokenNext(pDelimiters); // account
        ...
        } else if (!_strcmpi(pToken, "/account")) {
            // 케릭 이름으로 계정 얻기
            CWS_Client* pUSER = NULL;
            pUSER = g_pUserLIST->Find_CHAR(pArg1);
            if (pUSER) {
            ...
            if (pUSER /* && pUSER->GetZONE() */) {
                // m_TmpStr.Printf ("%s:%d ", pUSER->GetZONE()->Get_NAME(),
    ...
    } else if (!_strcmpi(pToken, "/call_r")) {
        // /move에 대한 GSV의 응답 :: 요청가 있는 서버로 ..
        CWS_Client* pUSER =
            (CWS_Client*)g_pUserLIST->GetSOCKET(pPacket->m_gsv_CHEAT_REQ.m_dwReqUSER);
        if (pUSER) {
            pUSER->GetSERVER()->Send_wsv_CHEAT_REQ(pPacket, 0, pUSER->Get_GSID(), (char*)"/call");
        }
    }
...
bool
CWS_Server::Recv_srv_USER_LIMIT(t_PACKET* pPacket) {
...
//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Recv_zws_ACCOUNT_LIST(t_PACKET* pPacket) {
    return true;
}

//-------------------------------------------------------------------------------------------------
bool
CWS_Server::Recv_zws_CREATE_CLAN(t_PACKET* pPacket) {
...
bool
CWS_Server::Recv_gsv_ADJ_CLAN_VAR(t_PACKET* pPacket) {
    g_pThreadGUILD->Add_ClanCMD(GCMD_ADJ_VAR, 0, pPacket, (char*)"null");
    return true;
}
...
// 존 서버로 부터 받은 패킷...
bool
CWS_Server::HandlePACKET(t_PACKETHEADER* pPacket) {
    //    LogString (LOG_DEBUG_, "        >> %d CWS_Server::HandlePACKET:: Type: 0x%x, Length:
...
void
CWS_ListSERVER::Send_ToWORLD(const classPACKET& pCPacket) {
    this->LockLIST();
...
//-------------------------------------------------------------------------------------------------
void
CWS_ListSERVER::Set_ChannelACTIVE(int iChannelNo, bool bActive, bool bToggle) {
...
//-------------------------------------------------------------------------------------------------
bool
CWS_ListSERVER::Set_ChannelSERVER(BYTE btChannelNO, CWS_Server* pServer) {
...
void
CWS_ListSERVER::Del_ChannelSERVER(int iChannelNo) {
    if (iChannelNo <= 0 || iChannelNo >= MAX_CHANNEL_SERVER)
        return;
    this->LockLIST();
...
//-------------------------------------------------------------------------------------------------
void
CWS_ListSERVER::Send_gsv_ANNOUNCE_CHAT(char* szMessage, char* szAccount) {
    if (NULL == szMessage)
        return;
...
//-------------------------------------------------------------------------------------------------
void
CWS_ListSERVER::Make_wls_CHANNEL_LIST(classPACKET* pCPacket) {
...
//-------------------------------------------------------------------------------------------------
void
CWS_ListSERVER::Make_srv_SERVER_LIST_REPLY(classPACKET* pCPacket) {
...
//-------------------------------------------------------------------------------------------------
bool
CWS_ListSERVER::Send_zws_SET_USER_CLAN(CWS_Client* pClient) {
...
//-------------------------------------------------------------------------------------------------
bool
CWS_ListSERVER::Send_zws_DEL_USER_CLAN(CWS_Client* pClient) {
    CWS_Server* pChannel = this->Get_ChannelSERVER(pClient->Get_ChannelNO());
    if (pChannel) {
        classPACKET pCPacket = classPACKET();
