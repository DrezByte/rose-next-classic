...
private:
    ...
    bool Send_wls_CONFIRM_ACCOUNT_REPLY(BYTE btResult,
        DWORD dwWSID,
        DWORD dwGSID,
        CWS_Client* pCLIENT = NULL); // DWORD dwLoginTIME, DWORD dwRight=0, char *szAccount=NULL,
                                     // char *szChar=NULL, bool bFirstZONE=false, DWORD dwLSID=0 );
...
public:
    ...
    CWS_Client* GetChannelCLIENT(WORD wGSClientIDX) { return m_ppChannelUSER[wGSClientIDX]; }

    inline bool SendPacket(const classPACKET& pCPacket) { return this->Send_Start(pCPacket); }
...
public:
    ...
    void Send_ToWORLD(const classPACKET& pCPacket);
    ...
    void ClosedClientSOCKET(iocpSOCKET* pSOCKET) { this->FreeClientSOCKET(pSOCKET); }
};
