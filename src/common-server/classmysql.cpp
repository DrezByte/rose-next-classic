...
//-------------------------------------------------------------------------------------------------
const char*
classMYSQL::GetERROR(void) {
    return ::mysql_error(m_pMySQL);
}

//-------------------------------------------------------------------------------------------------
bool
classMYSQL::Connect(char* szServerIP, char* szUser, char* szPassword) //, short nPortNO)
{
    ...
    sprintf(m_pQueryBuff,
        "MYSQL:: Connect Error[ ErrCODE: %d, %s ]\n",
        m_iErrCODE,
        this->GetERROR());
...
//-------------------------------------------------------------------------------------------------
int
classMYSQL::ExecMySQL(char* szQuery) {
...
bool
classMYSQL::QuerySQL(char* szFormat, ...) {
    va_list ArgPtr;
...
int
classMYSQL::ExecSQL(char* szFormat, ...) {
    va_list ArgPtr;
...
//-------------------------------------------------------------------------------------------------
bool
classMYSQL::QueryString(char* szQuery) {
    // int mysql_query(MYSQL *mysql, const char *query)
    unsigned long ulLength = strlen(szQuery);
    if (ulLength > 0) {
...
//-------------------------------------------------------------------------------------------------
bool
classMYSQL::QueryString(char* szQuery, unsigned long ulLength) {
    m_iErrCODE = ::mysql_real_query(m_pMySQL, szQuery, ulLength);
    if (0 == m_iErrCODE) {
        this->StoreResult();
...
//-------------------------------------------------------------------------------------------------
BYTE*
classMYSQL::GetDataPTR(WORD wColumnIDX) {
...
char*
classMYSQL::GetStrPTR(WORD wColumnIDX, bool bSetSpaceToNULL) {
    if (wColumnIDX >= m_iColumnCNT) {
        _ASSERT(wColumnIDX >= 0 && wColumnIDX < m_iColumnCNT);
        return NULL;
    }
...
int
classMYSQL::GetStrLen(WORD wColumnIDX) {
    if (wColumnIDX >= m_iColumnCNT) {
        _ASSERT(wColumnIDX >= 0 && wColumnIDX < m_iColumnCNT);
        return NULL;
    }
...
int
classMYSQL::GetInteger(WORD wColumnIDX) {
    if (wColumnIDX >= m_iColumnCNT) {
        _ASSERT(wColumnIDX >= 0 && wColumnIDX < m_iColumnCNT);
        return 0;
    }
...
short
classMYSQL::GetInteger16(WORD wColumnIDX) {
    if (wColumnIDX >= m_iColumnCNT) {
        _ASSERT(wColumnIDX >= 0 && wColumnIDX < m_iColumnCNT);
        return 0;
    }
...
__int64
classMYSQL::GetInt64(WORD wColumnIDX) {
    if (wColumnIDX >= m_iColumnCNT) {
        _ASSERT(wColumnIDX >= 0 && wColumnIDX < m_iColumnCNT);
        return 0;
    }
...
//-------------------------------------------------------------------------------------------------
bool
classMYSQL::BindPARAM(short nParamIDX, BYTE* pData, unsigned int uiDataSize) {
...
//-------------------------------------------------------------------------------------------------
DWORD
classMYSQL::MakeQuery(char* szCommand, ...) {
    va_list va;
    ...
    while ((type = va_arg(va, MQ_PARAM_TYPE)) != MQ_PARAM_END) {
        switch (type) {
            ...
            case MQ_PARAM_BINDIDX: {
                ...
                *pBuff++ = '\'';
                pBuff += ::mysql_real_escape_string(m_pMySQL,
                    pBuff,
                    (const char*)m_pBindPARAM[iParamIDX].m_pDATA,
                    m_pBindPARAM[iParamIDX].m_uiDataSize);
                *pBuff++ = '\'';
                break;
            }
        }
