...
//-------------------------------------------------------------------------------------------------
bool
IOCPSocketSERVER::Active(int iListenTCPPortNO, DWORD dwMaxSocketCount, int iKeepAliveSec) {
...
//-------------------------------------------------------------------------------------------------
void
IOCPSocketSERVER::StartACCEPT(int iListenTCPPortNO, int iKeepAliveSec) {
...
IOCPSocketSERVER::ShutdownACCEPT() {
    if (m_pAcceptTHREAD) {
        ...
        // ShutdownACCEPT()를 호출한 쓰레드와 m_pAcceptTHREAD가 틀린 쓰래드 인데.. Free에서 종료되기
        // 전에는 리턴 안됨
...
void
IOCPSocketSERVER::ShutdownWORKER() {
    if (m_ppWorkerTHREAD) {
        for (DWORD dwI = 0; dwI < m_dwWorkerThreadCNT; dwI++) {
            m_ppWorkerTHREAD[dwI]->Terminate();

            if (0
                == ::PostQueuedCompletionStatus(m_IOCP.GetHANDLE(),
                    -1 /*dwNumberOfBytesTransferred*/,
                    0 /*dwCompletionKey*/,
                    NULL /*lpOverlapped*/)) {
                g_LOG.CS_ODS(0xffff,
                    "ThreadWORKER::Free() PostQueuedCompletionStatus() return 0, LastERROR: "
                    "%d(0x%x)",
                    GetLastError(),
                    GetLastError());
            }
...
//-------------------------------------------------------------------------------------------------
void
IOCPSocketSERVER::CloseIdleSCOKET(DWORD dwIdleMilliSec) {
