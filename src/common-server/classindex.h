...
CIndexARRAY<DataType>::CIndexARRAY(char* szName, unsigned int uiBuffSize) {
    ...
    if (NULL == m_pINDEX) {
        g_LOG.CS_ODS(0xffff,
            "ERROR :: Out of memory - create index buffer[ %d ] failed ..",
            uiBuffSize);
        return;
    }
...
template<class DataType>
unsigned int
CIndexARRAY<DataType>::GetEmptySlot() {
    unsigned int iL;
...
template<class DataType>
unsigned int
CIndexARRAY<DataType>::AddData(DataType dtValue) {
    this->Lock();
...
template<class DataType>
void
CIndexARRAY<DataType>::DelData(unsigned int uiSlotNo) {
    this->Lock();
...
template<class DataType>
DataType
CIndexARRAY<DataType>::GetData(unsigned int uiSlotNo) {
...
//-------------------------------------------------------------------------------------------------
template<class DataType>
CSyncIdxARRAY<DataType>::CSyncIdxARRAY(char* szName, int iBuffSize):
    CIndexARRAY<DataType>(szName, iBuffSize) {
