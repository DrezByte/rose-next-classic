...
template<class dType>
dType*
classListBLOCK<dType>::Insert(t_HASHKEY HashKEY,
    char* szTag,
    WORD wAttackTYPE,
    DWORD dwBlockSecond) {
...
template<class dType>
dType*
classListBLOCK<dType>::Search(t_HASHKEY HashKEY, char* szTag, bool bUpdateList) {
            ...
            if (bUpdateList && pBlockDATA->m_DATA->m_dwBlockSECOND) {
                ...
                if (dwCurTIME - pBlockDATA->m_DATA->m_dwLastTryTIME
                    ...
                    LogString(LOG_NORMAL,
                        (char*)"del block data [ %s ] Expired: %d / %d \n",
                        szTag,
                        pBlockDATA->m_DATA->m_dwBlockSECOND,
                        dwCurTIME);
...
template<class dType>
void
classListBLOCK<dType>::Update(dType* pBlockDATA, WORD wAttackTYPE, DWORD dwAddBlockSecond) {
    pBlockDATA->m_wAttackTYPE |= wAttackTYPE;
