...
//-------------------------------------------------------------------------------------------------
bool
CLS_Server::Recv_zws_SERVER_INFO(t_PACKET* pPacket) {
    /********
    ...
    char* forceWorldServerIP = (char*)"";
    char forceWorldServerIP2[255];
    GetPrivateProfileString("FormLSCFG",
        "ForceWorldServerIP",
        "",
        forceWorldServerIP2,
        sizeof(forceWorldServerIP2),
        "./SHO_LS.ini");
...
//-------------------------------------------------------------------------------------------------
void
CLS_Server::Make_lsv_CHANNEL_LIST_REPLY(classPACKET* pCPacket) {
...
//-------------------------------------------------------------------------------------------------
bool
CLS_Server::Recv_wls_CHANNEL_LIST(t_PACKET* pPacket) {
...
//-------------------------------------------------------------------------------------------------
bool
CLS_Server::Send_wls_CONFIRM_ACCOUNT_REPLY(BYTE btResult,
    DWORD dwLSID,
    DWORD dwWSID,
    DWORD dwLoginTIME,
    DWORD dwRight,
...
//-------------------------------------------------------------------------------------------------
bool
CLS_Server::Recv_gsv_CHECK_ALIVE(t_PACKET* pPacket) {
...
//-------------------------------------------------------------------------------------------------
bool
CLS_Server::Recv_zws_CONFIRM_ACCOUNT_REQ(t_PACKET* pPacket) {
    CLS_Account* pCAccount;
...
bool
CLS_Server::Recv_zws_SUB_ACCOUNT(t_PACKET* pPacket) {
    ...
    if (pCAccount && pCAccount->m_pWorldServer) {
        ...
        m_AccountLIST.DeleteNode(
            pCAccount->m_pListNODE); // 뻑 :: 같은 계정에 대해 여러번 호출되는듯...
        g_pThreadSQL->Add_LogOutUSER(pCAccount);
    } else {
        LogString(0xffff,
            (char*)"Recv_gsv_SUB_ACCOUNT:: Account[ %s ] not found in JOIN list\n",
            szAccount);
    }
...
bool
CLS_Server::Recv_wls_ACCOUNT_LIST(t_PACKET* pPacket) {
    CLS_Account* pCAccount;
...
//-------------------------------------------------------------------------------------------------
bool
CLS_ListSERVER::Add_ServerList2Packet(classPACKET* pCPacket, DWORD dwRight) {
    classDLLNODE<CLS_Server*>* pConnNODE;

    this->LockLIST();
    pConnNODE = m_LIST.GetHeadNode();
    while (pConnNODE) {
        if (dwRight >= pConnNODE->DATA->m_dwAcceptRIGHT && pConnNODE->DATA->m_bActive) {
...
//-------------------------------------------------------------------------------------------------
void
CLS_ListSERVER::Send_lsv_ANNOUNCE_CHAT(void* pServer, char* szChatMSG) {
...
//-------------------------------------------------------------------------------------------------
void
CLS_ListSERVER::Send_lsv_CHECK_ALIVE(void) {
...
//-------------------------------------------------------------------------------------------------
void
CLS_ListSERVER::Make_srv_SERVER_LIST_REPLY(classPACKET* pCPacket) {
