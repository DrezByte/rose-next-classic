/*
{
    switch( iChargeType ) {
        case 0  :	// 금지
        case 1	:	// 시간제 - 매 1 시간 단위로 6 포인트 삭감, 1시간을 채우지 못했을 경우,
1시간으로 계산한다. case 2	:	// 월 정액제 - 매 30 일 단위로 350 포인트 삭감. case 3	:	// 시간
정액제 - 매 12 시간 단위로 20 포인트 삭감. case 4	:	// 3개월 정액제 - 매 90 일 단위로 888 포인트
삭감. case 5	:	// 홍콩 월정액제 case 6	:	// 홍콩 3개월 정액제 case 7	:	// 홍콩 시간제 case
8	:	// 홍콩 시간 정액제 case 9	:	// 분 정량제 - 매 1분 단위로 0.25포인트 삭감.
...

{
    switch( iResult ) {
        ...
        case 3	:	// 패스워드 에러 (login result)	계정 또는 패스워드 에러 또는 포인트 충전하지
않은 상태,  Joypark 회원 중심 참고. case 4	:	// ObjnetObiect 구축 실패, Login Server등록 불가능
(Connection State Result) case 5	:	// Login Server연결 중단 (Connection State Result)
        case 6	:	// Login Server연결 실패 (Connection State Result)
        case 7	:	// 없는 계정 (login result)	계정 또는 패스워드 에러 또는 포인트를 충전하지 않은
상태,  Joypark 회원 중심 참고. case 8	:	// 포인트 부족 (login result)	포인트 부족 case 9	:
// 시간 부족 (login result)	무료 체험기간 만기 case 10	:	// 없는 과금제도 (login result)	과금방식
문제 case 11	:	// 블록 (login result)	블록 중 case 12	:	// 블록３일 (login result)	블록 중
        case 13	:	// 블록７일 (login result)	블록 중
        case 14	:	// 블록 10 일 (login result)	블록 중
        case 15	:	// 로그 아웃 상태 (logout result)
        case 16	:	// 불확실한 에러 (데이터 베이스 에러 login, relogin, logout result) 불확실한
에러 case 17	:	// Ip중복 (홈 유저 대상 login, relogin, result) case 18	:	// 게임방 유효한
게임방 ip리스트 (사용 : 게임방 login, relogin, result)
        case 20	:	// 없는 계정 (relogin result)	계정 또는 패스워드 에러 도는 포인트를 충전하지
않은 상태, Joypark 회원 중심 참고. case 21	:	// 중복 로그인 (relogin result)	이미 온라인 상황인
계정 case 23	:	// 포인트 부족 (relogin result)	포인트 부족 case 24	:	// 시간 부족 (relogin
result)	무료 체험기간 만기 case 25	:	// 부정확한 과금방식 (relogin result)	과금방식 문제 case
26	:	// 블록 (relogin result)	블록 중 case 27	:	// 블록 ３일 (relogin result)	블록 중 case
28	:	// 블록 ７일 (relogin result)	블록 중 case 29	:	// 블록 10 일 (relogin result)	블록 중
        case 30	:	// 같은 ip  n초내 중복 로그인 (login result)	마우스를 잠깐 쉬게 하고, 몇 초
후 다시 접속. case 31	:	// Game Server등록 성공 (Connection State result) case 32	:	// Game
Server등록 ID중복(Connection State result) case 33	:	// Game Server미 등록(login, relogin result)
서버 접속 불가능, 잠시후 다시 연결 시도. case 34	:	// 유저 매일 접속 가능한 시간 초과.(login,
relogin result)	접속시간 초과.
...
#endif
{
    ...
    m_pLoginClient =
        CreateSoftstarLoginClient(szServerIP /*"127.0.0.1"*/, iPortNo /*7888*/, &sCallback);
...
//-------------------------------------------------------------------------------------------------
void
CBS_Taiwan::ConnectionState(int iResult) {
...
void
CBS_Taiwan::PreloginResult(char* strPlayerID,
    unsigned short u16ChargeType,
    double dPoint,
    unsigned int uIdentity,
    unsigned int uPreloginResult,
    int iLoginState,
    int right) {
            ...
            switch (u16ChargeType) {
                ...
                case 12: // 무료 체험
                {
                    ...
                    if (pCAccount) {
                        ...
                        if (dwCurTime - pCAccount->m_dwDisconnectTryTIME >= 2 * 60 * 1000) {
                            ...
                            if (pCAccount->m_pWorldServer) {
                                pCAccount->m_pWorldServer->Send_str_PACKET(WLS_KICK_ACCOUNT,
                                    strPlayerID);
                            }
                        }
                    ...
                    */
                    ...
                    pClient->m_dwPayFLAG =
                        PLAY_FLAG_KOREA_DEFAULT; // u16ChargeType;	우선 한국 개인 정액으루...
                    break;
                }
                // case 0  :	// 금지
                // case 101:	// 블록 3일
...
CBS_Taiwan::PreloginResult(char* strPlayerID,
    ...
    int right) {
    g_LOG.CS_ODS(0xffff,
        "prelogin result ID:%s ,ChargeType:%d ,Point:%f ,Identity:%d ,Result:%d\n",
        strPlayerID,
        u16ChargeType,
        dPoint,
        uIdentity,
...
void
CBS_Taiwan::LoginResult(char* strPlayerID,
    unsigned short u16ChargeType,
    double dPoint,
    unsigned int uIdentity,
    unsigned int uLoginResult,
    int iLoginState) {
    g_LOG.CS_ODS(0xffff,
        "login result ID:%s ,ChargeType:%d ,Point:%f ,Identity:%d ,Result:%d\n",
        strPlayerID,
        u16ChargeType,
        dPoint,
        uIdentity,
    ...
    if (0 == uLoginResult) {
        ...
        switch (u16ChargeType) {
            ...
            case 11: // 10일 - 매 10일 단위로 150 포인트 삭감.
            case 12: // 무료 체험
                g_pListACCOUNT->Gums_LoginSUC(EXT_BILLING_MSG_PAY_TAIWAN,
                    strPlayerID,
                    u16ChargeType /*iPayType*/,
                    PLAY_FLAG_TAIWAN_DEFAULT);
                return;
...
//-------------------------------------------------------------------------------------------------
void
CBS_Taiwan::ReloginResult(char* strPlayerID,
    unsigned short u16ChargeType,
    double dPoint,
    unsigned int uReloginResult,
    int iLoginState) {
    g_LOG.CS_ODS(0xffff,
        "relogin result ID:%s ,ChargeType:%d ,Point:%f ,Result:%d\n",
        strPlayerID,
        u16ChargeType,
        dPoint,
        uReloginResult);
...
//-------------------------------------------------------------------------------------------------
void
CBS_Taiwan::LogoutResult(char* strPlayerID, unsigned int uIdentity, unsigned int uLogoutResult) {
    // uIdentity == LSID
    g_LOG.CS_ODS(0xffff,
        "logout result ID:%s ,Identity:%d ,Result:%d\n",
        strPlayerID,
        uIdentity,
        uLogoutResult);
...
//-------------------------------------------------------------------------------------------------
void
CBS_Taiwan::BillingMessage(char* strPlayerID, char* strBillingMessage, unsigned int uIdentity) {
...
//-------------------------------------------------------------------------------------------------
void
CBS_Taiwan::EnforceLogout(int iPlayerNo, char* strPlayerID) {
    // iPlayerNo 무시해도 됨 !!!
