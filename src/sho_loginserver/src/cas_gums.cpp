...
//-------------------------------------------------------------------------------------------------
DWORD
AS_gumSOCKET::GetPlayingFLAG(eGumsNATION eNation, int iPayType) {
    DWORD dwPlayFlag = 0;
    switch (eNation) {
        case GUMS_KOREA: // 한국은...
...
bool
AS_gumSOCKET::RecvGums_LoginSUC(eGumsNATION eNation,
    char* szAccount,
    char* szGumsPKET,
    char* szUserTAG,
    int iPayType) {
...
bool
AS_gumSOCKET::RecvGums_LoginOnlyFREE(int iMsgTYPE, char* szAccount, char* szUserTAG) {
    // 프리섭 로그인... 프리섭 리스트만 전송..
    int iSocketIDX = atoi(szUserTAG);
    CLS_Client* pClient = (CLS_Client*)g_pListCLIENT->GetSOCKET(iSocketIDX);
    if (pClient && !_strcmpi(szAccount, pClient->m_Account.Get())) {
        ...
        pClient->m_dwPayFLAG =
            PLAY_FLAG_KOREA_DEFAULT; // this->GetPlayingFLAG( eNation, iPayType );
        pClient->m_nProcSTEP = CLIENT_STEP_LOGEDIN;
    }
...
bool
AS_gumSOCKET::RecvGums_LogInFailedNoRIGHT(int iMsgTYPE, char* szAccount, char* szUserTAG) {
    int iSocketIDX = atoi(szUserTAG);
    CLS_Client* pClient = (CLS_Client*)g_pListCLIENT->GetSOCKET(iSocketIDX);
    if (pClient && !_strcmpi(szAccount, pClient->m_Account.Get())) {
...
bool
AS_gumSOCKET::RecvGums_LogInFailedOutOfIP(int iMsgTYPE, char* szAccount, char* szUserTAG) {
    int iSocketIDX = atoi(szUserTAG);
    CLS_Client* pClient = (CLS_Client*)g_pListCLIENT->GetSOCKET(iSocketIDX);
    if (pClient && !_strcmpi(szAccount, pClient->m_Account.Get())) {
...
// 게임할 권한이 없는 사용자
bool
AS_gumSOCKET::RecvGums_LogInFailedNoRIGHT(int iMsgTYPE, char* szUserID, char* szUserTAG) {
    return g_pListACCOUNT->Gums_KickOutUSER(szUserID, BILLING_MSG_KICKOUT_NO_RIGHT);
}
bool
AS_gumSOCKET::RecvGums_LogInFailedOneDayEXPIRED(int iMsgTYPE, char* szUserID) {
    return g_pListACCOUNT->Gums_KickOutUSER(szUserID, BILLING_MSG_KICKOUT_TIME_EXPIRED);
}
bool
AS_gumSOCKET::RecvGums_LogInFailedAlreadyLOGIN(int iMsgTYPE, char* szUserID) {
    return g_pListACCOUNT->Gums_KickOutUSER(szUserID, BILLING_MSG_KICKOUT_ALREADY_LOGIN);
}
bool
AS_gumSOCKET::RecvGums_LogInFailedOutOfIP(int iMsgTYPE, char* szUserID, char* szUserTAG) {
    return g_pListACCOUNT->Gums_KickOutUSER(szUserID, BILLING_MSG_KICKOUT_OUT_OF_IP);
}
bool AS_gumSOCKET::RecvGums_LogInFailedKickedUSER(int iMsgTYPE,
    char* szUserID) { // 기존 사용자 접속을 끊었다.
    return g_pListACCOUNT->Gums_KickOutUSER(szUserID, BILLING_MSG_KICKOUT_EXIST_ACCOUNT);
}
bool
AS_gumSOCKET::RecvGums_LogInFailedTimeEXPIRED(int iMsgTYPE, char* szUserID, char* szString) {
    return g_pListACCOUNT->Gums_KickOutUSER(szUserID, BILLING_MSG_KICKOUT_TIME_EXPIRED);
}

// 프리 로그인 실패
bool
AS_gumSOCKET::RecvGums_PreLoginFAILED(int iMsgTYPE, char* szAccount) {
    return true;
}
...
bool
AS_gumSOCKET::RecvGums_TimeOUT(int iMsgTYPE, char* szAccount) {
    // KEEP ALIVE...
    return g_pListACCOUNT->Gums_TimeOUT(szAccount);
}
bool
AS_gumSOCKET::RecvGums_TimeALERT(int iMsgTYPE, char* szAccount, char* szRemainMin) {
    // 채널 섭에 남은 시간 보냄...
    return g_pListACCOUNT->Gums_Message(BILLING_MSG_TYPE_TIME_ALERT, szAccount, szRemainMin);
}
bool
AS_gumSOCKET::RecvGums_TimeALERT_EXT(int iMsgTYPE,
    char* szAccount,
    char* szRemainMin,
    DWORD dwWarnPlayFlag) {
    return g_pListACCOUNT->Gums_Message_EXT(
        EXT_BILLING_MSG_TYPE_TIME_ALERT, szAccount, szRemainMin, dwWarnPlayFlag);
}

bool
AS_gumSOCKET::RecvGums_SetExpireDATEorTIME(int iMsgTYPE, char* szAccount, char* szMsg) {
...
bool
AS_gumSOCKET::RecvGums_SetExpireDATEorTIME_EXT(int iMsgTYPE, char* szAccount, char* szMsg) {
...
// 사용 기간 만료
bool
AS_gumSOCKET::RecvGums_TimeEXPIRED(int iMsgTYPE, char* szUserID, char* szString) {
    return g_pListACCOUNT->Gums_KickOutUSER(
        szUserID, BILLING_MSG_KICKOUT_TIME_EXPIRED); // RecvGums_TimeEXPIRED
}
bool
AS_gumSOCKET::RecvGums_TimeEXPIRED_EXT(int iMsgTYPE, char* szUserID, DWORD dwDelPlayFlag) {
    return g_pListACCOUNT->Gums_Message_EXT(
        EXT_BILLING_MSG_TYPE_TIME_EXPIRED, szUserID, NULL, dwDelPlayFlag);
}

bool
AS_gumSOCKET::RecvGums_DupUserID(int iMsgTYPE, char* szUserID) {
    return g_pListACCOUNT->Gums_KickOutUSER(
        szUserID, BILLING_MSG_KICKOUT_DUP_LOGIN); // RecvGums_DupUserID
}
#endif

//-------------------------------------------------------------------------------------------------
VOID CALLBACK
AS_TimerProc(HWND hwnd /* handle to window */,
...
//-------------------------------------------------------------------------------------------------
bool
ASGUM_Socket::WndPROC(WPARAM wParam, LPARAM lParam) {
    AS_gumSOCKET* pInstance = AS_gumSOCKET::GetInstance();
    if (!pInstance)
        return false;
...
#ifdef __SHO_LS_LIB
    sprintf_s(szLogFile, "LSgums%d_%d_%d.log", ST.wYear, ST.wMonth, ST.wDay);
#else
    sprintf_s(szLogFile, "ASgums%d_%d_%d.log", ST.wYear, ST.wMonth, ST.wDay);
#endif

#ifdef __GUMS_LOG
    fopen_s(&m_fpLOG, szLogFile, "a");
    this->Log(true, (char*)"      << Log start >>");
#else
    m_fpLOG = NULL;
#endif
}
...
void
AS_gumSOCKET::Log(bool bRecv, char* szLog) {
...
//-------------------------------------------------------------------------------------------------
void
AS_gumSOCKET::Init(HWND hWND, char* szGumServerIP, int iGumServerPort, UINT uiSocketMSG) {
    //	m_iGameID = 1;
    m_iCpID = 1;
    ...
    if (NULL == m_pReconnectTimer)
        m_pReconnectTimer = new CTimer(
            m_hMainWND, AS_TIMER_GUMS_CONN, RECONNECT_TIME_TICK, (TIMERPROC)AS_TimerProc);

    if (NULL == m_pSendAliveTimer)
        m_pSendAliveTimer = new CTimer(
            m_hMainWND, AS_TIMER_GUMS_ALIVE, SEND_ALIVE_TIME_TICK, (TIMERPROC)AS_TimerProc);
}

//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Connect() {
    if (!m_bGumsINIT) {
        // m_bGumsINIT = false;
...
//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Send_GumsPACKET(int iMsgType, char* szString, int iSeqNO, bool bCheckInit) {
    ...
    //	m_csHEAD.Lock ();
    {
        ...
        sprintf_s(szGumHeader, "GUMP/1.2\t%u\x1d%d\t", dwSeqNO, iMsgType);

        pCPacket->m_HEADER.m_wType = 0xffff;
        pCPacket->m_HEADER.m_nSize = sizeof(t_PACKETHEADER);

        pCPacket->AppendData(
            szGumHeader, (short)(strlen(szGumHeader))); // 스트링으로 붙이면 \0이 붙어서 안됨...
    }
    //	m_csHEAD.Unlock ();
...
//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Send_IntializedMessage(char* szServerName,
    int iServerGroupNumber,
    int iServerNumber,
    int iZoneNumber,
    int iGumsKeepAlive,
    int iUserKeepAlive) {
...
//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Send_ServerKeepAliveMessage() {
    // GUMPHeader#MsgType/heart_beat/
    return this->Send_GumsPACKET(GUMS002_REQ_SERVER_KEEP_ALIVE, (char*)"heart_beat\t");
}
//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Send_RequestLOGIN(bool bKickAlreadyLogin,
    int iGameID,
    char* szUserID,
    char* szPassword,
    char* szRealIP,
    ...
    char szGumBody[256];
    // 로그인 요청
    // GUMPHeader#MsgType/Disconnected/GameId/UserID/CpID/UserPwd/UserRealIp/UserVirtualIp/
    sprintf_s(szGumBody,
        "%d\t%d\t%s\t%d\t%s\t%s\t%s\t",
        bKickAlreadyLogin,
        iGameID,
        ...
        szPassword,
        szRealIP,
        szVirtualIP ? szVirtualIP : szRealIP);
    return this->Send_GumsPACKET(GUMS010_REQ_LOGIN, szGumBody);
}
//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Send_RequestLOGOUT(int iGameID, char* szUserID) {
    char szGumBody[256];
    // 로그아웃 요청 GUMPHeader#MsgType/GameId/UserID/CpID/
    sprintf_s(szGumBody, "%d\t%s\t%d\t", iGameID, szUserID, m_iCpID);
    return this->Send_GumsPACKET(GUMS020_REQ_LOGOUT, szGumBody);
}
//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Send_UserKeepAliveMessage(int iGameID, char* szUserID) {
    char szGumBody[256];
...
//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Send_RequestReaminTime(int iGameID, char* szUserID) {
    char szGumBody[256];
    // GUMPHeader#MsgType/GameId/UserID/CpID/
    sprintf_s(szGumBody, "%d\t%s\t%d\t", iGameID, szUserID, m_iCpID);
...
//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Send_PreLogin(int iGameID,
    char* szUserID,
    char* szRealIP,
    char* szVirtualIP,
    int iUserTAG) {
...
//-------------------------------------------------------------------------------------------------
int
AS_gumSOCKET::GetPayType(char* szPayType) {
    static struct {
        int m_iPayType;
        char* m_szPayType;
    } sPayType[] = {{-1, NULL},
        {BILLING_MSG_PAY_FU, (char*)"FU"},
        {BILLING_MSG_PAY_FA, (char*)"FA"},
        {BILLING_MSG_PAY_FAP, (char*)"FAP"},
        {BILLING_MSG_PAY_FQ, (char*)"FQ"},
        {BILLING_MSG_PAY_GU, (char*)"GU"},
        {BILLING_MSG_PAY_GQ, (char*)"GQ"},
        {BILLING_MSG_PAY_IQ, (char*)"IQ"},
        {0, NULL}};
...
DWORD
AS_gumSOCKET::GetPayFlag(char* szPayType) {
    /*
        0	Free Play 	캐릭작성, 채팅
        1	Entry	배틀, 베이식 필드
...
//-------------------------------------------------------------------------------------------------
void
AS_gumSOCKET::Recv_GumsPACKET(char* szString, int iStrLen) {
    // static const char *szRecvGumHeader = "GUMPHeader\x1d";
    static char* pHeaderDelimiter = (char*)"\x1d";
    static char* pTabDelimiters = (char*)"\t";
    char* pGumVersion;
    ...
    if (!pGameID || !pUserID || !pCpID) {
        g_LOG.CS_ODS(0xffff,
            "Invalid GUM Token[ %s ]::: MsgType:%s, GamdID:%s, UserID:%s, CpID:%s\n",
            szString,
            pMsgType,
            pGameID,
            pUserID,
    ...
    switch (iMsgTYPE) {
        default:
            g_LOG.CS_ODS(0xffff,
                "Unknow GUM MsgType[ %s ]::: Type: %d, GamdID:%s, UserID:%s, CpID:%s\n",
                szString,
                iMsgTYPE,
                pGameID,
                pUserID,
        ...
        case GUMS1111_REPLY_FREE_LOGIN_SUCCESS: {
            if (pLast == NULL) {
                g_LOG.CS_ODS(0xffff,
                    "ERROR in GUMS  :: Type:%d, PayType == NULL[ %s ]\n",
                    iMsgTYPE,
                    szString);
                return;
            }
            ...
            switch (eNation) {
                ...
                case GUMS_KOREA: {
                    ...
                    switch (iPayType) {
                        case BILLING_MSG_PAY_FU: // FU	로그인 된 사용자는 무료 아이디 사용자임을
                                                 // 나타낸다.
                        case BILLING_MSG_PAY_FA: // FA	로그인 된 사용자는 개인정액 사용자임을
                                                 // 나타낸다.
                        case BILLING_MSG_PAY_FAP: // FAP	로그인 된 사용자는 개인정액
                                                  // 사용자(플레티넘)임을 나타낸다.
                        case BILLING_MSG_PAY_FQ: // FQ	로그인 된 사용자는 개인정량 사용자임을
                                                 // 나타낸다.
                        case BILLING_MSG_PAY_GU: // GU	로그인 된 사용자는 게임방 사용자임을
                                                 // 나타낸다.
                        case BILLING_MSG_PAY_GQ: // GQ	게임방에서 로그인을 시도하였으나 게임방의
                                                 // 과금기간(시간)이 만료되어 개인정량으로 로그인
                                                 // 되었음을 나타낸다.
                        case BILLING_MSG_PAY_IQ: // IQ	게임방에서 로그인을 시도하였으나 이미 계약된
                                                 // IP 숫자를 모두 사용하고 있어 개인정량으로 로그인
                                                 // 되었음을 나타낸다.
        ...
        case GUMS017_REPLY_LOGIN_FAILED_KICKED_USER: // 접속 해지 로그인 요청시 기존 사용자 로그
                                                     // 아웃됐다고 통보 받음..
        ...
        case GUMS042_REPLY_EXPIRE_DAY2: // 종량 사용자의 남은 시간을 돌려준다.
        {
            ...
            if (NULL == pLast) {
                g_LOG.CS_ODS(0xffff,
                    "DATE or TIME == NULL :: Recv GUMS %d _REPLY_EXPIRE_DAY:: %s, %s\n",
                    iMsgTYPE,
                    pUserID,
                    szString);
                return;
        ...
        case GUMS1042_REPLY_EXPIRE_DAY2: {
            ...
            // Entry 요금제의 만료시간은 2006년 9월 10일 10:25 이며 Stock Space 요금제의 만료시간은
            // 2006년 9월 25일 10:25이다.
            char szTmp1[256],
                szTmp2[256] = {
                    0,
                };
            ...
            if (iStrLen > 1) {
                szTmp2[iStrLen - 1] = 0;
                this->RecvGums_SetExpireDATEorTIME_EXT(
                    iMsgTYPE, pUserID, szTmp2 /* &szString[ xxxxx ] */ /* pLast */);
            }
            return;
        }
        ...
        //
        // 1. 경고 메세지
        case GUMS051_REPLY_DAY_EXPIRED_ALERT: // (1)	기간 만료 경고(정액) :: 결제한 사용기간이
                                              // 만료되기 몇 분전에 Game Server에게 전달되는
                                              // 메시지다. MsgType 이 51
        case GUMS052_REPLY_ONE_DAY_TIME_OVER: // (2)	일일 사용시간 한도 초과 경고:: 일일 사용시간
                                              // 한도 - 예를 들면 하루 1시간 사용가능 - 를 초과하기
                                              // 몇 분전에 Game Server에게 전달되는 메시지다.
                                              // MsgType 이 52
        case GUMS053_REPLY_ONE_DAY_TIME_OVER2: // (3)	일일 사용기간 범위 초과 경고:: 일일 사용기간
                                               // - 예를 들면 18:00 ~ 20:00 시 - 을 벗어나기 몇
                                               // 분전에 Game Server에게 전달되는 메시지다. MsgType
                                               // 이 53
        case GUMS054_REPLY_TIME_EXPIRED_ALERT: // (4)	사용시간(정량) 초과 경고 :: 정량제 사용시간
                                               // 만료 몇 분전에 Game Server에게 전달되는 메시지다.
                                               // MsgType 이 54
        ...
        // 2. 접속 해제 요청 메세지
        case GUMS061_REPLY_DAY_EXPIRED_DIS: // (1)	기간 만료 접속 해제 메시지(정액) :: 결제한
                                            // 사용기간이 만료되어 사용자의 접속을 해제하라는
                                            // 메시지이다. MsgType 이 61
        case GUMS062_REPLY_ONE_DAY_EXPIRED_DIS: // (2)	일일 사용시간 한도 초과 접속 해제 메시지 ::
                                                // 일일 사용시간 한도가 초과되어 사용자의 접속을
                                                // 해제하라는 메시지이다. MsgType 이 62
        case GUMS063_REPLY_ONE_DAY_EXPIRED_DIS2: // (3)	일일 사용기간 범위 초과 접속 해제 메시지 ::
                                                 // 일일 사용기간 범위가 초과되어 사용자의 접속을
                                                 // 해제하라는 메시지이다. MsgType 이 63
        ...
        case GUMS064_REPLY_DUP_USERID_DIS: // (4)	다른 사용자의 요구로 접속 해제 메시지 :: 같은
                                           // 아이디로 다른 사용자가 로그인을 하면서 이미 로그인
                                           // 되어 있던 기존 사용자를 로그 아웃 시킬 때 기존
                                           // 사용자에게 전달되는 메시지이다. MsgType 이 64
        ...
        case GUMS065_REPLY_USER_NOT_FOUND: // 사용자 Live 신호가 왔을때 사용자가 메모리 내에 없을
                                           // 경우 보내주는 메시지
        case GUMS1065_REPLY_USER_NOT_FOUND:
            ...
            if (this->RecvGums_TimeOUT(
                    iMsgTYPE, pUserID) /* g_pListACCOUNT->RecvGums_TimeOUT( pUserID ) */) {
    ...
    g_LOG.CS_ODS(0xffff,
        "!!!! Recved GUM Msg[ %s ]::: Type: %d, GamdID:%s, UserID:%s, CpID:%s, Last:%s\n",
        szString,
        iMsgTYPE,
        pGameID,
        pUserID,
...
//-------------------------------------------------------------------------------------------------
bool
AS_gumSOCKET::Proc_SocketMSG(WPARAM wParam, LPARAM lParam) {
        ...
        case FD_READ: {
            ...
            while (m_SockGUM.Peek_Packet(m_pRecvPket, true)) {
                ...
                if (0x0ffff == m_pRecvPket->m_HEADER.m_wType) {
                    ...
                    this->Recv_GumsPACKET(
                        szString, m_pRecvPket->m_HEADER.m_nSize - sizeof(t_PACKETHEADER));
                } else {
                    g_LOG.CS_ODS(0xffff,
                        "ERROR !!!! :: Undefined Account server packet... Type[ 0x%x ], Size[ %d "
                        "]\n",
...
#ifdef __SHO_LS_LIB
                this->Send_IntializedMessage((char*)"roseON_LS", 1, 1, 1, 3, 10);
#else
                this->Send_IntializedMessage((char*)"roseON_AS", 1, 2, 1, 3, 10);
#endif
            } else {
