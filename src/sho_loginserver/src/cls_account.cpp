...
CLS_AccountLIST::CLS_AccountLIST(char* szListName, int iHashTableSIZE): m_csListACCOUNT(4000) {
    if (NULL == m_pAccountPOOL) {
        m_pAccountPOOL = new CDataPOOL<CLS_Account>((char*)"CLS_AccountPOOL", 0x1ffff, 0xfff);
...
//-------------------------------------------------------------------------------------------------
bool
CLS_AccountLIST::Insert_ACCOUNT(CLS_Account* pCAccount, CLS_Server* pWorldServer) {
    bool bRet;
    ...
    m_csListACCOUNT.Lock();
    {
        ...
        if (bRet) {
            ...
            // m_pHTLSID->GetCount(), m_pHTAcnt->GetCount(), pCAccount->m_Account.Get() );
        }
    }
...
//-------------------------------------------------------------------------------------------------
CLS_Account*
CLS_AccountLIST::Search_ACCOUNT(DWORD dwLSID) {
...
//-------------------------------------------------------------------------------------------------
CLS_Account*
CLS_AccountLIST::Search_ACCOUNT(char* szAccount) {
    tagHASH<CLS_Account*>* pHashNode;
...
CLS_Account*
CLS_AccountLIST::Remove_FromLIST(DWORD dwLSID) {
    CLS_Account* pCAccount;
...
void
CLS_AccountLIST::Delete_ACCOUNT(CLS_Account* pCAccount) {
    if (pCAccount->m_HashACCOUNT) {
        m_csListACCOUNT.Lock();
        {
...
void
CLS_AccountLIST::Delete_IdleACCOUNT(int iTimeOutSecond) {
