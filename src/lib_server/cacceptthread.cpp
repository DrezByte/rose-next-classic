...
CAcceptTHREAD::Init(int iTCPPort, int iKeepAliveSec) {
    ...
    /*
        ...
        if ( setsockopt(m_ListenSocket, SOL_SOCKET, SO_KEEPALIVE, (char*)&bOptVal, bOptLen) ==
       SOCKET_ERROR) { assert("error:: Set SO_KEEPALIVE: ON");
        }
        if ( getsockopt(m_ListenSocket, SOL_SOCKET, SO_KEEPALIVE, (char*)&iOptVal, &iOptLen) ==
       SOCKET_ERROR) { assert("error:: Get SO_KEEPALIVE: ON");
        }

        //*KeepAliveTime = milliseconds
        //	Data Type: DWORD
        //	For Windows 98, the Data Type is a String Value.
        //	Specifies the connection idle time in milliseconds before TCP will begin sending
       keepalives, if keepalives are enabled on a connection. The default is 2 hours (7,200,000).

        //*KeepAliveInterval = 32-bit number
        //	Data Type: DWORD
        //	For Windows 98, the Data Type is a String value.
        //	Specifies the time in milliseconds between retransmissions of keepalives, once the
       KeepAliveTime has expired. Once KeepAliveTime has expired, keepalives are sent every
       KeepAliveInterval milliseconds until a response is received, up to a maximum of
       MaxDataRetries before the connection is terminated. The default is 1 second (1000).
       ...
       adaptive and will vary according to link conditions. The default is 5.
        ...
        #define IOCP_KEEPALIVE_INTERVAL		3000	//응답이 없을시 다시 보낼 시간 간격 3초(기본 5회
       시도후 짤름)
        ...
        DWORD			dwBytesRecvd;
        ::WSAIoctl( m_ListenSocket, SIO_KEEPALIVE_VALS, &KeepAlive, sizeof( KeepAlive ), 0, 0,
