...
public :
    ...
    static  iocpSOCKET* GetSocket (int iSocketIDX)			{	return
m_pInstance->m_pClientIDX->GetData(iSocketIDX);	}
    static	DWORD		GetMaxSocket ()						{	return m_pInstance->m_dwMaxSocket;
}

    static  int			AddClientSocket (iocpSOCKET *pSOCKET, SOCKET hSocket, char *szIP, t_HASHKEY
ipHashKEY); static	iocpSOCKET* DelClientSocket (int iSocketIDX); } ;
*/
