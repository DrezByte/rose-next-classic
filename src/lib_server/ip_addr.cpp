...
bool
CIPv4Addr::Add(char* szIPFrom, char* szIPTo) {
    in_addr iaFrom, iaTo;
...
CIPv4Addr::Find(in_addr& inADDR) {
    ...
    while (pNODE) {
        if (inREVS.S_un.S_addr >= pNODE->DATA.m_ulMIN && inREVS.S_un.S_addr <= pNODE->DATA.m_ulMAX)
            return true;
