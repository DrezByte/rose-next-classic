...
public:
    /*
        ...
        CStrVAR		m_DescSTR;
    */
    char* MakeItemDESC(CStrVAR* pStrVAR,
        tagITEM* pITEM,
        int iItmePrice,
        int iDupCnt,
        int iMoneyAmout,
        DWORD dwTradeMoney,
        int iTradeCnt);
    char* GetItemINFO(tagITEM* pITEM,
        int iItmePrice = CHECK_ITEM_LOG_PRICE,
        int iDupCnt = CHECK_ITEM_LOG_CNT,
        int iMoneyAmout = CHECK_ITEM_MONEY,
        DWORD dwTradeMoney = 0,
        int iTradeCnt = 0);
    /*
        GS_CThreadLOG ();
...
public:
    /*
        ...
        char	   *GetCurDateTimeSTR()
        {
            ...
            sprintf( m_szDateTime, "%04d-%02d-%02d %02d:%02d:%02d", m_locTime.wYear,
       m_locTime.wMonth, m_locTime.wDay, m_locTime.wHour, m_locTime.wMinute, m_locTime.wSecond);
            return m_szDateTime;
        }
    ...
    */
    ...
    bool When_TagItemLOG(int iAction,
        classUSER* pSourAVT,
        tagITEM* pItem,
        short nQuantity = 1,
        __int64 biTradeZuly = 0,
        classUSER* pDestAVT = NULL,
        bool bForce = false);
    ...
    // 팔거나 주기
    bool When_NpcTRADE(classUSER* pSourAVT,
        tagITEM* pITEM,
        CObjCHAR* pDestCHR,
        char* szLogType,
        int iPriceTOT,
        int iDupCnt);
    bool
    When_P2PTRADE(classUSER* pSourAVT, tagITEM* pITEM, classUSER* pDestAVT, DWORD dwTradeMoney);

    bool
    When_GiveITEM(classUSER* pSourAVT, tagITEM* pITEM, classUSER* pDestAVT, short nTradeSlotNO);
    bool
    When_RecvITEM(classUSER* pSourAVT, tagITEM* pITEM, classUSER* pDestAVT, short nTradeSlotNO);
    ...
    bool When_CreateOrDestroyITEM(classUSER* pSourAVT,
        tagITEM* pOutItem,
        tagITEM* pUseItem,
        short nUseCNT,
        BYTE btMakeOrBreak,
    ...
    // 재밍 관련
    bool
    When_UpgradeITEM(classUSER* pSourAVT, tagITEM* pEquipITEM, BYTE btBeforeGrade, BYTE btResult);
    bool When_GemmingITEM(classUSER* pSourAVT,
        tagITEM* pEquipITEM,
        tagITEM* pJewelITEM,
        BYTE btGemming,
        BYTE btSuccess);
