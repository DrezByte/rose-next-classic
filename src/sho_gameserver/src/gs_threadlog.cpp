...
bool GS_CThreadLOG::When_ERROR ( classUSER *pUSER, char *szFile, int iLine, char *szDesc )
{
    ...
    {
        // 시간, 케릭, 동작, 위치, IP
        this->m_QuerySTR.Printf("INSERT tblGS_ERROR ( dateREG, txtIP, txtACCOUNT, txtCHAR, txtFILE,
intLINE, txtDESC ) VALUES( \'%s\',\'%s\',\'%s\',\'%s\',\'%s\',%d,\'%s\' );",
...
#ifdef __NEW_LOG
bool
GS_CThreadLOG::When_LogInOrOut(classUSER* pUSER, BYTE btType) {
    m_csSQL.Lock();
    {
        {
...
#ifndef __NEW_LOG
bool
GS_CThreadLOG::When_LogIN(classUSER* pUSER) {
    m_csSQL.Lock();
    {
        this->m_ItemSTR.Printf("Channel:%d, Money: %I64d, LEV: %d, EXP: %d, BP: %d, SP: %d",
...
bool
GS_CThreadLOG::When_LogOUT(classUSER* pUSER) {
    m_csSQL.Lock();
    {
        SYSTEMTIME stIn;
        ...
        this->m_ItemSTR.Printf("Channel:%d, LogIn Time[%d-%d-%d,%d:%d:%d], Money: %I64d, LEV:%d, "
                               "EXP: %d, BP: %d, SP: %d",
            CLIB_GameSRV::GetInstance()->GetChannelNO(),
            stIn.wYear,
            stIn.wMonth,
...
txtITEM ) VALUES( \'%s\',\'%s\',\'%s\',\'%s\',\'%s\' );", this->GetCurDateTimeSTR(), szCharNAME,
                pSourAVT->Get_IP(),
...
*/
char*
GS_CThreadLOG::MakeItemDESC(CStrVAR* pStrVAR,
    tagITEM* pITEM,
    int iItmePrice,
    int iDupCnt,
    int iMoneyAmout,
    ...
    if (pITEM->IsEnableDupCNT()) {
        ...
        pStrVAR->Printf("[%d:%d]%s : %dEA, TradeZ: %d",
            pITEM->GetTYPE(),
            pITEM->GetItemNO(),
            ITEM_NAME(pITEM->GetTYPE(), pITEM->GetItemNO()),
            iTradeCnt ? iTradeCnt : pITEM->GetQuantity(),
            dwTradeMoney);
    } else if (ITEM_TYPE_MONEY == pITEM->GetTYPE()) {
        // 돈
        ...
        if (dwTradeMoney >= CHECK_TRADE_MONEY_AMOUNT
            || ITEM_BASE_PRICE(pITEM->GetTYPE(), pITEM->GetItemNO())
                >= iItmePrice) // 비싼 일반 장비.
        {
...
char*
GS_CThreadLOG::GetItemINFO(tagITEM* pITEM,
    int iItmePrice,
    int iDupCnt,
    int iMoneyAmout,
    DWORD dwTradeMoney,
...
#ifndef __NEW_LOG
bool
GS_CThreadLOG::When_NpcTRADE(classUSER* pSourAVT,
    tagITEM* pITEM,
    CObjCHAR* pDestCHR,
    char* szLogType,
    int iPriceTOT,
    ...
    m_csSQL.Lock();
    {
        if (this->GetItemINFO(pITEM, 10000, 100, 10000, iPriceTOT, iDupCnt)) {
            this->m_DescSTR.Printf("Money: %I64d, LEV: %d, EXP: %d, PriceTOT:%d",
                pSourAVT->Get_MONEY(),
                pSourAVT->Get_LEVEL(),
                pSourAVT->Get_EXP(),
                iPriceTOT);
...
bool
GS_CThreadLOG::When_P2PTRADE(classUSER* pSourAVT,
    tagITEM* pITEM,
    classUSER* pDestAVT,
    DWORD dwTradeMoney) {
    ...
    m_csSQL.Lock();
    {
        if (this->GetItemINFO(pITEM, 3000, 5, 5000, dwTradeMoney)) {
            this->m_DescSTR.Printf("%s sells %I32uZULY, SM: %I64d, DM: %I64d",
                pSourAVT->Get_NAME(),
                dwTradeMoney,
                pSourAVT->Get_MONEY(),
                pDestAVT->Get_MONEY());
...
bool
GS_CThreadLOG::When_GiveITEM(classUSER* pSourAVT,
    tagITEM* pITEM,
    classUSER* pDestAVT,
    short nTradeSlotNO) {
    // pSour가 pDest에게 줬다.
    m_csSQL.Lock();
    {
        if (this->GetItemINFO(pITEM, 3000, 5, 5000)) {
            this->m_DescSTR.Printf("Slot:%d, BeforeZULY: S:%I64d / O:%I64d",
                nTradeSlotNO,
                pSourAVT->Get_MONEY(),
                pDestAVT->Get_MONEY());
...
bool
GS_CThreadLOG::When_RecvITEM(classUSER* pSourAVT,
    tagITEM* pITEM,
    classUSER* pDestAVT,
    short nTradeSlotNO) {
...
bool
GS_CThreadLOG::When_DEPOSIT(classUSER* pSourAVT, tagITEM* pITEM) {
    m_csSQL.Lock();
    {
...
bool
GS_CThreadLOG::When_WithDRAW(classUSER* pSourAVT, tagITEM* pITEM) {
    m_csSQL.Lock();
    {
        if (this->GetItemINFO(pITEM, 3000, 5)) {
            this->m_QuerySTR.Printf(SP_M_DEFLOG,
...
#ifdef __NEW_LOG
bool
GS_CThreadLOG::When_CreateOrDestroyITEM(classUSER* pSourAVT,
    tagITEM* pOutItem,
    tagITEM* pUseItem,
    short nUseCNT,
    BYTE btMakeOrBreak,
    ...
    // if ( this->GetItemINFO( pITEM, 1000, 1, 1000 ) )
    {
        ...
        if (pOutItem) {
            ...
            this->m_QuerySTR.Printf(SP_CreateLog, // [AddCreateLog]
                ...
                nUseCNT > 0 ? ITEM_NAME(pUseItem[0].GetTYPE(), pUseItem[0].GetItemNO())
                            : "none", // @Stuff1 varchar(24),
                nUseCNT > 1 ? ITEM_NAME(pUseItem[1].GetTYPE(), pUseItem[1].GetItemNO())
                            : "none", // @Stuff2 varchar(24),
                nUseCNT > 2 ? ITEM_NAME(pUseItem[2].GetTYPE(), pUseItem[2].GetItemNO())
                            : "none", // @Stuff3 varchar(24),
                nUseCNT > 3 ? ITEM_NAME(pUseItem[3].GetTYPE(), pUseItem[3].GetItemNO())
                            : "none", // @Stuff4 varchar(24),
...
// 제조시 생성된 아이템
bool
GS_CThreadLOG::When_CreatedITEM(classUSER* pSourAVT, tagITEM* pITEM) {
    m_csSQL.Lock();
    {
        if (this->GetItemINFO(pITEM, 1000, 1, 1000)) {
            this->m_QuerySTR.Printf(SP_M_DEFLOG,
...
// 제조시 소모된 아이템
bool
GS_CThreadLOG::When_DestroyedITEM(classUSER* pSourAVT, tagITEM* pITEM) {
    m_csSQL.Lock();
    {
...
//-------------------------------------------------------------------------------------------------
bool
GS_CThreadLOG::When_DieBY(CObjCHAR* pKillOBJ, classUSER* pDeadAVT) {
    m_csSQL.Lock();
    {
#ifndef __NEW_LOG
        if (pKillOBJ->GetZONE()) {
            short nZoneNO = pKillOBJ->GetZONE()->Get_ZoneNO();
            this->m_ItemSTR.Printf("KillerPOS: %d:%s(%6.0f,%6.0f)",
                nZoneNO,
                ZONE_NAME(nZoneNO),
                pKillOBJ->m_PosCUR.x,
                pKillOBJ->m_PosCUR.y);
...
// 레벨업
bool
GS_CThreadLOG::When_LevelUP(classUSER* pSourAVT, int iGetEXP) {
    ...
    m_csSQL.Lock();
    {
        {
#ifndef __NEW_LOG
            this->m_ItemSTR.Printf(
                "LEVEL UP to %d, CurEXP: %d, GetEXP: %d, Money: %I64d, BP: %d, SP: %d",
                pSourAVT->Get_LEVEL(),
                pSourAVT->m_GrowAbility.m_lEXP,
                iGetEXP,
                pSourAVT->Get_MONEY(),
                pSourAVT->GetCur_BonusPOINT(),
                pSourAVT->GetCur_SkillPOINT());
            this->m_QuerySTR.Printf(SP_M_DEFLOG,
                pSourAVT->GetCur_MONEY() - pSourAVT->m_i64StartMoney,
                pSourAVT->Get_NAME(),
                pSourAVT->Get_IP(),
...
// 스킬...
bool
GS_CThreadLOG::When_LearnSKILL(classUSER* pSourAVT, short nSkillIDX) {
    m_csSQL.Lock();
    {
        {
#ifndef __NEW_LOG
            this->m_ItemSTR.Printf(
                "Skill Learn:%d[%s], SkillLEV: %d, Money: %I64d, LEV: %d, EXP: %d, BP: %d, SP: %d ",
                nSkillIDX,
                SKILL_NAME(nSkillIDX),
                SKILL_LEVEL(nSkillIDX),
                pSourAVT->Get_MONEY(),
                pSourAVT->Get_LEVEL(),
                pSourAVT->Get_EXP(),
                pSourAVT->GetCur_BonusPOINT(),
...
#ifndef __NEW_LOG
bool
GS_CThreadLOG::When_LevelUpSKILL(classUSER* pSourAVT, short nSkillIDX) {
    m_csSQL.Lock();
    {
        {
            this->m_ItemSTR.Printf("Skill LevelUP:%d[%s], SkillLEV: %d, Money: %I64d, LEV: %d, "
                                   "EXP: %d, BP: %d, SP: %d ",
                nSkillIDX,
                SKILL_NAME(nSkillIDX),
                SKILL_LEVEL(nSkillIDX),
...
bool
GS_CThreadLOG::When_QuestLOG(classUSER* pSourAVT, int iQuestIDX, BYTE btLogType) {
    m_csSQL.Lock();
    {
        {
...
// 퀘스트...
bool
GS_CThreadLOG::When_RecvQUEST(classUSER* pSourAVT, int iQuestIDX) {
    m_csSQL.Lock();
    {
...
bool
GS_CThreadLOG::When_DoneQUEST(classUSER* pSourAVT, int iQuestIDX) {
    m_csSQL.Lock();
    {
...
bool
GS_CThreadLOG::When_GiveUpQUEST(classUSER* pSourAVT, int iQuestIDX) {
    m_csSQL.Lock();
    {
...
bool
GS_CThreadLOG::When_ChangedQUEST(classUSER* pSourAVT, int iOldQuestIDX, int iNewQuestIDX) {
    m_csSQL.Lock();
    {
        this->m_ItemSTR.Printf("Quest_Change %d:%s=>%d:%s",
            iOldQuestIDX,
            QUEST_NAME(iOldQuestIDX),
            iNewQuestIDX,
            QUEST_NAME(iNewQuestIDX));
...
bool
GS_CThreadLOG::When_GemmingITEM(classUSER* pSourAVT,
    tagITEM* pEquipITEM,
    tagITEM* pJewelITEM,
    BYTE btGemming,
    BYTE btSuccess) {
    ...
    {
        {
            ...
            m_ItemSTR.Printf("[%d:%d]%s, G:%d,D:%d,L:%d, O:%d/%s S:%d:, A:%d",
                pEquipITEM->GetTYPE(),
                pEquipITEM->GetItemNO(),
                ITEM_NAME(pEquipITEM->GetTYPE(), pEquipITEM->GetItemNO()),
                pEquipITEM->GetGrade(),
                pEquipITEM->GetDurability(),
                pEquipITEM->GetLife(),
                pEquipITEM->GetGemNO(),
                GEMITEM_NAME(pEquipITEM->GetGemNO()),
                pEquipITEM->HasSocket(),
                pEquipITEM->IsAppraisal());

            m_DescSTR.Printf("[%d:%d]%s",
                pJewelITEM->GetTYPE(),
                pJewelITEM->GetItemNO(),
                ITEM_NAME(pJewelITEM->GetTYPE(), pJewelITEM->GetItemNO()));
...
#ifndef __NEW_LOG
bool
GS_CThreadLOG::When_UngemmingITEM(classUSER* pSourAVT, tagITEM* pEquipITEM, tagITEM* pJewelITEM) {
    m_csSQL.Lock();
    {
        m_ItemSTR.Printf("[%d:%d]%s, G:%d,D:%d,L:%d, O:%d/%s S:%d:, A:%d",
            pEquipITEM->GetTYPE(),
...
bool
GS_CThreadLOG::When_BreakupITEM(classUSER* pSourAVT, tagITEM* pITEM) {
    m_csSQL.Lock();
    {
...
bool
GS_CThreadLOG::When_UpgradeITEM(classUSER* pSourAVT,
    tagITEM* pEquipITEM,
    BYTE btBeforeGrade,
    BYTE btResult) {
    m_csSQL.Lock();
...
// 백업 로그.
bool
GS_CThreadLOG::When_BackUP(classUSER* pSourAVT, char* szBackUpType) {
    m_csSQL.Lock();
    {
        {
#ifndef __NEW_LOG
...
bool
GS_CThreadLOG::When_CheatCODE(classUSER* pSourAVT, char* szCode) {
    m_csSQL.Lock();
    {
        {
#ifndef __NEW_LOG
...
//-------------------------------------------------------------------------------------------------
bool
GS_CThreadLOG::When_ItemHACKING(classUSER* pSourAVT, tagITEM* pHackITEM, char* szLogType) {
    m_csSQL.Lock();
    {
        {
...
*/
bool
GS_CThreadLOG::When_gs_CLAN(classUSER* pSourAVT, char* szLogType, BYTE btSucType) {
    m_csSQL.Lock();
    {
        {
#ifndef __NEW_LOG
...
#ifdef __NEW_LOG
bool
GS_CThreadLOG::Check_ItemLogFilter(tagITEM* pITEM, short nQuantity, __int64 biZuly) {
    if (pITEM->IsEnableDupCNT()) {
        // 소모/기타
        if (biZuly < CHECK_TRADE_MONEY_AMOUNT
            && ITEM_BASE_PRICE(pITEM->GetTYPE(), pITEM->GetItemNO()) * nQuantity
                < CHECK_TRADE_MONEY_AMOUNT) {
        ...
        // if ( pITEM->IsCreated() ||	// 제조된 아이템이냐 ???
        if (biZuly >= CHECK_TRADE_MONEY_AMOUNT
            || biTotPrice >= CHECK_TRADE_MONEY_AMOUNT) { // 비싼 장비.
            ;
        } else
            return false;
    }
...
bool
GS_CThreadLOG::When_TagItemLOG(int iAction,
    classUSER* pSourAVT,
    tagITEM* pItem,
    short nQuantity,
    __int64 biTradeZuly,
    ...
    {
        {
            ...
            if (pItem) {
                ...
                if (!bForce && !this->Check_ItemLogFilter(pItem, nQuantity, biTradeZuly)) {
                    // 돈일 경우는 작은 금액이니까 여기 들어왔고 무조건 return. 안 그러면
                    // ITEM_USE_RESTRICTION에서 뻑남
                    ...
                    // 로그 기준 미달...
                    /*
                    1회용 폭죽이나, 1회용 모션 아이템 말고, 다른 마일리지 아이템은
                    3번칼럼(사용제한)이 6 6일경우 교환은 안되도 계정창고에 저장은 가능하게 따라서
                    6일경우에 마일리지 아이템으로 판단 */
                    // if ( pItem != 마일리지 아이템 )
...
bool
GS_CThreadLOG::When_ObjItemLOG(int iAction, classUSER* pSourAVT, CObjITEM* pItemOBJ) {
    m_csSQL.Lock();
    {
        {
            char* szZoneName = ZONE_NAME(pSourAVT->m_nZoneNO);
...
bool
GS_CThreadLOG::When_DeletedITEM(CObjITEM* pItemOBJ) {
    // 사용자가 버린 필드 아이템 사라짐.
    m_csSQL.Lock();
    {
#ifndef __NEW_LOG
        if (this->GetItemINFO(&pItemOBJ->m_ITEM)) {
            this->m_QuerySTR.Printf(
                "INSERT tblGS_LOG ( dateREG, txtACTION, txtLOC, txtITEM, txtSUBJECT, txtSBJIP ) "
                "VALUES( \'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\' );",
                this->GetCurDateTimeSTR(),
...
#else
        {
            ...
            if (ITEM_TYPE_MONEY == pItemOBJ->m_ITEM.GetTYPE()) {
                biTradeZuly = pItemOBJ->m_ITEM.GetMoney();
                nQuantity = 0;
            } else if (pItemOBJ->m_ITEM.IsEnableDupCNT()) {
                biTradeZuly = 0;
            ...
            if (ITEM_TYPE_MONEY != pItemOBJ->m_ITEM.GetTYPE()) {
                ...
                this->m_QuerySTR.Printf(SP_AddItemLog, // [AddItemLog]
                    ...
                    m_ItemSTR.Get(), // @ItemID varchar(10),
                    ITEM_NAME(pItemOBJ->m_ITEM.GetTYPE(),
...
#ifndef __NEW_LOG
bool
GS_CThreadLOG::When_PickITEM(classUSER* pSourAVT, CObjITEM* pItemOBJ) {
    m_csSQL.Lock();
    {
        if (this->GetItemINFO(&pItemOBJ->m_ITEM)) {
            this->m_QuerySTR.Printf(SP_M_OBJLOG,
...
bool
GS_CThreadLOG::When_DropITEM(classUSER* pSourAVT, CObjITEM* pItemOBJ) {
    m_csSQL.Lock();
    {
        if (this->GetItemINFO(&pItemOBJ->m_ITEM)) {
            this->m_QuerySTR.Printf(SP_M_DEFLOG,
...
bool
GS_CThreadLOG::When_ChangeABILITY(classUSER* pSourAVT, BYTE btAbilityTYPE, short nUsedBPOINT) {
#ifdef __NEW_LOG_2005_05
    m_csSQL.Lock();
    {
        // SP_M_CHAGEABILITY "{call AddChangeAbility(%d, \'%s\', \'%s\', %d, %d, \'%s\', \'%s\', %d,
        // %d)}"
