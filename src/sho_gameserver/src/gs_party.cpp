...
//-------------------------------------------------------------------------------------------------
bool
CPartyBUFF::CreatePARTY(classUSER* pOwner) {
...
void
CPartyBUFF::DeletePARTY(CParty* pCParty) {
        ...
        pCParty->Lock();
        {
            ...
            for (short nI = 0; nI < pCParty->GetMemberCNT(); nI++) {
                if (NULL == pCParty->m_pPartyUSERs[nI]) {
                    g_pThreadLOG->When_SysERR(
                        __FILE__, nI, "DeletePARTY:: NULL == pCParty->m_pPartyUSERs[ nI ]");
                    continue;
                }

                if (pCParty->m_pPartyUSERs[nI]->m_pUSER) {
                    pCParty->m_pPartyUSERs[nI]->m_pUSER->m_pPartyBUFF = NULL;
                    pCParty->m_pPartyUSERs[nI]->m_pUSER->Send_gsv_PARTY_REPLY(
                        0, PARTY_REPLY_DESTROY);
                }
...
void
CParty::OnConnect(classUSER* pUSER) {
    this->Lock();
    {
#ifdef __USE_ARRAY_PARTY_USER
        for (short nI = 0; nI < this->m_nMemberCNT; nI++) {
            if (NULL == this->m_pPartyUSERs[nI]) {
                g_pThreadLOG->When_SysERR(
                    __FILE__, nI, "OnConnect:: NULL == this->m_pPartyUSERs[ nI ]");
                continue;
            }
...
//-------------------------------------------------------------------------------------------------
void
CParty::OnDisconnect(classUSER* pUSER) {
    ...
    if (0 == pUSER->m_nPartyPOS) { // 파티 오너가 짤렸네...
        for (short nI = 1; nI < this->m_nMemberCNT; nI++) {
            if (NULL == this->m_pPartyUSERs[nI]) {
                g_pThreadLOG->When_SysERR(
                    __FILE__, nI, "OnDisconnect:: NULL == this->m_pPartyUSERs[ nI ]");
                continue;
            }

            if (this->m_pPartyUSERs[nI]->m_pUSER) {
                this->Change_PartyOWNER(
                    this->m_pPartyUSERs[nI]->m_pUSER, PARTY_REPLY_CHANGE_OWNERnDISCONN);
                return;
            }
        }
...
//-------------------------------------------------------------------------------------------------
void
CParty::Member_LevelUP(short nPartyPOS, short nLevelDIFF) {
    this->Lock();
    {
...
//-------------------------------------------------------------------------------------------------
bool
CParty::Kick_MEMBER(DWORD dwDBID) {
    this->Lock();
    {
#ifdef __USE_ARRAY_PARTY_USER
        for (short nI = 1; nI < this->m_nMemberCNT; nI++) {
            if (NULL == this->m_pPartyUSERs[nI]) {
                g_pThreadLOG->When_SysERR(
                    __FILE__, nI, "Kick_MEMBER:: NULL == this->m_pPartyUSERs[ nI ]");
                continue;
            }
...
//-------------------------------------------------------------------------------------------------
bool
CParty::SendWhisperToPartyMembers(char* szMessage) {
...
//-------------------------------------------------------------------------------------------------
bool
CParty::SendToPartyMembers(classPACKET* pCPacket) {
    this->Lock();
    {
#ifdef __USE_ARRAY_PARTY_USER
        for (short nI = 0; nI < this->m_nMemberCNT; nI++) {
            if (NULL == this->m_pPartyUSERs[nI]) {
                g_pThreadLOG->When_SysERR(
                    __FILE__, nI, "SendToPartyMembers:: NULL == this->m_pPartyUSERs[ nI ]");
                continue;
            }
...
CParty::LevelUP() {
    this->Lock();
    {
        ...
        for (short nI = 0; nI < this->m_nMemberCNT; nI++) {
            if (NULL == this->m_pPartyUSERs[nI]) {
                g_pThreadLOG->When_SysERR(
                    __FILE__, nI, "LevelUP:: NULL == this->m_pPartyUSERs[ nI ]");
                continue;
            }
...
//-------------------------------------------------------------------------------------------------
bool
CParty::SendToPartyMembersExceptME(classUSER* pSender, classPACKET* pCPacket) {
    this->Lock();
    {
#ifdef __USE_ARRAY_PARTY_USER
        for (short nI = 0; nI < this->m_nMemberCNT; nI++) {
...
//-------------------------------------------------------------------------------------------------
bool
CParty::SendPartyLEVnEXP(classUSER* pUSER, BYTE bitLevelUP) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return false;
...
CParty::AddEXP(CObjMOB* pMobCHAR, int iTotalEXP, int iPartyExpA, int iPartyExpB) {
    ...
    this->Lock();
    {
        ...
        for (short nI = 0; nI < this->m_nMemberCNT; nI++) {
            if (NULL == this->m_pPartyUSERs[nI]) {
                g_pThreadLOG->When_SysERR(
                    __FILE__, nI, "AddEXP:: NULL == this->m_pPartyUSERs[ nI ]");
                continue;
            }

            if (m_pPartyUSERs[nI]->m_pUSER
                && pMobCHAR->GetZONE() == m_pPartyUSERs[nI]->m_pUSER->GetZONE()) {
    ...
    if (iNearUSER > 1 && bFindOwner && m_btPartyLEV < MAX_PARTY_LEVEL) {
        ...
        // 일정 간격 감소되는 경험치.
        iCalEXP = (m_btPartyLEV / 2 + m_nMemberCNT) * (dwCurTIME - this->m_dwLastKillTIME)
            / PARTY_EXP_UPDATE_TIME;
        ...
        if (iDiffLEV >= 0) {
            // 얻는 파티 경험치 = (몹레벨-파티평균 레벨 + A) * (몹레벨-파티평균 레벨 + A + 1) * B/10
            // + 2 iCalEXP = ( iDiffLEV + 2 ) * ( iDiffLEV + 1 ) + 2;
...
//-------------------------------------------------------------------------------------------------
bool
CParty::Change_ObjectIDX(classUSER* pUSER) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return false;
...
//-------------------------------------------------------------------------------------------------
bool
CParty::Change_PartyOWNER(classUSER* pNewOWNER, BYTE btReply) {
    this->Lock();
    {
#ifdef __USE_ARRAY_PARTY_USER
...
//-------------------------------------------------------------------------------------------------
bool
CParty::Send_PartyUserLIST(classUSER* pUSER, DWORD dwUserTAG) {
    ...
    this->Lock();
    {
        ...
        for (short nI = 0; nI < this->m_nMemberCNT; nI++) {
            if (NULL == this->m_pPartyUSERs[nI]) {
                // 이런 경우는 뭐냐 ???
                g_pThreadLOG->When_SysERR(
                    __FILE__, nI, "Send_PartyUserLIST:: NULL == pCParty->m_pPartyUSERs[ nI ]");
                continue;
            }
...
//-------------------------------------------------------------------------------------------------
bool
CParty::Sub_PartyUSER(short nPartyPOS) {
    DWORD dwLeftUserID;
    DWORD dwOwnerID;
    ...
    if (m_nMemberCNT <= 1) {
        ...
        pCPacket->m_gsv_PARTY_MEMBER.m_dwObjectTAG[1] =
            dwOwnerID; // pOwnerNODE->m_VALUE.m_pUSER->m_dwDBID;
    ...
    this->Lock();
    {
        ...
        for (short nI = 0; nI < this->m_nMemberCNT; nI++) {
            if (NULL == this->m_pPartyUSERs[nI]) {
                // 이런 경우는 뭐냐 ???
                g_pThreadLOG->When_SysERR(
                    __FILE__, nI, "Share_MONEY:: NULL == pCParty->m_pPartyUSERs[ nI ]");
                continue;
            }
            if (this->m_pPartyUSERs[nI]
                    ->m_pUSER) { // 0x0008bce5 오류... NULL == this->m_pPartyUSERs[ nI ] ???
...
//-------------------------------------------------------------------------------------------------
bool
CParty::Send_gsv_PARTY_ITEM(classUSER* pPicker, CObjITEM* pItemOBJ) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return false;
...
bool
CParty::Give_Item2ORDER(classUSER* pTryOBJ, CObjITEM* pItemOBJ) {
    this->Lock();
    {
...
//-------------------------------------------------------------------------------------------------
bool
CParty::Send_gsv_PARTY_REPLY2Members(BYTE btReply, DWORD dwFromIDXorTAG) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        false;
...
//-------------------------------------------------------------------------------------------------
bool
CParty::Send_gsv_CHECK_NPC_EVENT(classUSER* pTryOBJ, short nNpcIDX) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return false;
...
//-------------------------------------------------------------------------------------------------
void
CParty::Set_PartyRULE(BYTE btRULE) {
...
//-------------------------------------------------------------------------------------------------
void
CParty::Warp_USERS(int iZoneNO, tPOINTF& PosGOTO) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return;
