...
public:
    ...
    int GetANI_Stop() {
        return (m_btRideMODE == RIDE_MODE_DRIVE) ? (m_nPatAniROW + PAT_ANI_STOP1) : AVT_ANI_STOP1;
    }
    int GetANI_Move() {
        if (m_btRideMODE) {
    ...
    int GetANI_Attack() {
        return (m_btRideMODE == RIDE_MODE_DRIVE) ? (m_nPatAniROW + PAT_ANI_ATTACK) : AVT_ANI_ATTACK;
    }
    int GetANI_Die() {
        return (m_btRideMODE == RIDE_MODE_DRIVE) ? (m_nPatAniROW + PAT_ANI_DIE) : AVT_ANI_DIE;
    }
    int GetANI_Hit() {
...
#define RIDE_ATTR_CART_SUB 0x08
    ...
    //  void    SetPartITEM (short nPartIdx, short nItemNo) {   m_nPartItemIDX[ nPartIdx ] =
    //  nItemNo;		}
    ...
    virtual void UpdateAbility() {
        if (this->GetCur_MOVE_MODE() == MOVE_MODE_DRIVE) {
            /*
            m_nPatAniROW = PAT_RELATIVE_MOTION_POS(	m_nRideItemIDX[ RIDE_PART_BODY ] );	// 몸체에서
            모션 줄 번호 m_nPatAniCOL = PAT_RELATIVE_MOTION_POS( m_nRideItemIDX[ RIDE_PART_ARMS ] );
            // 무기에서 모션 열 번호
            */
            m_nPatAniROW = PAT_RELATIVE_MOTION_POS(
                m_RideITEM[RIDE_PART_BODY].m_nItemNo); // 몸체에서 모션 줄 번호
            m_nPatAniCOL = PAT_RELATIVE_MOTION_POS(
                m_RideITEM[RIDE_PART_ARMS].m_nItemNo); // 무기에서 모션 열 번호
        }
    ...
    bool Is_CartDriver() {
        return (RIDE_MODE_DRIVE == this->GetCur_RIDE_MODE() && this->m_iLinkedCartObjIDX);
    }
