...
CObjAVT::Set_TargetIDX(int iTargetIndex, bool bCheckHP) {
    ...
    if (Get_TargetIDX()) {
        pTarget = (CObjCHAR*)this->Get_TargetOBJ(
            true); // HP 체크된 CAI_OBJ를 상속받은 CObjCHAR가 넘어 온다.
        if (pTarget) {
    ...
    if (Get_TargetIDX()) {
        pTarget = (CObjCHAR*)this->Get_TargetOBJ(
            bCheckHP); // HP 체크된 CAI_OBJ를 상속받은 CObjCHAR가 넘어 온다.
        if (pTarget) {
...
CObjAVT::Save_Damage(int iAttackerIDX, int iDamage) {
    ...
    if (this->Get_MoveMODE() == MOVE_MODE_DRIVE) {
        if (iRand >= 51)
            nInvIDX = INVENTORY_RIDE_ITEM0 + RIDE_PART_BODY; // 바디
        else if (iRand >= 21)
            nInvIDX = INVENTORY_RIDE_ITEM0 + RIDE_PART_LEG; // 레그
        else
        // if ( iRand >= 16 )
        {
        ...
        if (pItem->GetLife()
            ...
            // 방패 있음
            if (iRand > 60)
                nInvIDX = EQUIP_IDX_WEAPON_L; // 방패, 왼손무기
            else if (iRand > 30)
                nInvIDX = EQUIP_IDX_ARMOR; // 갑옷
            else if (iRand > 22)
                nInvIDX = EQUIP_IDX_BOOTS; // 신발
            else if (iRand > 16)
                nInvIDX = EQUIP_IDX_HELMET; // 머리
            else if (iRand > 9)
                nInvIDX = EQUIP_IDX_KNAPSACK; // 등
            else if (iRand > 5)
                nInvIDX = EQUIP_IDX_GAUNTLET; // 장갑
            else
                nInvIDX = EQUIP_IDX_FACE_ITEM; // 얼굴(악세사리)
        } else {
            // 방패 없음
            if (iRand > 64)
                nInvIDX = EQUIP_IDX_ARMOR; // 갑옷
            ...
            if (m_dwRecoverTIME >= dwCheckTime) {
                ...
                // if ( this->GetCur_STAMINA() >= YELLOW_STAMINA )
                {
                    ...
                    if (CMD_SIT == Get_COMMAND()) {
                        //[앉기 상태]
                        // RECOVER_HP= (MAX_HP)/12+1+ ITEM_RECOVER_HP (1이하 버림) 대만 2005.6.16
                        // kchs
                        ...
                        // RECOVER_MP= (MAX_MP)/12+1+ ITEM_RECOVER_MP (1이하 버림) 대만 2005.6.19
                        // kchs
                        ...
                        // RECOVER_MP= (MAX_MP)/35+1+ ITEM_RECOVER_MP (1이하 버림) 대만 2005.6.19
                        // kchs
...
CObjAVT::Send_gsv_PATSTATE_CHAGE(BYTE btOnOff, DWORD dwCoolTIME) {
    ...
    if (!btOnOff) {
        ...
        this->Send_gsv_TOGGLE(TOGGLE_TYPE_DRIVE + this->Get_MoveMODE(),
            true); // 클라에서는 이게 필요하다.. 이거 없으면 클라에서 속도 못 맞춘다.
