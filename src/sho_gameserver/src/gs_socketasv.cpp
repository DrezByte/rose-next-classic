...
bool
GSASV_Socket::WndPROC(WPARAM wParam, LPARAM lParam) {
...
//-------------------------------------------------------------------------------------------------
void
GS_asvSOCKET::Init(HWND hWND, char* szLoginServerIP, int iLoginServerPort, UINT uiSocketMSG) {
    m_hMainWND = hWND;
    ...
    if (NULL == m_pReconnectTimer)
        m_pReconnectTimer =
            new CTimer(m_hMainWND, GS_TIMER_ASV, RECONNECT_TIME_TICK, (TIMERPROC)GS_TimerProc);
}
...
//-------------------------------------------------------------------------------------------------
bool
GS_asvSOCKET::Send_zws_ADD_ACCOUNT(char* szAccount, char* szMD5Pass, char* szIP) {
...
//-------------------------------------------------------------------------------------------------
void
GS_asvSOCKET::Send_zas_SUB_ACCOUNT(char* szAccount, BYTE btWhy, short nLevel, short nZoneNo) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return;
...
//-------------------------------------------------------------------------------------------------
void
GS_asvSOCKET::Send_zas_CHECK_ACCOUNT(BYTE btType, char* szAccount) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return;
...
//-------------------------------------------------------------------------------------------------
bool
GS_asvSOCKET::Recv_zas_KICK_ACCOUNT() {
...
bool
GS_asvSOCKET::Recv_zas_MESSAGE_EXT() {
    ...
    if (pUser) {
        ...
        pUser->Send_gsv_BILLING_MESSAGE_EXT(m_pRecvPket->m_zas_MESSAGE_EXT.m_wMsgTYPE,
...
bool
GS_asvSOCKET::Recv_zas_CHECK_ACCOUNT() {
...
//-------------------------------------------------------------------------------------------------
bool
GS_asvSOCKET::Send_cli_ALIVE(char* szAccount) {
    // GUMS에 alive 패킷 전송.
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return false;
...
bool
GS_asvSOCKET::Send_zas_SERVER_TYPE(BYTE btType) {
    classPACKET* pCPacket = Packet_AllocNLock();
    if (!pCPacket)
        return false;
...
//-------------------------------------------------------------------------------------------------
bool
GS_asvSOCKET::Proc_SocketMSG(WPARAM wParam, LPARAM lParam) {
        ...
        case FD_READ: {
            ...
            while (m_SockASV.Peek_Packet(m_pRecvPket, true)) {
                ...
                switch (m_pRecvPket->m_HEADER.m_wType) {
                    ...
                    default:
                        g_LOG.CS_ODS(0xffff,
                            "Undefined Account server packet... Type[ 0x%x ], Size[ %d ]\n",
