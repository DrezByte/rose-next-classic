...
void
CGameOBJ::ResetSECTOR(short nSectorSize) {
...
int
CGameOBJ::GetCur_WorldTIME(void) {
    return ::Get_WorldTIME();
}

//-------------------------------------------------------------------------------------------------
bool
CGameOBJ::Make_gsv_SUB_OBJECT(classPACKET* pCPacket) {
    if (!Get_INDEX()) {
        assert(0);
        return false;
    }
