...
//-------------------------------------------------------------------------------------------------
void
CZoneLIST::Set_WorldVAR(short nVarIDX, short nValue) {
...
//-------------------------------------------------------------------------------------------------
bool
CZoneLIST::InitZoneLIST(char* szBaseDIR) {
#ifdef __STRESS_TEST
    return true;
#endif
    ...
    for (nZ = 1; nZ < m_nZoneCNT; nZ++) {
        ...
        else if (this->m_pSectorSIZE[nZ] > MAX_SECTOR_SIZE)
...
CZoneLIST::Add_EventPOS(t_HASHKEY HashKEY,
    ...
    float fXPos,
    float fYPos) {
    tagEVENTPOS* pEventPos = new tagEVENTPOS;
...
tagEVENTPOS*
CZoneLIST::Get_EventPOS(short nZoneNO, char* szPosName) {
...
CZoneTHREAD*
CZoneLIST::GetZONE(short nZoneNO) {
...
CObjMOB*
CZoneLIST::RegenCharacter(short nZoneNO,
    float fCenterX,
    float fCenterY,
    int iRange,
    int iIndex,
...
void
CZoneLIST::Send_gsv_ANNOUNCE_CHAT(CObjVAR* pObjVAR, char* szMessage) {
    CGameOBJ* pObj = (CGameOBJ*)pObjVAR;
    if (pObj->GetZONE()) {
        this->Send_gsv_ANNOUNCE_CHAT(pObj->GetZONE()->Get_ZoneNO(),
            szMessage,
            /* pObj->Get_NAME() */ "SERVER");
    }
}

void
CZoneLIST::Send_gsv_ANNOUNCE_CHAT(short nZoneNO, char* szMessage, char* szAccount) {
...
//-------------------------------------------------------------------------------------------------
void
CZoneLIST::Send_gsv_ANNOUNCE_CHAT(char* szMessage, char* szAccount) {
    if (NULL == szMessage)
        return;
...
//-------------------------------------------------------------------------------------------------
void
CZoneLIST::Send_gsv_ANNOUNCE_CHAT(t_PACKET* pSendPacket) {
...
//-------------------------------------------------------------------------------------------------
void
CZoneLIST::Set_PK_FLAG(short nZoneNO, BYTE btOnOff) {
    CZoneTHREAD* pZONE = this->GetZONE(nZoneNO);
    if (pZONE)
...
//-------------------------------------------------------------------------------------------------
bool
CZoneLIST::Do_QuestTrigger(short nZoneNO, short nTeamNO, t_HASHKEY HashKEY) {
    CZoneTHREAD* pZONE = this->GetZONE(nZoneNO);
    if (pZONE) {
...
//-------------------------------------------------------------------------------------------------
void
CZoneLIST::Toggle_RegenSYSTEM(CGameOBJ* pObj) {
    if (pObj && pObj->GetZONE()) {
        pObj->GetZONE()->Toggle_REGEN();
    }
}
void
CZoneLIST::Set_RegenSYSTEM(CGameOBJ* pObj, BYTE btOnOff) {
    if (pObj && pObj->GetZONE()) {
...
//-------------------------------------------------------------------------------------------------
void
CZoneLIST::Toggle_RegenSYSTEM(CAI_OBJ* pChar) {
    CObjCHAR* pSourCHAR = static_cast<CObjCHAR*>(pChar);
    if (pSourCHAR) {
...
void
CZoneLIST::Set_RegenSYSTEM(CAI_OBJ* pChar, BYTE btOnOff) {
    CObjCHAR* pSourCHAR = static_cast<CObjCHAR*>(pChar);
    if (pSourCHAR) {
...
void
CZoneLIST::Toggle_RegenSYSTEM(short nZoneNO) {
    CZoneTHREAD* pZONE = this->GetZONE(nZoneNO);
    if (pZONE)
        pZONE->Toggle_REGEN();
}
void
CZoneLIST::Set_RegenSYSTEM(short nZoneNO, BYTE btOnOff) {
    CZoneTHREAD* pZONE = this->GetZONE(nZoneNO);
    if (pZONE)
