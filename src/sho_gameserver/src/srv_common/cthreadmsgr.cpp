...
CThreadMSGR::LogIN(tagMSGR_CMD* pCMD) {
    ...
    if (!this->db->GetNextRECORD()) {
        // insert !!!
        if (this->db->ExecSQL("INSERT tblWS_FRIEND ( intCharID ) VALUES(%d);", pCMD->m_dwDBID)
            < 1) {
...
CThreadMSGR::LogOUT(CMessenger* pMSGR) {
    ...
    if (pMSGR->MSGR_IsUPDATE()) {
        if (iFriendCNT > 0) {
            ...
            this->db->MakeQuery("UPDATE tblWS_FRIEND SET blobFRIENDS=",
                MQ_PARAM_BINDIDX,
                1,
                MQ_PARAM_ADDSTR,
                ",intFriendCNT=",
            ...
            this->db->MakeQuery("UPDATE tblWS_FRIEND SET blobFRIENDS=NULL",
                MQ_PARAM_ADDSTR,
                ",intFriendCNT=",
                MQ_PARAM_INT,
                iFriendCNT,
    ...
    while (pNODE) {
        ...
        if (pFindMSGR) {
            if (pFR->m_btSTATUS == FRIEND_STATUS_OFFLINE) {
                // 접속해 있는데, 나에게 상태를 등록안했다는것은 나를 삭제했거나, 나를
                // 삭제했다는것...
        ...
        /*
           ...
           pNODE->m_VALUE.m_btSTATUS, pNODE->m_VALUE.m_dwDBID, pNODE->m_VALUE.m_Name.Get() );
                ...
                if ( pFindMSGR && pFR->m_btSTATUS == FRIEND_STATUS_OFFLINE ) {
                    // 접속해 있는데, 나에게 상태를 등록안했다는것은 나를 삭제했거나, 나를
           삭제했다는것... pFR->m_btSTATUS = FRIEND_STATUS_DELETED;
                }
        */
...
CMessenger::MSGR_Status2ONE(DWORD dwDBID, BYTE btNewStatus, CMessenger* pMessenger) {
    ...
    while (pNODE) {
        if (dwDBID == pNODE->m_VALUE.m_dwDBID) { // 04. 10. 11 : 0x0013bfe오류...
            ...
            if (!(FRIEND_STATUS_REFUSED & pNODE->m_VALUE.m_btSTATUS)) {
                // 내가 거부한 상태거나, 넘이 나를 삭제한 상태가 아니면??? (나를 삭제한 넘이 나한테
                // 보내-삭제할 당시)
