    ...
    if (MAX_CLAN_SKILL_SLOT == this->FindClanSKILL(nSkillNo, nSkillNo)) {
        ...
        for (short nI = 0; nI < MAX_CLAN_SKILL_SLOT; nI++) {
            if (0 == ClanBIN.m_SKILL[nI].m_nSkillIDX) {
                ...
                // skill log...
                g_pThreadLOG->When_ws_CLAN("Clan master",
                    "?",
                    "?",
                    this,
...
CClan::DelClanSKILL(short nSkillNo) {
    ...
    for (short nI = 0; nI < MAX_CLAN_SKILL_SLOT; nI++) {
        if (ClanBIN.m_SKILL[nI].m_nSkillIDX == nSkillNo) {
            ...
            // skill log...
            g_pThreadLOG
                ->When_ws_CLAN("Clan master", "?", "?", this, NEWLOG_CLAN_DEL_SKILL, nSkillNo);
...
CThreadGUILD::Test_add(char* pGuildName, char* pGuildDesc) {
    ...
    if (this->db->QuerySQL("{?=call ws_ClanCREATE(\'%s\',\'%s\',%d,%d)}",
            pGuildName,
            pGuildDesc,
            1,
            2)) {
...
CThreadGUILD::Test_del(char* pGuildName) {
    ...
    this->db->SetParam_long(1, iResultSP, cbSize1);
    if (this->db->QuerySQL("{?=call ws_ClanDELETE(\'%s\')}", pGuildName)) {
        DWORD dwClanID = 0;
        while (this->db->GetNextRECORD()) {
            // 성공...
            dwClanID = (DWORD)this->db->GetInteger(0);
...
CThreadGUILD::Load_CLAN(DWORD dwClanID) {
    if (this->db->QuerySQL("{call ws_ClanSELECT(%d)}", dwClanID)) {
        if (this->db->GetNextRECORD()) {
            ...
            // 모든 클랜멤버 얻기...
            if (this->db->QuerySQL("{call ws_ClanCharALL(%d)}", dwClanID)) {
                int iClanPos;
                CDLList<CClanUSER>::tagNODE* pNode;
                char* pCharName;
                while (this->db->GetNextRECORD()) {
...
CThreadGUILD::Query_CreateCLAN(int iSocketIDX, t_PACKET* pPacket) {
    ...
    this->db->SetParam_long(1, iResultSP, cbSize1);
    if (this->db->QuerySQL("{?=call ws_ClanINSERT(\'%s\',\'%s\',%d,%d)}",
            pGuildName,
            pGuildDesc,
...
CThreadGUILD::Query_DeleteCLAN(char* szClanName) {
    ...
    this->db->SetParam_long(1, iResultSP, cbSize1);
    if (this->db->QuerySQL("{?=call ws_ClanDELETE(\'%s\')}", szClanName)) {
        DWORD dwClanID = 0;
        // while( this->db->GetNextRECORD() ) {
        //	// 성공...
        //	dwClanID = (DWORD)this->db->GetInteger(0);
...
CThreadGUILD::Query_InsertClanMember(char* szCharName, DWORD dwClanID, int iClanPos) {
    ...
    this->db->SetParam_long(1, iResultSP, cbSize1);
    if (this->db->QuerySQL("{?=call ws_ClanCharADD(\'%s\',%d,%d)}",
            szCharName,
            dwClanID,
            iClanPos)) {
        while (this->db->GetMoreRESULT()) {
        ...
        switch (iResultSP) {
            ...
            case -1: // 중복된 이름
                LogString(0xffff,
                    "duplicated clan user name : ClanID:%d / %s \n",
                    dwClanID,
                    szCharName);
                break;
            case -2: // insert 오류( 디비 오류 )
...
CThreadGUILD::Query_DeleteClanMember(char* szCharName) {
    ...
    this->db->SetParam_long(1, iResultSP, cbSize1);
    if (this->db->QuerySQL("{?=call ws_ClanCharDEL(\'%s\')}", szCharName)) {
        DWORD dwClanID = 0;
        while (this->db->GetNextRECORD()) {
            dwClanID = (DWORD)this->db->GetInteger(0);
        }
        ...
        switch (iResultSP) {
            case 0: // 성공
                LogString(0xffff,
                    "Delete clan user success : dwCladID::%d / %s\n",
                    dwClanID,
                    szCharName);
                return true;
            default: // 실패
...
CThreadGUILD::Query_AdjustClanMember(char* szCharName, int iAdjContr, int iAdjPos) {
    ...
    this->db->SetParam_long(1, iResultSP, cbSize1);
    if (this->db->QuerySQL("{?=call ws_ClanCharADJ(\'%s\',%d,%d)}",
            szCharName,
            iAdjContr,
            iAdjPos)) {
        while (this->db->GetMoreRESULT()) {
...
CThreadGUILD::Query_UpdateClanMOTD(DWORD dwClanID, char* szMessage) {
    ...
    this->db->SetParam_long(1, iResultSP, cbSize1);
    if (this->db->QuerySQL("{?=call ws_ClanMOTD(%d,\'%s\')}", dwClanID, szMessage)) {
        while (this->db->GetMoreRESULT()) {
            ;
        }
        switch (iResultSP) {
...
CThreadGUILD::Query_UpdateClanSLOGAN(DWORD dwClanID, char* szMessage) {
    ...
    this->db->SetParam_long(1, iResultSP, cbSize1);
    if (this->db->QuerySQL("{?=call ws_ClanSLOGAN(%d,\'%s\')}", dwClanID, szMessage)) {
        while (this->db->GetMoreRESULT()) {
            ;
        }
        switch (iResultSP) {
...
CThreadGUILD::Query_LoginClanMember(char* szCharName, int iSenderSockIDX) {
    if (this->db->QuerySQL("{call ws_ClanCharGET(\'%s\')}", szCharName)) {
        if (this->db->GetNextRECORD()) {
            // 클랜 있다.
    ...
    t_PACKET* pPacket) // BYTE btAdjType, int iAdjValue)
{
    ...
    this->db->SetParam_long(1, iResultSP, cbSize1);
    if (this->db->QuerySQL("{?=call ws_ClanUPDATE(%d,\'%s\',%d)}",
...
CThreadGUILD::Query_UpdateClanBINARY(DWORD dwClanID, BYTE* pDATA, unsigned int uiSize) {
    ...
    if (this->db->QuerySQL("{?=call ws_ClanBinUPDATE(%d,?)}", dwClanID)) {
        while (this->db->GetMoreRESULT()) {
            ;
        }
        switch (iResultSP) {
...
CThreadGUILD::Query_UpdateClanMARK(CClan* pClan, WORD wMarkCRC, BYTE* pDATA, unsigned int uiSize) {
    ...
    if (this->db->QuerySQL("{?=call ws_ClanMarkUPDATE(%d,%d,%d,?)}",
            pClan->m_dwClanID,
            (short)(wMarkCRC),
            uiSize)) {
        while (this->db->GetMoreRESULT()) {
        ...
        switch (iResultSP) {
            case 1: // 성공
                ...
                if (this->db->QuerySQL("select dateMarkREG from tblWS_CLAN where intID=%d",
                        pClan->m_dwClanID)) {
                    if (this->db->GetNextRECORD()) {
                        sqlTIMESTAMP sTimeStamp;
