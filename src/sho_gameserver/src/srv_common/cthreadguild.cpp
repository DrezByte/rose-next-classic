...
CThreadGUILD::Test_add(char* pGuildName, char* pGuildDesc) {
    ...
    if (this->m_pSQL->QuerySQL("{?=call ws_ClanCREATE(\'%s\',\'%s\',%d,%d)}",
            pGuildName,
            pGuildDesc,
            1,
            2)) {
...
CThreadGUILD::Query_InsertClanMember(char* szCharName, DWORD dwClanID, int iClanPos) {
    ...
    this->m_pSQL->SetParam_long(1, iResultSP, cbSize1);
    if (this->m_pSQL->QuerySQL("{?=call ws_ClanCharADD(\'%s\',%d,%d)}",
            szCharName,
            dwClanID,
            iClanPos)) {
        while (this->m_pSQL->GetMoreRESULT()) {
...
CThreadGUILD::Query_AdjustClanMember(char* szCharName, int iAdjContr, int iAdjPos) {
    ...
    this->m_pSQL->SetParam_long(1, iResultSP, cbSize1);
    if (this->m_pSQL->QuerySQL("{?=call ws_ClanCharADJ(\'%s\',%d,%d)}",
            szCharName,
            iAdjContr,
            iAdjPos)) {
        while (this->m_pSQL->GetMoreRESULT()) {
    ...
    if (this->m_pSQL->QuerySQL("{?=call ws_ClanMarkUPDATE(%d,%d,%d,?)}",
        ...
        switch (iResultSP) {
            case 1: // 성공
                ...
                if (this->m_pSQL->QuerySQL("select dateMarkREG from tblWS_CLAN where intID=%d",
                        pClan->m_dwClanID)) {
                    if (this->m_pSQL->GetNextRECORD()) {
                        sqlTIMESTAMP sTimeStamp;
