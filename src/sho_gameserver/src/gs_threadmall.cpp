...
//-------------------------------------------------------------------------------------------------
bool
GS_CThreadMALL::Add_SqlPacketWithACCOUNT(classUSER* pUSER, t_PACKET* pPacket) {
    return CSqlTHREAD::Add_SqlPACKET(
        (int)pUSER->m_iSocketIDX, pUSER->Get_ACCOUNT(), (BYTE*)pPacket, pPacket->m_HEADER.m_nSize);
}
bool
GS_CThreadMALL::Add_SqlPacketWithAVATAR(classUSER* pUSER, t_PACKET* pPacket) {
    return CSqlTHREAD::Add_SqlPACKET(
...
GS_CThreadMALL::Execute() {
    ...
    LOG_DEBUG(
        "<  < << GS_CThreadMALL::Execute() ThreadID: %d(0x%x)\n", this->ThreadID, this->ThreadID);
}

//-------------------------------------------------------------------------------------------------
bool
GS_CThreadMALL::Run_SqlPACKET(tagQueryDATA* pSqlPACKET) {
...
//-------------------------------------------------------------------------------------------------
bool
GS_CThreadMALL::Give_MallITEM(tagQueryDATA* pSqlPACKET, BYTE btMallInvIDX, short nDupCnt) {
    ...
    if (this->m_pSQL->QuerySQL(
            "{?=call GiveITEM(%I64d,\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',%d)}",
...
bool
GS_CThreadMALL::Bring_MallITEM(tagQueryDATA* pSqlPACKET, BYTE btMallInvIDX, short nDupCnt) {
    classUSER* pUSER = (classUSER*)g_pUserLIST->GetSOCKET(pSqlPACKET->m_iTAG);
    if (NULL == pUSER)
        return true;
    ...
    SDWORD cbSize1 = SQL_NTS;
    this->m_pSQL->SetParam_long(1, iResultSP, cbSize1);
    if (this->m_pSQL->QuerySQL("{?=call PickOutITEM(\'%s\',\'%s\',\'%s\',\'%s\',%I64d,%d)}",
            pUSER->Get_ACCOUNT(),
            pUSER->Get_NAME(),
            pUSER->Get_IP(),
            CLIB_GameSRV::GetInstance()->GetServerIP(),
        ...
        if (iResultSP) {
            ...
            if (nUserInvIDX <= 0) {
                // 30분 * 2 * 10 -> 10시간
                pUSER->Save_ItemToFILED(sOutItem,
                    ITEM_OBJ_DROP_TIME * 2
                        * 10); // 몰 구입후 인벤토리가 추가가 불가능해서 밖에 30분간 보관...
            }
        } else {
            // failed
            nUserInvIDX = -1;
        ...
        if (pCPacket) {
            ...
            if (iResultSP) {
                pCPacket->m_HEADER.m_nSize =
                    sizeof(gsv_MALL_ITEM_REPLY) + sizeof(tag_SET_INVITEM) * 2;
                pCPacket->m_gsv_MALL_ITEM_REPLY.m_btReplyTYPE = REPLY_MALL_ITEM_BRING_SUCCESS;

                // 사용자 인벤...
                pCPacket->m_gsv_MALL_ITEM_REPLY.m_BringITEM[0].m_btInvIDX =
                    static_cast<BYTE>(nUserInvIDX);
                ...
                pCPacket->m_gsv_MALL_ITEM_REPLY.m_BringITEM[1].m_ITEM =
                    pUSER->m_MALL.m_ITEMS[btMallInvIDX].m_ITEM;
            } else {
...
bool
GS_CThreadMALL::List_MallITEM(int iScoketIDX, char* szAccount) {
    __int64 biMallID;
    short nItemTYPE, nItemNO, nDupCNT;
    ...
    //__int64 biStartID=0;
    // if ( this->m_pSQL->QuerySQL( "{call GetItemLIST(\'%s\',%I64d)}", pSqlPACKET->m_Name.Get(),
    // biStartID ) ) {
