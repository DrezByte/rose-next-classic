...
#ifdef __SERVER
bool
CRegenPOINT::Load(FILE* fp) {
#else
bool
CRegenPOINT::Load(CFileSystem* pFileSystem) {
    if (pFileSystem == NULL)
        return false;
#endif
...
void
CRegenPOINT::RegenCharacter(int iIndex, int iCount) {
    ...
    #if defined(_DEBUG)
    g_itMGR.AppendChatMsg(CStr::Printf("      %s  %d마리 생성", NPC_NAME(iIndex), iCount),
...
//-------------------------------------------------------------------------------------------------
void
CRegenPOINT::ClearCharacter(CObjCHAR* pCharOBJ) {
#ifdef __VIRTUAL_SERVER
...
//-------------------------------------------------------------------------------------------------
void
CRegenPOINT::Proc(DWORD dwCurTIME) {
    /*
    ...
    */
    ...
    if (iVar <= 10) {
        ...
        if (m_iTacticsCNT > SLOT0)
            this->RegenCharacter(m_pTacticsMOB[SLOT0].m_iMobIDX,
                m_pTacticsMOB[SLOT0].m_iMobCNT - 1);
