...
CUserDATA::Cal_MaxHP() {
    ...
    switch (this->GetCur_JOB()) {
        ...
        case CLASS_MAGICIAN_211: // 뮤즈(211)		[(LV+4)*SQRT(LV+26)*2.36+(STR*2)] + 아이템
                                 // 증가치
            iA = 4, iM1 = 26, fC = 2.36f;
            break;
        case CLASS_MAGICIAN_221: // 매지션(221)		[(LV+5)*SQRT(LV+26)*2.37+(STR*2)] + 아이템
                                 // 증가치
        ...
        // case CLASS_VISITOR :		// 무직	비지터(00)	[(LV+4)*SQRT(LV+26)*2.36+(STR*2)] + 아이템
        // 증가치
        default:
            iA = 4, iM1 = 26, fC = 2.36f;
            break;
        ...
        if (ShotTYPE < MAX_SHOT_TYPE) {
            ...
            switch (WEAPON_TYPE(pRightWPN->m_nItemNo) / 10) {
                ...
                case 25: // 카타르
                    ...
                    break;
                case 0: // 스킬맨손 공격력 : 무도가 직업에 특한된 스킬로, 패시브 성격의 맨손
                        // 공격력을 제공한다.
    ...
    } else // PAT 탑승 모드
    {
            ...
            switch (ShotTYPE) {
                case SHOT_TYPE_ARROW:
                    iAP = ((GetCur_DEX() * 0.52f + GetCur_STR() * 0.1f + GetCur_LEVEL() * 0.1f + 3)
                              + iWeaponTERM * (GetCur_DEX() * 0.04f + GetCur_SENSE() * 0.03f + 29)
                                  / 60.f)
                        * 1.03f;
                    break;
                case SHOT_TYPE_BULLET:
                    iAP =
                        ((GetCur_DEX() * 0.3f + GetCur_CON() * 0.47 + GetCur_LEVEL() * 0.1f + 6.4f)
                            + iWeaponTERM * (GetCur_CON() * 0.04f + GetCur_SENSE() * 0.05f + 29)
                                / 60.f)
                        * 1.032f;
                    break;

                case SHOT_TYPE_THROW:
                    iAP = ((GetCur_STR() * 0.32f + GetCur_CON() * 0.45f + GetCur_LEVEL() * 0.1f + 8)
                              + iWeaponTERM * (GetCur_CON() * 0.04f + GetCur_SENSE() * 0.05f + 29)
                                  / 60.f)
                        * 1.033f;
                    break;
        ...
        } else // 타이완 근거리 무기
        {
            switch (WEAPON_TYPE(pRightWPN->m_nItemNo) / 10) {
                case 21: // 한손
                case 22: // 양손		// 근접 무기
                    iAP = ((GetCur_STR() * 0.75f + GetCur_LEVEL() * 0.2f)
                              + iWeaponTERM * (GetCur_STR() * 0.05f + 29) / 60.f)
                        * 1.033f;
                    break;
                case 24: // 마법 무기
                {
                    if (WEAPON_TYPE(pRightWPN->m_nItemNo) == 241) {
                        // 마법 지팡이
                        iAP = ((GetCur_STR() * 0.4f + GetCur_INT() * 0.4f + GetCur_LEVEL() * 0.2f)
                                  + iWeaponTERM * (GetCur_INT() * 0.05f + 29) / 60.f)
                            * 1.03f;
                        break;
                    } else { // 마법도구
                        iAP = ((GetCur_INT() * 0.6f + GetCur_LEVEL() * 0.2f)
                                  + iWeaponTERM * (GetCur_SENSE() * 0.1f + 26) / 54.f)
                            * 1.025f;
                    }
                } break;
                case 25:
                    if (252 == WEAPON_TYPE(pRightWPN->m_nItemNo)) { // 이도류
                        iAP = ((GetCur_STR() * 0.63f + GetCur_DEX() * 0.45f + GetCur_LEVEL() * 0.2f)
                                  + iWeaponTERM * (GetCur_DEX() * 0.05f + 25) / 52.f)
                            * 1.032f;
                    } else { // 카타르
                        iAP = ((GetCur_STR() * 0.42f + GetCur_DEX() * 0.55f + GetCur_LEVEL() * 0.2f)
                                  + iWeaponTERM * (GetCur_DEX() * 0.05f + 20) / 42.f)
                            * 1.032f;
                    }
                    break;
                case 0:
...
CUserDATA::Skill_ToUseAbilityVALUE(short nSkillIDX, short nPropertyIDX) {
    ...
    if (AT_PATHP == SKILL_USE_PROPERTY(nSkillIDX, nPropertyIDX)) {
        if (GetCur_PatHP() < iValue && GetCur_PatHP() > 0)
            return GetCur_PatHP(); // __KCHS_BATTLECART__ 이렇게 하지 않으면 카트 체력은 영원히 0이
                                   // 안 될수 있고. 그래서 쿨타임 절대로 안 돌아간다.
    }
#endif
...
CUserDATA::Skill_GetAbilityValue(short nAbilityType) {
    switch (nAbilityType) {
        ...
        case AT_HP:
            return (GetCur_HP() - 1); //// 실제보다 1작게...피인 경우 소모시 다빼면 뒈진다..
        case AT_MP:
            return GetCur_MP();
        case AT_EXP:
            return GetCur_EXP();
