...
public:
    ...
    int GetUserLimitCNT() { return s_iUserLimit[m_nClanLEVEL]; }
    int GetPositionLimitCNT(int iPos) {
        int iLimitCnt = GetUserLimitCNT();
        switch (iPos) {
            case GPOS_RENDER:
                iLimitCnt = (int)(m_nClanLEVEL * 1.8f) + 2;
                break;
            case GPOS_COMMANDER:
                iLimitCnt = m_nClanLEVEL + 2;
                break;
            case GPOS_SUB_MASTER:
                iLimitCnt = (int)(m_nClanLEVEL * 0.3f) + 1;
                break;
            case GPOS_MASTER:
                iLimitCnt = 1;
                break;
    ...
    bool Insert_MEMBER(BYTE btResult,
        CWS_Client* pMember,
        int iClanPos = GPOS_JUNIOR,
        char* szMaster = NULL);
    bool
    Delete_MEMBER(t_HASHKEY HashCommander, char* szCharName, BYTE btCMD, char* szKicker = NULL);
    bool Update_MEMBER();
    bool Adjust_MEMBER(t_HASHKEY HashCommander,
        char* szCharName,
        int iAdjPoint,
        int iAdjPos,
        char* szMaster,
