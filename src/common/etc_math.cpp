...
//-------------------------------------------------------------------------------------------------
float
distance(tPOINTF& From, tPOINTF& To) {
    tPOINTF from_to;
...
float
distance_square(tPOINTF& From, tPOINTF& To) {
    tPOINTF from_to;
    from_to.x = To.x - From.x;
    from_to.y = To.y - From.y;
...
//-------------------------------------------------------------------------------------------------
void
quat_2_euler_ogl(const quat& q, float& yaw, float& pitch, float& roll) {
    float sqw = q.w * q.w;
    float sqx = q.x * q.x;
    float sqy = q.y * q.y;
    float sqz = q.z * q.z;
    ...
    /*
        ...
        roll  = atan2f(2.0f * (q.w*q.y - q.x*q.z),(-sqx - sqy + sqz + sqw)); // rotation about
       y-axis yaw   = atan2f(2.0f * (q.w*q.z - q.x*q.y),(-sqx + sqy - sqz + sqw)); // rotation about
       z-axis
    */
...
float
quaternionToModelDirection(float fQuatW, float fQuatX, float fQuatY, float fQuatZ) {
