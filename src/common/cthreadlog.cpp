...
//-------------------------------------------------------------------------------------------------
bool
CThreadLOG::When_SysERR(char* szFile, int iLine, char* szDesc) {
    /*
    txtIP
    txtACCOUNT
    txtCHAR
...
//-------------------------------------------------------------------------------------------------
bool
CThreadLOG::When_ERROR(classUSER* pUSER, char* szFile, int iLine, char* szDesc) {
    /*
    txtIP
    txtACCOUNT
    txtCHAR
...
bool CThreadLOG::When_LogIN ( classUSER *pUSER )
{
    ...
    {
        // 시간, 케릭, 동작, 위치, IP
        this->m_QuerySTR.Printf("INSERT tblGS_LOG ( dateREG, txtSUBJECT, txtOBJECT, txtSBJIP,
txtACTION, txtLOC ) VALUES( \'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\' );",
                this->GetCurDateTimeSTR(),
                pUSER->Get_NAME(),
                pUSER->Get_ACCOUNT(),
                pUSER->Get_IP(),
...
bool CThreadLOG::When_LogOUT ( classUSER *pUSER )
{
    m_csSQL.Lock ();
    {
        ...
        this->m_ItemSTR.Printf("LogIn Time[%d-%d-%d,%d:%d:%d]", stIn.wYear, stIn.wMonth, stIn.wDay,
stIn.wHour, stIn.wMinute, stIn.wSecond );

        if ( 0 == pUSER->m_nZoneNO ) {
            m_PosSTR.Printf( "lobby :: Account: %s", pUSER->Get_ACCOUNT() );
            this->m_QuerySTR.Printf("INSERT tblGS_LOG ( dateREG, txtSUBJECT, txtOBJECT, txtSBJIP,
txtACTION, txtLOC, txtITEM ) VALUES( \'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\' );",
                    this->GetCurDateTimeSTR(),
                    "not selected",
                    pUSER->Get_ACCOUNT(),
                    pUSER->Get_IP(),
...
#ifdef __NEW_LOG
bool
CThreadLOG::When_CharacterLOG(classUSER* pSourAVT, char* szCharNAME, BYTE btDelOrNew) {
    /*
        [AddCharacterLog]
...
#else
bool
CThreadLOG::When_CreateCHAR(classUSER* pSourAVT, char* szCharNAME) {
    m_csSQL.Lock();
    {
        this->m_QuerySTR.Printf("INSERT tblGS_LOG ( dateREG, txtSUBJECT, txtSBJIP, txtACTION, "
                                "txtITEM ) VALUES( \'%s\',\'%s\',\'%s\',\'%s\',\'%s\' );",
            this->GetCurDateTimeSTR(),
            szCharNAME,
            pSourAVT->Get_IP(),
...
bool
CThreadLOG::When_DeleteCHAR(classUSER* pSourAVT, char* szCharNAME) {
    m_csSQL.Lock();
    {
        this->m_QuerySTR.Printf("INSERT tblGS_LOG ( dateREG, txtSUBJECT, txtSBJIP, txtACTION, "
                                "txtITEM ) VALUES( \'%s\',\'%s\',\'%s\',\'%s\',\'%s\' );",
...
//-------------------------------------------------------------------------------------------------
bool
CThreadLOG::When_ws_CLAN(char* szName,
    char* szIP,
    char* szLogType,
    CClan* pClan,
    BYTE btSucType,
...
CThreadLOG::CThreadLOG():
    CSqlTHREAD(true), m_PosSTR(128), m_ItemSTR(256), m_QuerySTR(1024), m_DescSTR(256) {
    m_bWaiting = false;
}
CThreadLOG::~CThreadLOG() {
    ;
...
void
CThreadLOG::Execute() {
    LOG_TRACE("CThreadLOG::Execute() ThreadID: %d(0x%x)", this->ThreadID, this->ThreadID);
