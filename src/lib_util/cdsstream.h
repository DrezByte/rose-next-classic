...
private:
    ...
    HRESULT CreateStreamBuffer(LPDIRECTSOUND pDirectSound,
        WAVEFORMATEX* pWFX,
        int iBufferingSec = 8,
        short nNumPlayNotifications = 32);
...
public:
    ...
    // nNumPlayNotifications	:: 전체 버퍼를 몇번에 걸처서 읽어들일 것인가 .. 디스크 액세스 횟수를
    // 결정.
    //
    bool OpenMP3StreamFile(char* szMP3File,
        LPDIRECTSOUND pDirectSound,
        int iBufferingSec,
        short nNumPlayNotifications,
        bool bAlreadyCoInitialized);
    bool OpenWAVStreamFile(char* szWAVFile,
        LPDIRECTSOUND pDirectSound,
        int iBufferingSec,
        short nNumPlayNotifications,
        long lFilePtr);
