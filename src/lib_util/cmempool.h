...
CMemPOOL<pDataType>::~CMemPOOL() {
    _ASSERT(m_NODEs.GetNodeCount()
        == 0); // 상속 받은 클레스의 파괴자에서 this->DeletePool()이 호출되어야 한다.
...
template<class pDataType>
bool
CMemPOOL<pDataType>::CreatePool(int iIncSize) {
    int iC;
...
template<class pDataType>
void
CMemPOOL<pDataType>::Pool_Free(pDataType pData) {
    this->FreeData(pData);
