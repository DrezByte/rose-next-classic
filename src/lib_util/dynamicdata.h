...
template<class dType>
bool
CDynamicDATA<dType>::_Init(short nMaxDataCount) {
    if (nMaxDataCount < 1)
        return false;
...
template<class dType>
void
CDynamicDATA<dType>::SetData(short nIndex, int iTag, dType DATA, ULONG lTime) {
...
template<class dType>
short
CDynamicDATA<dType>::GetData(int iTag, ULONG lTime) {
    short nI;
    ...
    for (nI = 1; nI < m_nMaxDataCount; nI++) {
        ...
        if (!m_nOldDataIndex
            || ((lTime - m_pITEM[nI].m_lTime) > (lTime - m_pITEM[m_nOldDataIndex].m_lTime))) {
            m_nOldDataIndex = nI;
        }
    }
