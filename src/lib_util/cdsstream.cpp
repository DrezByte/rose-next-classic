...
//-------------------------------------------------------------------------------------------------
bool
classDSOUNDSTREAM::OpenMP3StreamFile(char* szMP3File,
    LPDIRECTSOUND pDirectSound,
    int iBufferingSec,
    short nNumPlayNotifications,
    bool bAlreadyCoInitialized) {
    SAFE_DELETE(m_pClassMP3);
...
//-------------------------------------------------------------------------------------------------
bool
classDSOUNDSTREAM::OpenWAVStreamFile(char* szWAVFile,
    LPDIRECTSOUND pDirectSound,
    int iBufferingSec,
    short nNumPlayNotifications,
    long lFilePtr) {
...
//-------------------------------------------------------------------------------------------------
void
classDSOUNDSTREAM::SetVolume(long lVolume) {
    HRESULT hR;
    ...
    if (m_pDSBuffer) {
        ...
        switch (hR) {
            case DSERR_CONTROLUNAVAIL:
                CLOG::OutputString(LOG_NORMAL,
                    "DSStream::SetVolume return  DSERR_CONTROLUNAVAIL\n");
                break;
            case DSERR_GENERIC:
            ...
            case DSERR_PRIOLEVELNEEDED:
                CLOG::OutputString(LOG_NORMAL,
                    "DSStream::SetVolume return  DSERR_PRIOLEVELNEEDED\n");
                break;
            default:
...
//-------------------------------------------------------------------------------------------------
HRESULT
classDSOUNDSTREAM::CreateStreamBuffer(LPDIRECTSOUND pDirectSound,
    WAVEFORMATEX* pWFX,
    int iBufferingSec,
    short nNumPlayNotifications) {
    ...
    */
    ...
    //    hResult = m_pDSNotify->SetNotificationPositions( NUM_PLAY_NOTIFICATIONS + 1,
    //    m_sDSBPosNotify );
...
//-------------------------------------------------------------------------------------------------
HRESULT
classDSOUNDSTREAM::PlayStreamBuffer(bool bLooped) {
    HRESULT hResult;
    VOID* pbBuffer = NULL;
...
//-------------------------------------------------------------------------------------------------
void
classDSOUNDSTREAM::StopStreamBuffer(void) {
    if (NULL == m_pDSBuffer)
        return;
    ...
    if (FAILED(
            hr = m_pDSBuffer->Lock(0, m_dwBufferSize, &pbBuffer, &dwBufferLength, NULL, NULL, 0L)))
        return;
...
//-------------------------------------------------------------------------------------------------
HRESULT
classDSOUNDSTREAM::RestoreBuffers(void) {
    HRESULT hResult;
...
//-------------------------------------------------------------------------------------------------
HRESULT
classDSOUNDSTREAM::FillBuffer(void) {
    HRESULT hr;
    void* pbBuffer = NULL;
    DWORD dwBufferLength;
...
//-------------------------------------------------------------------------------------------------
HRESULT
classDSOUNDSTREAM::WriteToBuffer(void* pbBuffer, DWORD dwBufferLength) {
    HRESULT hr;
    ...
    if (!m_bFoundEnd) {
        ...
        if (m_bMP3Stream) {
            // MP3
            if (FAILED(hr = m_pClassMP3->ReadData(dwBufferLength,
                           (BYTE*)pbBuffer,
                           &nActualBytesWritten)))
                return hr;
            ...
            // WAV
            if (FAILED(hr = m_pClassWAV->ReadData(dwBufferLength,
                           (BYTE*)pbBuffer,
                           &nActualBytesWritten)))
                return hr;
...
//-------------------------------------------------------------------------------------------------
HRESULT
classDSOUNDSTREAM::UpdateProgress(void) {
    HRESULT hr;
    DWORD dwPlayPos;
    DWORD dwWritePos;
    DWORD dwPlayed;
...
//-------------------------------------------------------------------------------------------------
HRESULT
classDSOUNDSTREAM::HandleNotification(void) {
    HRESULT hr;
    VOID* pbBuffer = NULL;
    DWORD dwBufferLength;
