...
zz_mem::status() {
    LogString(LOG_NORMAL,
        "mem: status(): total(%d) = used(%d) + remained(%d). highest(%d)\n",
        _total,
        _used,
        _total - _used,
        _max);
...
void*
zz_mem::malloc(size_t size) {
    void* p;

    if (_used + size > _total) {
        LogString(LOG_NORMAL,
            "mem: cannot allocate memory(size:%d bytes out of total %d bytes)!\n",
            size,
            _total);
        return NULL;
    }
