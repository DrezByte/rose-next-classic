...
Socket_Error(char* szMsg) {
    ...
    switch (iErr) {
        case WSAEAFNOSUPPORT:
            g_LOG.OutputString(LOG_DEBUG_, "The specified address family is not supported.");
            break;
        case WSAEFAULT:
            g_LOG.OutputString(LOG_DEBUG_, "The lpWSAData is not a valid pointer.\n");
            g_LOG.OutputString(LOG_DEBUG_,
                "The buf or from parameters are not part of the user address space, or the fromlen "
        ...
        case WSAEINPROGRESS:
            ...
            break;
        case WSAEMFILE:
            g_LOG.OutputString(LOG_DEBUG_, "No more socket descriptors are available.");
            break;
        case WSAENETDOWN:
            g_LOG.OutputString(LOG_DEBUG_,
        ...
        case WSAEPROCLIM:
            g_LOG.OutputString(LOG_DEBUG_,
                "Limit on the number of tasks supported by the Windows Sockets implementation has "
                "been reached.");
            break;
        case WSAEPROTONOSUPPORT:
            g_LOG.OutputString(LOG_DEBUG_, "The specified protocol is not supported.");
            break;
        case WSAEPROTOTYPE:
            g_LOG.OutputString(LOG_DEBUG_,
        ...
        case WSAEINVAL:
            g_LOG.OutputString(LOG_DEBUG_,
                "The socket has not been bound with bind, or an unknown flag was specified, or "
        ...
        case WSAEISCONN:
            g_LOG.OutputString(LOG_DEBUG_,
                "The socket is connected. This function is not permitted with a connected socket, "
                "whether the socket is connection-oriented or connectionless. ");
            break;
        case WSAENETRESET:
            g_LOG.OutputString(LOG_DEBUG_,
                "The connection has been broken due to the keep-alive activity detecting a failure "
                "while the operation was in progress. ");
            break;
        case WSAENOTSOCK:
            g_LOG.OutputString(LOG_DEBUG_, "The descriptor is not a socket. ");
            break;
        case WSAEOPNOTSUPP:
            g_LOG.OutputString(LOG_DEBUG_,
                "MSG_OOB was specified, but the socket is not stream-style such as type "
                "SOCK_STREAM, OOB data is not supported in the communication domain associated "
                "with this socket, or the socket is unidirectional and supports only send "
                "operations. ");
            break;
        case WSAESHUTDOWN:
            g_LOG.OutputString(LOG_DEBUG_,
                "The socket has been shut down; it is not possible to recvfrom on a socket after "
        ...
        case WSAEMSGSIZE:
            g_LOG.OutputString(LOG_DEBUG_,
                "The	message was too large to fit into the specified buffer and was "
                "truncated. ");
            break;
        case WSAETIMEDOUT:
            g_LOG.OutputString(LOG_DEBUG_,
        ...
        case WSAECONNRESET:
            g_LOG.OutputString(LOG_DEBUG_,
                ...
                "close. The application should close the socket as it is no longer usable. On a "
                "UPD-datagram socket this error would indicate that a previous send operation "
                "resulted in an ICMP \"Port Unreachable\" message. ");
            break;
        default:; // LogString (LOG_DEBUG_, "Unknown socket iError. CODE[ 0x%X, %d ]\n", iErr,
                  // iErr);
    }
