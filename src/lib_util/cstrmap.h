...
public:
    ...
    KEY MakeKey(const char* szKeyName) { return ::StrToHashKey(szKeyName); }
...
template<typename T>
bool
CStrMAP<T>::GetFirst(KEY* pHashKey, T* pHashData) {
    for (int iL = 0; iL < m_iTableCount; iL++)
...
template<typename T>
int
CStrMAP<T>::GetEntryCount(int iEntryIDX) {
    int iCnt = 0;
...
template<typename T>
bool
CStrMAP<T>::Delete(KEY ulHashKey, T HashData) {
...
template<typename T>
T
CStrMAP<T>::Search(KEY ulHashKey, T HashData) {
    m_ulPos = (ulHashKey & 0xFFFF) % m_iTableCount;

    m_pNODE = m_pTALBE[m_ulPos]->m_NEXT;
    while (m_pNODE) {
        i(m_pNODE->m_DATA == HashData) { return m_pNODE->m_DATA; }
        m_pNODE = m_pNODE->m_NEXT;
    }
