...
//-------------------------------------------------------------------------------------------------
bool
classMP3::_Init(bool bAlreadyCoInitialized) {
    HRESULT hResult;
    ...
    hResult = ::CoCreateInstance(CLSID_AMMultiMediaStream,
        NULL,
        CLSCTX_INPROC_SERVER,
        IID_IAMMultiMediaStream,
        reinterpret_cast<void**>(&m_pAMStream));
    if (FAILED(hResult))
        return false;
...
//-------------------------------------------------------------------------------------------------
bool
classMP3::OpenFile(char* szMP3File, int iBufferingSec, int iBufferBlockCnt) {
    HRESULT hResult;
    WCHAR* pWszName;
    int iLength, iRet;
    DWORD dwBlockAlign;
    ...
    iRet = ::CoCreateInstance(CLSID_AMAudioData,
        NULL,
        CLSCTX_INPROC_SERVER,
        IID_IAudioData,
        (void**)&m_pAudioData);
...
//-------------------------------------------------------------------------------------------------
HRESULT
classMP3::ReadData(UINT uiSizeToRead, void* pBuffer, UINT* puiActualBytesWritten) {
    HRESULT hResult;
    DWORD dwLength;
