...
void
CSocketEVENT::Execute() {
    // Window Message Pumpper !!!
    MSG sMSG;
    BOOL bRet;

    g_LOG.CS_ODS(0xffff,
        ">  > >> CSocketEVENT::Execute() ThreadID: %d(0x%x)\n",
        this->ThreadID,
        this->ThreadID);

    // bRet == -1 then the function fails if hWnd is an invalid window handle or lpMsg is an invalid
    // pointer !!! bRet == 0 then MSG == WM_QUIT !!!
...
//-------------------------------------------------------------------------------------------------
LRESULT CALLBACK
CSockWNDProc(HWND hWnd, UINT uiMsg, WPARAM wParam, LPARAM lParam) {
...
CSocketWND::CSocketWND(HINSTANCE hInstance, UINT uiMaxSocket) {
    ...
    m_hWND = ::CreateWindow(szWndClassName, // 윈도우 클래스 이름
        "", // 윈도우 타이틀 제목
        0, // 윈도우 스타일
        CW_USEDEFAULT,
        CW_USEDEFAULT, // 위치 = 디폴트
        CW_USEDEFAULT,
        CW_USEDEFAULT, // 크기 = 디폴트
...
//-------------------------------------------------------------------------------------------------
void
CSocketWND::DelSocket(UINT uiSocketWndMSG) {
...
//-------------------------------------------------------------------------------------------------
bool
CSocketWND::AddSocket(CClientSOCKET* pSocket, UINT uiSocketWndMSG) {
    UINT uiSocketIDX = uiSocketWndMSG - WM_SOCKETWND_MSG;
    if (uiSocketIDX >= m_uiMaxSocket) {
        _ASSERT(0);
        return false;
