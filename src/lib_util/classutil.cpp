...
//-------------------------------------------------------------------------------------------------
char*
CUtil::GetTokenFirst(char* pStr, char* pDelimiters) {
    char* pToken;
...
char*
CUtil::GetTokenNext(char* pDelimiters) {
    char* pToken;
...
//-------------------------------------------------------------------------------------------------
bool
CUtil::Is_FileExist(char* szFileName) {
#ifdef __BORLANDC__
...
//  주민등록 번호 검증.
bool
CUtil::Check_PersonalNumber(char* szNumber) {
...
//  사업자 등록번호 검증.
bool
CUtil::Check_BussinessNumber(char* szNumber) {
...
//-------------------------------------------------------------------------------------------------
DWORD
CUtil::Is_InternetConnected(void) {
    struct {
        DWORD m_dwFlag;
        char* m_szDesc;
...
//-------------------------------------------------------------------------------------------------
bool
CUtil::Get_HostName(char* szRecvBuff, int iBuffLen) {
    return (SOCKET_ERROR != ::gethostname(szRecvBuff, iBuffLen));
}

bool
CUtil::Get_IPAddressFromHostName(const char* szHostName, char* szBuffer, short nBufferSize) {
    if (!szBuffer)
        return false;
...
//-------------------------------------------------------------------------------------------------
int
CUtil::ExtractFilePath(char* pFullFilePath, char* pOutResult, WORD wOutBuffLEN) {
    short nI, nLen;
    ...
    if (nI >= 0 && nI < nLen) {
        ...
        if (pOutResult) {
            strncpy_s(pOutResult,
                wOutBuffLEN - 1,
                pFullFilePath,
                nI >= wOutBuffLEN ? wOutBuffLEN - 1 : nI);
        }
...
int
CUtil::ExtractFileName(char* pResult, char* pFullPath) {
    short nI, nLen;
...
//-------------------------------------------------------------------------------------------------
char*
CUtil::GetLastErrorMsg(DWORD dwLastError) {
    DWORD dwReturn;
    LPVOID lpMsgBuf;
