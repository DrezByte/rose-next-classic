...
//---------------------------------------------------------------------------
DWORD
CProcess::FindThread(DWORD dwProcessID) {
    HANDLE hSnapshot;
        ...
        do {
            if (dwProcessID == sEntry32.th32OwnerProcessID) {
                ...
                sprintf_s(szTmp,
                    ...
                    ">>> %x == %x : %x ]]] \n",
                    dwProcessID,
                    sEntry32.th32OwnerProcessID,
                    sEntry32.th32ThreadID);
...
DWORD
CProcess::FindProcessWithCallBack(CALLBACK_FINDPROCESS CallBackFunc) {
    HANDLE hSnapshot;
...
DWORD
CProcess::FindProcess(char* szExeName) {
    HANDLE hSnapshot;
    PROCESSENTRY32 sEntry32 = {0};
    DWORD dwPID = 0;
    /*
...
BOOL
CProcess::KillProcess(DWORD dwPID) {
    HANDLE hProcess;

    hProcess = ::OpenProcess(PROCESS_TERMINATE | PROCESS_QUERY_INFORMATION /* PROCESS_ALL_ACCESS */,
        FALSE,
        dwPID);
    if (hProcess) {
...
BOOL CALLBACK
CSystem_EnumWindowsProc(HWND hwnd, LPARAM lParam) {
...
HWND
CProcess::GetWindowHandleWithProcessID(DWORD dwPID) {
    if (0 == dwPID)
        return NULL;
...
HWND
CProcess::GetWindowHandleWithFileName(char* szFileName) {
