...
//-------------------------------------------------------------------------------------------------
bool
CRawSOCKET::Socket(UINT uiWindowMsg,
    int nSocketType,
    long lEvent,
    int nProtocolType,
    int nAddressFormat) {
...
//-------------------------------------------------------------------------------------------------
bool
CRawSOCKET::Bind(UINT uiSocketPort, LPCTSTR lpszSocketAddress) {
...
//-------------------------------------------------------------------------------------------------
bool
CRawSOCKET::AsyncSelect(UINT uiWindowMsg, long lEvent) {
    _ASSERT(m_hSocket != INVALID_SOCKET);
...
//-------------------------------------------------------------------------------------------------
bool
CRawSOCKET::Connect(LPCTSTR lpszHostAddress, UINT nHostPort) {
...
//-------------------------------------------------------------------------------------------------
bool
CRawSOCKET::IOCtl(long lCommand, DWORD* lpArgument) {
...
//-------------------------------------------------------------------------------------------------
int
CRawSOCKET::Receive(void* lpBuf, int nBufLen, int nFlags) {
    return recv(m_hSocket, (LPSTR)lpBuf, nBufLen, nFlags);
}

//-------------------------------------------------------------------------------------------------
int
CRawSOCKET::Send(const void* lpBuf, int nBufLen, int nFlags) {
    return send(m_hSocket, (LPSTR)lpBuf, nBufLen, nFlags);
}

//-------------------------------------------------------------------------------------------------
int
CRawSOCKET::ReceiveFrom(void* lpBuf, int nBufLen, SOCKADDR_IN* pSockAddr, int nFlags) {
    int iSockAddrLen = sizeof(SOCKADDR_IN);

    memset(pSockAddr, 0, sizeof(SOCKADDR_IN));
    int iResult =
        recvfrom(m_hSocket, (LPSTR)lpBuf, nBufLen, nFlags, (SOCKADDR*)pSockAddr, &iSockAddrLen);
    /*
...
//-------------------------------------------------------------------------------------------------
int
CRawSOCKET::SendTo(const void* lpBuf, int nBufLen, SOCKADDR_IN* pSockAddr, int nFlags) {
    return sendto(m_hSocket,
        (LPSTR)lpBuf,
        nBufLen,
        nFlags,
        (SOCKADDR*)pSockAddr,
...
//-------------------------------------------------------------------------------------------------
int
CRawSOCKET::SendTo(const void* lpBuf,
    int nBufLen,
    UINT nHostPort,
...
void
CRawSOCKET::OnReceive(int /*nErrorCode*/) {}
void
CRawSOCKET::OnSend(int /*nErrorCode*/) {}
void
CRawSOCKET::OnOutOfBandData(int /*nErrorCode*/) {}
void
CRawSOCKET::OnAccept(int /*nErrorCode*/) {}
void
CRawSOCKET::OnConnect(int /*nErrorCode*/) {}
void
CRawSOCKET::OnClose(int /*nErrorCode*/) {}
