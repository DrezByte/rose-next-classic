...
//-------------------------------------------------------------------------------------------------
void
CBITARRAY::SetArray(BYTE* pDATA, short nDataByteCnt) {
    int iMinBytes;
...
void
CBITARRAY::ClearAll(void) {
    memset(m_pDATA, 0, m_iByteLength);
}
void
CBITARRAY::FillAll(void) {
...
void
C1BITARRAY::SetBit(int iIndex) {
    if (iIndex >= 0 && iIndex < m_iMaxDataCount)
        m_pDATA[iIndex >> BIT_SHIFT] |= m_btBitMask[iIndex & WHICH_BIT];
}

void
C1BITARRAY::ClearBit(int iIndex) {
    if (iIndex >= 0 && iIndex < m_iMaxDataCount)
        m_pDATA[iIndex >> BIT_SHIFT] &= ~(m_btBitMask[iIndex & WHICH_BIT]);
}

BYTE
C1BITARRAY::FlipBit(int iIndex) {
    if (GetBit(iIndex))
        ClearBit(iIndex);
    else
        SetBit(iIndex);
...
BYTE
C2BITARRAY::GetValue(int iIndex) {
    if (iIndex >= 0 && iIndex < m_iMaxDataCount) {
        BYTE btValue;
...
void
C2BITARRAY::SetValue(int iIndex, BYTE btValue) {
    if (iIndex >= 0 && iIndex < m_iMaxDataCount) {
        btValue &= MAX_VALUE;
...
void
C2BITARRAY::ClearValue(int iIndex) {
    if (iIndex >= 0 && iIndex < m_iMaxDataCount)
        m_pDATA[iIndex >> BIT_SHIFT] &= ~(m_btBitMask[iIndex & WHICH_BIT]);
}
...
BYTE
C4BITARRAY::GetValue(int iIndex) {
    if (iIndex >= 0 && iIndex < m_iMaxDataCount) {
        BYTE btValue;
...
void
C4BITARRAY::SetValue(int iIndex, BYTE btValue) {
    if (iIndex >= 0 && iIndex < m_iMaxDataCount) {
        btValue &= MAX_VALUE;
...
void
C4BITARRAY::ClearValue(int iIndex) {
    if (iIndex >= 0 && iIndex < m_iMaxDataCount)
        m_pDATA[iIndex >> BIT_SHIFT] &= ~(m_btBitMask[iIndex & WHICH_BIT]);
}
