...
void
CChatMember::SetServerIdx(WORD wServerIdx) {
    m_wServerIdx = wServerIdx;
}
void
CChatMember::SetName(const char* pszName) {
    assert(pszName);
    if (pszName)
        m_strName = pszName;
}
WORD
CChatMember::GetServerIdx() {
    return m_wServerIdx;
}
const char*
CChatMember::GetName() {
...
//*----------------------------------------------------------------------//
void
CChatRoom::JoinMember(WORD wServerIdx, const char* pszName) {
...
//*----------------------------------------------------------------------//
void
CChatRoom::LeaveMember(WORD wServerIdx) {
...
void
CChatRoom::LeaveMember(const char* pszName) {
    assert(pszName);
    if (pszName == NULL)
        return;
...
//*----------------------------------------------------------------------//
void
CChatRoom::Made(WORD wMyID) {
    JoinMember(wMyID, g_pAVATAR->Get_NAME());
...
void
CChatRoom::SendReqMakeRoom(BYTE btRoomType, BYTE btMaxUser, char* pszTitle, char* pszPwd) {
...
void
CChatRoom::SendReqJoinRoom(BYTE btRoomType, WORD wRoomID, char* pszPwd) {
    assert(m_iState == STATE_DEACTIVATED);
    if (m_iState == STATE_DEACTIVATED) {
...
void
CChatRoom::SetMaster(WORD wServerIdx) {
...
void
CChatRoom::SetTitle(const char* pszTitle) {
    assert(pszTitle);
    if (pszTitle)
        m_strTitle = pszTitle;
}
