...
void
CClan::SetName(const char* pszName) {
    assert(pszName);
    if (pszName)
        m_strName = pszName;
}
void
CClan::SetSlogan(const char* pszSlogan) {
    assert(pszSlogan);
    if (pszSlogan)
        m_strSlogan = pszSlogan;
}
void
CClan::SetMarkCenter(short nID) {
    m_nMarkCenter = nID;
}
void
CClan::SetMarkBack(short nID) {
    m_nMarkBack = nID;
}
void
CClan::SetLevel(short nLevel) {
    m_nLevel = nLevel;
}
void
CClan::SetPoint(int iPoint) {
    m_iPoint = iPoint;
}
void
CClan::SetAlly(int iIndex, int iClanID, const char* pszName) {
    assert(pszName);
    if (pszName == NULL)
        return;
...
void
CClan::SetStoreRate(int iRate) {
    m_iStoreRate = iRate;
}
void
CClan::SetMoney(__int64 i64Money) {
    m_i64Money = i64Money;
}
...
void
CClan::AddMember(const char* pszName,
    int iClass,
    int iClanPoint,
    int iChannelNo,
    short nLevel,
...
void
CClan::RemoveMember(const char* pszName) {
    assert(pszName);
    if (pszName) {
...
void
CClan::ChangeClass(const char* pszName, int iClass) {
    assert(pszName);
    if (pszName) {
        if (g_pAVATAR != NULL && strcmpi(pszName, g_pAVATAR->Get_NAME()) == 0)
...
void
CClan::ChangeConnectStatus(const char* pszName, int iChannelNo) {
    assert(pszName);
    if (pszName) {
...
void
CClan::SetNotice(const char* pszNotice) {
    assert(pszNotice);
    if (pszNotice)
...
int
CClan::GetMemberCount() {
    return (int)m_Members.size();
}

int
CClan::GetMemberMaxCount() {
...
bool
CClan::IsValidClassUp(int iMemberClass) {
...
int
CClan::GetCurrClassCount(int iClass) {
    int iCount = 0;
    std::list<S_ClanMember>::iterator iter;
...
int
CClan::GetMaxClassCount(int iClanLevel, int iClass) {
...
int
CClan::GetMyClanPoint() {
    return m_iMyClanPoint;
}

void
CClan::SetMemberLevel(const char* pszName, short nLev) {
    assert(pszName);
    if (pszName) {
        std::list<S_ClanMember>::iterator iter;
        int iIndex = 0;
...
void
CClan::SetMemberJob(const char* pszName, short nJob) {
    assert(pszName);
    if (pszName) {
        std::list<S_ClanMember>::iterator iter;
        int iIndex = 0;
...
void
CClan::SetSkill(tagClanSKILL* skilldata, int count) {
    CClanSkillSlot* clan_skill_slot = CClan::GetInstance().GetClanSkillSlot();

    for (int i = 0; i < count; ++i) {
        if (skilldata[i].m_nSkillIDX)
            clan_skill_slot->SetSkillSlot(i,
                skilldata[i].m_nSkillIDX,
                1,
                skilldata[i].m_dwExpiredAbsSEC);
        else
...
CClanSkillSlot*
CClan::GetClanSkillSlot() {
    return &m_ClanSkillSlot;
}

void
CClan::SetClanMarkRegTime(SYSTEMTIME& time) {
    m_ClanMarkRegTime = time;
}
SYSTEMTIME&
CClan::GetClanMarkRegTime() {
