...
CUpgrade&
CUpgrade::GetInstance() {
    static CUpgrade s_Instance;
    return s_Instance;
}

void
CUpgrade::SetTargetItem(CItem* pItem) {
...
void
CUpgrade::SetMaterialItem(CItem* pItem) {
    assert(pItem);
    if (pItem == NULL)
        return;
...
void
CUpgrade::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);
    ...
    if (m_pTargetItem && pEvent->GetID() == CTEventItem::EID_DEL_ITEM
...
void
CUpgrade::SetNpcSvrIdx(int iNpcSvrIdx) {
    m_iNpcSvrIdx = iNpcSvrIdx;
    m_iSkillSlotIdx = 0;
    m_iUseItemInvenIdx = 0;
}
void
CUpgrade::SetSkillSlotIdx(int iSkillSlotIdx) {
    m_iNpcSvrIdx = 0;
...
void
CUpgrade::SetUseItemInvenIdx(int iUseItemInvenIdx) {
    m_iNpcSvrIdx = 0;
    m_iSkillSlotIdx = 0;
...
bool
CUpgrade::Send_CRAFT_UPGRADE_REQ() {
...
int
CUpgrade::GetRequireMp() {
    return m_iRequireMp;
}

int
CUpgrade::GetSuccessProb() {
    return m_iSuccessProb;
}

int
CUpgrade::ComputeSuccessProb() {
    assert(m_pTargetItem);
    if (m_pMaterialItems[0] == NULL)
        return 0;
...
const char*
CUpgrade::GetRequireMaterialName(int iIndex) {
...
int
CUpgrade::GetRequireMaterialCount(int iIndex) {
    assert(iIndex >= 0 && iIndex < 3);
    if (iIndex >= 0 && iIndex < 3 && m_pRequireMaterialItems[iIndex])
...
CItemFragment*
CUpgrade::GetTargetItem() {
    return m_pTargetItem;
}

int
CUpgrade::IsValidUpgradeCondition() {
    if (m_pTargetItem == NULL)
        return INVALID_TARGETITEM;
    if (!m_pTargetItem->GetItem().IsEnableUpgrade())
        return INVALID_TARGETITEM;
    switch (GetType()) {
        case TYPE_SKILL:
    ...
    for (int iIndex = 0; iIndex < 3; ++iIndex) {
        if (m_pRequireMaterialItems[iIndex]) {
            ...
            if (m_pRequireMaterialItems[iIndex]->GetRequireCount()
                > m_pMaterialItems[iIndex]->GetQuantity())
...
void
CUpgrade::ClearResultItemSet() {
    m_ResultItemSet.clear();
}

void
CUpgrade::AddResultItemSet(tag_SET_INVITEM& Item) {
...
void
CUpgrade::SetResultSuccessProb(int iSuccessProb) {
    m_iResultSuccessProb = iSuccessProb;
}

int
CUpgrade::GetResultSuccessProb() {
