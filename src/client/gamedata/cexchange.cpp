...
///교환을 요청하는 Packet
bool
CExchange::SendCliTradeReq(WORD wObjSvrIdx) {
    return g_pNet->Send_cli_TRADE_P2P(wObjSvrIdx, RESULT_TRADE_REQUEST);
}

const std::string&
CExchange::GetOtherName() {
    return m_strTargetAvatarName;
}
__int64
CExchange::GetMyTradeMoney() {
    return m_i64MyTradeMoney;
}

__int64
CExchange::GetOtherTradeMoney() {
    return m_i64OtherTradeMoney;
}

void
CExchange::SetReadyMe(bool bReady) {
    m_bReadyMe = bReady;
}

void
CExchange::SetReadyOther(bool bReady) {
...
bool
CExchange::IsReadyOther() {
    return m_bReadyOther;
}

bool
CExchange::IsReadyAll() {
...
void
CExchange::StartExchange(WORD wServerObjectIdx, const char* pszName) {
    m_bExchange = true;
    assert(pszName);
...
/// @brief 현재 교환중인가?
bool
CExchange::IsExchange() {
    return m_bExchange;
}

void
CExchange::SendTradePacket(BYTE btResult, char cSlotIdx) {
...
int
CExchange::GetCountTradeItem(t_InvTYPE InvenType) {
    int iCount = 0;
...
void
CExchange::RemoveMyItemByInvenIndex(int iInvenIndex) {
    for (int iSlot = 0; iSlot < TOTAL_EXCHANGE_INVENTORY; ++iSlot) {
...
void
CExchange::RemoveMyItemBySlotIndex(int iSlotIndex, bool bSendPacket) {
...
bool
CExchange::UpdateOtherItem(int iIndex, tagITEM& item) {
    ...
    if (iIndex == TRADE_MONEY_SLOT_NO) //돈
    {
        ...
        if (item.GetTYPE() == 0) {
            ...
            if (m_OtherItems[iIndex]) {
                ...
                assert(m_OtherItems[iIndex]
                    && "없는 아이템을 빼려고 합니다 @CExchange::UpdateOtherItem ");
            }
        } else {
            if (m_OtherItems[iIndex]) {
                assert(m_OtherItems[iIndex]
                    && "비어 있지 않은 슬롯에 아이템을 넣으려고 합니다 "
                       "@CExchange::UpdateOtherItem ");
            } else {
                ...
                ///만약 상대방이 올린 품목의 갯수 - 내가 올린 품목의 갯수가 현재 인벤토리의 빈
                ///슬롯갯수보다 클경우 상대방에게 내 인벤이 꽉찼다고 알려주고 이미 올린것을 빼라고
                ///한다.
...
void
CExchange::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
