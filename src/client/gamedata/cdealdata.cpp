...
void
CDealData::AddItem2BuyList(int iIndex, int iQuantity) {
...
void
CDealData::AddItem2SellList(int iIndex, int iQuantity) {
...
void
CDealData::RemoveItemFromBuyList(CItem* pItem) {
    if (pItem) {
        std::vector<CItemFragment*>::iterator iter;
        CItemFragment* pFragmentItem = NULL;
...
void
CDealData::RemoveItemFromSellList(CItem* pItem) {
    if (pItem) {
        std::vector<CItemFragment*>::iterator iter;
        CItemFragment* pFragmentItem = NULL;
        for (iter = m_SellItems.begin(); iter != m_SellItems.end(); ++iter) {
...
unsigned
CDealData::GetTradeItemCnt() {
    return GetBuyItemCnt() + GetSellItemCnt();
}

unsigned
CDealData::GetBuyItemCnt() {
    return m_BuyItems.size();
}

unsigned
CDealData::GetSellItemCnt() {
    return m_SellItems.size();
}

void
CDealData::SendTradeReq() {
...
long
CDealData::GetTotal_BUY() {
    long lPrice = 0;
    std::vector<CItemFragment*>::iterator iter;
    if (CStore::GetInstance().IsUnionStore()) {
        for (iter = m_BuyItems.begin(); iter != m_BuyItems.end(); ++iter)
            lPrice +=
                ITEM_TRADE_UNIONPOINT((*iter)->GetItem().GetTYPE(), (*iter)->GetItem().GetItemNO())
                * (*iter)->GetQuantity();
    } else {
        for (iter = m_BuyItems.begin(); iter != m_BuyItems.end(); ++iter) {
            lPrice += g_pTerrain->m_Economy.Get_ItemBuyPRICE((*iter)->GetItem().GetTYPE(),
...
long
CDealData::GetTotal_SELL() {
    long lPrice = 0;
    std::vector<CItemFragment*>::iterator iter;

    for (iter = m_SellItems.begin(); iter != m_SellItems.end(); ++iter) {
        lPrice += g_pTerrain->m_Economy.Get_ItemSellPRICE((*iter)->GetItem(),
                      g_pAVATAR->GetSellSkillVALUE())
            * (*iter)->GetQuantity();
    }
