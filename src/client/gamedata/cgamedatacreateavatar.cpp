...
const float c_AvatarPositions[5][3] = {{520500, 520500, 100},
    {520270, 520653, 100},
    {520000, 520707, 100},
    {519730, 520653, 100},
    {519500, 520500, 100}};
...
CGameDataCreateAvatar&
CGameDataCreateAvatar::GetInstance() {
    static CGameDataCreateAvatar s_Instance;
    return s_Instance;
}

void
CGameDataCreateAvatar::Clear() {
...
void
CGameDataCreateAvatar::AddCreateAvatar(CJustModelAVT* pAvatar) {
    assert(pAvatar);
    ...
    if (pAvatar->IsPlatinum()) {
        ...
        for (riter = m_avatars_platinum.rbegin(); riter != m_avatars_platinum.rend();
             ++riter, ++count) {
        ...
        for (riter = m_avatars_premium.rbegin(); riter != m_avatars_premium.rend();
             ++riter, ++count) {
            setPosition(riter->second->GetModelNode(),
...
void
CGameDataCreateAvatar::RemoveCreateAvatar(const char* pszName) {
    assert(pszName);
    std::multimap<int, CJustModelAVT*>::iterator iter;
...
HNODE
CGameDataCreateAvatar::GetModelNode(const char* pszName) {
    assert(pszName);
    std::multimap<int, CJustModelAVT*>::iterator iter;
...
CJustModelAVT*
CGameDataCreateAvatar::FindAvatar(const char* pszName) {
    if (pszName == NULL)
        return NULL;
    std::multimap<int, CJustModelAVT*>::iterator iter;
...
const char*
CGameDataCreateAvatar::Pick_AVATAR(long x, long y) {
...
void
CGameDataCreateAvatar::SelectAvatar(const char* pszName) {
    assert(pszName);
...
void
CGameDataCreateAvatar::SelectAvatar(int iIndex) {
    std::multimap<int, CJustModelAVT*>::reverse_iterator riter;
    int iCount = 0;
...
void
CGameDataCreateAvatar::ClearSelectedAvatar() {
    m_strSelectedAvatarName.clear();
}

const char*
CGameDataCreateAvatar::GetSelectedAvatarName() {
    return m_strSelectedAvatarName.c_str();
}

int
CGameDataCreateAvatar::GetSelectedAvatarIndex() {
...
CJustModelAVT*
CGameDataCreateAvatar::GetSelectedAvatar() {
    std::multimap<int, CJustModelAVT*>::iterator iter;
...
CGameDataCreateAvatar::Update() {
    ...
    if (CJustModelAVT* pAVT = FindAvatar(m_strSelectedAvatarName.c_str())) {
        ...
        if (getPosition(pAVT->GetModelNode(), fPositionXYZ)) {
            ...
            Info.AddString(pAVT->GetName().c_str(),
                g_dwYELLOW,
            ...
            Info.AddString(CStr::Printf("%s: %s",
                CStringManager::GetSingleton().GetAbility(AT_CLASS),
                CStringManager::GetSingleton().GetJobName(pAVT->GetJob())));

            if (pAVT->GetDeleteRemainSEC())
                Info.AddString(CStr::Printf("%s: %3.1f%s",
                                   STR_REMAIN_REMOVE_WAITTIME,
                                   pAVT->GetDeleteRemainSEC() / 3600.f,
                                   STR_HOUR),
                    g_dwWHITE);
        ...
        if (getAnimatableState(pAVT->GetModelNode()) == 0) {
            CSystemProcScript::GetSingleton().CallLuaFunction("OnSelectAvatarMotionEnd",
                ZZ_PARAM_STRING,
                m_strSelectedAvatarName.c_str(),
                ZZ_PARAM_END);
        }
    }
}
int
CGameDataCreateAvatar::GetAvatarCount() {
