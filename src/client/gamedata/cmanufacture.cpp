...
CManufacture&
CManufacture::GetInstance() {
    static CManufacture s_Instance;
    return s_Instance;
}

int
CManufacture::GetSkillIndex() {
    return m_iSkillIndex;
}

int
CManufacture::GetSkillSlotIndex() {
    return m_iSkillSlotIndex;
}

void
CManufacture::SetSkillSlotIndex(int iIndex) {
...
int
CManufacture::GetCosumeMP() {
    return m_iConsumeMP;
}

int
CManufacture::GetSuccessRate() {
...
int
CManufacture::GetSuccessPoint(int iIndex) {
...
void
CManufacture::SetSuccessPoint(int iIndex, int iValue) {
...
CRequireMaterial&
CManufacture::GetRequireMaterial(int iIndex) {
    if (iIndex < 0 && iIndex >= g_iMaxCountMaterial) {
        assert(0 && "Invalid RequireMaterial Index @CManufacture::GetRequireMaterial");
...
int
CManufacture::GetEstimateProbability(int iItemDif,
    int iItemQuality,
    int iMatQuality,
    int iNumMat,
    short nWorldProd) {
    ...
    ///진행포인트
    double fProPoint1 =
        (iMatQuality * (50 + 70) * (iCon / 2 + iItemDif / 2 + 530) * nWorldProd) / 800000.;
    double fProPoint2 = ((iMatQuality + iItemDif / 2)
                            * ((50 + 100) * (iSen / 2 + iP_Lv * 6 + iMatQuality * 2 + 770)))
        / (iNumMat + 7) / 1600.;
    double fProPoint3 = ((fProPoint1 / 5 + iItemQuality) * (50 + 80)
                            * (iCon / 2 + iP_Lv * 5 + iMatQuality * 2 + 630))
        / (iNumMat + 7) / 2000.;
    ...
    fSucProb1 =
        (170
            - (iSucPoint1 / (iMatQuality * (iCon / 2 + iItemDif / 2 + 530) * nWorldProd / 800000.)))
        / 105.;
    if (fSucProb1 > 1)
        fSucProb1 = 1;
    ...
    fSucProb2 =
        (200
            - (iSucPoint2
                / ((iMatQuality + iItemDif / 2) * (iCon / 2 + iP_Lv * 6 + iMatQuality * 2 + 770)
                    / (iNumMat + 7) / 1600.)))
        / 105.;
    if (fSucProb2 > 1)
        fSucProb2 = 1;
    ...
    fSucProb3 =
        (180
            - (iSucPoint3
                / ((fProPoint1 / 6 + iItemQuality)
                    * (iCon * 0.3f + iP_Lv * 5 + iMatQuality * 2 + 600) / (iNumMat + 7) / 2000.)))
        / 105.;
    if (fSucProb3 > 1)
        fSucProb3 = 1;
...
int
CManufacture::GetSuccessPoint(int iSlot, int iItemDif, int iItemQuality, int iNumMat) {
    int iSuccessPoint = 0;
    switch (iSlot) {
        case 1:
...
short
CManufacture::GetRandShort(short nMin, short nMax) {
    return rand() % (nMax - nMin + 1) + nMin;
}

bool
CManufacture::IsRequireMaterialItem(int iInvenIdx) {
    ...
    for (int i = 0; i < g_iMaxCountMaterial; ++i) {
        if (m_RequireMaterials[i].IsEmpty()) {
            ...
            {
                if (m_RequireMaterials[i].GetItemClass()
                    == ITEM_TYPE(Item.GetTYPE(), Item.GetItemNO()))
                    return true;
            } else {
...
/// Inventory에서도 뺀다.
void
CManufacture::SubItemsAfterRecvResult(int iSlotNum) {
...
int
CManufacture::GetMaterialIndex(CItem* pItem) {
    if (pItem == NULL) {
    ...
    assert(pItem
        && "재료 리스트에 없는 CItem의 인덱스를 구하려고 합니다@CManufacture::GetMaterialIndex");
    return -1;
}

void
CManufacture::SetMaterialItem(CItem* pItem) {
    for (int i = 0; i < g_iMaxCountMaterial; ++i) {
        if (!m_RequireMaterials[i].IsEmpty()) {
            if (m_RequireMaterials[i].GetItemClass()) ///종류일경우
            {
...
//////////////////////////////////////////////////////////////////////////
void
CManufacture::RemoveMaterialItem(CItem* pItem) {
    if (pItem == NULL) {
...
void
CManufacture::AddMaterialItem(CItem* pItem, int index, int iQuantity) {
    ...
    if (index == 0) ///예상확률을 재계산한다.
    {
        ...
        m_iSuccessRate = GetEstimateProbability(m_iItemDif,
            m_iItemQuality,
            iMatQuality,
            m_iNumMat,
            Get_WorldPROD());
...
bool
CManufacture::LoadMakableClasses(int iSkillNo, int iSkillLv) {
...
bool
CManufacture::LoadMakableItem(int iClass, int iSkillNo, int iSkillLv) {
    if (iSkillNo <= 0 || iSkillLv <= 0)
        return false;
...
const std::list<tagITEM>&
CManufacture::GetMakableItems() {
    return m_MakableItems;
}

const std::list<int>&
CManufacture::GetMakableClasses() {
    return m_MakableClasses;
}

void
CManufacture::SetMakeClass(int iClass) {
...
void
CManufacture::SetMakeItem(tagITEM& Item) {
    if (m_pMakeItem) {
    ...
    int iMaterialNo = ITEM_PRODUCT_IDX(Item.GetTYPE(),
        Item.GetItemNO()); ///아이템을 만들기 위하여 필요한 재료들이 있는 STB의 Line Number
    ...
    if (iMaterialItemNo > 0 && iMaterialItemCount > 0) {
        if (iMaterialItemNo > 1000) {
            ...
            m_RequireMaterials[0].SetName(
                ITEM_NAME(m_RequireMaterials[0].GetItemType(), m_RequireMaterials[0].GetItemNo()));
        } else {
            ...
            m_RequireMaterials[0].SetRequireCount(iMaterialItemCount);
            m_RequireMaterials[0].SetName(
                CStringManager::GetSingleton().GetItemType(iMaterialItemNo));
        }
        ++m_iNumMat;
    }
    ...
    for (int i = 1; i < g_iMaxCountMaterial; ++i) {
        ...
        if (iMaterialItemNo > 0 && iMaterialItemCount > 0) {
            if (iMaterialItemNo > 1000) {
                ...
                m_RequireMaterials[i].SetName(ITEM_NAME(m_RequireMaterials[i].GetItemType(),
                    m_RequireMaterials[i].GetItemNo()));
            } else {
                m_RequireMaterials[i].SetItemClass(iMaterialItemNo);
                m_RequireMaterials[i].SetItemNo(0);
                m_RequireMaterials[i].SetItemType(0);
                m_RequireMaterials[i].SetRequireCount(iMaterialItemCount);
                m_RequireMaterials[i].SetName(
                    CStringManager::GetSingleton().GetItemType(iMaterialItemNo));
            }
            ++m_iNumMat;
        }
...
int
CManufacture::IsValidSendMakeItemReq() {
    CIcon* pIcon = NULL;
...
//////////////////////////////////////////////////////////////////////////
void
CManufacture::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
...
bool
CManufacture::IsLoadedClass(int iClass) {
...
int
CManufacture::GetMakeType() {
    return m_iMakeType;
}

CItem*
CManufacture::GetMakeItem() {
    return m_pMakeItem;
}