...
CServerList&
CServerList::GetInstance() {
    static CServerList s_Instance;
    return s_Instance;
}

const std::map<int, WORLDSERVER_INFO>&
CServerList::GetWorldServerList() {
    return m_worldserver_list;
}

const std::list<CHANNELSERVER_INFO>&
CServerList::GetChannelServerList(int worldserver_id) {
...
void
CServerList::ClearChannelServerList(int worldserver_id) {
    std::map<int, WORLDSERVER_INFO>::iterator iter;
    for (iter = m_worldserver_list.begin(); iter != m_worldserver_list.end(); ++iter) {
        if (iter->second.m_world_id == worldserver_id) {
...
void
CServerList::ClearWorldServerList() {
...
void
CServerList::AddChannelServerList(int worldserver_id,
    BYTE channelserver_id,
    const char* channelserver_name,
    short user_perent) {
...
void
CServerList::AddWorldServerList(int index, DWORD worldserver_id, const char* worldserver_name) {
...
void
CServerList::ShowChannel() {
    m_show_channel = true;
}

void
CServerList::HideChannel() {
    m_show_channel = false;
}

bool
CServerList::IsShowChannel() {
