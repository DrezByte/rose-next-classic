...
//*----------------------------------------------------------------------
bool
CParty::JoinMember(tag_PARTY_MEMBER& memberinfo, const char* pszName) {
    assert(pszName);
    if (pszName == NULL)
        return false;
    ...
    if (memberinfo.m_wObjectIDX != g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->Get_INDEX())) {
        CObjAVT* pObjAVT =
            g_pObjMGR->Get_CharAVT(g_pObjMGR->Get_ClientObjectIndex(memberinfo.m_wObjectIDX),
                false);
...
//*----------------------------------------------------------------------
bool
CParty::LeaveMember(DWORD dwObjectTAG) {
...
bool
CParty::BanMember(DWORD dwObjectTag) {
    ///내가 Ban당했을경우
...
//*----------------------------------------------------------------------
bool
CParty::GetMemberInfoByObjSvrIdx(WORD wObjSvrIdx, PartyMember& member) {
    std::list<PartyMember>::iterator iter;
    for (iter = m_Members.begin(); iter != m_Members.end(); ++iter) {
...
bool
CParty::GetMemberInfoByTag(DWORD dwObjectTAG, PartyMember& member) {
    std::list<PartyMember>::iterator iter;
    for (iter = m_Members.begin(); iter != m_Members.end(); ++iter) {
        if (iter->m_Info.m_dwUserTAG == dwObjectTAG) {
            member = *iter;
...
//*----------------------------------------------------------------------
bool
CParty::IsPartyMember(WORD wObjSvrIdx) {
    std::list<PartyMember>::iterator iter;
    for (iter = m_Members.begin(); iter != m_Members.end(); ++iter) {
        if (iter->m_Info.m_wObjectIDX == wObjSvrIdx)
            return true;
...
//*----------------------------------------------------------------------
int
CParty::GetNeedExp4Levelup(int iLevel) {
    // return ( ( iLevel+3) * ( iLevel+5)*5 ) + 40;
    return CCal::Get_PartyLevelUpNeedEXP(
        iLevel); // 2004. 7. 22 서버와 공통으로 사용하는 함수로 교체
}
//*----------------------------------------------------------------------
/// @brief 파티레벨을 구한다.
//*----------------------------------------------------------------------
...
bool
CParty::ChangeLeaderByObjSvrIdx(WORD wObjSvrIdx) {
    PartyMember member;
...
bool
CParty::ChangeLeaderByTag(DWORD dwObjectTag) {
    PartyMember member;
...
//*----------------------------------------------------------------------
bool
CParty::ChangeMemberInfoByUserTag(tag_PARTY_MEMBER& MemberInfo) {
    std::list<PartyMember>::iterator iter;
    for (iter = m_Members.begin(); iter != m_Members.end(); ++iter) {
...
CParty::Update() {
    ...
    if (dwTimeSpace && (dwTimeSpace > c_iAutoExpDecreaseTimeSpace))
        SetExp(m_iCurrExp
            - (GetAutoDecreaseExp(m_iLevel, GetMemberCount()) * dwTimeSpace)
                / c_iAutoExpDecreaseTimeSpace);
...
//*-----------------------------------------------------------------------------------//
int
CParty::GetAutoDecreaseExp(int iPartyLv, int iCountPartyMember) {
...
WORD
CParty::GetLeaderSvrIndex() {
    return m_wLeaderObjSvrIdx;
}

DWORD
CParty::GetLeaderObjectTAG() {
    return m_dwLeaderObjectTAG;
}

void
CParty::SetLeaderObjSvrIdx(WORD wObjSvrIdx) {
    m_wLeaderObjSvrIdx = wObjSvrIdx;
}

void
CParty::SetLeaderObjectTag(DWORD dwObjectTag) {
    m_dwLeaderObjectTAG = dwObjectTag;
}
...
//*-----------------------------------------------------------------------------------//
void
CParty::MemberDisconnected(DWORD dwUserTag) {
    std::list<PartyMember>::iterator iter;
    for (iter = m_Members.begin(); iter != m_Members.end(); ++iter) {
...
void
CParty::SendChangePartyRule(BYTE btPartyRule) {
...
void
CParty::RecvPartyRule(BYTE btPartyRule) {
    m_btPartyRule = btPartyRule;
