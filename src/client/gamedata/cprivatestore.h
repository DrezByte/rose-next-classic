 ...
 * - Observer로 등록전에 WishList를 가져가서 매입리스트에 추가하고, BuyList, SellList( in UI )는
 *Clear한다.
 ...
 *		-- Data는 CPrivateStore를 쓰는것으로 하고 이용에 주의하자.( WishList는 항상 유지하고,
 *BuyList와 SellList를 Clear해준다.)
 *
 * - 네트워크( 2004 / 6 / 9 )
 *		-- 개인상점개설시 판매물품
 *			--- 구매자에게는 gsv_SET_MONEYnINV 와 gsv_P_STORE_RESULT가 온다.
 *			--- 개인상점개설자에게는 gsv_P_STORE_RESULT만 온다. ->여기서 인벤토리에서 빼자, 물론
 *판매리스트에서도
 *		-- 개인상점개설시 매입물품
