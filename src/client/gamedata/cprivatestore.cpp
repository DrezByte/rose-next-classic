...
//*---------------------------------------------------------------------------//
void
CPrivateStore::AddItemSellList(CItem* pItem, int iQuantity, int iPrice) {
    if (IsOpened())
        return;
...
void
CPrivateStore::SortItemSellList() {
...
//*---------------------------------------------------------------------------//
void
CPrivateStore::RemoveItemSellList(int iIndex) {
    if (IsOpened())
        return;
...
//*---------------------------------------------------------------------------//
CItem*
CPrivateStore::GetItemSellList(int iIndex) {
...
//*---------------------------------------------------------------------------//
void
CPrivateStore::AddItemBuyList(int iIndex, int iUnitCost, int iQuantity) {
    if (IsOpened())
        return;
...
//*---------------------------------------------------------------------------//
void
CPrivateStore::RemoveItemBuyList(int iIndex) {
    if (IsOpened())
        return;
...
//*---------------------------------------------------------------------------//
void
CPrivateStore::AddItemWishList(tagITEM& Item, bool bSendPacket, int iSlotIndex) {
    if (IsOpened())
        return;
...
//*---------------------------------------------------------------------------//
void
CPrivateStore::RemoveItemWishList(int iIndex) {
    if (IsOpened())
        return;
...
CPrivateStore::Open() {
    ...
    for (iterBuyList = m_BuyItems.begin(); iterBuyList != m_BuyItems.end();
         ++iterBuyList, ++iBuyItemCount) {
    ...
    g_pNet->Send_cli_P_STORE_OPEN((BYTE)iSellItemCount,
        (BYTE)iBuyItemCount,
        SellItems,
        BuyItems,
        (char*)m_strTitle.c_str());
    m_bOpened = true;
    return true;
}
...
//*---------------------------------------------------------------------------//
void
CPrivateStore::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);
...
const char*
CPrivateStore::GetTitle() {
    return m_strTitle.c_str();
}
void
CPrivateStore::SetTitle(const char* pszTitle) {
    m_strTitle = pszTitle;
}

CItem*
CPrivateStore::GetWishItem(int iIndex) {
    assert(iIndex >= 0 && iIndex < MAX_WISH_ITEMS);
    if (iIndex >= 0 && iIndex < MAX_WISH_ITEMS)
...
bool
CPrivateStore::IsOpened() {
    return m_bOpened;
}

void
CPrivateStore::UpdateBuyList(tagPS_SLOT_ITEM& SlotITEM) {
    std::list<CItemFragment*>::iterator iter;
    CItemFragment* pFragmentItem;
    for (iter = m_BuyItems.begin(); iter != m_BuyItems.end(); ++iter) {
        pFragmentItem = *iter;
...
void
CPrivateStore::UpdateSellList(tagPS_SLOT_ITEM& SlotITEM) {
    ...
    if (SlotITEM.m_btSLOT >= 0 && SlotITEM.m_btSLOT < m_SellItems.size()) {
        ...
        if (pFragmentItem) ///서버에서 Recv_gsv_P_STORE_MONEYnINV을 먼저 받았을경우 삭제되었을수
                           ///있다
        {
            ...
            {
                if (SlotITEM.m_SlotITEM.IsEnableDupCNT()) {
                    // int iSubQuantity = pFragmentItem->GetQuantity() -
                    // SlotITEM.m_SlotITEM.GetQuantity();
                    pFragmentItem->SetQuantity(SlotITEM.m_SlotITEM.GetQuantity());
...
CPrivateStore::Clear() {
    ...
    CTEventPrivateStore Event;
    int iIndex = 0;
    for (iIndex = 0, iterSell = m_SellItems.begin(); iterSell != m_SellItems.end();
         ++iIndex, ++iterSell) {
        pFragmentItem = *iterSell;
        if (pFragmentItem) {
            Event.SetID(CTEventPrivateStore::EID_REMOVE_SELLLIST);
    ...
    std::list<CItemFragment*>::iterator iterBuy;
    for (iIndex = 0, iterBuy = m_BuyItems.begin(); iterBuy != m_BuyItems.end();
         ++iIndex, ++iterBuy) {
        pFragmentItem = *iterBuy;
...
CPrivateStore::IsValidOpen() {
    ...
    for (; begin != npcInfoInMapList.end(); ++begin) {
        ...
        int iDistance = CD3DUtil::distance((int)npcInfoInMap.m_Position.x,
            (int)npcInfoInMap.m_Position.y,
            (int)vecCenter.x,
            (int)vecCenter.y);
