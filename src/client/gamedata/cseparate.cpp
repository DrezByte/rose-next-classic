...
CSeparate&
CSeparate::GetInstance() {
    static CSeparate s_Instance;
    return s_Instance;
}

void
CSeparate::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);
...
CSeparate::SetItem(CItem* pItem) {
    ...
    switch (m_iType) {
        case TYPE_SEPARATE: {
            ...
            m_iRequireMp =
                ITEM_QUALITY(MainMaterialItem.GetTYPE(), MainMaterialItem.GetItemNO()) / 2
        ...
        case TYPE_DECOMPOSITION: {
            ...
            if (int iRawMaterial = PRODUCT_RAW_MATERIAL(iProductIdx)) {
                ...
                if (MaterialItem.IsEnableDupCNT())
                    MaterialItem.m_uiQuantity =
                        GetQuantity(Item, PRODUCT_NEED_ITEM_CNT(iProductIdx, 0));
                ...
                for (int i = 1; i < 4; ++i) {
                    if (iMaterialIdx = PRODUCT_NEED_ITEM_NO(iProductIdx, i)) {
                        ...
                        if (MaterialItem.IsEnableDupCNT())
                            MaterialItem.m_uiQuantity =
                                GetQuantity(Item, PRODUCT_NEED_ITEM_CNT(iProductIdx, i));
...
int
CSeparate::GetRequireMp() {
    return m_iRequireMp;
}

int
CSeparate::GetQuantity(tagITEM& Item, int iRequireQuantity) {
    int iDuration = 0;
...
void
CSeparate::Send_CRAFT_BREAKUP_REQ() {
    ...
    if (m_pMaterialItem) {
        if (m_iSkillSlotIdx && m_wNpcSvrIdx == 0) {
            g_pNet->Send_cli_CRAFT_BREAKUP_REQ(CRAFT_BREAKUP_USE_SKILL,
                m_iSkillSlotIdx,
                m_pMaterialItem->GetIndex());
            m_bWaitServerReply = true;
        } else if (m_wNpcSvrIdx && m_iSkillSlotIdx == 0) {
            g_pNet->Send_cli_CRAFT_BREAKUP_REQ(CRAFT_BREAKUP_FROM_NPC,
                m_wNpcSvrIdx,
                m_pMaterialItem->GetIndex());
            m_bWaitServerReply = true;
        } else {
...
void
CSeparate::SetSkillSlotIdx(int iSkillSlotIdx) {
    m_iSkillSlotIdx = iSkillSlotIdx;
    m_wNpcSvrIdx = 0;
}

void
CSeparate::SetNpcSvrIdx(WORD wNpcSvrIdx) {
    m_wNpcSvrIdx = wNpcSvrIdx;
    m_iSkillSlotIdx = 0;
}

WORD
CSeparate::GetNpcSvrIdx() {
    return m_wNpcSvrIdx;
}
