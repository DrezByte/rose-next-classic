...
bool
CStore::ChangeStore(int iNpcObjIndex, bool bSpecialTab) {
                ...
                for (nI = 0; nI < c_iSlotCountPerTab; nI++) {
                    ...
                    if (nItemNo) {
                        ...
                        if (pItem) {
                            ...
                            m_pItems.insert(
                                std::map<int, CItem*>::value_type(i * c_iSlotCountPerTab + nI,
                                    pItem));
...
int
CStore::GetNpcObjIndex() {
    return m_iNpcObjIndex;
}
std::string&
CStore::GetTabName(int i) {
    if (i < 0 || i >= 4) {
