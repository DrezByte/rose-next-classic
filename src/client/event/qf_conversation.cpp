...
void
QF_gotoCon(int hEvent, int iDataIDX, int iItemIDX) {
    /*
    CEvent *pEvent=(CEvent*)hEvent;
    ...
    LogString (LOG_DEBUG_, "%d/%d: %s \n", iDataIDX, iItemIDX, pEvent->m_pScrDATA[ iDataIDX
    ].m_pScrITEM[ iItemIDX ].m_Message.Get()); if ( pEvent->m_pScrDATA[ iDataIDX ].m_pScrITEM[
    iItemIDX ].m_lChildDataIDX >= 0 ) { iDataIDX = pEvent->m_pScrDATA[ iDataIDX ].m_pScrITEM[
    iItemIDX ].m_lChildDataIDX; LogString (LOG_DEBUG_, "%d/%d: %s \n", iDataIDX, iItemIDX,
    pEvent->m_pScrDATA[ iDataIDX ].m_pScrITEM[ iItemIDX ].m_Message.Get());
    }
    */
}

void
QF_ChangetalkImage(ZSTRING szTalkImage) {
...
void
QF_ChangetalkName(ZSTRING szTalkName) {
    CDialogDlg* pDlg = NULL;
...
void
QF_EffectCallSelf(ZSTRING szEffectName) {
...
void
QF_EffectCallNpc(ZSTRING szEffectName, int iNpcIndex) {
    int iEffectHash = (int)g_pEffectLIST->Add_EffectFILE((char*)szEffectName);
...
void
QF_MotionCallSelf(ZSTRING szEffectName) {
...
void
QF_MotionCallNpc(ZSTRING szEffectName, int iNpcIndex) {
    int iHashMOTION = g_MotionFILE.Add_FILE((char*)szEffectName);
...
void
QF_CameraworkingSelf(ZSTRING szCameraName) {
...
void
QF_CameraworkingNpc(ZSTRING szCameraName, int iNpcIndex) {
    HNODE hCameraMotion = ::findNode(szCameraName);
    if (hCameraMotion == 0) {
        hCameraMotion = ::loadMotion(szCameraName, szCameraName, 1, 4, 3, 1, 0);
    }
...
void
QF_CameraworkingPoint(ZSTRING szCameraName, int PosX, int PosY) {
    D3DVECTOR PosENZIN;
