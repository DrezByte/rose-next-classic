...
unsigned int
get_param_uint(lua_State* L, int& index_from_end, const char* where) {
...
int
get_param_int(lua_State* L, int& index_from_end, const char* where) {
    int index = lua_gettop(L) - index_from_end;
    _ASSERT(index >= 1);
    index_from_end++;
    if (!lua_isnumber(L, index)) {
...
float
get_param_float(lua_State* L, int& index_from_end, const char* where) {
    int index = lua_gettop(L) - index_from_end;
    _ASSERT(index >= 1);
    index_from_end++;
    if (!lua_isnumber(L, index)) {
...
float*
get_param_float3(lua_State* L, int& index_from_end, const char* where) {
    int index = lua_gettop(L) - index_from_end;
    _ASSERT(index >= 1);
    index_from_end += 3;
...
const char*
get_param_string(lua_State* L, int& index_from_end, const char* where) {
    int index = lua_gettop(L) - index_from_end;
    _ASSERT(index >= 1);
    index_from_end++;
...
//
void
set_param_uint(lua_State* L, int& return_num, unsigned int val) {
    f_return_uint = val;
...
void
set_param_int(lua_State* L, int& return_num, int val) {
    f_return_int = val;
    lua_pushnumber(L, val);
    return_num++;
}
...
void
set_param_float(lua_State* L, int& return_num, float val) {
    f_return_float = val;
    lua_pushnumber(L, val);
    return_num++;
}
...
void
set_param_float3(lua_State* L, int& return_num, const float* val) {
    f_return_float3 = val;
...
void
set_param_string(lua_State* L, int& return_num, const char* val) {
    f_return_string = val;
...
void
set_global(lua_State* L, const char* variable_name, int value) {
    lua_pushnumber(L, value);
    lua_setglobal(L, variable_name);
}

void
set_global(lua_State* L, const char* variable_name, float value) {
    lua_pushnumber(L, value);
    lua_setglobal(L, variable_name);
}

void
set_global(lua_State* L, const char* variable_name, const char* value) {
...
lua_CallFUNC(lua_State* L, const char* function_name, va_list va) {
    ...
    if (err == 0) {
        ...
        lua_tonumber( L, lua_gettop( L ) ) );		break; case LUA_TTABLE	: LogString (LOG_DEBUG_,
        "script returned a table \n" ); break; case LUA_TSTRING	: LogString (LOG_DEBUG_, "script
        returned %s \n", lua_tostring( L, lua_gettop( L ) ) );		break;
        //				case LUA_TBOOLEAN	: LogString (LOG_DEBUG_, "script returned %d \n",
        lua_toboolean( L, lua_gettop( L ) ) );	break; default: LogString (LOG_DEBUG_, "script
        returned unknown param  %d \n", iType );	break;
                    }
                    lua_pop( L, 1 );
               }
        */
    ...
    switch (err) {
        ...
        case LUA_ERRSYNTAX: // 3
            ...
            break;
        case LUA_ERRMEM: // 4
            szErrMSG = "script: memory allocation error. For such errors, Scriptor does not call "
                       "_ERRORMESSAGE."; // (see Section 4.7). ");
            break;
        case LUA_ERRERR: // 5
            szErrMSG = "script: error while running _ERRORMESSAGE. For such errors, Scriptor does "
                       "not call _ERRORMESSAGE again, to avoid loops.";
            break;
    }
...
//-------------------------------------------------------------------------------------------------
int
lua_CallIntFUNC(lua_State* pLUA, const char* szFuncName, ...) {
    if (!szFuncName)
        return 0x80000000;
...
//-------------------------------------------------------------------------------------------------
char*
lua_CallCharFUNC(lua_State* pLUA, const char* szFuncName, ...) {
    va_list va;
    va_start(va, szFuncName);
    int iResultCnt = lua_CallFUNC(pLUA, szFuncName, va);
    va_end(va);
...
//-------------------------------------------------------------------------------------------------
bool
lua_GetRETURN(lua_State* pLUA, int& iValue) {
    int iResult;
...
bool
lua_GetRETURN(lua_State* pLUA, char** ppChar) {
    if (lua_gettop(pLUA) < 1 || !lua_isstring(pLUA, -1)) {
        return false;
    }
...
bool
lua_GetRETURN(lua_State* pLUA, float& fValue) {
    int iResult;
