...
void
CJustModelAVT::SwitchState(int iID) {
...
//-------------------------------------------------------------------------------------------------
bool
CJustModelAVT::LoadModelNODE(const char* szName) {
...
void
CJustModelAVT::Init(const char* strName,
...
void
CJustModelAVT::Init(int iPartsArray[MAX_BODY_PART]) {
    for (int i = 0; i < MAX_BODY_PART; i++) {
...
/// set race and sex
void
CJustModelAVT::SetRace(BYTE btRace) {
    m_btRace = btRace;
...
bool
CJustModelAVT::SetPart(int iPartType, int iPartIDX) {
    ...
    m_phPartVIS[iPartType] =
        m_CharMODEL.CreatePART((char*)(m_strName.c_str()), m_hNodeMODEL, iPartType);
...
bool
CJustModelAVT::LoadModelVisible() {
...
void
CJustModelAVT::UnloadModelVisible() {
...
void
CJustModelAVT::SetPosition(float x, float y, float z) {
    ::setPosition(m_hNodeMODEL, x, y, z);
}

bool
CJustModelAVT::SetMotion(int iMotionIDX, int iRepeatCount) {
...
bool
CJustModelAVT::SetMotion(const char* strMotion, int iRepeatCount) {
    if (strMotion == NULL)
        return false;
...
void
CJustModelAVT::Update() {
    m_pCurrState->Update(this);
}

bool
CJustModelAVT::IsPlatinum() {
    return m_bPlatinum;
}

void
CJustModelAVT::SetPlatinum(bool platinum) {
    m_bPlatinum = platinum;
}

void
CJustModelAVT::SetIndex(int index) {
    m_iIndex = index;
}
int
CJustModelAVT::GetIndex() {
    return m_iIndex;
}
