...
CUserDATA::Cal_DEFENCE() {
    ...
    if ( pITEM->GetHEADER() && pITEM->GetLife() && ITEM_TYPE( pITEM->GetTYPE(), pITEM->GetItemNO() )
    == 261 )
    */
    // 방패.
...
CUserDATA::Skill_GetAbilityValue(short nAbilityType) {
    switch (nAbilityType) {
        ...
        case AT_STAMINA:
            return this->m_GrowAbility.m_nSTAMINA;
        case AT_PATHP:
            return GetCur_PatHP();
...
CUserDATA::GetPassiveSkillAttackSpeed(short nRightWeaponItemNo) {
    switch (WEAPON_TYPE(nRightWeaponItemNo)) {
        case 231:
            return this->GetPassiveSkillValue(AT_PSV_ATK_SPD_BOW); // 활
        case 233: // 투척기
        case 232:
            return this->GetPassiveSkillValue(AT_PSV_ATK_SPD_GUN); // 총
        case 251: // 카타르
        case 252:
            return this->GetPassiveSkillValue(AT_PSV_ATK_SPD_PAIR); // 이도류
    }
            ...
            for (nI = 0; nI < SKILL_INCREASE_ABILITY_CNT; nI++) {
                ...
                if (SKILL_INCREASE_ABILITY(nSkillIDX, nI) >= AT_STR
                    ...
                    if ((nPassiveTYPE >= AT_PSV_RES && nPassiveTYPE < AT_AFTER_PASSIVE_SKILL_2ND)
                        || (nPassiveTYPE >= AT_PSV_ATK_POW_NO_WEAPON
                            && nPassiveTYPE <= AT_AFTER_PASSIVE_SKILL)) {
                        if (SKILL_CHANGE_ABILITY_RATE(nSkillIDX, nI)) {
                            // 05.05.26 비율계산 이면 ???
