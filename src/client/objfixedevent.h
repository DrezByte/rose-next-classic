...
private:
    ...
    /// 한번 충돌해서 이벤트를 발생시킨 오브젝트는 다시 충돌하지 않음 -> 충돌함이 되기전에는
    /// 이벤트를 다시 발생시키지 않는다.
...
public:
    ...
    /*override*/ char* Make_ZNAME(int iCreateOrder, short nPartIdx) {
        return CStr::Printf(NAME_TREEOBJ, iCreateOrder, nPartIdx);
    }
    ...
    void ExecEventScript(int iCurrentState,
        bool bJustEnd = false,
        bool bCollisionEvent = false,
        bool bInsertToScene = false);

    void ExecTrigger();

    /// 현재 충돌을 체크할수 있는 상태인가?
    /// 한번 충돌후 특정한 상황이( 일정거리가 떨어진다던지 아니면 기타등 ) 일어나기 전에는 disable
    /// 된다.
