...
//-------------------------------------------------------------------------------------------------
bool
CObjMORPH::Create(int iMorphID, D3DVECTOR& Position) {
    ...
    ::setMaterialBlendTypeCustom(hMaterial,
        pMorpher->m_nSrcBlend,
...
//-------------------------------------------------------------------------------------------------
void
CObjMORPH::Scale(D3DVECTOR& Scale) {
    if (m_hMorpher)
        ::setScale(m_hMorpher, Scale.x, Scale.y, Scale.z);
}

void
CObjMORPH::Rotate(D3DXQUATERNION& Rotate) {
    if (m_hMorpher)
...
//-------------------------------------------------------------------------------------------------
bool
CObjMORPH::IsIntersect(float& fCurDistance) {
    return false;
}

void
CObjMORPH::InsertToScene(void) {
...
void
CObjMORPH::RemoveFromScene(bool bIncludeEFFECT) {
    if (!m_bIsVisible || !m_hMorpher)
        return;
