...
/// 로딩될 텍스쳐의 밉맵 레벨 설정하기.
/// @Level -1이면 텍스쳐(dds)에 지정된 그대로 사용하기. 0이면 가능한한 많이. 1이면 하나만. 1이상이면
/// 개수대로 리턴값은 이전에 저장되어 있는 레벨값
ZZ_SCRIPT
...
ZZ_SCRIPT
HNODE
loadAnimatable(ZSTRING pAnimatableName, HNODE hMesh, HNODE hMotion, HNODE hMaterial, HNODE hLight);
...
/// 라이트 속성 설정
/// @param hLight 라이트 핸들
/// @param pProperty 속성 스트링("ambient", "diffuse", "specular", "direction", "position",
/// "attenuation")
/// @param Value3    속성 값
/// @return          성공하면 1, 실패하면 0
ZZ_SCRIPT
...
/// @iFormat : 포맷 번호(주의: 순차적으로 지정해야 한다. 즉, 0 다음에, 1. 1 다음에 2 식으로. 스킵
/// 가능)
ZZ_SCRIPT
int
setShaderFormat(HNODE hShader, ZSTRING pVertexShaderPath, ZSTRING pPixelShaderPath, int iFormat);

/// @iVertexFormat : use combination of ZZ_VF_XXXXs, in zz_mesh.h
ZZ_SCRIPT
HNODE loadShader(ZSTRING pShaderName,
...
ZZ_DLL
HNODE loadTerrainMesh(ZSTRING pMeshName,
    float fOrigX,
    float fOrigY,
    int iDetailLevel,
    int iUVType0,
...
ZZ_DLL
HNODE loadTerrainMaterial(ZSTRING pMatName,
    HNODE hShader,
    HNODE pFirstTexture,
...
/// 예를 들면, 글로우 효과 처리
/// endScene()  이전에 호출.
/// 일반적으로 beginSprite() 이전에 호출해준다. 그렇지 않으면 스프라이트로 출력된 인터페이스도
/// 후처리가 적용되기 때문.
ZZ_SCRIPT
...
/// @return 성공하면 1, 실패하면 0
ZZ_SCRIPT int
rotateAxis(HNODE hVisible, float fAngleDegree, float fAxisX, float fAxisY, float fAxisZ);
...
ZZ_SCRIPT
int rotateByVectorRelativeScript(HNODE hVisible,
    float fLocalStartAxisX,
    float fLocalStartAxisY,
    float fLocalStartAxisZ,
    float fWorldEndTargetX,
    float fWorldEndTargetY,
    float fWorldEndTargetZ);
...
/// 자동적으로 링크가 깨진다. unlinkBone()이나 unlinkDummy(), unlinkNode() 등을 따로 호출할 필요
/// 없음.
/// @return   : 성공하면 1, 실패하면 0
ZZ_SCRIPT
...
ZZ_DLL
HNODE loadTerrainBlock(ZSTRING pTerrainBlockName,
    float fOrigX,
    float fOrigY,
    int iDetailLevel,
    int iUVType0,
...
/// 비저블 노드의 포그 상태 설정
/// 주의: 비저블의 모든 렌더유닛을 추가한 다음 호출해야 한다. 즉, 모든 유닛들을 addRenderUnit() 후에
/// 호출
///       그렇지 않으면, 현재 등록된 렌더유닛의 재질에만 적용된다.
/// 할일: 재질의 상태를 변경하지 말고, 재질에 포그받을 여부를 설정하거나, 비저블 자체에 포그받을
/// 여부를 설정토록.
ZZ_SCRIPT
...
int intersectRay(HNODE hNode,
    ...
    float dirY,
    float dirZ,
    float* pContactPointX,
    float* pContactPointY,
    float* pContactPointZ,
    float* pDistance);
...
/// 사용하는 실수형 인덱스 개수 : 4
ZZ_SCRIPT
int intersectRayScript(HNODE hNode,
    float originX,
    float originY,
    float originZ,
    float dirX,
...
ZZ_DLL
int intersectNodeTriSphereMoving(HNODE hNodeByTri_In,
    float vSphereCenter_In[3],
    float fSphereRadius_In,
    float vPrevCenter_In[3],
    float vClosestCenter_Out[3],
    float fMaxDistanceSquare_In);
...
/// 성공한다면, float_array의 처음 네 개를 각각 fContactPointX, fContactPointY, fContactPointZ,
/// fDistance 로 사용한다.
ZZ_SCRIPT
...
// 주의: 이전 인터페이스(getPointHeight)에는 float fPointZ 가 있었으나, 이 함수가 현재 2D좌표 상에서
// 최대 높이를 구하는 것이기 때문에,
...
ZZ_SCRIPT
int setMaterialLightmap(HNODE hMaterial,
    ZSTRING pLightmapFilename,
    int iX,
    int iY,
    int iWidth,
    int iHeight);
...
/// hChild 노드가 iType(예, ZZ_TYPE_MODEL)의 타입인 오브젝트의 자식(또는 후손)이라면 1을, 아니라면
/// 0을 리턴한다. 예를 들어, 현재 hChild 노드가 iType타입 그 차일드(또는 그 후손)일 경우 1을
/// 리턴한다. 주의: 자기 자신이 iType인 경우에도 1을 리턴.
ZZ_SCRIPT
...
// 그 개수를 리턴
// 주의 : collectByMinMax() 와 collectNode() 사이에는 다른 collectByMinMax()나 collectByNodeBBox()를
// 호출할 수 없음.
ZZ_SCRIPT
...
// 주의 : collectByNodeBBox() 와 collectNode() 사이에는 다른 collectByMinMax()나
// collectByNodeBBox()를 호출할 수 없음.
ZZ_SCRIPT
...
/// endScene() 이후에 호출.
/// 이 함수를 호출한 다음, 모델의 좌표를 점검하고, 알맞지 않다면, 다시 직접 갱신할 수
/// 있다(setPosition()...)
ZZ_SCRIPT
...
// 모델의 애니메이션 속도 조절.
// fSpeed : 원본 애니메이션의 속도에 곱해질 값. 1.0이면 원래 모션의 속도 그대로 유지. 0.5 면 두 배
// 느리게, 2.0이면 두 배 빠르게 리턴값 : 1이면 성공. 0이면 실패 주의 :
...
// 검잔상 객체 생성(스크립트 전용)
// fDistancePerPoint : 1개의 포인트가 새로 생성되는 거리(cm 단위). 즉, fDistancePerPoint 거리만큼
// 이동되면 새로 포인트가 생성된다. iDurationInMilliSecond : 밀리 세컨드 단위의 라이프 타임.
// 포인트들은 이 시간만큼 살아있다 죽는다. pTexturePath : 사용될 텍스쳐 경로 fColorRed/Green/Blue :
// 텍스쳐에 블렌딩될 색상값(0 ~ 1.0) fSP_X/Y/Z : 검의 원점에 대한 시작 포인트의 로컬 좌표 fEP_X/Y/Z
// : 검의 원점에 대한 종료 포인트의 로컬 좌표
ZZ_SCRIPT
...
// fDistancePerPoint : 1개의 포인트가 새로 생성되는 거리(cm 단위). 즉, fDistancePerPoint 거리만큼
// 이동되면 새로 포인트가 생성된다. iDurationInMilliSecond : 밀리 세컨드 단위의 라이프 타임.
// 포인트들은 이 시간만큼 살아있다 죽는다. pTexturePath : 사용될 텍스쳐 경로 Color : 텍스쳐에
// 블렌딩될 색상값. D3DCOLOR. ARGB vStrat : 검의 원점에 대한 시작 포인트의 로컬 좌표 vEnd : 검의
// 원점에 대한 종료 포인트의 로컬 좌표
ZZ_DLL
...
ZZ_DLL
void worldToScreen(float fWorldX,
    float fWorldY,
    float fWorldZ,
    float* fScreenX,
    float* fScreenY,
...
ZZ_SCRIPT
HNODE loadOceanBlock(ZSTRING pOceanBlockName,
    float fOrigX,
    float fOrigY,
    float fOrigZ,
    float fWidth,
...
*/
ZZ_DLL
float getSliding(HNODE hVisible,
    float fMin,
    float fMax,
...
ZZ_SCRIPT
float
getSlidingScript(HNODE hVisible, float fMin, float fMax, float fNeighborDistance, float fVelocity);
...
ZZ_SCRIPT
HNODE loadTerrainBlockTest(ZSTRING pTerrainBlockName,
    int iX,
    int iY,
    int iType,
    HNODE hFirstMat,
...
/// 디폴트 : 1(점진적)
/// @iDelayedLoad : 0(즉시 로딩), 1(지연 로딩), 2(현재 지연 로딩된 상태를 플러시하고, 현재 상태
/// 유지) 점진적로딩모드 : 일정 프레임간격으로 메시와 텍스쳐를 파일에서 읽는다. 즉시로딩모드 :
/// loadMesh() 또는 loadColormapMaterial() 등의 호출시 즉시 파일을 읽는다.
ZZ_SCRIPT
...
ZZ_SCRIPT
int
setLazyBufferSize(int iTextureSize, int iNormalMeshSize, int iTerrainMeshSize, int iOceanMeshSize);
...
/// @iMipLevels 밉맵레벨. -1 이면 파일(dds)에 지정된 대로, 0이면 가능한한 많이 생성. 1이면 밉맵 사용
/// 안함.
...
/// 안함.
/// @bUseFilter 필터링 적용 여부. 0이면 필터링 사용 안함.
/// @iPoolType 0 : Default Pool(그래픽카드에 생성), 1 : Managed Pool(DX가 관리), 2 : SysetmMem
/// Pool(시스템 메모리에 생성)
/// @return 성공하면 텍스쳐 핸들, 실패하면 0
ZZ_SCRIPT
HNODE loadTextureWithPool(ZSTRING pTextureName,
    ZSTRING pTextureFileName,
    int iMipLevels,
    int bUseFilter,
    int iPoolType);
...
ZZ_SCRIPT
HNODE loadFont(ZSTRING pFontIDName,
    ZSTRING pFontName,
    int iFontSize,
    int iBold,
    int iItalic,
...
ZZ_SCRIPT
HNODE
loadSkyMaterial(ZSTRING pMaterialName, HNODE hShader, ZSTRING pMapFileName1, ZSTRING pMapFileName2);
...
ZZ_DLL
int intersectRayNormal(HNODE hVisible_In,
    float vRayOrigin_In[3],
    float vRayDirection_In[3],
    float vContactPoint_Out[3],
    float vContactNormal_Out[3]);
...
ZZ_SCRIPT
int setVisibleRangeFromCamera(HNODE hVisible,
    float fDistanceFromCameraStart,
    float fDistanceFromCameraEnd);
...
//--------------------------------------------------------------------------------
/// @hFont loadFont() 로부터 얻은 핸들.
/// @bUseSprite 스프라이트 사용 여부. 1이면, 스프라이트의 beginSprite()/endSprite() 블록 안에서
/// 호출되어야 한다.
/// @iX 스크린 x좌표. 0부터 시작.
/// @iY 스크린 y좌표. 0부터 시작. 아래 방향으로 증가.
/// @pMsg 출력할 메세지.
...
ZZ_DLL
int
drawFontf(HNODE hFont, int bUseSprite, int iX, int iY, ZZ_COLOR Color, const char* pMsgFormat, ...);

ZZ_DLL
int drawFontf(HNODE hFont, int bUseSprite, ZZ_RECT* pRect, const char* pMsgFormat, ...);

ZZ_DLL
int drawFontf(HNODE hFont,
    int bUseSprite,
    ZZ_RECT* pRect,
    ZZ_DWORD dwFormat,
