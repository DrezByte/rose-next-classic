...
CClientDB&
CClientDB::GetInstance() {
    static CClientDB s_Instance;
    return s_Instance;
}
bool
CClientDB::Initialize() {
    CoInitialize(NULL);
    return true;
}
...
CClientDB::Connect() {
    ...
    HRESULT hr = CoCreateInstance(CLSID_JETOLEDB_4_00,
        NULL,
...
bool
CClientDB::UnInitialize() {
    CoUninitialize();
    return true;
}

bool
CClientDB::CheckError(HRESULT hr) {
    switch (hr) {
        case S_OK:
            return true;
...
ICommandText*
CClientDB::CreateCommand(IDBInitialize* pIDBInitialize, LPCWSTR wCmdString) {
...
CClientDB::CreateInventoryTable() {
    ...
    WCHAR wCmdString[] =
        L"CREATE TABLE INVENTORY ( name TEXT(30), realindex long , virtualindex long )";
    ...
    if (FAILED(hr = pICommandText->Execute(NULL, IID_NULL, NULL, &cNumRows, NULL))) {
...
void
CClientDB::GetInventoryData(const char* pszName, std::list<S_InventoryData>& Inventory) {
    ICommandText* pICommandText = NULL;
    ...
    wsprintfW(wCmdString,
        L"SELECT realindex,virtualindex FROM INVENTORY WHERE name = '%S'",
        pszName);
    ...
    if (FAILED(hr = pICommandText
                        ->Execute(NULL, IID_IRowset, NULL, &cNumRows, (IUnknown**)&pIRowset))) {
...
bool
CClientDB::UpdateInventoryData(const char* pszName, S_InventoryData Data) {
    ICommandText* pICommandText = NULL;
    HRESULT hr;
    LONG cNumRows = 0;
    ...
    wsprintfW(wCmdString,
        L"UPDATE INVENTORY SET virtualindex = %d WHERE name = '%S' AND realindex = %d",
        Data.lVirtualIndex,
        pszName,
        Data.lRealIndex);
...
bool
CClientDB::InsertInventoryData(const char* pszName, S_InventoryData Data) {
    ICommandText* pICommandText = NULL;
    HRESULT hr;
    LONG cNumRows = 0;
    ...
    wsprintfW(wCmdString,
        L"INSERT INTO INVENTORY ( name, realindex, virtualindex ) VALUES ( '%S', %d, %d )",
        pszName,
        Data.lRealIndex,
        Data.lVirtualIndex);
...
bool
CClientDB::CheckErrorGetNextRows(HRESULT hr) {
    switch (hr) {
        case S_OK:
            return true;
...
bool
CClientDB::CheckErrorGetData(HRESULT hr) {
    switch (hr) {
        case S_OK:
            return true;
            break;
