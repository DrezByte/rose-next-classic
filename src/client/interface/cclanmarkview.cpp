...
void
CClanMarkView::Draw(CObjCHAR* pObjChar /*, const POINT& ptDraw*/, const D3DVECTOR& vDrawPos) {
    assert(pObjChar);
    ...
    if (pObjChar->GetClanID()) {
        if (pObjChar->GetClanMarkBack() != 0) {
            ...
            g_DrawImpl.Draw(vDrawPos.x,
                vDrawPos.y,
                vDrawPos.z,
                IMAGE_RES_CLANBACK,
                pObjChar->GetClanMarkBack());
            g_DrawImpl.Draw(vDrawPos.x,
                vDrawPos.y,
                vDrawPos.z,
                IMAGE_RES_CLANCENTER,
                pObjChar->GetClanMarkCenter());
        } else ///유저등록 이미지 사용시
        {
            ...
            if (pUserDefinedClanMark == NULL) {
                ...
                CClanMarkUserDefined::GetClanMarkFileName(
                    CGame::GetInstance().GetSelectedServerID(),
                    pObjChar->GetClanID(),
                    file_name);
                pUserDefinedClanMark =
                    CClanMarkManager::GetSingleton().GetClanMark(file_name.c_str(),
                        pObjChar->GetClanMarkCenter(),
                        pObjChar->GetClanID());
