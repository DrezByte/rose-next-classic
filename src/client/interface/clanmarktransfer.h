...
private:
    ...
    bool GetImageFileCRC(HANDLE hBmpFile,
        WORD* pBmpCRC16,
    ...
    bool SendToServerTempZipFile(const char* TempZipFileName,
        WORD* pBumCRC16,
        DWORD* pdwFileSizeHigh,
        DWORD* pdwFileSizeLow);

    bool SaveZipFileFromServer(const char* TempZipFileName,
        DWORD* pdwNumberOfBytesWritten,
        BYTE* pData,
        int iSize);
    void CreateImageFileFromZipFile(const char* TempZipFileName,
        int iClanID,
        DWORD* pdwNumberOfBytesWritten);
};
