...
CItem*
CItemSlot::CreateItem(int iInvenIdx, tagITEM& Item) {
    ...
    if (iInvenIdx >= 0 && iInvenIdx < MAX_EQUIP_IDX) {
        ...
        switch (Item.GetTYPE()) {
            ...
            default:
                assert(
                    0 && CStr::Printf("CItemSlot::AddItem Invalid ItemType(%d)", Item.GetTYPE()));
                break;
        }
...
CItem*
CItemSlot::AddItem(int iInvenIdx, tagITEM& Item) {
    CItem* pItem = NULL;
...
CItem*
CItemSlot::AddItem(tagITEM& Item) {
...
int
CItemSlot::SetItem(int iInvenIdx, tagITEM& Item) {
    assert(iInvenIdx >= 0 && iInvenIdx < INVENTORY_TOTAL_SIZE);
...
CItem*
CItemSlot::GetItem(int iInvenIdx) {
    assert(iInvenIdx >= 0 && iInvenIdx < INVENTORY_TOTAL_SIZE);
...
void
CItemSlot::ChangeItemStatus(int iInvenIdx) {
...
int
CItemSlot::GetEmptySlot(int iStartIdx, int iEndIdx) {
...
void
CItemSlot::UpdateItem(int iInvenIdx, tagITEM& Item) {
    assert(iInvenIdx >= 0 && iInvenIdx < INVENTORY_TOTAL_SIZE);
