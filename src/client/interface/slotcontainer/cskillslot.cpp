...
void
CSkillSlot::SetSkillSlot(short nSlotIndex, short nSkillIndex, short nSkillLevel) {
...
//----------------------------------------------------------------------------------------------------
void
CSkillSlot::SkillLevelUp(int iSkillSlotNo, int iSkillIndex) {
...
///			서버로 부터는 스킬번호만 받기 때문에 역으로 번호로 슬롯을 얻어올 필요가 있다.( 타이머
///세팅
///)
//----------------------------------------------------------------------------------------------------

CSkill*
CSkillSlot::GetSkillBySkillIDX(int iSkillIDX) {
    CSkill* pSkill = NULL;
...
//----------------------------------------------------------------------------------------------------
CSkill*
CSkillSlot::GetSkillByBaseSkillIDX(int iBaseSkillIDX) {
    CSkill* pSkill = NULL;
...
short
CSkillSlot::GetSkillIndex(short nSlotIndex) {
    if (nSlotIndex >= MAX_LEARNED_SKILL_CNT || nSlotIndex < 0 || m_SkillSlot[nSlotIndex] == NULL)
        return 0;
...
short
CSkillSlot::GetSkillIndex(short nIndex, short nTabIndex) {
    if (nIndex < 0)
        return 0;
...
short
CSkillSlot::GetSkillLevel(short nSlotIndex) {
    if (nSlotIndex >= MAX_LEARNED_SKILL_CNT || nSlotIndex < 0 || m_SkillSlot[nSlotIndex] == NULL)
        return 0;
...
short
CSkillSlot::GetSkillLevel(short nIndex, short nTabIndex) {
    if (nIndex < 0)
        return 0;
...
short
CSkillSlot::GetSkillDelayTime(short nSlotIndex) {
    if (nSlotIndex >= MAX_LEARNED_SKILL_CNT || nSlotIndex < 0 || m_SkillSlot[nSlotIndex] == NULL)
        return 0;
...
short
CSkillSlot::GetSkillDelayTime(short nIndex, short nTabIndex) {
    if (nIndex < 0)
        return 0;
...
short
CSkillSlot::GetSkillIconIndex(short nSlotIndex) {
...
short
CSkillSlot::GetSkillIconIndex(short nIndex, short nTabIndex) {
...
void
CSkillSlot::RemoveBySlotIndex(short nSlotIndex) {
...
//--------------------------------------------------------------------------------------------------------/
void
CSkillSlot::SetActiveSkillEnableByRideState(bool ride) {
    ...
    if (ride) /// pat 스킬만 Enable 나머지는 Disable
    {
        if (false == g_pAVATAR->m_Inventory.m_ItemRIDE[0].IsEmpty()) {
            int item_type = ITEM_TYPE(g_pAVATAR->m_Inventory.m_ItemRIDE[0].GetTYPE(),
                g_pAVATAR->m_Inventory.m_ItemRIDE[0].GetItemNO());
            switch (item_type) {
                case TUNING_PART_BODY_CART: ///카트 본체
                    for (int index = MAX_LEARNED_SKILL_PER_PAGE;
                         index < MAX_LEARNED_SKILL_PER_PAGE * 2;
                         ++index) {
                        if (m_SkillSlot[index] != NULL) {
                            if (SKILL_ATTRIBUTE_CART
                                == SKILL_ATTRIBUTE(m_SkillSlot[index]->GetSkillIndex()))
    ...
    {
        for (int index = MAX_LEARNED_SKILL_PER_PAGE; index < MAX_LEARNED_SKILL_PER_PAGE * 2;
             ++index) {
            if (m_SkillSlot[index] != NULL) {
...
// 2인승 카트
void
CSkillSlot::SetActiveSkillEnableByRideState_Aid(bool ride) {
