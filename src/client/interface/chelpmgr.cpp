...
inline char*
GetHelpMsg(int i) {
    return (char*)CStringManager::GetSingleton().GetTipContent(i);
}
inline char*
GetHelpTitle(int i) {
    return (char*)CStringManager::GetSingleton().GetTipHeader(i);
}
...
CHelpMgr::SetNextHelpMsg() {
    ...
    if (pDlg) {
        ...
        if (pMsgDlg->SetMessage(szTitle,
                szMsg,
                CSystemMsgDlg::MSG_TYPE_HELP,
                color,
...
int
CHelpMgr::GetNextIndex(int iPrevIndex) {
    if (m_HelpData.empty())
        return -1;
