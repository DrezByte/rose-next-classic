...
int
CCursor::SetCursorType(int iType, CInfo* pMouseInfo) {
...
CInfo&
CCursor::GetMouseInfo() {
    return m_MouseInfo;
}

int
CCursor::RefreshCursor() {
    assert(m_pCurrState);
...
int
CCursor::GetCurrCursorType() {
    return m_iType;
}

void
CCursor::ReloadCursor() {
    for (int i = 0; i < STATE_MAX; ++i)
