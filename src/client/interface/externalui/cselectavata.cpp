...
void
CSelectAvata::SetEUIObserver(CExternalUIManager* pObserver) {}

UINT
CSelectAvata::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
/// 캐릭터가 선택되고 서버에 알리기 전에 Msg Queue를 모두 지워서 두번 보내어지는것을 막는다. 2004 /6
/// /20
void
CSelectAvata::SendSelectAvataReq() {
    ...
    if (CJustModelAVT* pSelectedAvt = refData.GetSelectedAvatar()) {
        if (pSelectedAvt->IsPlatinum()
            ...
            g_EUILobby.ShowMsgBox(STR_ONLY_PLATINUM_SELECT_CHAR,
                CTMsgBox::BT_OK,
                true,
                GetDialogType());
        } else if (!m_bWaitServerReply) {
            if (pSelectedAvt->GetDeleteRemainSEC()) {
                CTCommand* pCmd = new CTCmdReviveCharacter(refData.GetSelectedAvatarIndex(),
                    refData.GetSelectedAvatarName());
                ...
                g_pNet->Send_cli_SELECT_CHAR(refData.GetSelectedAvatarIndex(),
                    (char*)refData.GetSelectedAvatarName());
                LogString(LOG_NORMAL, "Send_cli_SELECT_CHAR\n");
                m_bWaitServerReply = true;
            }
        }
    } else
        g_EUILobby.ShowMsgBox(STR_REQUEST_SELECT_CHARACTER,
            CTMsgBox::BT_OK | CTMsgBox::BT_CANCEL,
            true,
            GetDialogType());
}
...
void
CSelectAvata::RecvAvataList(t_PACKET* recvPacket) {
    short nI = 0;
    ...
    for (nI = 0; nI < recvPacket->m_wsv_CHAR_LIST.m_btCharCNT; nI++) {
        ...
        pnPartItemIDX = (tagPartITEM*)Packet_GetDataPtr(recvPacket,
            nOffset,
...
void
CSelectAvata::OnLButtonUp(unsigned iProcID) {
    switch (iProcID) {
        case IID_BTN_CREATE:
        ...
        case IID_BTN_DELETE: {
            ...
            if (CJustModelAVT* pSelectedAvt = refData.GetSelectedAvatar()) {
                if (pSelectedAvt->GetDeleteRemainSEC()) {
                    ...
                    CTCommand* pCmd = new CTCmdDeleteCharacter(refData.GetSelectedAvatarIndex(),
                        refData.GetSelectedAvatarName());
                    g_EUILobby.ShowMsgBox(CStr::Printf("%s%s",
                                              refData.GetSelectedAvatarName(),
                                              STR_QUERY_DELETE_CHARACTER),
                        CTMsgBox::BT_OK | CTMsgBox::BT_CANCEL,
                        true,
                        GetDialogType(),
                        pCmd);
