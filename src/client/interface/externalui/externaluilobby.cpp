...
bool
CExternalUILobby::InitEUIManager() {
...
bool
CExternalUILobby::CreateLoginDlg() {
    //	::CreateLoginDlg();
...
void
CExternalUILobby::CloseLoginDlg() {
...
bool
CExternalUILobby::CreateServerListDlg() {
    m_pCurrDialog->Hide();
...
void
CExternalUILobby::CloseServerListDlg() {
    ((CTDialog*)m_EUIManager.GetEUI(EUI_SELECT_SERVER))->Hide();
}

bool
CExternalUILobby::ShowDialog(int iType) {
...
bool
CExternalUILobby::HideDialog(int iType) {
    assert(iType >= EUI_LOGIN && iType < EUI_MAX);
    if (iType >= EUI_LOGIN && iType < EUI_MAX) {
...
void
CExternalUILobby::CloseWaitAvataListDlg() {
    HideMsgBox();
    CreateAvataListDlg();
    //	DestroyWaitAvataDlg();
}
...
void
CExternalUILobby::CloseAvataListDlg() {
    ((CTDialog*)m_EUIManager.GetEUI(EUI_SELECT_AVATA))->Hide();
}

// Create Avata
bool
CExternalUILobby::CreateCreateAvataDlg() {
    m_pCurrDialog->Hide();
...
void
CExternalUILobby::CloseCreateAvataDlg() {
...
void
CExternalUILobby::CloseWaitJoinServerDlg() {
    HideMsgBox();
}

void
CExternalUILobby::Update() {
    POINT ptMouse;
...
void
CExternalUILobby::ReSetAvataListBox() {
...
void
CExternalUILobby::ShowMsgBox(char* szMsg,
    short int iButtonType,
    bool bModal,
    unsigned int iInvorkerDlgType,
    CTCommand* pCmdOk,
    ...
    if (m_pMsgBox) {
        ...
        POINT pt = {g_pCApp->GetWIDTH() / 2 - m_pMsgBox->GetWidth() / 2,
...
void
CExternalUILobby::HideMsgBox() {
    if (m_pMsgBox)
        m_pMsgBox->Hide();
}
void
CExternalUILobby::DrawMsgBox() {
...
void
CExternalUILobby::FailCreateAvata() {
...
void
CExternalUILobby::ShowWaitMsgBox() {
...
void
CExternalUILobby::AddTCommand(int iInvorkerDlgID, CTCommand* pCmd) {
    if (pCmd == NULL)
        return;
...
void
CExternalUILobby::SendReqFirstServerChannelList() {
    ((CSelectServer*)m_EUIManager.GetEUI(EUI_SELECT_SERVER))->SendReqFirestServerChannelList();
}

CExternalUIManager&
CExternalUILobby::GetExternalUIManager() {
    return m_EUIManager;
}