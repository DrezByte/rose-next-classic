...
///
void
CSelectServer::SetEUIObserver(CExternalUIManager* pObserver) {
...
///
UINT
CSelectServer::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    UINT Ret = 0;
...
void
CSelectServer::SendSelectServerReq() {
...
void
CSelectServer::SendReqFirestServerChannelList() {
...
void
CSelectServer::RecvSelectServer(t_PACKET* recvPacket) {
    if (recvPacket->m_lsv_SELECT_SERVER.m_btResult != RESULT_SELECT_SERVER_OK) {
        switch (recvPacket->m_lsv_SELECT_SERVER.m_btResult) {
            case RESULT_SELECT_SERVER_FAILED:
                g_EUILobby.ShowMsgBox(STR_RESULT_SELECT_SERVER_FAILED,
                    CTMsgBox::BT_OK,
                    true,
                    GetDialogType());
                break;
            case RESULT_SELECT_SERVER_FULL:
                g_EUILobby.ShowMsgBox(STR_RESULT_SELECT_SERVER_FULL,
                    CTMsgBox::BT_OK,
                    true,
                    GetDialogType());
                break;
            case RESULT_SELECT_SERVER_INVALID_CHANNEL:
                g_EUILobby.ShowMsgBox(STR_RESULT_SELECT_SERVER_INVALID_CHANNEL,
                    CTMsgBox::BT_OK,
                    true,
                    GetDialogType());
                break;
            ...
            case RESULT_SELECT_SERVER_NEED_CHARGE:
                g_EUILobby.ShowMsgBox(STR_RESULT_SELECT_SERVER_NEED_CHARGE,
                    CTMsgBox::BT_OK,
                    true,
                    GetDialogType());
                break;
...
void
CSelectServer::OnLButtonUp(unsigned iProcID) {
    switch (iProcID) {
        case IID_BTN_OK:
            SendSelectChannel();
            break;
...
void
CSelectServer::OnLButtonDblClk(unsigned iProcID) {
    switch (iProcID) {
...
unsigned int
CSelectServer::FindSvrId(short ItemID) {
...
ServerInfo*
CSelectServer::GetServerInfo(int iIndex) {
...
CSelectServer::Show() {
    ...
    POINT pt = {g_pCApp->GetWIDTH() - GetWidth() - 30, 100};
...
void
CSelectServer::MsgBoxProc(unsigned int iMsgType, short int iButton) {
    switch (iMsgType) {
        case MSG_TYPE_EXIT:
            g_pCApp->SetExitGame();
            break;
...
int
CSelectServer::FindChannelNo(int iIndex) {
...
void
CSelectServer::SendSelectChannel() {
    CWinCtrl* pCtrl = Find(IID_LISTBOX_SERVER);
    assert(pCtrl);
    if (pCtrl == NULL)
        return;
    ...
    if (pCtrl && pCtrl->GetControlType() == CTRL_LISTBOX) {
        ...
        if (iChannelNo) {
            ...
            g_EUILobby.ShowMsgBox(STR_RECEIVE_CHARACTER_LIST,
                CTMsgBox::BT_CANCEL,
                true,
                GetDialogType(),
                NULL,
...
void
CSelectServer::OnLButtonDown(unsigned iProcID) {
    switch (iProcID) {
        case IID_LISTBOX_SERVER: {
            SendSelectServerReq();
            break;
...
void
CSelectServer::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);
    ...
    switch (pEvent->GetID()) {
        ...
        case CTEventServerList::EID_ADD_CHANNELSERVER: {
            ...
            if (pCtrl && pCtrl->GetControlType() == CTRL_LISTBOX) {
                ...
                int index = 0;
                for (iter = m_ServerInfoList.begin(); iter != m_ServerInfoList.end();
                     ++iter, ++index) {
