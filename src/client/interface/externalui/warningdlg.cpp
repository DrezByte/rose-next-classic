...
///
void
CWarningDlg::SetEUIObserver(CExternalUIManager* pObserver) {
    m_pEUIManager = pObserver;
}

bool
CWarningDlg::Create(const char* szIDD) {
    CTDialog::Create(szIDD);
    return true;
}

UINT
CWarningDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    UINT Ret = 0;
    CWinCtrl* pCtrl = NULL;
...
void
CWarningDlg::MsgBoxProc(unsigned int iMsgType, short int iButton) {
    //???
}

void
CWarningDlg::OnLButtonUp(unsigned iProcID) {
    switch (iProcID) {
        case IID_BTN_OK:
...
void
CWarningDlg::OnLButtonDown(unsigned iProcID) {}
