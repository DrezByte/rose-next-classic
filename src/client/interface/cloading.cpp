...
bool
CLoading::InitLoadingImageManager() {
...
bool
CLoading::LoadTexture(int iZoneNo, int iPlanetNO) {
...
CLoading::Draw() {
    ...
    if (iWidth != 0 && iHeight != 0)
        D3DXMatrixScaling(&mat,
            (float)g_pCApp->GetWIDTH() / (float)iWidth,
            (float)g_pCApp->GetHEIGHT() / (float)iHeight,
            1.0f);
