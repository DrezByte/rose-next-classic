...
/// Load TSI file
bool
CImageRes::LoadRES(CFileSystem* pFileSystem, const char* strResPath) {
    if (pFileSystem == NULL)
        return false;
...
void
ReleaseTexture(stTexture texture) {
...
stTexture*
CImageRes::GetTexture(int iSprIndex) {
...
stSprite*
CImageRes::GetSprite(int iSprIndex) {
    if ((unsigned int)iSprIndex >= m_SpriteVec.size() || iSprIndex < 0) {
        assert(0 && "Argument is Invalid @CImageRes::GetSprite()");
        return NULL;
    }
...
bool
CImageResManager::LoadImageResources() {
...
void
CImageResManager::ReleaseResources() {
...
void
CImageResManager::ReleaseResource(short i) {
...
CImageRes*
CImageResManager::GetImageRes(int iType) {
...
HNODE
CImageResManager::Load_NpcFace(int index) {
    CFileSystem* pFileSystem = (CVFSManager::GetSingleton()).GetFileSystem();
    if (pFileSystem == NULL)
        return false;
