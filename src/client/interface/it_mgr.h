 ...
 *  - WM_LBUTTONDOWN이 일어나거나 Show가 된Dlg는 리스트의 맨뒤로 옮겨서 그리기는 맨 마지막으로 하고
 * 처리는 제일 먼저 하게 한다.
 *
 *
 * @Author		최종진
...
public:
    ...
    enum { /// 채팅 입력 방식 타입
        ...
        INPUTTYPE_AUTOENTER, /// Show시에 해당 Dialog에 EditBox가 있다면 자동 입력된다.Enter후
                             /// 자동으로 다시 Focus를 가진다.
    };
    ...
    void
    OpenPrivateChatDlg(DWORD dwUserTag, BYTE btStatus, const char* pszName); /// 1:1 대화창 오픈
    ...
    void OpenMsgBox(const char* szMsg,
        ...
        bool bModal = true,
        unsigned int iInvokerDlgID = 0,
        CTCommand* pCmdOk = NULL,
        CTCommand* pCmdCancel = NULL,
        int iMsgType = 0);
    ...
    /*---------------------------------------------------------------------------*/
    void InitInterfacePos(); /// 인터페이스 초기화, 해상도가 바뀌거나 인터페이스를 최초 위치로
                             /// 옮기고 싶을때 사용하는 Method
    void SetInterfacePosBySavedData(); ///	저장되있던 Dialog Position이 있을경우 가져와서 Dialog의
                                       ///위치들을 수정한다.
    ...
    /*---------------------------------------------------------------------------*/
    void CloseDialogAllExceptDefaultView(); /// 기본적으로 보이는 다이얼로그를 제외한 모든
                                            /// 다이얼로그를 닫는다.
...
private:
    ...
    void
    AppendDlg(short nDlgType, CTDialog* pDLG, int iID = 0); /// Dialog들을 보관하는 컨테이너에 추가
    void InsertDlg(short nDlgType,
        CTDialog* pDlg,
        int iNextID,
        int iID = 0); /// 컨테이너에서 특정 Dialog뒤에 해당 다이얼로그를 추가한다.

    void MoveDlg2ListEnd(
        CTDialog* pDlg); /// 특정 Dialog를 리스트의 맨뒤로 이동 ( 화면에서 제일위로 오게된다)
    void MoveDlgIcon2ListEnd(CIconDialog*
            pIcon); /// 특정 Dialog Icon을 리스트의 맨뒤로 이동 ( 화면에서 제일위로 오게된다)

    void ProcessEscKeyDown(); /// ESC 키가 눌렸을때 처리하는 Method

private:
    list_dlgs m_Dlgs; /// 게임내 사용되는 인터페이스 다이얼로그들을 보관하는 컨테이너
    CTCommandQ m_CommandQ; /// Command Queue( 다이얼로그에 종속적인 Command는 각 다이얼로그가 가지고
                           /// 있는다)
    ...
    std::list<CIconDialog*>
        m_Icons; /// 열린 다이얼로그들이 화면상에 아이콘화 되었을때 저장되는 컨테이너
    std::map<int, POINT> m_ptIcons; /// 아이콘화된 다이얼로그의 위치를 저장해두는 컨테이너

    CIconDialog*
        m_pDialogIconFromMenu; /// 메뉴창에서 버튼을 드래그할때 필요한 기본 아이콘을 보관한다.
