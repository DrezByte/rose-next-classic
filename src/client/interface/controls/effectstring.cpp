...
void
CEffectString::SetColor(BYTE red, BYTE green, BYTE blue) {
    m_color_red = red;
    m_color_green = green;
    m_color_blue = blue;
}
...
bool
CEffectString::SetString(int font,
    char* message,
    POINT draw_position,
    int display_width,
    DWORD display_time) {
...
/**
 * Fade In/Out 기능의 글자색깔 변화는 현재 아웃라인폰트에 적용시 그림자가 남게 되어서 안됨 ,
 * 주석처리함 추후에 엔진에서 그림자의 색깔까지 바꿀수 있도록 수정되면 적용
 *
 * @Author		최종진
 * @Date			2005/9/6
 */
int
CEffectString::Update() {
    // if end return -1;
    ...
    if (int line_count = m_parser.GetLineCount()) {
        ...
        for (int i = 0; i < line_count; ++i) {
            if (message = m_parser.GetString(i)) {
                drawFont(g_GameDATA.m_hFONT[m_font],
                    true,
                    0,
                    offset_y,
                    D3DCOLOR_ARGB(m_color_alpha, m_color_red, m_color_green, m_color_blue),
                    message);
...
void
CEffectString::SetType(int type) {
    m_type = type;
}

int
CEffectString::GetType() {
    return m_type;
}
