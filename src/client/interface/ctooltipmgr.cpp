...
CToolTipMgr&
CToolTipMgr::GetInstance() {
    static CToolTipMgr s_Instance;
    return s_Instance;
}

void
CToolTipMgr::RegToolTip(int x, int y, const char* pszMsg) {
    m_Info.Clear();
    m_bHasDrawInfo = true;
    POINT pt = {x + 20, y};
...
void
CToolTipMgr::RegUIInfo(const int x, const int y, const DWORD dwDlgType, const int iToolTipID) {
    m_Info.Clear();
    m_bHasDrawInfo = true;
    POINT pt = {x + 20, y};
...
void
CToolTipMgr::RegistInfo(const CInfo& Info) {
    m_Info.Clear();
    m_Info = Info;
    m_bHasDrawInfo = true;
}