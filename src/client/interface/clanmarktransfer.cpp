...
void
CClanMarkTransfer::RequestMarkFromServer(int iClanID) {
    g_pNet->Send_cli_CLANMARK_REQ(iClanID);
}

bool
CClanMarkTransfer::RegisterMarkToServer(int iClanID, const char* pStrFileName) {
    assert(pStrFileName);
    ...
    if (hBmpFile == INVALID_HANDLE_VALUE) {
        ...
        g_itMGR.OpenMsgBox(
            CStr::Printf("%s %s", STR_CLANMARK_FILE_NOTFOUNT_REGISTER, STR_CLANMARK_HELP_HOMEPAGE));
        return false;
    }
...
CClanMarkTransfer::Check_BmpFile(HANDLE hFile, SIZE size) {
    ...
    if (dwFileSizeLow < BmpHeaderSize) {
        ...
        g_itMGR.OpenMsgBox(
            CStr::Printf("%s %s", STR_CLANMARK_INVALID_FORMAT, STR_CLANMARK_HELP_HOMEPAGE));
        return false;
    }
    ...
    if (size.cx > 0 && size.cy > 0) {
        if (BitmapInfoHeader->biWidth != size.cx || BitmapInfoHeader->biHeight != size.cy) {
            _RPTF2(_CRT_ASSERT,
                "Invalid Canvas Size( Require - Width:20,Height:20 ) ,File (Width:%d, Height:%d)",
                BitmapInfoHeader->biWidth,
                BitmapInfoHeader->biHeight);

            g_itMGR.OpenMsgBox(
                CStr::Printf("%s %s", STR_CLANMARK_INVALID_SIZE, STR_CLANMARK_HELP_HOMEPAGE));
            return false;
        }
    }
    return true;
...
CClanMarkTransfer::SendToServerTempZipFile(const char* TempZipFileName,
    ...
    DWORD* pdwFileSizeHigh,
    DWORD* pdwFileSizeLow) {
    HANDLE hTempZipFile = CreateFile(TempZipFileName,
        GENERIC_READ,
        FILE_SHARE_READ,
        NULL,
        OPEN_EXISTING,
    ...
    if (hTempZipFile == INVALID_HANDLE_VALUE) {
        DWORD dwErr = GetLastError();
        _RPTF1(_CRT_ASSERT,
            "CreateFile Error : Temporary ZipFile - Invalid_handle_value(%d)",
            dwErr);
        ...
        if (*pdwFileSizeHigh == 0) {
            ...
            if (ReadFile(hTempZipFile, buffer, *pdwFileSizeLow, &NumberOfBytesReadZipFile, NULL)) {
                ...
                if (NumberOfBytesReadZipFile <= 1000) {
                    ...
                    g_itMGR.OpenMsgBox(CStr::Printf("%s %s",
                        STR_CLANMARK_INVALID_SIZE,
...
CClanMarkTransfer::CreateImageFileFromZipFile(const char* TempZipFileName,
    ...
    HANDLE hBmpFile = CreateFile(strBmpFileName.c_str(),
        GENERIC_WRITE,
        0,
        0,
        CREATE_ALWAYS,
