...
void
CTFontImpl::Draw(int iFont, bool bUseSprite, int x, int y, D3DCOLOR Color, const char* pMsg) {
...
int
CTFontImpl::GetFontHeight(int iFont) {
    if (iFont < 0 || iFont >= MAX_FONT)
        return 0;
...
SIZE
CTFontImpl::GetFontTextExtent(int iFont, const char* pszText) {
    SIZE size = {0, 0};
    if (iFont < 0 || iFont >= MAX_FONT)
        return size;
    if (pszText == NULL)
...
void
CTFontImpl::SetTransformSprite(float x, float y, float z) {
    D3DXMATRIX mat;
