...
CDragNDropMgr&
CDragNDropMgr::GetInstance() {
    static CDragNDropMgr s_Instance;
    return s_Instance;
}

void
CDragNDropMgr::Draw(POINT ptMouse) {
    if (m_pItem) {
...
void
CDragNDropMgr::DragStart(CDragItem* pItem) {
    assert(pItem);
...
void
CDragNDropMgr::DragEnd(int iTargetType) {
    if (m_pItem) {
 ...
 */
void
CDragNDropMgr::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
