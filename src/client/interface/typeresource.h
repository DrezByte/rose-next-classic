...
#define STR_CHANGE_TRADEITEM_IN_MYREADY       \
    CStringManager::GetSingleton().GetString( \
        52) /// 1:1 거래중 내가 확인 버튼을 누른 상태에서 상대방의 물품이 변경되었다.
...
#define STR_SKILLPOWER_EFFECT_0 \
    CStringManager::GetSingleton().GetString(80) ///계산식에 따른 스킬파워에 영향을 미치는 것
...
#define STR_INVALID_TARGET \
    CStringManager::GetSingleton().GetString(128) // 타겟이 유효하지 않습니다.
#define STR_WEAPON_MISMATCH \
    CStringManager::GetSingleton().GetString(129) // 필요한 무기가 장비 되어 있지 않습니다.
#define STR_GET_EXP CStringManager::GetSingleton().GetString(130) // 경험치 %d을 얻었습니다.
#define STR_NOTIFY_01 \
    CStringManager::GetSingleton().GetString(131) // 더이상 아이템을 갖을수 없습니다.
#define STR_NOTIFY_02 \
    CStringManager::GetSingleton().GetString(132) // 소유권이 없습니다. 잠시후 획득하세요.
...
#define STR_NOTIFY_06 \
    CStringManager::GetSingleton().GetString(136) // 조건이 맞지 않아 사용할수 없습니다.
...
#define STR_DONT_DROP_ITEM \
    CStringManager::GetSingleton().GetString(202) /// 버릴수 없는 아이템입니다.
#define STR_DONT_EXCHANGE_ITEM \
    CStringManager::GetSingleton().GetString(203) /// 교환할수 없는 아이템입니다.

/// 스킬 습득에 관련한 스트링
#define STR_LEARN_SKILL_FAILED CStringManager::GetSingleton().GetString(204) /// 스킬 배우기 실패.
#define STR_LEARN_SKILL_JOB_FAILED \
    CStringManager::GetSingleton().GetString(205) /// 직업이 맞지 않습니다.
#define STR_LEARN_SKILL_NEED_PRESKILL_FAILED \
    CStringManager::GetSingleton().GetString(206) /// 필요 스킬이 없습니다.
#define STR_LEARN_SKILL_NEED_ABILITY_FAILED \
    CStringManager::GetSingleton().GetString(207) /// 능력치가 부족합니다.
#define STR_LEARN_SKILL_SLOT_FULL_FAILED \
    CStringManager::GetSingleton().GetString(208) /// 더이상 스킬을 배울수 없습니다.
#define STR_LEARN_SKILL_INVALID_SKILL_FAILED \
    CStringManager::GetSingleton().GetString(209) /// 잘못된 스킬번호 입니다.

/// 명령, 스킬 사용시
#define STR_ACTION_COMMAND_STOP_STATE_FAILED \
    CStringManager::GetSingleton().GetString(210) /// 정지상태 에서만 사용가능합니다.

///파티 관련
#define FORMAT_STR_PARTY_MAKE_REQ \
    CStringManager::GetSingleton().GetString(211) ///%s님이 파티 결성을 요청했습니다
#define FORMAT_STR_PARTY_INVITE \
    CStringManager::GetSingleton().GetString(212) ///%s님이 파티에 초대하였습니다
#define FORMAT_STR_PARTY_CHANGE_LEADER \
    CStringManager::GetSingleton().GetString(213) ///%s님이 파티의 리더가 되었습니다
#define STR_PARTY_ENTRUSTED_LEADER \
    CStringManager::GetSingleton().GetString(214) ///파티의 리더로 위임 받았습니다
#define STR_PARTY_BANNED \
    CStringManager::GetSingleton().GetString(215) ///파티에서 강제 퇴장 당했습니다
#define FORMAT_STR_PARTY_BANNED \
    CStringManager::GetSingleton().GetString(216) ///%s님이 파티에서 강제 퇴장 당했습니다
#define STR_PARTY_NOTFOUND_DEST \
    CStringManager::GetSingleton().GetString(217) ///대상을 찾을수 없습니다
#define FORMAT_STR_PARTY_BUSY \
    CStringManager::GetSingleton().GetString(218) ///%s님은 지금 요청을 받을 수 없는 상태입니다
#define STR_PARTY_MADE CStringManager::GetSingleton().GetString(219) ///새로운 파티가 결성되었습니다
#define FORMAT_STR_PARTY_REJECT_INVITE \
    CStringManager::GetSingleton().GetString(220) ///%s님이 파티 요청을 거부했습니다
#define FORMAT_STR_PARTY_LEAVE \
    CStringManager::GetSingleton().GetString(221) ///%s님이 파티에서 탈퇴했습니다
#define FORMAT_STR_PARTY_JOIN \
    CStringManager::GetSingleton().GetString(222) ///%s님이 파티에 추가되었습니다

#define FORMAT_STR_PARTY_QUERY_ENTRUST \
    CStringManager::GetSingleton().GetString(223) ///%s님을 파티리더로 위임하시겠습니까?
#define STR_PARTY_QUERY_LEAVE \
    CStringManager::GetSingleton().GetString(224) ///파티에서 탈퇴하겠습니까?
#define FORMAT_STR_PARTY_QUERY_BAN \
    CStringManager::GetSingleton().GetString(225) ///%s님을 강제로 퇴장 시키겠습니까?
#define FORMAT_STR_PARTY_LEVEL CStringManager::GetSingleton().GetString(226) ///파티레벨 %d

#define STR_SKILL_APPLY_FAILED \
    CStringManager::GetSingleton().GetString(227) ///스킬 적용에 실패 했습니다.
#define STR_UNKNOWN_STATE_FLAG \
    CStringManager::GetSingleton().GetString(228) ///알수 없는 상태 플래그.
...
#define STR_NOT_ENOUGH_PROPERTY \
    CStringManager::GetSingleton().GetString(246) ///필요한 에너지가 부족합니다.
#define STR_MISMATCH_CASTING_NEED_EQUIP \
    CStringManager::GetSingleton().GetString(247) ///필요 장비가 부족합니다.

#define STR_DOING_SKILL_ACTION \
    CStringManager::GetSingleton().GetString(248) ///필요 장비가 부족합니다.
#define STR_NOTIFY_CANT_DUPLICATE \
    CStringManager::GetSingleton().GetString(249) ///중복 젹용 불가 스킬
...
#define STR_SUCCESS_MAKE_ITEM \
    CStringManager::GetSingleton().GetString(327) ///아이템 제조에 성공했습니다
#define STR_FAIL_MAKE_ITEM \
    CStringManager::GetSingleton().GetString(328) ///아이템 제조에 실패했습니다
#define STR_NOT_ENOUGH_MAKE_SKILL_LEVEL \
    CStringManager::GetSingleton().GetString(329) ///제조 스킬 레벨이 부족합니다

///<- 2004 / 3 / 4 : 추가 nAvy
#define STR_BANK_NOT_ENOUGH_MONEY \
    CStringManager::GetSingleton().GetString(330) ///보관료가 부족해서 더 이상 보관할 수 없습니다
#define STR_BANK_CANT_STORE_ITEM \
    CStringManager::GetSingleton().GetString(331) ///보관할 수 없는 아이템입니다
#define STR_BANK_NOT_ENOUGH_SPACE \
    CStringManager::GetSingleton().GetString(332) ///보관할 여유 공간이 없습니다
...
#define STR_USING_MAKESKILL \
    CStringManager::GetSingleton().GetString(339) ///제조스킬을 사용중입니다.
#define STR_CHANGE_PRICES CStringManager::GetSingleton().GetString(340) ///물가가 변동되었습니다.
#define STR_QUERY_STOP_TRADE \
    CStringManager::GetSingleton().GetString(341) ///거래를 중단하시겠습니까?
#define STR_WHISPER CStringManager::GetSingleton().GetString(342) ///귓속말
#define STR_CANT_JOIN_PARTY_REASON_LEVEL \
    CStringManager::GetSingleton().GetString(343) ///님은 레벨이 맞지 않아 파티에 들어올 수 없습니다
...
#define STR_ITEM_IN_TRADE_LIST \
    CStringManager::GetSingleton().GetString(346) ///판매목록에 올려진 아이템입니다
#define STR_CANT_USEITEM_REASON_SYNCOPE \
    CStringManager::GetSingleton().GetString(347) ///기절한 상태에서는 사용할수 없습니다
#define STR_INVALID_CHARACTER_NAME \
    CStringManager::GetSingleton().GetString(348) ///이름에 허용되지 않는 글자가 있습니다
#define STR_FAIL_MAKE_NEW_CHARACTER \
    CStringManager::GetSingleton().GetString(349) ///캐릭터 생성 실패
#define STR_REQUEST_SELECT_CHARACTER \
    CStringManager::GetSingleton().GetString(350) ///캐릭터를 선택해 주세요
#define STR_QUERY_DELETE_CHARACTER \
    CStringManager::GetSingleton().GetString(351) ///캐릭터를 지우시겠습니까?
#define STR_RECEIVE_CHARACTER_LIST \
    CStringManager::GetSingleton().GetString(352) ///아바타 리스트를 받습니다
#define STR_SHOUT CStringManager::GetSingleton().GetString(353) ///외침
#define STR_FAIL_SKILL_LEVELUP \
    CStringManager::GetSingleton().GetString(354) ///스킬 레벨업이 실패했습니다
#define STR_NOT_ENOUGH_SKILLPOINT \
    CStringManager::GetSingleton().GetString(355) ///스킬포인트가 모자랍니다
...
#define TT_AVATAR_MENU_BTN_EXCHANGE \
    CStringManager::GetSingleton().GetString(360) ///"교환 신청" 버튼
...
#define STR_SKILL_TARGET_FILTER_NOTIFY_01 \
    CStringManager::GetSingleton().GetString(363) /// 같은 그룹원이 아닙니다.
#define STR_SKILL_TARGET_FILTER_NOTIFY_02 \
    CStringManager::GetSingleton().GetString(364) /// 같은 길드원이 아닙니다.
#define STR_SKILL_TARGET_FILTER_NOTIFY_03 \
    CStringManager::GetSingleton().GetString(365) /// 우호적인 플레이어가 아닙니다.
#define STR_SKILL_TARGET_FILTER_NOTIFY_04 \
    CStringManager::GetSingleton().GetString(366) /// 몬스터가 아닙니다.
#define STR_SKILL_TARGET_FILTER_NOTIFY_05 \
    CStringManager::GetSingleton().GetString(367) /// 적대관계의 플레이어가 아닙니다.
#define STR_SKILL_TARGET_FILTER_NOTIFY_06 \
    CStringManager::GetSingleton().GetString(368) /// 플레이어가  아닙니다.
#define STR_SKILL_TARGET_FILTER_NOTIFY_07 \
    CStringManager::GetSingleton().GetString(369) /// 적용할수 있는 대상이 아닙니다.

#define STR_NOT_ENOUGH_CONDITION \
    CStringManager::GetSingleton().GetString(370) /// 조건이 맞지 않아 사용할수 없습니다.
#define STR_SO_FAR_DISTANCE \
    CStringManager::GetSingleton().GetString(371) /// 너무 멀리 떨어져 있습니다.
#define STR_CANT_SKILL_IN_AGIT \
    CStringManager::GetSingleton().GetString(372) /// 아지트에선 사용할수 없습니다.
...
/// PAT
///
#define STR_ERROR_NOT_ASSEMBLED \
    CStringManager::GetSingleton().GetString(381) /// 조합이 되어있지 않습니다.
#define STR_PAT_ERROR_NOT_EQUIP_BODY \
    CStringManager::GetSingleton().GetString(382) /// 바디부품을 먼저 장착하세요
#define STR_PAT_ERROR_NOT_EQUAL_CLASS \
    CStringManager::GetSingleton().GetString(383) /// 바디와 부품종류가 틀립니다
#define STR_PAT_ERROR_NOT_VERSION \
...
#define F_STR_QUEST_GETTING_MONEY \
    CStringManager::GetSingleton().GetString(419) //돈 %d원을 얻었습니다
#define F_STR_QUEST_GETTING_ITEMS \
    CStringManager::GetSingleton().GetString(420) //%s %d개를 얻었습니다
#define F_STR_QUEST_GETTING_ITEM CStringManager::GetSingleton().GetString(421) //%s을 얻었습니다
#define F_STR_QUEST_SUBTRACT_MONEY \
    CStringManager::GetSingleton().GetString(422) //돈 %d원을 가져갔습니다
#define F_STR_QUEST_SUBTRACT_ITEMS \
    CStringManager::GetSingleton().GetString(423) //%s %d개를 가져갔습니다
#define F_STR_QUEST_SUBTRACT_ITEM CStringManager::GetSingleton().GetString(424) //%s를 가져갔습니다
#define F_STR_SET_ABILITY \
    CStringManager::GetSingleton().GetString(425) //%s 능력치가 %d으로 변경되었습니다
#define F_STR_INCREASE_ABILITY \
    CStringManager::GetSingleton().GetString(426) //%s 능력치가 %d 상승했습니다
#define STR_CANT_USE_DRIVESKILL \
    CStringManager::GetSingleton().GetString(427) //드라이브 카트 스킬을 사용 할 수 없습니다
#define STR_PARTY_IS_FULL \
    CStringManager::GetSingleton().GetString(428) //더 이상 인원을 추가할 수 없습니다
#define STR_CANT_INVITE_TO_PARTY \
    CStringManager::GetSingleton().GetString(429) //파티초대는 파티리더만이 가능합니다
...
#define STR_CANT_EQUIP_USING_DRIVESKILL \
    CStringManager::GetSingleton().GetString(438) // PAT사용시에는 아이템을 장착할수 없습니다
#define STR_CANT_UNEQUIP_USING_DRIVESKILL \
    CStringManager::GetSingleton().GetString(439) // PAT사용시에는 아이템을 탈착할수 없습니다
#define STR_CANT_UNEQUIP_BULLET_USING_DRIVESKILL \
    CStringManager::GetSingleton().GetString(440) // PAT사용시에는 소모탄을 탈착할수 없습니다
#define STR_CANT_SUMMON_IN_PVPZONE \
    CStringManager::GetSingleton().GetString(441) // PVP 존에서는 소환몹을 소환할수 없습니다
#define STR_CANT_USESKILL_USING_DRIVESKILL \
    CStringManager::GetSingleton().GetString(442) //탑승중에는 시전할수 없습니다
...
#define STR_CANT_OPEN_PRIVATESTORE \
    CStringManager::GetSingleton().GetString(445) //개인상점을 개설 할 수 없습니다
...
#define STR_ITEM_CANT_SEPARATED \
    CStringManager::GetSingleton().GetString(448) //분리/분해 할 수 없는 아이템입니다
#define STR_ITEM_CANT_UPGRADED \
    CStringManager::GetSingleton().GetString(449) //제련 할 수 없는 아이템입니다
...
#define STR_DUPLICATED_FRIENDNAME \
    CStringManager::GetSingleton().GetString(483) //친구목록에 이미 있는 이름입니다
#define F_STR_BANK_TITLE CStringManager::GetSingleton().GetString(484) //%s'의 창고
#define F_STR_QUERY_DELETE_FRIEND \
    CStringManager::GetSingleton().GetString(485) //%s를 친구목록에서 삭제하시겠습니까?
#define F_STR_RESTRICT_SHOUT \
    CStringManager::GetSingleton().GetString(486) //외치기는 %d초에 한번만 사용할수 있습니다
#define STR_HELP CStringManager::GetSingleton().GetString(487) //도움말
#define STR_CHANGED_PARTY_CONFIG \
    CStringManager::GetSingleton().GetString(488) //파티 설정이 변경되었습니다
...
#define STR_PARTY_ITEM_PICKUP_RIGHT_ALL \
    CStringManager::GetSingleton().GetString(491) //파티원 전원 부여
...
#define STR_NOT_FOUND_FRIEND \
    CStringManager::GetSingleton().GetString(499) //상대방을 친구목록에서 찾을수 없습니다
#define STR_DEFAULT_PRIVATESTORE_TITLE CStringManager::GetSingleton().GetString(500) //%s' 개인상점

#define F_STR_QUERY_ABANDON_QUEST \
    CStringManager::GetSingleton().GetString(501) //%s 퀘스트를 포기하겠습니까?
#define STR_SEPARATE_EMPTY_TARGETITEM \
    CStringManager::GetSingleton().GetString(502) //분리/분해할 아이템이 없습니다
#define STR_NOTENOUGH_EMPTY_INVENTORYSLOT \
    CStringManager::GetSingleton().GetString(503) //인벤토리에 여유공간이 없습니다
#define F_STR_FRIEND_DELETED \
    CStringManager::GetSingleton().GetString(504) //%s이 나를 친구 목록에서 제외시켰습니다
#define F_STR_MESSANGER_BLOCKED \
    CStringManager::GetSingleton().GetString(505) //%s이 대화를 차단하였습니다
#define STR_REQUIRE_SKILLPOINT CStringManager::GetSingleton().GetString(506) //필요 스킬포인트
#define STR_CANT_LEVELUP \
...
#define STR_LOGIN_REPLY_TOO_MANY_USER         \
    CStringManager::GetSingleton().GetString( \
        510) //최대 접속인원이 넘었습니다. 잠시후에 다시 이용해주시기 바랍니다
#define STR_LOGIN_REPLY_NO_RIGHT_TO_CONNECT   \
    CStringManager::GetSingleton().GetString( \
        511) //로즈온라인은 9월에 오픈 베타 테스트를 시작할 예정으로 현재는 접속하실수 없습니다
#define STR_QUERY_REVIVE_CHARACTER \
    CStringManager::GetSingleton().GetString(512) //캐릭터를 부활시키겠습니까?
#define STR_CANT_CREATE_MORE_CHARACTER \
    CStringManager::GetSingleton().GetString(513) //캐릭터를 더 이상 생성 할 수 없습니다
...
#define F_STR2_CHAT_BLOCKED_BY_GM \
    CStringManager::GetSingleton().GetString(517) // GM[%s)이 %d분간 채팅금지 시켰습니다
#define STR_OTHER_WHISPER_REJECT_STATE \
    CStringManager::GetSingleton().GetString(518) //상대방은 현재 귓속말 거부상태입니다
...
#define STR_ITEM_IS_BUY_ALL \
    CStringManager::GetSingleton().GetString(532) //더이상 사지 않는 아이템입니다

#define STR_BANK_REPLY_NEED_PASSWORD \
    CStringManager::GetSingleton().GetString(533) //창고 비밀번호를 입력하세요
#define STR_BANK_REPLY_CHANGED_PASSWORD \
    CStringManager::GetSingleton().GetString(534) //창고 비밀번호가 변경되었습니다
#define STR_BANK_REPLY_INVALID_PASSWORD \
    CStringManager::GetSingleton().GetString(535) //창고 비밀번호가 틀렸습니다
#define STR_BANK_REPLY_CLEARED_PASSWORD \
    CStringManager::GetSingleton().GetString(536) //창고 비밀번호가 삭제되었습니다
#define STR_BANK_MOVEITEM_FAILED \
    CStringManager::GetSingleton().GetString(537) //아이템 이동이 실패했습니다
#define STR_CHAT_REPLY_MAKE_FAILED \
    CStringManager::GetSingleton().GetString(538) //방만들기에 실패했습니다
#define STR_CHAT_REPLY_INVALIED_PASS \
    CStringManager::GetSingleton().GetString(539) //비밀번호가 틀립니다
#define STR_CHAT_REPLY_ROOM_NOT_FOUND \
    CStringManager::GetSingleton().GetString(540) //없어진 채팅방입니다

#define STR_CHAT_REPLY_FULL_USERS \
    CStringManager::GetSingleton().GetString(541) //더이상 참여할수 없습니다
#define STR_CHAT_REPLY_KICKED \
    CStringManager::GetSingleton().GetString(542) //채팅방에서 강퇴 당했습니다
#define F_STR_QUERY_APPEND_FRIEND_REQ \
    CStringManager::GetSingleton().GetString(543) //%s가 친구로 등록하려고 합니다. 승낙하시겠습니까?
#define F_STR_REJECT_APPEND_FRIEND_REQ \
    CStringManager::GetSingleton().GetString(544) //%s이 친구 추가 요청을 거절했습니다
#define F_STR_NOT_FOUNDORCONNECT_FRIEND \
    CStringManager::GetSingleton().GetString(545) //%s을 찾을수 없거나 접속중이 아닙니다
#define STR_OTHERSTATE_IS_REJCTMESSANGER \
    CStringManager::GetSingleton().GetString(546) //상대방은 현재 메신저 거부상태입니다
#define STR_MEMO_REPLY_SEND_NOT_EXISTORINVALID \
    CStringManager::GetSingleton().GetString(547) //대상을 찾을수 없습니다
#define STR_MEMO_REPLY_SEND_REFUSED CStringManager::GetSingleton().GetString(548) //쪽지 수신 거부
#define STR_MEMO_REPLY_SEND_FULL_MEMO \
    CStringManager::GetSingleton().GetString(549) //상대방의 수신함이 꽉차서 더이상 받을수 없습니다
#define STR_MEMO_REPLY_SEND_INVALID_CONTENT CStringManager::GetSingleton().GetString(550) //내용오류

#define STR_CRAFT_BREAKUP_SUCCESS_GEM \
    CStringManager::GetSingleton().GetString(551) //아이템 분리가 성공했습니다
#define STR_CRAFT_BREAKUP_DEGRADE_GEM         \
    CStringManager::GetSingleton().GetString( \
        552) //아이템 분리가 성공했습니다. 분리 과정에서 보석의 등급감소가 발생했습니다
#define STR_CRAFT_BREAKUP_CLEARED_GEM         \
    CStringManager::GetSingleton().GetString( \
        553) //아이템 분리가 성공했습니다. 분리 과정에서 보석이 손실되었습니다
#define STR_CRAFE_BREAKUP_SUCCESS \
    CStringManager::GetSingleton().GetString(554) //아이템 분해가 성공했습니다
#define STR_CRAFT_UPGRADE_INVALID_MAT \
    CStringManager::GetSingleton().GetString(555) //재료 아이템이 잘못됬습니다
...
#define STR_BILLING_MSG_FEE_TYPE1             \
    CStringManager::GetSingleton().GetString( \
        593) //  /요금: 귀하는 정액요금제 사용자 입니다. (2004년06월01일)
#define STR_BILLING_MSG_FEE_TYPE2             \
    CStringManager::GetSingleton().GetString( \
        594) //         귀하는 정량요금제 사용자 입니다. (18시간20분20초)
#define STR_BILLING_MSG_TIME_ALERT CStringManager::GetSingleton().GetString(595) // 남은 시간 통보
#define STR_BILLING_MSG_TIME_EXPIRED \
    CStringManager::GetSingleton().GetString(596) // <종료> 결제 시간이 다되어 게임을 종료합니다.

#define STR_BILLING_MSG_KICKOUT_CHEAT_DETECT \
    CStringManager::GetSingleton().GetString(597) // 스핵등 치트 발견되서 종료
#define STR_BILLING_MSG_KICKOUT_ALREADY_LOGIN \
    CStringManager::GetSingleton().GetString(598) // 이미 접속된 계정이라 접속 종료
#define STR_BILLING_MSG_KICKOUT_TIME_EXPIRED \
    CStringManager::GetSingleton().GetString(599) // 과금 시간 만료
#define STR_BILLING_MSG_KICKOUT_DUP_LOGIN \
    CStringManager::GetSingleton().GetString(600) // 다른 넘이 접속해서 접속 종료
#define STR_BILLING_MSG_KICKOUT_NO_RIGHT \
    CStringManager::GetSingleton().GetString(601) // 접속할 권한이 없다...
#define STR_BILLING_MSG_KICKOUT_OUT_OF_IP \
    CStringManager::GetSingleton().GetString(602) // pc방에서 접속할수 있는 ip 갯수가 초과
...
#define STR_GETTING_ITEM_PARTY \
    CStringManager::GetSingleton().GetString(728) //%s님이 %s 아이템을 획득하였습니다
#define STR_GETTING_ITEMS_PARTY \
    CStringManager::GetSingleton().GetString(729) //%s님이 %s %d개 획득하였습니다
#define STR_GETTING_MONEY_PARTY \
    CStringManager::GetSingleton().GetString(730) //%s님이 돈 %원을 획득하였습니다

#define STR_CANT_REFAIR_PAT_ENGINE            \
    CStringManager::GetSingleton().GetString( \
        731) //팻 엔진은 수리 할 수 없습니다. 엔진 연료를 충전해야 합니다.
