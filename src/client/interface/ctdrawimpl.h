 ...
 *  : TGameCtrl내부적으로 2D이미지를 그릴때 이 인터페이스를 통해서 znzin의 해당 인터페이스를
 * 호출한다.
 *
 * @Author		이동현,최종진
 *
...
public:
    ...
    virtual void
    DrawFitH(int iX, int iY, int iModuleID, int iGraphicID, int iHeight, D3DCOLOR color);
    virtual void
    DrawFit(int iX, int iY, int iModuleID, int iGraphicID, int iWidth, int iHeight, D3DCOLOR color);
