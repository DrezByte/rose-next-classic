...
void
CIconDialog::SetDragItem(CDragItem* pDragItem) {
    m_pDragItem = pDragItem;
}

CIconDialog*
CIconDialog::CreateIcon(int iDialogType, bool bCreateDragItem) {
    int iGraphicID = 0;
    CInfo Info;
    Info.Clear();
    switch (iDialogType) {
        case DLG_TYPE_CLAN:
            iGraphicID =
                CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI21_ICON_DIALOG_GUILD");
            Info.AddString(STR_CLAN);
            break;
        case DLG_TYPE_ITEM:
            iGraphicID =
                CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI21_ICON_DIALOG_ITEM");
            Info.AddString(STR_ITEM);
            break;
        case DLG_TYPE_CHAR:
            iGraphicID =
                CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI21_ICON_DLG_AVATAR");
            Info.AddString(STR_AVATARINFO);
            break;
        case DLG_TYPE_SKILL:
            iGraphicID =
                CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI21_ICON_DIALOG_SKILL");
            Info.AddString(STR_SKILL);
            break;
        case DLG_TYPE_QUEST:
            iGraphicID =
                CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI21_ICON_DIALOG_QUEST");
            Info.AddString(STR_QUEST);
            break;
        case DLG_TYPE_COMMUNITY:
            iGraphicID = CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI,
                "UI21_ICON_DIALOG_COMMUNITY");
            Info.AddString(STR_COMMUNITY);
            break;
            //	case DLG_TYPE_GUILD:
            //		iGraphicID = CResourceMgr::GetInstance()->GetImageNID( IMAGE_RES_UI,
            //"UI21_ICON_DIALOG_GUILD" ); 		break;
        case DLG_TYPE_HELP:
            iGraphicID =
                CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI21_ICON_DIALOG_HELP");
            Info.AddString(STR_HELP);
            break;
        case DLG_TYPE_OPTION:
            iGraphicID =
                CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI21_ICON_DIALOG_OPTION");
            Info.AddString(STR_OPTION);
            break;
        case DLG_TYPE_PARTY:
            iGraphicID =
                CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI21_ICON_DIALOG_PARTY");
            Info.AddString(STR_PARTY);
            break;
        case DLG_TYPE_SYSTEM:
            // iGraphicID = CResourceMgr::GetInstance()->GetImageNID( IMAGE_RES_UI,
            // "UI21_ICON_DIALOG_SYSTEM" ); break;
        default:
            iGraphicID = 0;
            break;
    }
...
void
CIconDialog::SetDialogType(int iDialogType) {
    m_iDialogType = iDialogType;
}
int
CIconDialog::GetDialogType() {
    return m_iDialogType;
}

void
CIconDialog::Update(POINT ptMouse) {
...
unsigned
CIconDialog::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CIconDialog::GetToolTip(CInfo& ToolTip, DWORD dwDialogType, DWORD dwType) {
    ToolTip = m_ToolTip;
}

void
CIconDialog::SetPosition(POINT pt) {
    if (pt.x < 0)
        pt.x = 0;
    if (pt.y < 0)
        pt.y = 0;
...
CDragItem*
CIconDialog::GetDragItem() {
    return m_pDragItem;
}

int
CIconDialog::GetIndex() {
