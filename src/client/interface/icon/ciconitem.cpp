        ...
        if (Item.GetGemNO() > 300) {
            ...
            if (Item.GetGemNO() <= g_TblGEMITEM.m_nDataCnt)
                g_DrawImpl.Draw(m_ptPosition.x,
                    ...
                    GEMITEM_MARK_IMAGE(Item.GetGemNO()));
        } else {
...
void
CIconItem::Update(POINT ptMouse) {
    ...
    {
        g_itMGR.DrawItemINFO( (short)ptMouse.x, (short)ptMouse.y, m_sItem,
    m_pParentDlg->GetDialogType(), 0 );
    }
    */
}
...
void
CIconItem::GetToolTip(CInfo& strToolTip, DWORD dwDialogType, DWORD dwType) {
    assert(m_pItem);
...
unsigned
CIconItem::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
