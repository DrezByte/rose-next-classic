...
void
CIcon::SetDrawColor(DWORD color) {
...
bool
CIcon::IsInside(POINT pt) {
    RECT rcThis = {m_ptPosition.x,
        m_ptPosition.y,
...
void
CIcon::SetSlot(CSlot* pSlot) {
    m_pSlot = pSlot;
}
void
CIcon::SetCommand(CTCommand* pCommand) {
    m_pCommand = pCommand;
}

void
CIcon::ExecuteCommand() {}
...
void
CIcon::GetToolTip(CInfo& ToolTip, DWORD dwDialogType, DWORD dwType) {
...
unsigned
CIcon::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    return 0;
}

void
CIcon::Update(POINT ptMouse) {
    if (IsEnable())
        m_dwIconDrawColor = D3DCOLOR_ARGB(255, 255, 255, 255);
    else
...
void
CIconStatus::SetGraphicID(int iModuleID, int iGraphicID) {
