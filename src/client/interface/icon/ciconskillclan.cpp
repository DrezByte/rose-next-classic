...
int
CIconSkillClan::GetIndex() {
    return GetSkillSlotFromIcon();
}

void
CIconSkillClan::SetSkillSlotToIcon(int iSkillSlot) {
    m_iSkillSlot = iSkillSlot;
}

int
CIconSkillClan::GetSkillSlotFromIcon() {
    return m_iSkillSlot;
}

int
CIconSkillClan::GetSkillLevel() {
...
CIcon*
CIconSkillClan::Clone() {
    return new CIconSkillClan(m_iSkillSlot);
}
void
CIconSkillClan::GetToolTip(CInfo& ToolTip, DWORD dwDialogType, DWORD dwType) {
    ...
    if (pSkill && pSkill->HasExpiredTime()) {
        ...
        ToolTip.AddString(CStr::Printf("%s : %d/%d/%d %d:%02d",
            STR_EXPIRED_TIME,
            st.wYear,
            st.wMonth,
            st.wDay,
