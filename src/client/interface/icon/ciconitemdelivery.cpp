...
CIconItemDelivery::CIconItemDelivery(tagITEM* pItem, const char* pszFrom): CIconItem(pItem) {
    m_border_grahicid =
        CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI13_BORDER_SELECTED_ITEM");
    if (pszFrom)
        m_from = pszFrom;
    else
        m_from.clear();
...
unsigned
CIconItemDelivery::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CIconItemDelivery::GetToolTip(CInfo& strToolTip, DWORD dwDialogType, DWORD dwType) {
