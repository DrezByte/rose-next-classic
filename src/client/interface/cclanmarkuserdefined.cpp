...
void
CClanMarkUserDefined::Draw(const D3DVECTOR& vDrawPos) {
    Draw(vDrawPos.x, vDrawPos.y, vDrawPos.z);
}

void
CClanMarkUserDefined::Draw(float x, float y, float z) {
    if (m_hNode) {
...
void
CClanMarkUserDefined::SetNode(HNODE hNode) {
    assert(m_hNode == NULL);
    m_hNode = hNode;
}

void
CClanMarkUserDefined::SetName(const char* pszName) {
...
const std::string&
CClanMarkUserDefined::GetName() {
    return m_strTextureName;
}

DWORD
CClanMarkUserDefined::GetLastUsedTime() {
    return m_dwLastUsedTime;
}
void
CClanMarkUserDefined::AddRef() {
    m_iRefCount++;
}

void
CClanMarkUserDefined::Release() {
    m_iRefCount--;
...
int
CClanMarkUserDefined::GetRefCount() {
    return m_iRefCount;
}

void
CClanMarkUserDefined::GetClanMarkFileName(int server_id, int clan_id, std::string& filename) {
