...
void
CITStateRepair::Enter() {
    if (CTDialog* pDlg = g_itMGR.FindDlg(DLG_TYPE_ITEM)) {
        CItemDlg* pItemDlg = (CItemDlg*)pDlg;
        pItemDlg->AddActionEventListener2Slots(); /// 아이템창 슬롯에서의 아이콘 클릭시 행위(수리)를
                                                  /// 바꾸어 준다.
    }
}
void
CITStateRepair::Leave() {
    if (CTDialog* pDlg = g_itMGR.FindDlg(DLG_TYPE_ITEM)) {
        CItemDlg* pItemDlg = (CItemDlg*)pDlg;
...
unsigned
CITStateRepair::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    UINT uiRet = 0;
