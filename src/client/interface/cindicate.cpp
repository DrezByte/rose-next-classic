...
CIndicate::Draw() {
    ...
    drawFont(g_GameDATA.m_hFONT[FONT_TITLE],
        true,
        &rcTitle,
        g_dwWHITE,
        DT_CENTER,
    ...
    for (iter = m_Descriptions.begin(); iter != m_Descriptions.end(); ++iter) {
        drawFont(g_GameDATA.m_hFONT[FONT_DESCRIPTION],
            true,
            &rcDrawDesc,
            g_dwWHITE,
            DT_CENTER,
...
void
CIndicate::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
void
CIndicate::SetTitle(const char* pszTitle, const char* pszSubTitle) {
    assert(pszTitle);
    if (pszTitle == NULL)
        return;
...
void
CIndicate::AddDescription(const char* pszDescription) {
    assert(!IsVision());
