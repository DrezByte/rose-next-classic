    ...
    if (p->depth == 1) /// base skill
    {
        ...
        for (int index = 0; atts[index] != NULL; index += 2) {
            if (strcmpi(atts[index], "INDEX") == 0) {
                ...
                // setDelayedLoad(0);
                node_base.m_texture = loadTexture(file.c_str(), file.c_str(), 1, 0);
...
CSkillTreeDlg::Draw() {
    ...
    if (!m_userdata.base_skills.empty()) {
        ...
        for (iter_base = m_userdata.base_skills.begin(); iter_base != m_userdata.base_skills.end();
             ++iter_base) {
            assert(iter_base->m_icon);
        ...
        for (iter_children = iter_base->m_children.begin();
             iter_children != iter_base->m_children.end();
             ++iter_children) {
...
CIconSkill*
CSkillTreeDlg::MakeIcon(int skillindex, int level, int level_limit) {
    if (!g_pAVATAR)
        return NULL;
...
void
CSkillTreeDlg::MoveWindow(POINT pt) {
...
unsigned
CSkillTreeDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
    if (unsigned uiProcID = CTDialog::Process(uiMsg, wParam, lParam)) {
        switch (uiMsg) {
            ...
            case WM_LBUTTONDOWN: {
                ...
                for (iter_base = m_userdata.base_skills.begin();
                     iter_base != m_userdata.base_skills.end();
                     ++iter_base) {
                    assert(iter_base->m_icon);
...
void
CSkillTreeDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
void
CSkillTreeDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);
...
void
CSkillTreeDlg::LevelupSkill(int skillslotindex) {
    if (!g_pAVATAR)
        return;
    ...
    for (iter_base = m_userdata.base_skills.begin(); iter_base != m_userdata.base_skills.end();
        ...
        if (iter_base->m_index == base_skillindex
            && iter_base->m_level == pSkill->GetSkillLevel()) {
            assert(iter_base->m_icon);
        ...
        for (iter_child = iter_base->m_children.begin(); iter_child != iter_base->m_children.end();
             ++iter_child) {
            if (iter_child->m_index == base_skillindex) {
                if (iter_child->m_level == pSkill->GetSkillLevel()) {
                    ...
                    MoveWindow(m_sPosition);
                } else if (iter_child->m_level_limit
                    && iter_child->m_level_limit + 1 == pSkill->GetSkillLevel()) {
                    assert(iter_child->m_icon);
                    SAFE_DELETE(iter_child->m_icon);
...
void
CSkillTreeDlg::AddSkill(int skillslotindex) {
    if (!g_pAVATAR)
        return;
    CSkillSlot* pSlot = g_pAVATAR->GetSkillSlot();
    assert(pSlot);
...
void
CSkillTreeDlg::RemoveSkill(int skillslotindex) {
    if (!g_pAVATAR)
        return;
    CSkillSlot* pSlot = g_pAVATAR->GetSkillSlot();
    assert(pSlot);
