 ...
 *	npc 이미지 그리기 방식 수정, map load시 추가된 Info로 그린다.기존 루틴은 혹시 다음에 필요할수도
 *있을것 같아서 남겨둔다( 움직이는 NPC 의 경우 map에서 로드된 위치정보로만 표시할수 없다 );
 *
 * Revision 1.0	2004/4/xx
 *	1. 현재 미니맵의 중앙으로 표시되는 World 좌표로부터 그려질 Object간의 거리를 구한다.
 *	2. 미니맵의 중앙이 그려지는 Screen 좌표를 구한다.
 *	3. 1,2번과 현재 Scale및 World/Screen 대비,이미지사이즈에 따라 그려질 Left-Top Screen좌표를
 *구한다.
 *
 */
#include "stdafx.h"
...
CMinimapDLG::CalculateDisplayPos() {
    m_fMinMinimapWorldPosX = (float)(PATCH_COUNT_PER_MAP_AXIS * m_dwMinimapStartX)
        * ((float)GRID_COUNT_PER_PATCH_AXIS * nGRID_SIZE); // 미니맵의 시작 월드 좌표( 가로 )
    ...
    getTextureSize(m_hMiniMap,
        iTextureWidth,
        iTextureHeight); /// setDelayedLoad(1)의 경우 Size가 제대로 반환 안될수 있다.

    int iMapRowCount = iTextureHeight / MINIMAP_RESOLUTION_PER_MAP - 2;
    int iMapColCount = iTextureWidth / MINIMAP_RESOLUTION_PER_MAP - 2;

    m_fMaxMinimapWorldPosX = m_fMinMinimapWorldPosX
        + iMapColCount * PATCH_COUNT_PER_MAP_AXIS * GRID_COUNT_PER_PATCH_AXIS * nGRID_SIZE;
    m_fMinMinimapWorldPosY = m_fMaxMinimapWorldPosY
        - iMapRowCount * PATCH_COUNT_PER_MAP_AXIS * GRID_COUNT_PER_PATCH_AXIS * nGRID_SIZE;
}
...
CMinimapDLG::Draw() {
    ...
    if (m_bMinimize == false && m_hMiniMap && m_hArrow) {
        ...
        ::drawSprite(m_hMiniMap,
            &rcMinimap,
            NULL,
            &D3DXVECTOR3((m_sPosition.x + c_iMargin) / m_fCurrentScale,
                (m_sPosition.y + c_iCaptionHeight) / m_fCurrentScale,
                0.0f),
            color);
        ...
        float fDrawPosY =
            fScreenCenterY - m_fCurrentScale * fDistanceY / fGetWorldDistancePerPixel();

        if (IsInsideMinimap(fDrawPosX - MINIMAP_CURSOR_WIDTH / 2,
                fDrawPosY - MINIMAP_CURSOR_HEIGHT / 2,
                MINIMAP_CURSOR_WIDTH,
                MINIMAP_CURSOR_HEIGHT)) {
            ...
            ::drawSprite(m_hArrow,
                NULL,
                &D3DXVECTOR3((float)(MINIMAP_CURSOR_WIDTH / 2),
                    (float)(MINIMAP_CURSOR_HEIGHT / 2),
                    0.0f), // rotation center,
                NULL,
        ...
        ::drawFontf(g_GameDATA.m_hFONT[FONT_NORMAL_BOLD],
            ...
            g_dwWHITE,
            "%d,%d",
            (int)g_pAVATAR->Get_CurPOS().x / 100,
            (int)g_pAVATAR->Get_CurPOS().y / 100);
    }
}
void
CMinimapDLG::DrawCoordinatesIndicator() {
...
void
CMinimapDLG::Update(POINT ptMouse) {
    if (!IsVision())
        return;
        ...
        if (IsInsideMinimap((float)ptMouse.x, (float)ptMouse.y, 0, 0)) {
            ...
            if (abs(fDistanceX) > 5 || abs(fDistanceY) > 5) {
                ...
                float fTempY = m_fMinimapCenterWorldPosY
                    - fDistanceY * fGetWorldDistancePerPixel() / m_fCurrentScale;
    ...
    if (m_zonename.is_ellipsis()) {
        ...
        if (PtInRect(&rect_zonename, ptPoint)) {
            CToolTipMgr::GetInstance().RegToolTip(m_sPosition.x - 20,
                m_sPosition.y - 20,
                m_zonename.get_string());
            return;
        }
    }
...
unsigned int
CMinimapDLG::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
bool
CMinimapDLG::On_LButtonUP(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
    m_bDragStart = false;
    CWinCtrl* pCtrl = NULL;
    switch (iProcID) {
...
bool
CMinimapDLG::On_LButtonDN(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
...
CMinimapDLG::DrawAVTPosition() {
    ...
    for (short nI = 0; nI < nRegObjCnt; nI++) {
        ...
        switch (pCharOBJ->Get_TYPE()) {
            case OBJ_NPC: { ///서버에서 내려주는 정보는 일단 무시한다.
            ...
            case OBJ_AVATAR: {
                fDrawPosX = fScreenCenterX
                    - m_fCurrentScale * fDistanceX / fGetWorldDistancePerPixel()
                    - AVATAR_POINT_SIZE / 2;
                fDrawPosY = fScreenCenterY
                    - m_fCurrentScale * fDistanceY / fGetWorldDistancePerPixel()
                    - AVATAR_POINT_SIZE / 2;
                if (IsInsideMinimap(fDrawPosX, fDrawPosY, AVATAR_POINT_SIZE, AVATAR_POINT_SIZE)) {

                    if (pCharOBJ->Get_TeamNO() != g_pAVATAR->Get_TeamNO()) {
                        pDrawObj->Draw((int)fDrawPosX - 6,
                            (int)fDrawPosY - 6,
                        ...
                        partyMemberMarkInfo.rcDraw.right =
                            partyMemberMarkInfo.rcDraw.left + AVATAR_POINT_SIZE;
                        partyMemberMarkInfo.rcDraw.bottom =
                            partyMemberMarkInfo.rcDraw.top + AVATAR_POINT_SIZE;
                        m_listPartyMemberMarkInfo.push_back(partyMemberMarkInfo);
                    } else {
                        pDrawObj->Draw((int)fDrawPosX,
                            (int)fDrawPosY,
                            IMAGE_RES_UI,
                            m_iAvatarImageID);
                    }
...
void
CMinimapDLG::DrawPartyMember() {
    CTDrawImpl* pDrawObj = g_itMGR.GetDrawImplPtr();
    /// PartyMember
    std::list<PartyMemberMarkInfo>::iterator iterPartyMember;
    for (iterPartyMember = m_listPartyMemberMarkInfo.begin();
         iterPartyMember != m_listPartyMemberMarkInfo.end();
         ++iterPartyMember)
        pDrawObj->Draw((int)iterPartyMember->rcDraw.left,
            (int)iterPartyMember->rcDraw.top,
            IMAGE_RES_UI,
            iterPartyMember->iIconNo);
}
void
CMinimapDLG::DrawNpcFromMap() {
    CTDrawImpl* pDrawObj = g_itMGR.GetDrawImplPtr();
...
float
CMinimapDLG::fGetWorldDistancePerPixel() {
    return (float)(PATCH_COUNT_PER_MAP_AXIS * GRID_COUNT_PER_PATCH_AXIS * nGRID_SIZE
        / (MINIMAP_RESOLUTION_PER_MAP));
}

bool
CMinimapDLG::IsInsideMinimap(float x, float y, int width, int height) {
    if (!m_bShowMiniMap)
        return false;

    if (x > m_sPosition.x + c_iMargin && (x + width) < m_sPosition.x + m_iWidth - c_iMargin
        && y > m_sPosition.y + c_iCaptionHeight && (y + height) < m_sPosition.y + m_iHeight)
        return true;
...
CWinCtrl*
CMinimapDLG::FindPaneChild(unsigned uiPaneID, unsigned uiChildID) {
    ...
    if (pCtrl->GetControlType() != CTRL_PANE)
        return NULL;
...
void
CMinimapDLG::ToggleShowMinimapSmall() {
    CWinCtrl* pCtrl;
    if (m_bMinimize) {
        m_bMinimize = false;
        if (pCtrl = Find(IID_PANE_SMALL)) {
...
void
CMinimapDLG::ToggleShowMinimapBig() {
    CWinCtrl* pCtrl;
    if (m_bMinimize) {
        m_bMinimize = false;
        if (pCtrl = Find(IID_PANE_BIG)) {
...
void
CMinimapDLG::AddIndicatorNpc(int npcno, int auto_delete) {
...
void
CMinimapDLG::AddIndicatorCoordinates(int index, int zoneno, float x, float y) {
...
void
CMinimapDLG::RemoveIndicatorNpc(int npcno) {
...
void
CMinimapDLG::RemoveAutodeleteIndicatorNpc(int npcno) {
    std::multimap<int, S_IndicatorNpc>::iterator iter;
...
void
CMinimapDLG::RemoveIndicatorCoordinates(int index) {
    std::map<int, S_IndicatorCoordinates>::iterator iter;
    iter = m_indicators_coordinates.find(index);
    if (iter != m_indicators_coordinates.end())
...
int
CMinimapDLG::GetIndicatorNpc(int npcno) {
    std::multimap<int, S_IndicatorNpc>::iterator iter;
    iter = m_indicators_npc.find(npcno);
    if (iter != m_indicators_npc.end())
        return 1;
...
void
CMinimapDLG::SetAvataInfoPos(int y) {
#ifdef _NEWUI
...
void
CMinimapDLG::RefreshDlg() {
    POINT ptNewPos = {getScreenWidth() - m_iWidth, m_sPosition.y};
    MoveWindow(ptNewPos);
}

void
CMinimapDLG::SetInterfacePos_After() {
    RefreshDlg();
}