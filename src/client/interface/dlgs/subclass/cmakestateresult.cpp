...
void
CMakeStateResult::MoveWindow(POINT ptPos) {
    std::vector<CGuage*>::iterator iter;
        ...
        if (m_listGuage[i]->Update(ptMouse, m_dwPrevTime, dwCurrTime) == CGuage::UPDATE_END) {
            if (m_pParent->m_btRESULT == RESULT_CREATE_ITEM_SUCCESS) {
                        ...
                        - 1) ///맨밑의 게이지 갱신이 끝까지 갔다
                {
                    ...
                    g_pNet->Send_cli_ITEM_RESULT_REPORT(REPORT_ITEM_CREATE_SUCCESS,
                        itemtype,
                        itemno);
                ...
                if (i == m_pParent->m_nStepORInvIDX) {
                    ...
                    g_pNet->Send_cli_ITEM_RESULT_REPORT(REPORT_ITEM_CREATE_FAILED,
                        itemtype,
                        itemno);
                    break;
                }
...
void
CMakeStateResult::Show() {}

void
CMakeStateResult::Hide() {}

void
CMakeStateResult::Init() {
    std::vector<CGuage*>::iterator iter;
    for (iter = m_listGuage.begin(); iter != m_listGuage.end(); ++iter) {
        (*iter)->SetValue(0);
    ...
    struct gsv_CREATE_ITEM_REPLY : public t_PACKETHEADER {
        BYTE	m_btRESULT;								// 결과...
        short	m_nStepORInvIDX;						// 제조 실패시 실패된 스텝, 성공시 들어간
    인벤토리번호 tagITEM	m_CreatedITEM;							// 제조 성공시 제조된 아이템 } ;
    */
    ...
    switch (m_pParent->m_btRESULT) {
        ...
        case RESULT_CREATE_ITEM_NEED_SKILL_LEV: // 제조 스킬 레벨이 부족하다
        {
            m_bWaitState = true;
            CTCmdChangeState* pCmd = new CTCmdChangeState(CMakeDLG::STATE_NORMAL);
            g_itMGR.OpenMsgBox(STR_NOT_ENOUGH_MAKE_SKILL_LEVEL,
                CMsgBox::BT_OK,
                true,
                m_pParent->GetDialogType(),
                pCmd,
...
unsigned int
CMakeStateResult::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    return 0;
}

bool
CMakeStateResult::IsModal() {
    return true;
}
