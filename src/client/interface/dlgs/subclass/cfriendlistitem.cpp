...
CFriendListItem::Draw() {
    ...
    ::drawFont(g_GameDATA.m_hFONT[FONT_SMALL],
        ...
        dwColor,
        DT_VCENTER,
        CStr::Printf("%-16s(%s)", m_strName.c_str(), pszStatus));
}

unsigned
CFriendListItem::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CFriendListItem::SetStatus(BYTE btStatus) {
    m_btStatus = btStatus;
...
const DWORD
CFriendListItem::GetUserTag() {
    return m_dwUserTag;
}

const char*
CFriendListItem::GetName() {
    return m_strName.c_str();
}

const BYTE
CFriendListItem::GetStatus() {
    return m_btStatus;
}