...
unsigned
CQuestListItem::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CQuestListItem::Update(POINT ptMouse) {
    if (IsInside(ptMouse.x, ptMouse.y)) {
        if (m_title.is_ellipsis())
            CToolTipMgr::GetInstance().RegToolTip(m_sPosition.x + 10,
                m_sPosition.y - 18,
                m_title.get_string());
    }
}
