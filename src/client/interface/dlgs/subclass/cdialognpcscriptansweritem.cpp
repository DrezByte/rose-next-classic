...
CDialogNpcScriptAnswerItem::CDialogNpcScriptAnswerItem(int iIndex,
    char* pszScript,
    int iEventID,
    void (*fpEventHandle)(int iEventID),
    int iImageID,
    int iLineHeight,
    int iWidth) {
    m_highlight = false;
    ...
    switch (iIndex) {
        ...
        default: {
            /*	assert( pszScript );
                ...
                {
                    m_iIndent = 15;
                    m_SplitScript.Split( FONT_NORMAL, pszScript , m_iWidth - m_iIndent -
               c_iBorderLeft * 2,CLocalizing::GetSingleton().GetCurrentCodePageNO()); SetHeight(
               m_iLineHeight * m_SplitScript.GetLineCount() );
                }
            */
            assert(pszScript);
            if (pszScript) {
                ...
                m_Script.SetString(CStr::Printf("%d. %s", iIndex, pszScript),
                    m_iWidth - m_iBorderWidth * 2 - 12);
...
void
CDialogNpcScriptAnswerItem::Draw() {
...
unsigned
CDialogNpcScriptAnswerItem::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CDialogNpcScriptAnswerItem::Update(POINT ptMouse) {
