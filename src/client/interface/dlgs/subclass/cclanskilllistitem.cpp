            ...
            if (pIcon->GetName()) {
                if (iLevel <= 0)
                    ...
                    drawFont(g_GameDATA.m_hFONT[FONT_NORMAL],
                        ...
                        3,
                        g_dwBLACK,
                        CStr::Printf("%s(Lv:%d)", pIcon->GetName(), iLevel));
            }
...
void
CClanSkillListItem::MoveWindow(POINT pt) {
    CWinCtrl::MoveWindow(pt);
    m_Slot.MoveWindow(m_sPosition);
}

void
CClanSkillListItem::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
unsigned
CClanSkillListItem::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CClanSkillListItem::Show() {
    CWinCtrl::Show();
    m_Slot.Show();
}

void
CClanSkillListItem::Hide() {
    CWinCtrl::Hide();
    m_Slot.Hide();
}

void
CClanSkillListItem::SetIcon(CIconSkillClan* pIcon) {
    m_Slot.DetachIcon();
