...
CGuage::Draw() {
    ...
    g_DrawImpl.Draw((int)m_ptPosition.x,
        (int)m_ptPosition.y,
        iDrawGuageWidth,
        m_iModuleID,
        m_iGuageImageID);
    if (!m_strText.empty()) {
        RECT rcDraw = {m_ptPosition.x,
            m_ptPosition.y,
            m_ptPosition.x + m_iWidth,
            m_ptPosition.y + m_iHeight};
...
CGuage::Update(POINT ptMouse, DWORD dwPrevTime, DWORD dwCurrTime) {
    if (m_bAutoIncrement) {
        ...
        LogString(LOG_NORMAL,
            "UPDATE_NORMAL:Guage Value:%d %d %d\n",
            m_iValue,
            dwCurrTime,
            dwPrevTime);
    }
...
void
CGuage::SetAutoIncrement(int iAutoIncrementValue, int iAutoIncrementMaxValue) {
...
void
CGuage::SetPosition(POINT ptPos) {
    m_ptPosition = ptPos;
}
void
CGuage::SetOffset(POINT ptOffset) {
    m_ptOffset = ptOffset;
}

void
CGuage::MoveWindow(POINT pt) {
    POINT ptNew = {pt.x + m_ptOffset.x, pt.y + m_ptOffset.y};
    SetPosition(ptNew);
}

void
CGuage::SetText(const char* szTxt) {
    if (szTxt == NULL)
...
void
CGuage::SetWidth(int iWidth) {
    m_iWidth = iWidth;
}
void
CGuage::SetHeight(int iHeight) {
    m_iHeight = iHeight;
}
void
CGuage::SetGuageImageID(int iImageID) {
    m_iGuageImageID = iImageID;
}
void
CGuage::SetAutoIncrementMaxValue(int iAutoIncrementMaxValue) {
    m_iAutoIncrementMaxValue = iAutoIncrementMaxValue;
}