...
CDialogNpcScriptAnswerItemNew::CDialogNpcScriptAnswerItemNew(int iIndex,
    char* pszScript,
    int iEventID,
    ...
    if (pszScript) {
        ...
        int iOnelineWidth =
            m_Script.SetString(CStr::Printf("%d. %s", iIndex, pszScript), m_iWidth - 60);
...
void
CDialogNpcScriptAnswerItemNew::Draw() {
...
unsigned
CDialogNpcScriptAnswerItemNew::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CDialogNpcScriptAnswerItemNew::Update(POINT ptMouse) {
    if (m_Script.GetStringCount()) {
        if (!IsInside(ptMouse.x, ptMouse.y)) {
            m_highlight = false;
        } else {
