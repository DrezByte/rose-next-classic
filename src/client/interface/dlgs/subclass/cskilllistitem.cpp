...
void
CSkillListItem::SetIcon(CIconSkill* pIcon) {
    m_Slot.DetachIcon();
        ...
        if (pSkill) {
            ...
            if (pIcon->GetName()) {
                if (iLevel <= 0) {
                    ...
                    sStrBuf.set_string(CStr::Printf("%s(Lv:%d)", pIcon->GetName(), iLevel),
                        rtDraw,
                        FONT_NORMAL);
                    sStrBuf.draw();
                }
...
void
CSkillListItem::MoveWindow(POINT pt) {
...
void
CSkillListItem::Update(POINT ptMouse) {
    if (!IsVision())
        return;
        ...
        if (pIcon) {
            ...
            if (m_strUseAbility.is_ellipsis()) {
                ...
                if (PtInRect(&rect_useability, ptPoint)) {
                    CToolTipMgr::GetInstance().RegToolTip(m_sPosition.x,
                        m_sPosition.y + 20,
                        m_strUseAbility.get_string());
                    return;
                }
            }
            ...
            if (pCtrl && pCtrl == m_pButton) {
                ...
                if (IsHasNextLevel(pSkillIcon->GetSkillSlotFromIcon())) {
                    int iNeedSkillPoint =
                        g_pAVATAR->Get_NeedPoint2SkillUP(pSkillIcon->GetSkillSlotFromIcon());
                    int iRequireJuly =
                        SKILL_LEVELUP_NEED_ZULY(pSkillIcon->GetSkillIndex() + 1) * 100;
                    int iRequireLevel =
                        SKILL_NEED_ABILITY_VALUE(pSkillIcon->GetSkillIndex() + 1, 0);
                    ...
                    Info.AddString(CStr::Printf("%s: %d", STR_REQUIRE_SP, iNeedSkillPoint),
                        text_color);
...
unsigned
CSkillListItem::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
    ...
    if (m_pButton && m_pButton->Process(uiMsg, wParam, lParam)) {
        if (uiMsg
            == WM_LBUTTONUP) ///서버에 Level Up을 요청한다.그전에 레벨업이 가능한지 한번더 체크할것
        {
            CIcon* pIcon = m_Slot.GetIcon();
            if (pIcon) {
                CIconSkill* pSkillIcon = (CIconSkill*)pIcon;
...
bool
CSkillListItem::IsHasNextLevel(int iSkillSlotIdx) {
    if (iSkillSlotIdx < 0)
        return false;
...
bool
CSkillListItem::IsValidLevelUp(int iSkillSlotIdx) {
    if (iSkillSlotIdx < 0)
        return false;
    ...
    */
    ...
    for (int i = 0; i < SKILL_NEED_ABILITY_TYPE_CNT; ++i) {
        ...
        if (iNeedAbilityType) {
            if (g_pAVATAR->Get_AbilityValue(iNeedAbilityType)
