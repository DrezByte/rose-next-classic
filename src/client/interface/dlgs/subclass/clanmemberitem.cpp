...
CClanMemberItem::CClanMemberItem(const char* pszName,
    int iClass,
    int iClanPoint,
    int iChannelNo,
    short nLevel,
    ...
    m_iOnlineMarkImageID =
        CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "CLAN01_MARK_ONLINE");
    m_iOfflineMarkImageID =
        CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "CLAN01_MARK_OFFLINE");
}

CClanMemberItem::~CClanMemberItem(void) {}

unsigned
CClanMemberItem::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CClanMemberItem::SetConnectStatus(int iChannelNo) {
    m_iChannelNo = iChannelNo;
    if (iChannelNo == 0xff) {
        m_strChannel.clear();
...
CClanMemberItem::Draw() {
    ...
    ::drawFontf(g_GameDATA.m_hFONT[FONT_SMALL],
        true,
        20,
        2,
        dwColor,
...
const char*
CClanMemberItem::GetName() {
    return m_strName.c_str();
}

void
CClanMemberItem::SetClass(int iClass) {
    m_iClass = iClass;
    m_strClass = CStringManager::GetSingleton().GetClanClass(m_iClass);
}

int
CClanMemberItem::GetClass() {
    return m_iClass;
}

int
CClanMemberItem::GetClanPoint() {
    return m_iClanPoint;
}

int
CClanMemberItem::GetChannelNo() {
    return m_iChannelNo;
}

void
CClanMemberItem::Update(POINT ptMouse) {
    ...
    if (IsInside(ptMouse.x, ptMouse.y) && m_iChannelNo != 0xFF) {
        ...
        m_Info.AddString(
            CStr::Printf("%s: %d", CStringManager::GetSingleton().GetAbility(AT_LEVEL), m_nLevel));
        m_Info.AddString(CStr::Printf("%s: %s",
...
void
CClanMemberItem::SetLevel(short nLevel) {
    m_nLevel = nLevel;
}

void
CClanMemberItem::SetJob(short nJob) {
    m_nJob = nJob;
    m_strJob = CStringManager::GetSingleton().GetJobName(nJob);
}

short
CClanMemberItem::GetJob() {
    return m_nJob;
}

short
CClanMemberItem::GetLevel() {
    return m_nLevel;
}