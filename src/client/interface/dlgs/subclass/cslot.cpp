...
unsigned int
CSlot::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CSlot::SetIconPosition(POINT pt) {
    if (m_pIcon)
...
void
CSlot::SetParent(int iID) {
    m_iParentID = iID;
}

void
CSlot::SetDragAvailable() {
    m_bDragAvailable = true;
}

void
CSlot::SetDragDisable() {
    m_bDragAvailable = false;
}

void
CSlot::SetDragItem(CDragItem* pDragItem) {
    assert(pDragItem);
...
void
CSlot::SetTooltipType(DWORD dwType) {
...
void
CSlotBuyPrivateStore::SetSelected() {
    assert(GetIcon());
    s_pSelectedSlot = this;
}
bool
CSlotBuyPrivateStore::IsSelected() {
    assert(GetIcon());
...
void
CSlotBuyPrivateStore::DetachIcon() {
    CSlot::DetachIcon();
    if (s_pSelectedSlot == this)
...
void
CSlotBuyPrivateStore::SetExhibition(bool b) {
    m_bExhibition = b;
}

unsigned int
CSlotBuyPrivateStore::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    POINT pt = {LOWORD(lParam), HIWORD(lParam)};
...
bool
CSlotBuyPrivateStore::IsExhibition() {
