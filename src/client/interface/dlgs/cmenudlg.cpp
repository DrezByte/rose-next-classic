...
unsigned
CMenuDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CMenuDlg::OnLButtonUp(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    m_uiClickedButtonID = 0;
...
void
CMenuDlg::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
        case IID_BTN_CHARACTER:
        case IID_BTN_ITEM:
        case IID_BTN_SKILL:
...
void
CMenuDlg::SetInterfacePos_After() {
    Show();
    RefreshDlg();
    SetOpen();
}
...
void
CMenuDlg::SetOpenType(int iType) {
    m_iOpenType = iType;
}

int
CMenuDlg::GetOpenType() {
    return m_iOpenType;
}
