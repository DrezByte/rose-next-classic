...
void
CBankWindowDlg::PostMsg(unsigned msg, unsigned param1, unsigned param2) {
    switch (msg) {
        case MSG_SETYPE: {
            m_Type = param1;
            RECT rc = {8, 8, 168, 22};
...
unsigned
CBankWindowDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CBankWindowDlg::OnLButtonUp(unsigned proc_id) {
    switch (proc_id) {
        case IID_BTN_OK: {
...
void
CBankWindowDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
