...
unsigned int
CTargetMenu::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
bool
CTargetMenu::OnLButtonUp(unsigned iProcID) {
    switch (iProcID) {
        case IID_BTN_FRIEND: {
        ...
        case IID_BTN_EXCHANGE: {
            ...
            if (pObjAVT) {
                if (CExchange::GetInstance().SendCliTradeReq(
                        g_pObjMGR->Get_ServerObjectIndex(pObjAVT->Get_INDEX()))) {
        ...
        case IID_BTN_PARTY: {
            ...
            /*				if( abs(pObjChar->Get_LEVEL() - g_pAVATAR->Get_LEVEL()) > 5 )
                            {
                                char szBuf[256];
                                sprintf( szBuf,"%s님은 레벨이 맞지 않아 파티에 들어올 수 없습니다",
...
void
CTargetMenu::Update(POINT ptMouse) {
    if (!IsVision())
        return;
