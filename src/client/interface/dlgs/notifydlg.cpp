...
bool
CNotifyDlg::Create(const char* IDD) {
...
void
CNotifyDlg::SetScript(const char* pszScript) {
    assert(pszScript);
    if (pszScript == NULL)
        return;
...
CNotifyDlg::Draw() {
    ...
    if (int iStringCount = m_Script.GetStringCount()) {
        ...
        int y = 10;
        int iEndLine = (iStringCount > m_iDrawLine + m_iExtentLine) ? m_iDrawLine + m_iExtentLine
                                                                    : iStringCount;
...
unsigned
CNotifyDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
