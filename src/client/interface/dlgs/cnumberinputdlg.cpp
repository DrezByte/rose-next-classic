...
unsigned int
CNumberInputDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CNumberInputDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
void
CNumberInputDlg::OnLButtonUp(unsigned iProcID) {
    switch (iProcID) {
        case IID_BTN_0:
            AppendText("0");
            break;
...
void
CNumberInputDlg::AppendText(char* szText) {
    if (szText == NULL)
        return;
...
__int64
CNumberInputDlg::GetNumberFromEditBox() {
...
void
CNumberInputDlg::Show() {
    CTDialog::Show();
    SetFocusEditBox();
}

void
CNumberInputDlg::SetFocusEditBox() {
    CWinCtrl* pCtrl = Find(IID_EDITBOX);
...
void
CNumberInputDlg::SetCommand(CTCmdNumberInput* pCmd, CTObject* pObj) {
    m_pCmd = pCmd;
