    ...
    if (g_UserInputSystem.GetCurrentTarget()) {
        ...
        if (pObj->Get_TYPE() == OBJ_MOB) {
            ...
            if (iMaxHP != 0) {
                ...
                //				if( nWidth > 151 )
                //					nWidth = 151;
                pDrawObj->Draw(m_sPosition.x + 60,
                    m_sPosition.y + 33,
                    nWidth,
                    IMAGE_RES_UI,
                    pResourceMgr->GetImageNID(IMAGE_RES_UI, "ID_GUAGE_RED"));
            }
        }
    }
}
...
unsigned int
CTargetObjectDLG::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
    if (g_UserInputSystem.GetCurrentTarget() == 0)
        return 0;
