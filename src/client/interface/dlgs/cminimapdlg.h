...
private:
    ...
    float
        m_fMaxMinimapWorldPosX; /// 미니맵 Texture의 Size로 이동가능한 최대 월드 X좌표를 구한 값(
                                /// 실제 이동가능 월드 좌표와 틀릴수 있다:미니맵이 잘못 들어간 경우)
    ...
    bool m_bExploreMode; /// 마우스 드래그에 따라 미니맵의 중심이 변경되는 Mode(Draw에서 false조건
                         /// 체크)
    ...
    float m_fPrevUserPosX; /// Explore Mode를 종료하기 위하여 임시적으로 내 아바타의 위치를
                           /// 저장해두는 변수
...
public:
    ...
    bool
    SetMinimap(const char* pFname, const char* pCursorName, DWORD dwMapStartX, DWORD dwMapStartY);
    void FreeMinimap();
...
protected:
    ...
    bool IsInsideMinimap(float x,
        float y,
        int width,
        int height); /// 미니맵Dialog내부의 실제 미니맵위인가?

    void DrawPartyMember(); /// 파티 멤버 출력
    void
    DrawNpcFromMap(); /// 맵에 찍혀힌 NPC들 출력( 서버에서 받은 CObjNPC이 아닌 맵에서 직접 로드한 )
