...
void
CDealDLG::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    if (pObj && strcmp(pObj->toString(), "CTEventDeal") == 0) {
        ...
        switch (pEvent->GetID()) {
            ...
            case CTEventDeal::EID_REMOVE_BUYITEM: {
                ...
                assert(0
                    && "Trade List에 없는 아이템을 지우려고 합니다 "
                       "@CDealDlg::Update@EID_REMOVE_BUYITEM");
                break;
            }
...
CDealDLG::Draw() {
    ...
    if (CStore::GetInstance().IsUnionStore())
        g_DrawImpl.Draw(m_sPosition.x + 12,
            m_sPosition.y + 135,
            IMAGE_RES_STATE_ICON,
            UNION_MARK(CStore::GetInstance().GetUnionNo()));
}
...
unsigned int
CDealDLG::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
bool
CDealDLG::On_LButtonUP(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
    switch (iProcID) {
        case DEAL_BTN_CLOSE:
            g_itMGR.Close_store();
            return true;
