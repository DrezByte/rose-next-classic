...
void
CPrivateStoreDlg::MoveWindow(POINT pt) {
...
void
CPrivateStoreDlg::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
        case IID_BTN_SELL:
            m_iTab = 0;
            HideChild(IID_BTN_ADD_BUYLIST);
...
void
CPrivateStoreDlg::OnLButtonUp(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
            ...
            if (CPrivateStore::GetInstance().IsValidOpen()) {
                ...
                if (pCtrl && pCtrl->GetControlType() == CTRL_EDITBOX) {
                    ...
                    if (strlen(pszTitle) == 0) {
                        std::string strTitle =
                            CStr::Printf(STR_DEFAULT_PRIVATESTORE_TITLE, g_pAVATAR->Get_NAME());
...
void
CPrivateStoreDlg::SortItemSellList() {
...
unsigned
CPrivateStoreDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CPrivateStoreDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
...
void
CPrivateStoreDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
