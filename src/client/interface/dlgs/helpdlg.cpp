...
bool
CHelpDlg::LoadHelpContents(const char* szFileName, CTree* pTree) {
...
bool
CHelpDlg::LoadPageList(CFileSystem* pFileSystem, CTDefaultTreeModel* pTreeModel) {
    int iPageCount = 0;
...
bool
CHelpDlg::Create(const char* IDD) {
    CTDialog::Create(IDD);
    m_pTree = new CTree;
    ...
    if (stbFiles.Load("3DData\\STB\\List_Help.STB", true)) {
        ...
        if (stbFiles.m_ppNAME[iCurrentLang] != NULL) {
            if (LoadHelpContents(stbFiles.m_ppNAME[iCurrentLang], m_pTree)) {
                ...
                CTImage* pImage =
                    CTControlMgr::GetInstance()->MakeImage(100, 0, 0, 9, 9, iGraphicID, iModuleID);
                pImage->Show();
                m_pTreeNodeRenderer->setOpenImage(pImage);

                iGraphicID = CResourceMgr::GetInstance()->GetImageNID(iModuleID, "UI14_PLUS");
                pImage =
                    CTControlMgr::GetInstance()->MakeImage(101, 0, 0, 9, 9, iGraphicID, iModuleID);
                pImage->Show();
                m_pTreeNodeRenderer->setCloseImage(pImage);

                iGraphicID =
                    CResourceMgr::GetInstance()->GetImageNID(iModuleID, "UI14_CIRCLE_FILLED");
                pImage =
                    CTControlMgr::GetInstance()->MakeImage(102, 0, 0, 9, 9, iGraphicID, iModuleID);
                m_pTreeNodeRenderer->setParentImage(pImage);

                iGraphicID = CResourceMgr::GetInstance()->GetImageNID(iModuleID, "UI14_CIRCLE");
                pImage =
                    CTControlMgr::GetInstance()->MakeImage(103, 0, 0, 9, 9, iGraphicID, iModuleID);
...
UINT
CHelpDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
                                ...
                                if (pNode && pNode->isLeaf()) {
                                    ...
                                    if (pContainer) {
                                        ...
                                        if (pCtrl && pCtrl->GetControlType() == CTRL_LISTBOX) {
                                            ...
                                            for (int i = 0; i < pHelpPage->GetPageContentsCount();
                                                 ++i) {
                                                pList->AppendText(pHelpPage->GetPageContent(i),
...
int
CHelpPage::GetPageContentsCount() {
    return m_PageContents.size();
}

const char*
CHelpPage::GetPageContent(int i) {
