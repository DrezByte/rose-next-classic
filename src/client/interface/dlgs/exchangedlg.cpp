...
CExchangeDLG::Draw() {
    ...
    drawFontf(g_GameDATA.m_hFONT[FONT_NORMAL],
        ...
        g_dwWHITE,
        "%d",
        Data.GetOtherTradeMoney());
}

unsigned int
CExchangeDLG::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CExchangeDLG::OnLButtonUp(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
    switch (iProcID) {
        case IID_BTN_MONEY: {
...
void
CExchangeDLG::OnLButtonDown(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
    switch (iProcID) {
        case IID_BTN_OK: {
...
void
CExchangeDLG::SetOtherReadyState(bool b) {
...
void
CExchangeDLG::SetMeReadyState(bool b) {
    ...
    switch (pEvent->GetID()) {
        ...
        case CTEventExchange::EID_REMOVE_MYITEM: {
            ...
            if (m_MySlots[iSlotIndex].GetIcon() == NULL)
                assert(m_MySlots[iSlotIndex].GetIcon()
                    && "Detach Icon From Empty Slot @CExchangeDLG::Update");
        ...
        case CTEventExchange::EID_REMOVE_OTHERITEM: {
            ...
            if (m_OtherSlots[iSlotIndex].GetIcon() == NULL)
                assert(m_OtherSlots[iSlotIndex].GetIcon()
                    && "Detach Icon From Empty Slot @CExchangeDLG::Update");
...
void
CExchangeDLG::MoveWindow(POINT pt) {
...
void
CExchangeDLG::Update(POINT ptMouse) {
    if (!IsVision())
        return;
