...
const POINT c_ptDrawMemberName[] =
    {{18, 60}, {18, 80}, {18, 100}, {18, 120}, {105, 60}, {105, 80}, {105, 100}, {105, 120}};
...
unsigned
CChatRoomDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CChatRoomDlg::AddChatMsg(const char* pszMsg, DWORD dwColor) {
...
void
CChatRoomDlg::OnLButtonUp(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
...
#pragma message("현재 모든 채팅방은 공개방으로한다.")
                ...
                if (pCtrl->GetControlType() != CTRL_COMBOBOX)
                    return;
...
bool
CChatRoomDlg::Create(const char* IDD) {
...
CChatRoomDlg::Draw() {
    ...
    if (pszTitle)
        ::drawFont(g_GameDATA.m_hFONT[FONT_NORMAL_BOLD], true, 40, 8, g_dwBLUE, pszTitle);
...
void
CChatRoomDlg::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
...
void
CChatRoomDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObj);
    if (pObj == NULL)
        return;

    assert(strcmp(pObj->toString(), "ChatRoom") == 0);
    if (strcmp(pObj->toString(), "ChatRoom"))
        return;
...
CChatRoomDlg::RecvChatMsg(WORD wUserID, const char* pszMsg) {
    ...
    for (iter = m_Members.begin(); iter != m_Members.end(); ++iter) {
        if (iter->GetServerIdx() == wUserID) {
            AddChatMsg(CStr::Printf("%s> %s", iter->GetName(), pszMsg), g_dwBLACK);
            break;
        }
    }
}
CWinCtrl*
CChatRoomDlg::FindChildInPane(unsigned uiPaneID, unsigned uiChildID) {
...
unsigned
CChatRoomDlg::ActionPerformed(CActionEvent* e) {
    assert(e);
    if (e == NULL)
        return 0;
