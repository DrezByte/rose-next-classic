...
bool
CAvatarInfoDlg::Create(const char* IDD) {
...
void
CAvatarInfoDlg::MoveWindow(POINT pt) {
    ...
    if (ctrl) {
        ((CTStatic*)ctrl)
            ->SetString(CStr::Printf(
                ...
                g_pAVATAR->GetCur_MaxWEIGHT(), // WEIGHT
                g_pAVATAR->Get_EXP(),
                g_pAVATAR->Get_NeedEXP(g_pAVATAR->Get_LEVEL()), // EXP
                money_buffer // MONEY
                ));
    }
...
void
CAvatarInfoDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
unsigned
CAvatarInfoDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    unsigned iProcID = 0;
...
void
CAvatarInfoDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
        ...
        switch (pEvent->GetID()) {
            case CTEventItem::EID_ADD_ITEM: /// 인벤토리에 아이템이 추가/장착 되었을때
            {
                ...
                if (iIndex == EQUIP_IDX_WEAPON_R) /// 오른손 무기가 장착되었을경우
                {
                    ...
                    if (AddItem.GetShotTYPE()
                        == MAX_SHOT_TYPE) /// 소모탄을 사용하는 무기가 아닐경우
                    {
                    ...
                    && iIndex < INVENTORY_SHOT_ITEM0 + MAX_SHOT_TYPE) /// 소모탄이 장착되었을 경우
                {
            ...
            case CTEventItem::EID_DEL_ITEM: /// 인벤토리에서  아이템을 버리거나 탈착하였을때
            {
                if (iIndex == EQUIP_IDX_WEAPON_R) /// 오른손 무기의 경우
                {
                    ...
                    && iIndex < INVENTORY_SHOT_ITEM0 + MAX_SHOT_TYPE) ///소모탄일경우
                {
                    ...
                    if (pIcon) {
                        ...
                        if (pItemIcon->GetIndex() == EQUIP_IDX_WEAPON_R) ///장착된무기가 있을경우
                        {
                            ...
                            if (EquipItem.GetShotTYPE()
                                == iIndex
                                    - INVENTORY_SHOT_ITEM0) /// 장착되어 있는 무기의 소모탄일경우
                            {
                                tagITEM TempBullet;
                                TempBullet.Clear();
                                TempBullet.m_cType = ITEM_TYPE_NATURAL;
                                TempBullet.m_nItemNo =
                                    c_iRepresentBulletItemNo[EquipItem.GetShotTYPE()];
                                ...
                                m_Slot.AttachIcon(
                                    m_pRepresentBullet
                                        ->CreateItemIcon()); ///대표 Item의 Icon을 Attach한다.
                            }
                        } else if (pItemIcon->GetIndex()
                            == iIndex) /// 소모탄이 탈착되어 지는 경우 한번더 체크
                        {
                            CItemSlot* pItemSlot = g_pAVATAR->GetItemSlot();
...
void
CAvatarInfoDlg::SetMaxView(bool bIsMax) {
...
void
CAvatarInfoDlg::SetMinView() {
    SetMaxView(false);
}

bool
CAvatarInfoDlg::GetIsMaxView() {
    return m_bIsMaxView;
}

void
CAvatarInfoDlg::SetInterfacePos_After() {
    SetMiniMapPos();
}

void
CAvatarInfoDlg::SetMiniMapPos() {
