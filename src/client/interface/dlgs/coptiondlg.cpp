...
bool
COptionDlg::Create(const char* szIDD) {
    char* pszName;
    ...
    for (auto& video_mode: g_pCApp->get_video_modes()) {
        AddSupportedScreen(video_mode.width,
            video_mode.height,
            video_mode.depth,
    ...
    if (pCtrl && pCtrl->GetControlType() == CTRL_TABBEDPANE) {
        ...
        if (pCtrl && pCtrl->GetControlType() == CTRL_COMBOBOX) {
            ...
            for (iter = m_SupportedScreens.begin(); iter != m_SupportedScreens.end(); ++iter) {
                pComboBox->AddItem(
                    CStr::Printf("%d*%d*%d", iter->m_iWidth, iter->m_iHeight, iter->m_iDepth));
            }
        }
...
void
COptionDlg::OnLButtonDown(unsigned iProcID) {
    switch (iProcID) {
...
void
COptionDlg::OnLButtonUp(unsigned iProcID) {
    switch (iProcID) {
...
unsigned int
COptionDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
COptionDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
void
COptionDlg::ChangeResolution(t_OptionResolution Resolution) {
...
COptionDlg::DoCancel() {
    CMusicMgr::GetSingleton().SetVolume(
        g_ClientStorage.GetBgmVolumeByIndex(m_SoundOption.iBgmVolume));
    g_pSoundLIST->SetVolume(g_ClientStorage.GetEffectVolumeByIndex(m_SoundOption.iEffectVolume));
}

void
COptionDlg::ChangeKeyboardOption() {
    CWinCtrl* pCtrl = Find(IID_TABBEDPANE);
    if (pCtrl && pCtrl->GetControlType() == CTRL_TABBEDPANE) {
        CTabbedPane* pPane = (CTabbedPane*)pCtrl;
        ...
        if (pCtrl && pCtrl->GetControlType() == CTRL_COMBOBOX) {
            ...
            if (iID >= 0 && iID < (int)m_SupportedScreens.size()) {
                ...
                if (pCtrl && pCtrl->GetControlType() == CTRL_COMBOBOX) {
                    ...
                    assert(iFrequencyIndex >= 0
                        && iFrequencyIndex < (int)m_SupportedScreens[iID].m_Frequencys.size());

                    if (iFrequencyIndex >= 0
                        && iFrequencyIndex < (int)m_SupportedScreens[iID].m_Frequencys.size()) {
                        ...
                        Resolution.iFrequency =
                            m_SupportedScreens[iID].m_Frequencys[iFrequencyIndex];
    ...
    if (pCtrl && pCtrl->GetControlType() == CTRL_TABBEDPANE) {
        ...
        if (pCtrl && pCtrl->GetControlType() == CTRL_LISTBOX) {
            ...
            g_pSoundLIST->SetVolume(
                g_ClientStorage.GetEffectVolumeByIndex(m_SoundOption.iEffectVolume));
        }
    }
...
CTComboBox*
COptionDlg::GetComboBox(int ComboBoxID) {
...
void
COptionDlg::ChangeScreenModeByHotKey() {
    CWinCtrl* pCtrl = Find(IID_TABBEDPANE);
    if (pCtrl && pCtrl->GetControlType() == CTRL_TABBEDPANE) {
        CTabbedPane* pPane = (CTabbedPane*)pCtrl;
        CJContainer* pContainer = pPane->GetTabContainer(IID_TABVIDEO);
...
void
COptionDlg::AddSupportedScreen(int iWidth, int iHeight, int iDepth, int iFrequency) {
    std::vector<SupportedScreen>::iterator iter;
    for (iter = m_SupportedScreens.begin(); iter != m_SupportedScreens.end(); ++iter) {
...
void
COptionDlg::AddSupportedAntiAlising(int iOption) {
    std::vector<SupportedAntiAlising>::iterator iter;
...
int
COptionDlg::GetCurrentResolutionIndex() {
    std::vector<SupportedScreen>::iterator iter;
    int iCount = 0;
...
void
COptionDlg::ChangeCommunityOption() {
    CWinCtrl* pCtrl = Find(IID_TABBEDPANE);
    if (pCtrl && pCtrl->GetControlType() == CTRL_TABBEDPANE) {
        CTabbedPane* pPane = (CTabbedPane*)pCtrl;
        CJContainer* pContainer = pPane->GetTabContainer(IID_TABCOMMUNITY);
...
void
COptionDlg::InitVideoOption() {}

void
COptionDlg::InitAudioOption() {}

void
COptionDlg::InitPlayOption() {}

void
COptionDlg::InitCommunityOption() {}
void
COptionDlg::Draw() {
    if (!IsVision())
        return;
