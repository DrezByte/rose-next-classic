...
void
CGoodsDlg::MoveWindow(POINT pt) {
    CTDialog::MoveWindow(pt);
    m_Slot.MoveWindow(m_sPosition);
}

void
CGoodsDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
unsigned
CGoodsDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
