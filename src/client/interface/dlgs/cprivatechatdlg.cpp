...
bool
CPrivateChatDlg::Create(const char* IDD) {
...
void
CPrivateChatDlg::SetOther(DWORD dwUserTag, BYTE btStatus, const char* pszName) {
...
unsigned
CPrivateChatDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
///메세지를 보낼때마다 상대방의 상태를 체크해서 m_btStatus를 Update한다.만약 리스트에서 삭제되었다면
///창을 닫는다.
void
...
void
CPrivateChatDlg::AddChatMsg(const char* pszMsg) {
    ...
    if (pCtrl->GetControlType() != CTRL_LISTBOX)
        return;
    ...
    if (pCtrl->GetControlType() != CTRL_SCROLLBAR)
        return;
...
unsigned
CPrivateChatDlg::ActionPerformed(CActionEvent* e) {
    assert(e);
    if (e == NULL)
        return 0;
