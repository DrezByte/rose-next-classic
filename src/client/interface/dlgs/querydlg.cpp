...
unsigned int
CQueryDLG::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CQueryDLG::DrawQuery() {
    if (m_sQuestion.m_pQuestionPage == NULL)
        return;
    ...
    pDrawMgr->Draw(ptDlg.x,
        ptDlg.y + nYOffset + (nI * 19),
        IMAGE_RES_UI,
        pResourceMgr->GetImageNID(IMAGE_RES_UI, "ID_QUERY_BOTTOM"));
    pDrawMgr->Draw(ptDlg.x,
        ptDlg.y,
        IMAGE_RES_UI,
        ...
        if (strlen(pTXT)) {
            if (m_vecEXAM[nI].m_nLnCNT == nSelLN)
                ::drawFontf(g_GameDATA.m_hFONT[FONT_NORMAL],
                    true,
                    QUERY_START_X,
                    m_nLastPosY + (nI * QUERY_HEIGHT) + QUERY_START_Y,
                    g_dwRED,
                    "%s",
                    pTXT);
...
bool
CQueryDLG::AppendExam(char* szExam, int iEventID, void (*fpExamEvent)(int iEventID)) {
...
void
CQueryDLG::ProcessEvent(short nNum) {
...
bool
CQueryDLG::On_LButtonDN(unsigned iProcID) {
    MouseClickEventProc();
    return true;
}
bool
CQueryDLG::On_KeyDown(WPARAM wParam) {
    return KeyDownEventProc(wParam);
}

void
CQueryDLG::Update(POINT ptMouse) {
    if (!IsVision())
        return;
