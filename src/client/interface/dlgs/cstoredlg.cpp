...
unsigned int
CStoreDLG::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
bool
CStoreDLG::On_LButtonUP(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
    switch (iProcID) {
        case IID_BTN_CLOSE:
            g_itMGR.Close_store();
            break;
...
void
CStoreDLG::SetEnableTabButton(int iInvType, bool bEnable) {
...
void
CStoreDLG::SetTabButtonText(short iIndex, const char* szText) {
    unsigned int iID = 0;
...
void
CStoreDLG::MoveWindow(POINT pt) {
    CTDialog::MoveWindow(pt);
    UpdateSlotPosition(m_nInvType);
}

void
CStoreDLG::UpdateSlotPosition(int iType) {
    for (int iSlot = 0; iSlot < c_iSlotCountPerTab; ++iSlot)
        m_Slots[iType][iSlot].MoveWindow(m_sPosition);
}
void
CStoreDLG::Update(POINT ptMouse) {
    if (!IsVision())
        return;
        ...
        if (pDlg == (CTDialog*)this) {
            ...
            if (pCtrl = Find(IID_BTN_TAB1)) {
                ...
                if (text.is_ellipsis() && pBtn->IsInside(ptMouse.x, ptMouse.y))
                    CToolTipMgr::GetInstance().RegToolTip(pBtn->GetPosition().x - 10,
...
void
CStoreDLG::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    if (pObj == NULL) {
