...
unsigned
CClanDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CClanDlg::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
...
void
CClanDlg::OnLButtonUp(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
        case IID_BTN_PREVIEW: {
            ...
            if (false
                == CClanMarkTransfer::GetSingleton().Check_BmpFile(hBmpFile,
                    CClanMarkUserDefined::ClanMarkSize)) {
                // g_itMGR.OpenMsgBox( CStr::Printf("%s %s",STR_CLANMARK_INVALID_FORMAT ,
            ...
            CloseHandle(hBmpFile);
            HNODE hNode = loadTexture(CClanMarkUserDefined::NewClanMarkFileName.c_str(),
    ...
    if (Clan.GetClanNo()) {
        ...
        ::drawFont(g_GameDATA.m_hFONT[FONT_NORMAL],
            ...
            CStringManager::GetSingleton().GetClanClass(
                CClan::GetInstance().GetClass())); ///자신의 직위
        ::drawFontf(g_GameDATA.m_hFONT[FONT_NORMAL],
            true,
            84,
            309,
        ...
        g_DrawImpl.Draw(m_sPosition.x + 229,
            m_sPosition.y + 283,
            IMAGE_RES_UI,
...
void
CClanDlg::DrawMembers() {

    if (!m_strSelectedMemberView.empty()) {
        D3DXMATRIX mat;
        D3DXMatrixTranslation(&mat,
            (float)m_sPosition.x + 20,
            (float)m_sPosition.y + m_iHeight - 28,
            0.0f);
        ::setTransformSprite(mat);
...
void
CClanDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);

    if (pObservable == NULL || pObj == NULL)
        return;
        ...
        switch (pEvent->GetID()) {
            ...
            case CTEventClan::EID_ADD_MEMBER: {
                ...
                if (pCtrl && pCtrl->GetControlType() == CTRL_TABBEDPANE) {
                    ...
                    if (pCtrl && pCtrl->GetControlType() == CTRL_ZLISTBOX) {
                        CZListBox* pList = (CZListBox*)pCtrl;
                        CClanMemberItem* pItem = new CClanMemberItem(pEvent->GetName(),
                            pEvent->GetClass(),
                            pEvent->GetPoint(),
                            pEvent->GetChannelNo(),
                            pEvent->GetLevel(),
...
void
CClanDlg::RemoveClanSkillListItem(CZListBox* pListBox, int iSkillSlotIndex) {
...
CWinCtrl*
CClanDlg::FindCtrlInTabbedPane(CTabbedPane* pPane, int iTabID, int iCtrlID) {
    CWinCtrl* pCtrl = NULL;
...
void
CClanDlg::SortMemberlistByClass() {
    ...
    if (pCtrl && pCtrl->GetControlType() == CTRL_TABBEDPANE) {
        ...
        if (pCtrl && pCtrl->GetControlType() == CTRL_ZLISTBOX) {
            ...
            for (int i = 0; i < pList->GetSize(); ++i) {
                ...
                pNewItem = new CClanMemberItem(pItem->GetName(),
                    pItem->GetClass(),
                    pItem->GetClanPoint(),
                    pItem->GetChannelNo(),
                    pItem->GetLevel(),
...
bool
CClanDlg::Create(const char* IDD) {
    bool bRet = CTDialog::Create(IDD);
    if (bRet) {
        m_iClassMarkImageNo[0] =
            CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "CLAN01_MARK_JUNIOR");
        m_iClassMarkImageNo[1] =
            CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "CLAN01_MARK_JUNIOR");
        m_iClassMarkImageNo[2] =
            CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "CLAN01_MARK_SENIOR");
        m_iClassMarkImageNo[3] =
            CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "CLAN01_MARK_LANDER");
        m_iClassMarkImageNo[4] =
            CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "CLAN01_MARK_COMMANDER");
        m_iClassMarkImageNo[5] =
            CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "CLAN01_MARK_SUBMASTER");
        m_iClassMarkImageNo[6] =
            CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "CLAN01_MARK_MASTER");
...
void
CClanDlg::SetSelectedMember(int iClanPoint) {
