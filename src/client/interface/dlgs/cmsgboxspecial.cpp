...
unsigned int
CMsgBoxSpecial::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CMsgBoxSpecial::SetCommand(CTCommand* pCmdOk, CTCommand* pCmdCancel) {
    ClearCommand();
...
void
CMsgBoxSpecial::SetInvokerDlgID(unsigned int iDlgID) {
    m_iInvokerDlgID = iDlgID;
}

void
CMsgBoxSpecial::SendCommand(unsigned int iDlgID, CTCommand* pCmd) {
    if (pCmd) {
