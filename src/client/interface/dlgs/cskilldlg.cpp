...
unsigned int
CSkillDLG::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    unsigned iProcID = 0;
...
bool
CSkillDLG::On_LButtonUP(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
    switch (iProcID) {
...
CSkillDLG::DrawTexts() {
    ...
    D3DXMatrixTranslation(&mat,
        (float)(m_sPosition.x + 41),
        (float)(m_sPosition.y + m_iHeight - 22),
        0.0f);
    ...
    ::drawFontf(g_GameDATA.m_hFONT[FONT_NORMAL],
        ...
        g_dwWHITE,
        "%d",
        g_pAVATAR->GetCur_SkillPOINT());
}

bool
CSkillDLG::IsValidLevelUp(int iSkillSlotIdx) {
    if (iSkillSlotIdx < 0)
        return false;
...
void
CSkillDLG::UpdateSkillListBySkillLevelUp(CZListBox* pListBox) {
...
void
CSkillDLG::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);
...
void
CSkillDLG::RemoveSkillListItem(CZListBox* pListBox, int iSkillSlotIndex) {
...
bool
CSkillDLG::Create(const char* IDD) {
