...
unsigned int
CMsgBox::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CMsgBox::SetCommand(CTCommand* pCmdOk, CTCommand* pCmdCancel) {
    ClearCommand();
...
void
CMsgBox::SetInvokerDlgID(unsigned int iDlgID) {
    m_iInvokerDlgID = iDlgID;
}

void
CMsgBox::SendCommand(unsigned int iDlgID, CTCommand* pCmd) {
    if (pCmd) {
        if (CGame::GetInstance().GetCurrStateID() == CGame::GS_MAIN)
...
void
CMsgBox::SetIcons(std::list<CIcon*>& Icons) {
...
void
CMsgBox::SetString(const char* szString) {
