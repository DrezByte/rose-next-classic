...
public:
    ...
    void UpdateSellList(
        tagPS_SLOT_ITEM& SlotITEM); /// 판매 희망아이템 리스트에 변동사항이 있을경우 업데이트
    void UpdateBuyList(
        tagPS_SLOT_ITEM& SlotITEM); /// 구입 희망아이템 리스트에 변동사항이 있을경우 업데이트

    int IsBuyItem(
        tagITEM& Item); /// 개인상점에 내가 팔려는 아이템이 구입희망리스트에 있는 아이템인가?
...
private:
    ...
    CTCmdNumberInput* m_pBuyItemAtAvatarStore; /// 판매 희망 아이템을 구입할때 사용될 TCommand( 숫자
                                               /// 입력기를 띄운다 )
