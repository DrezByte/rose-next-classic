...
CPartyMember::Draw() {
    ...
    if (pObjAVT) {
        ...
        g_DrawImpl.Draw(m_rcThis.right - nMaxWidth,
            m_rcThis.top + 25,
            nWidth,
            IMAGE_RES_UI,
            m_iGuageIdx);
...
unsigned int
CPartyMember::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
...
bool
CPartyMember::IsInside(POINT pt) {
    return (PtInRect(&m_rcThis, pt)) ? true : false;
}

void
CPartyMember::SetPosition(POINT pt) {
    m_rcThis.left = pt.x;
    m_rcThis.top = pt.y;
...
bool
CPartyMember::AddStatusIcon(DWORD dwFlag) {
...
void
CPartyMember::ResetStatusIconsPosition() {
...
void
CPartyMember::Update(POINT ptMouse) {
    CObjAVT* pObjAVT =
        g_pObjMGR->Get_CharAVT(g_pObjMGR->Get_ClientObjectIndex(m_wObjSvrIdx), false);
...
int
CPartyMember::GetHeight() {
    return m_iHeight;
}

DWORD
CPartyMember::GetObjectTag() {
    return m_dwObjectTag;
}

WORD
CPartyMember::GetObjSvrIdx() {
    return m_wObjSvrIdx;
}

void
CPartyMember::SetObjSvrIdx(WORD wObjSvrIdx) {
    m_wObjSvrIdx = wObjSvrIdx;
}
