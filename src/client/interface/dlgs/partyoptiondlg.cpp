...
unsigned
CPartyOptionDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CPartyOptionDlg::OnLButtonUp(unsigned uiProcID) {
    switch (uiProcID) {
        case IID_BTN_CLOSE:
            Hide();
            break;
        case IID_BTN_CONFIRM: {
            ...
            if (!CParty::GetInstance().IsPartyLeader()) ///파티장이 아닐경우에는 다른 옵션 변경은
                                                        ///무시( 정상적으로는 변하지도 않는다. );
            {
                Hide();
                break;
            ...
            if (pCtrl == NULL)
                break;
            if (pCtrl->GetControlType() != CTRL_RADIOBOX)
                break;
...
void
CPartyOptionDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);
    if (pObj == NULL)
        return;
        ...
        case CTEventParty::EID_CHANGE_RULE: {
            ...
            if (btRule & BIT_PARTY_RULE_ITEM_TO_ORDER) {
                g_itMGR.AppendChatMsg(
                    CStr::Printf("%s: %s", STR_ITEM_PICKUP_RIGHT, STR_PARTY_ITEM_PICKUP_SEQENCE),
                    IT_MGR::CHAT_TYPE_SYSTEM);
                pRadioBox->SetPressedButton(IID_RADIOBUTTON_ITEM_SEQUENCE);
            } else {
                g_itMGR.AppendChatMsg(
                    CStr::Printf("%s: %s", STR_ITEM_PICKUP_RIGHT, STR_PARTY_ITEM_PICKUP_RIGHT_ALL),
                    IT_MGR::CHAT_TYPE_SYSTEM);
                pRadioBox->SetPressedButton(IID_RADIOBUTTON_ITEM_PICK);
            }
...
CPartyOptionDlg::Draw() {
    ...
    ::drawFont(g_GameDATA.m_hFONT[FONT_NORMAL],
        ...
        122,
        g_dwBLACK,
        STR_PARTY_ITEM_PICKUP_RIGHT_ALL);
    ...
    ::drawFont(g_GameDATA.m_hFONT[FONT_NORMAL],
        ...
        182,
        g_dwBLACK,
        STR_SHOW_PARTYMEMBER_HPGUAGE);
}

void
CPartyOptionDlg::Show() {
    CTDialog::Show();
...
bool
CPartyOptionDlg::Create(const char* IDD) {
