...
void
CAvatarStoreDlg::SetMasterSvrObjIdx(WORD wSvrObjIdx) {
    m_wMasterSvrObjIdx = wSvrObjIdx;
}

WORD
CAvatarStoreDlg::GetMasterSvrObjIdx() {
    return m_wMasterSvrObjIdx;
}
void
CAvatarStoreDlg::Draw() {
    if (!IsVision())
        return;
    CTDialog::Draw();
...
unsigned
CAvatarStoreDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CAvatarStoreDlg::OnLButtonUp(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
        case IID_BTN_CLOSE:
            Hide();
            break;
...
void
CAvatarStoreDlg::OnLButtonDown(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
        case IID_BTN_SELL:
            m_iTab = 0;
            break;
...
void
CAvatarStoreDlg::MoveWindow(POINT pt) {
...
void
CAvatarStoreDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
    ...
    if (CTDialog* pDlg = CTDialog::GetProcessMouseOverDialog()) {
        if (pDlg == (CTDialog*)this) {
            ...
            if (m_title.is_ellipsis() && PtInRect(&m_title.get_rect(), pt))
                CToolTipMgr::GetInstance().RegToolTip(m_sPosition.x + m_title.get_rect().left - 10,
                    m_sPosition.y - 18,
                    m_title.get_string());
        }
    }
    ...
    CObjAVT* pAvt =
        g_pObjMGR->Get_CharAVT(g_pObjMGR->Get_ClientObjectIndex(m_wMasterSvrObjIdx), false);
...
void
CAvatarStoreDlg::AddItem2SellList(tagPS_SLOT_PRICE& SlotITEM) {
...
void
CAvatarStoreDlg::AddItem2BuyList(tagPS_SLOT_PRICE& SlotITEM) {
    assert(SlotITEM.m_btSLOT >= 0 && SlotITEM.m_btSLOT < c_iAvatarStoreMaxSlotCount);
    if (SlotITEM.m_btSLOT >= 0 && SlotITEM.m_btSLOT < c_iAvatarStoreMaxSlotCount) {
        CItem* pItem = new CItem;
...
void
CAvatarStoreDlg::UpdateSellList(tagPS_SLOT_ITEM& SlotITEM) {
    assert(SlotITEM.m_btSLOT >= 0 && SlotITEM.m_btSLOT <= c_iAvatarStoreMaxSlotCount);
    if (SlotITEM.m_btSLOT >= 0 && SlotITEM.m_btSLOT <= c_iAvatarStoreMaxSlotCount) {
...
CAvatarStoreDlg::IsBuyItem(tagITEM& Item) {
    ...
    for (int iSlot = 0; iSlot < c_iAvatarStoreMaxSlotCount; ++iSlot) {
        ...
        if (pItem && pItem->GetItem().IsEqual(Item.GetTYPE(), Item.GetItemNO())) {
            if (bCheckError) {
                bFound = true;
                if (pItem->GetItem().GetOption()
                    && pItem->GetItem().GetOption() != Item.GetOption()) {
                    iErrCode = ERR_OPTION;
                    continue;
                }
    ...
    if (bFound && bCheckError) /// 상점에서 구입을 원하는 아이템의 옵션( 옵션, 제련등급, 내구도,
                               /// 소켓유무) 비교
    {
        switch (iErrCode) {
            case ERR_OPTION:
 ...
 */
int
CAvatarStoreDlg::GetWishIndex(tagITEM& Item, int& iUnitPrice) {
    int iWishIndex = -1;
    CItem* pItem;
    m_BuyItems[c_iAvatarStoreMaxSlotCount];
    for (int iSlot = 0; iSlot < c_iAvatarStoreMaxSlotCount; ++iSlot) {
...
void
CAvatarStoreDlg::SetTitle(const char* pszTitle) {
    assert(pszTitle);
    if (pszTitle) {
