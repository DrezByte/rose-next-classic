...
const int g_iAutoExpDecreaseTimeSpace =
    10000; ///클라이언트에서 파티 경험치를 얼마의 시간마다 파티레벨만큼 빼주나
...
unsigned int
CPartyDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CPartyDlg::OnLButtonUp(unsigned iProcID) {
    ...
    switch (iProcID) {
        ...
        case IID_BTN_ENTRUST: {
            if (CPartyMember* pMember = GetSelectedMember()) {
                ...
                CTCmdSendPacketPartyReq* pCmd =
                    new CTCmdSendPacketPartyReq(PARTY_REQ_CHANGE_OWNER, pMember->GetObjSvrIdx());
                sprintf(g_MsgBuf, FORMAT_STR_PARTY_QUERY_ENTRUST, pMember->GetName());
                g_itMGR
                    .OpenMsgBox(g_MsgBuf, CMsgBox::BT_OK | CMsgBox::BT_CANCEL, true, 0, pCmd, NULL);
            }
            break;
        }
        case IID_BTN_CLOSE:
        case IID_BTN_LEAVE: {
            ...
            CTCmdSendPacketPartyReq* pSubCmd1 = new CTCmdSendPacketPartyReq(PARTY_REQ_LEFT,
        ...
        case IID_BTN_BAN: {
            if (CPartyMember* pMember = GetSelectedMember()) {
                ...
                CTCmdSendPacketPartyReq* pCmd =
                    new CTCmdSendPacketPartyReq(PARTY_REQ_BAN, pMember->GetObjectTag());
...
WORD
CPartyDlg::GetSelectedMemberSvrIdx() {
    LIST_PARTY_MEMBER_ITER iter;
    for (iter = m_listPartyMember.begin(); iter != m_listPartyMember.end(); ++iter) {
        if ((*iter)->IsSelected())
...
DWORD
CPartyDlg::GetSelectedMemberUserTag() {
    LIST_PARTY_MEMBER_ITER iter;
    for (iter = m_listPartyMember.begin(); iter != m_listPartyMember.end(); ++iter) {
        if ((*iter)->IsSelected())
...
CPartyMember*
CPartyDlg::FindMember(WORD wObjSvrIdx) {
    LIST_PARTY_MEMBER_ITER iter;
    for (iter = m_listPartyMember.begin(); iter != m_listPartyMember.end(); ++iter) {
...
CPartyDlg::Show() {
    ...
    m_iGuagePartyExpIdx =
        CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI18_GUAGE_PARTYLEVEL");
    m_iGuagePartyExpBGIdx =
        CResourceMgr::GetInstance()->GetImageNID(IMAGE_RES_UI, "UI18_GUAGE_PARTYLEVEL_BASE");
...
void
CPartyDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
    ...
    for_each(m_listPartyMember.begin(),
        m_listPartyMember.end(),
        bind2nd(mem_fun(&CPartyMember::Update), ptMouse));
    ...
    if (pCtrl && pCtrl->IsInside(ptMouse.x, ptMouse.y)) {
        CToolTipMgr::GetInstance().RegToolTip(pCtrl->GetPosition().x - 20,
            pCtrl->GetPosition().y - 20,
            TT_PARTY_LEAVE);
        return;
    }
...
void
CPartyDlg::MoveWindow(POINT pt) {
    CTDialog::MoveWindow(pt);
    ResetMemberPositions();
}

void
CPartyDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);
    if (pObj == NULL)
        return;
    ...
    switch (pEvent->GetID()) {
        ...
        case CTEventParty::EID_JOIN_MEMBER: {
            CPartyMember* pMember =
                new CPartyMember(pEvent->GetObjectTag(), pEvent->GetObjSvrIdx(), pEvent->GetName());
            m_listPartyMember.push_back(pMember);
            ResetMemberPositions();

            g_itMGR.AppendChatMsg(CStr::Printf(FORMAT_STR_PARTY_JOIN, pEvent->GetName()),
        ...
        case CTEventParty::EID_LEAVE_MEMBER: {
            ...
            for (iter = m_listPartyMember.begin(); iter != m_listPartyMember.end(); ++iter) {
                if ((*iter)->GetObjSvrIdx() == iObjSvrIdx) {
                    ...
                    g_itMGR.AppendChatMsg(CStr::Printf(FORMAT_STR_PARTY_LEAVE, pMember->GetName()),
                        IT_MGR::CHAT_TYPE_SYSTEM);
                ...
                if (pMember->GetObjSvrIdx() == iObjSvrIdx) {
                    ...
                    if (Party.IsPartyLeader()) {
                        ...
                        g_itMGR.AppendChatMsg(
                            CStr::Printf(FORMAT_STR_PARTY_CHANGE_LEADER, pMember->GetName()),
                            IT_MGR::CHAT_TYPE_SYSTEM);
                    }
                    break;
                }
        ...
        case CTEventParty::EID_BAN_MEMBER: {
            ...
            for (iter = m_listPartyMember.begin(); iter != m_listPartyMember.end(); ++iter) {
                if ((*iter)->GetObjectTag() == pEvent->GetObjectTag()) {
                    ...
                    g_itMGR.AppendChatMsg(CStr::Printf(FORMAT_STR_PARTY_BANNED, pMember->GetName()),
                        IT_MGR::CHAT_TYPE_SYSTEM);
