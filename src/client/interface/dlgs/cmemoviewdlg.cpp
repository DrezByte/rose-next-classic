...
CMemoViewDlg::~CMemoViewDlg(void) {}
bool
CMemoViewDlg::Create(const char* IDD) {
...
unsigned
CMemoViewDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
        ...
        if (uiMsg == WM_LBUTTONUP) {
            switch (uiProcID) {
                case IID_BTN_DELETE:
                    ...
                    // CSqliteDB::GetInstance().DeleteMemo( m_iID );				/// 클라이언트 DB에서
                    // 삭제
...
void
CMemoViewDlg::SetTargetName(const char* pszName) {
    assert(pszName);
    if (pszName)
        m_strName = pszName;
}
...
void
CMemoViewDlg::SetContent(const char* pszContent) {
    assert(pszContent);
    if (pszContent)
