...
bool
CBankDlg::Create(const char* szIDD) {
...
CBankDlg::Draw() {
    ...
    ::drawFontf(g_GameDATA.m_hFONT[FONT_NORMAL_BOLD],
        ...
        g_dwBLACK,
        F_STR_BANK_TITLE,
        (char*)g_GameDATA.username.c_str());

    const int money_buffer_size = 64;
    char money_buffer[money_buffer_size];
    CGameUtil::ConvertMoney2String(CBank::GetInstance().GetMoney(),
        money_buffer,
        money_buffer_size);
...
unsigned int
CBankDlg::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
    unsigned iProcID = 0;
...
void
CBankDlg::OnLButtonDown(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
    switch (iProcID) {
        case IID_BTN_TAB1:
            m_iCurrBankPage = 0;
            break;
...
void
CBankDlg::OnLButtonUp(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
    switch (iProcID) {
        case IID_BTN_SAVE:
            g_itMGR.PostMsg2Dlg(DLG_TYPE_BANKWINDOW,
                CBankWindowDlg::MSG_SETYPE,
                CBankWindowDlg::TYPE_SAVE,
                0);
...
void
CBankDlg::SetTabButtonText(int iID, char* szText) {
...
void
CBankDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    if (pObj == NULL) {
...
int
CBankDlg::GetCurrentTab() {
    return m_iCurrBankPage; /// 0 ~ 3
}

bool
CBankDlg::IsFullFirstTab() {
