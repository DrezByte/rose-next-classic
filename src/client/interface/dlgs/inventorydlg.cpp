...
// *------------------------------------------------------------------------* //
bool
CInventoryDLG::On_LButtonUP(unsigned iProcID, WPARAM wParam, LPARAM lParam) {
    switch (iProcID) {
        case INV_BTN_CLOSE:
            Hide();
            return true;
...
bool CInventoryDLG::UseItem( short nRealIndex )
{
    ...
    {
        g_itMGR.AppendChatMsg(STR_CANT_USEITEM_REASON_SYNCOPE,g_dwYELLOW,
...
/*				g_DragNDrop.CharDLG_DragProc( false );
                        ...
                        case 313:
                            {
                                ...
                                if( CSkillManager::CheckConditionForFireSkill( iSkillIDX,
   iCurrentTarget ) == false )
                                {
   ...
   ); int iCount = pDlg->GetItemQuantityInMaterialList( nRealIndex ); int iMaxCount =
   sITEM.GetQuantity() - iCount; if( iMaxCount > 1 )
                        {
                            g_itMGR.ShowNumberInputDlg( DLG_TYPE_MAKE, pCmd, DLG_TYPE_MAKE ,
   iMaxCount );
                        }
                        else if( iMaxCount == 1 )
                        {
...
*/
void
CInventoryDLG::MoveIcon(int iVirtualInvenIdx1, int iVirtualInvenIdx2, int iInvenType) {
    /*	CIconItem tempItem = m_VirtualInventory[ iInvenType ][ iVirtualInvenIdx1 ];
    m_VirtualInventory[ iInvenType ][ iVirtualInvenIdx1 ] = m_VirtualInventory[ iInvenType ][
    iVirtualInvenIdx2 ]; m_VirtualInventory[ iInvenType ][ iVirtualInvenIdx2 ] = tempItem;
...
void
CInventoryDLG::MoveIcon(int iInvenIdx) {
    /*
    ...
    if( iSrcSlotIdx >= 0 )
    {
        ...
        if( iDestSlotIdx >= 0 )
        {
            ...
            {
                m_VirtualInventory[m_nInvType][iDestSlotIdx] =
    m_VirtualInventory[m_nInvType][iSrcSlotIdx];
            ...
            else
            {
                ...
                m_VirtualInventory[m_nInvType][iDestSlotIdx] =
    m_VirtualInventory[m_nInvType][iSrcSlotIdx]; ptDraw.x = m_sPosition.x + ITEM_START_POS_X + (
    (iDestSlotIdx%10) * ITEM_WIDTH ); ptDraw.y = m_sPosition.y + ITEM_START_POS_Y + (
    (iDestSlotIdx/10) * ITEM_HEIGHT ); m_VirtualInventory[m_nInvType][iDestSlotIdx].SetPosition(
    ptDraw );
...
*/
void
CInventoryDLG::UpdateSlotPosition(int iType) {
    for (int j = 0; j < INVENTORY_PAGE_SIZE; ++j)
        m_ItemSlots[iType][j].MoveWindow(m_sPosition);
}

void
CInventoryDLG::MoveWindow(POINT ptMouse) {
...
int
CInventoryDLG::FindItem(int iItemType, int iItemNo) {
...
int
CInventoryDLG::FindItem(int iClass) {
    ///*
    // CIconItem* pItemIcon	= NULL;
    // tagITEM*   pItem		= NULL;
    // for( int i = 0; i < MAX_INV_TYPE; ++i )
