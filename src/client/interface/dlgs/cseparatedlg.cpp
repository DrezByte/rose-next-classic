...
void
CSeparateDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj && strcmp(pObj->toString(), "Separate") == 0);
    if (pObj == NULL || strcmp(pObj->toString(), "Separate"))
        return;
...
unsigned
CSeparateDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CSeparateDlg::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
//*-------------------------------------------------------------------/
bool
CSeparateDlg::HasEnoughInventoryEmptySlot(CIconItem* pItemIcon, std::vector<CSlot>& OutputItems) {
    assert(pItemIcon);
    if (pItemIcon == NULL)
        return false;
...
void
CSeparateDlg::MoveWindow(POINT pt) {
