...
unsigned
CCommDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CCommDlg::OnLButtonUp(unsigned uiProcID, WPARAM wParam, LPARAM lParam) {
    switch (uiProcID) {
...
void
CCommDlg::AddFriend(DWORD dwUserTag, BYTE btStatus, const char* pszName) {
...
void
CCommDlg::ChangeFriendStatus(DWORD dwUserTag, BYTE btStatus) {
    CWinCtrl* pCtrl = NULL;
    CZListBox* pZlist = GetZListBox(TAB_FRIEND, IID_ZLIST_FRIEND);
    assert(pZlist);
    if (pZlist == NULL)
...
CZListBox*
CCommDlg::GetZListBox(int iTab, int iListID) {
    CWinCtrl* pCtrl = Find(IID_TABBEDPANE);
...
void
CCommDlg::RemoveFriend(DWORD dwUserTag) {
    CWinCtrl* pCtrl = NULL;
    CZListBox* pZlist = GetZListBox(TAB_FRIEND, IID_ZLIST_FRIEND);
    assert(pZlist);
    if (pZlist == NULL)
...
CFriendListItem*
CCommDlg::FindFriendByName(const char* pszName) {
    assert(pszName);
    if (pszName == NULL)
        return NULL;
...
void
CCommDlg::AddMemo(int iRowid, const char* pszFromName, const char* pszMemo, DWORD dwRecvTime) {
    CZListBox* pZlist = GetZListBox(TAB_MAILBOX, IID_ZLIST_MAILBOX);
    assert(pZlist);
    if (pZlist == NULL)
        return;
...
void
CCommDlg::DeleteMemo(int iIndex) {
    CZListBox* pZlist = GetZListBox(TAB_MAILBOX, IID_ZLIST_MAILBOX);
    assert(pZlist);
    if (pZlist == NULL)
        return;
...
void
CCommDlg::AddChatRoom(const char* pszTitle, BYTE btRoomType, short nRoomID, BYTE btUserCount) {
    CZListBox* pZList = GetZListBox(TAB_CHATROOM, IID_ZLIST_CHATROOM);
    assert(pZList);
    if (pZList == NULL)
        return;
...
void
CCommDlg::LoadMemoFromDB() {
    ClearMemoList();
    // CSqliteDB::GetInstance().LoadMemo( g_pAVATAR->Get_NAME(), callback );
}

int
CCommDlg::callback(void* NotUsed, int argc, char** argv, char** azColName) {
    assert(argc == 4);
    if (argc == 4) {
