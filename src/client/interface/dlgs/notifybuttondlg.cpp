...
unsigned
CNotifyButtonDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CNotifyButtonDlg::UpdatePosition() {
    std::list<CTButton*>::iterator iter;
    int iCount = 0;
...
void
CNotifyButtonDlg::AddButton(CTButton* pBtn) {
    assert(pBtn);
    if (pBtn) {
        pBtn->Show();
...
void
CNotifyButtonDlg::Draw() {
    for_each(m_Children.begin(), m_Children.end(), [](auto b) { b->Draw(); });
}

void
CNotifyButtonDlg::Update(POINT ptMouse) {
    for_each(m_Children.rbegin(), m_Children.rend(), [ptMouse](auto b) { b->Update(ptMouse); });
}

void
CNotifyButtonDlg::Clear() {
    std::list<CTButton*>::iterator iter;
