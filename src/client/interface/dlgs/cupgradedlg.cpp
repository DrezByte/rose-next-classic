...
void
CUpgradeDlg::ChangeState(int iID) {
    ...
    for (int i = 0; i < 3; ++i) {
        if (Upgrade.GetRequireMaterialName(i)) {
            drawFont(g_GameDATA.m_hFONT[FONT_NORMAL],
                ...
                g_dwWHITE,
                DT_CENTER,
                Upgrade.GetRequireMaterialName(i));
            drawFontf(g_GameDATA.m_hFONT[FONT_NORMAL],
                true,
                &rcDrawCount,
                DT_RIGHT,
                "%d",
                Upgrade.GetRequireMaterialCount(i));
        }
        rcDraw.top += 47;
        rcDraw.bottom += 47;
        rcDrawCount.top += 47;
...
unsigned
CUpgradeDlg::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CUpgradeDlg::Update(CObservable* pObservable, CTObject* pObj) {
    assert(pObservable);
    assert(pObj);
    if (pObj == NULL)
        return;
    ...
    if (strcmp(pObj->toString(), "Upgrade"))
        return;
...
void
CUpgradeDlg::MoveWindow(POINT pt) {
