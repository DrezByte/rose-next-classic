...
/*----------------------------------------------------------------------------------------------------*/
bool
CTCmdDropItem::Exec(CTObject* pObj) {
    // if( m_iNumber <= 0  )
    //	return true;
    if (pObj == NULL) {
    ...
    long lPrice = g_pTerrain->m_Economy.Get_ItemSellPRICE(Item, g_pAVATAR->GetSellSkillVALUE())
        * (int)m_iNumber;
...
bool
CTCmdDropMoney::Exec(CTObject* pObj) {
    if (m_iNumber >= 1000) {
...
bool
CTCmdAddMyItem2Exchange::Exec(CTObject* pObj) {
...
bool
CTCmdAddMyMoney2Exchange::Exec(CTObject* pObj) {
    if (m_iNumber <= 0)
        return true;
...
bool
CTCmdRemoveMyMoneyFromExchange::Exec(CTObject* pObj) {
    if (m_iNumber <= 0)
        return true;
...
bool
CTCmdRemoveMyItemFromExchange::Exec(CTObject* pObj) {
    if (pObj == NULL) {
        assert(pObj && "pObj is NULL @CTCmdRemoveMyItemFromExchange");
        return true;
    }
...
bool
CTCmdMoveItemBank2Inv::Exec(CTObject* pObj) {
    if (pObj == NULL) {
...
bool
CTCmdAddItem2DealFromStore::Exec(CTObject* pObj) {
    if (pObj == NULL) {
...
///*--------------------------------------------------------------------------------------*/
bool
CTCmdBuyItem::Exec(CTObject* pObj) {
    ...
    if (CStore::GetInstance().IsUnionStore()) {
        ...
        if (lPrice > g_pAVATAR->Get_AbilityValue(AT_UNION_POINT1 - 1 + g_pAVATAR->Get_UNION())) {
            ...
            g_pAVATAR->Set_AbilityValue(AT_UNION_POINT1 - 1 + g_pAVATAR->Get_UNION(),
                iValue - lPrice);
        }
    } else {
        lPrice = g_pTerrain->m_Economy.Get_ItemBuyPRICE(Item.GetTYPE(),
                     Item.GetItemNO(),
    ...
    g_pNet->Send_cli_STORE_TRADE_REQ(
        g_pObjMGR->Get_ServerObjectIndex(CStore::GetInstance().GetNpcObjIndex()),
        1,
        0,
        &BuyItem,
        NULL);
...
bool
CTCmdSellItem::Exec(CTObject* pObj) {
    if (!g_itMGR.IsDlgOpened(DLG_TYPE_STORE))
        return true;
...
/*----------------------------------------------------------------------------------------------------*/
bool
CTCmdBuyItemAtAvatarStore::Exec(CTObject* pObj) {
    if (pObj == NULL) {
    ...
    if (pDlg) {
        ...
        CTCommand* pCmd =
            new CTCmdQueryBuyItemFromPrivateStore(pStoreDlg->GetMasterSvrObjIdx(), 1, Item);

        std::string strMsg = Item.m_SlotITEM.GetName();
        strMsg.append(" ");
        strMsg.append(
            CStr::Printf(STR_QUERY_BUYITEMFROMPRIVATESTORE, Item.m_SlotITEM.GetQuantity()));
...
bool
CTCmdSellItem2AvatarStore::Exec(CTObject* pObj) {
    if (pObj == NULL)
        return true;
    ...
    if (pDlg) {
        ...
        if (iWishIndex >= 0 && iWishIndex < MAX_WISH_ITEMS) {
            ...
            strMsg.append(" ");
            strMsg.append(
                CStr::Printf(STR_QUERY_SELLITEM2PRIVATESTORE, SellItem.m_SellITEM.GetQuantity()));
            char buf[128];
            ...
            CTCommand* pCmd =
                new CTCmdQuerySellItem2PrivateStore(pStoreDlg->GetMasterSvrObjIdx(), 1, SellItem);
            g_itMGR.OpenMsgBox(strMsg.c_str(), CMsgBox::BT_OK | CMsgBox::BT_CANCEL, true, 0, pCmd);
