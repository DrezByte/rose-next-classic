 ...
 *
 * @warning		메세지 박스가 닫힐경우 같은 타입의 모든 메세지 박스를 삭제한다.( 그렇지 않을경우
 *인덱싱에 문제 발생할수 있다.)
 * @Author		최종진
 * @Date			2005/9/5
 */
bool
CTCmdCreateMsgBox::Exec(CTObject* pObj) {
    list_dlgs_itor iter;
    ...
    POINT pt = {g_pCApp->GetWIDTH() / 2 - pMsgBox->GetWidth() / 2,
...
bool
CTCmdDeleteMsgBox::Exec(CTObject* pObj) {
    list_dlgs_itor iter;
...
/*-------------------------------------------------------------------------------------------------------------------------------*/
CTCmdOpenPrivateChatDlg::CTCmdOpenPrivateChatDlg(DWORD dwUserTag,
    BYTE btStatus,
    const char* pszName) {
...
CTCmdOpenPrivateChatDlg::Exec(CTObject* pObj) {
    if (g_itMGR.GetPrivateChatDlg(
            m_dwUserTag)) ///기존에 해당 유저와의 대화창이 열려있다면 열지 않는다.
        return true;
    ...
    if (pDlg)
        ...
        g_itMGR.AppendDlg(DLG_TYPE_PRIVATECHAT,
            pPrivateChatDlg,
            m_dwUserTag); ///처음 요청된 1:1대화창이라면 맨뒤로 가서 제일 먼저 프로세스 되도록 한다.
...
bool
CTCmdClosePrivateChatDlg::Exec(CTObject* pObj) {
    list_dlgs_itor iter;
    CTDialog* pDlg;
    for (iter = g_itMGR.m_Dlgs.begin(); iter != g_itMGR.m_Dlgs.end(); ++iter) {
        pDlg = *iter;
