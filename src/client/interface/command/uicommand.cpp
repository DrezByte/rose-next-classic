...
bool
CTCmdTakeOutItemFromMakeDlg::Exec(CTObject* pObj) {
    if (pObj == NULL) {
...
bool
CTCmdTakeOutItemFromSeparateDlg::Exec(CTObject* pObj) {
    if (pObj == NULL) {
        assert(pObj);
        return true;
    }
...
///*-----------------------------------------------------------------------------------------
// CTCmdAssembleRideItem::CTCmdAssembleRideItem( short nPartIdx, short nInvenIdx , short
// nEquipedBodyPartItemNo )
//{
//	m_nPartIdx	= nPartIdx;
...
//}
bool
CTCmdAssembleRideItem::Exec(CTObject* pObj) {
    ...
    } else ///바디 부품일경우
    {
        if (g_pAVATAR->GetPetMode() >= 0) ///드라이브 스킬 사용중이라면
        {
            ...
            for (int i = 1; i < MAX_RIDING_PART; ++i) {
                ...
                if (pPartItem) {
                    if (PAT_ITEM_PART_TYPE(pItem->GetItemNo())
                        != PAT_ITEM_PART_TYPE(pPartItem->GetItemNo())) {
                        g_itMGR.AppendChatMsg(STR_PAT_ERROR_NOT_EQUAL_CLASS,
                            IT_MGR::CHAT_TYPE_SYSTEM);
                        return true;
                    }

                    if (PAT_ITEM_PART_VER(pItem->GetItemNo())
                        < PAT_ITEM_PART_VER(pPartItem->GetItemNo())) {
                        g_itMGR.AppendChatMsg(STR_ERROR_EQUIP_PAT_BODY_VER,
                            IT_MGR::CHAT_TYPE_SYSTEM);
                        return true;
                    }
                }
        ...
        } else ///드라이브 스킬 사용중이 아니라면
        {
            ...
            for (int i = 1; i < MAX_RIDING_PART; ++i) {
                ...
                if (pPartItem) {
                    ...
                    else if (PAT_ITEM_PART_VER(pItem->GetItemNo())
                        < PAT_ITEM_PART_VER(pPartItem->GetItemNo()))
                        g_pNet->Send_cli_ASSEMBLE_RIDE_ITEM(i, 0);
                }
            }
...
bool
CTCmdDisAssembleRideItem::Exec(CTObject* pObj) {
    if (pObj == NULL) {
...
bool
CTCmdOpenNumberInputDlg::Exec(CTObject* pObj) {
    CTDialog* pDlg = NULL;
...
void
CTCmdOpenNumberInputDlg::SetCommand(CTCmdNumberInput* pCmd) {
    m_pCmd = pCmd;
}

void
CTCmdOpenNumberInputDlg::SetMaximum(__int64 i64Maximum) {
...
bool
CTCmdDragItem2AvatarStoreDlg::Exec(CTObject* pObj) {
...
bool
CTCmdAcceptAddFriend::Exec(CTObject* pObj) {
    g_pNet->Send_cli_MCMD_APPEND_REPLY(MSGR_CMD_APPEND_ACCEPT,
        m_wUserIdx,
...
bool
CTCmdRejectAddFriend::Exec(CTObject* pObj) {
    g_pNet->Send_cli_MCMD_APPEND_REPLY(MSGR_CMD_APPEND_REJECT,
        m_wUserIdx,
        (char*)m_strName.c_str());
    return true;
}
...
bool
CTCmdRemoveFriend::Exec(CTObject* pObj) {
...
bool
CTCmdAbandonQuest::Exec(CTObject* pObj) {
...
bool
CTCmdSendPacketDropItem::Exec(CTObject* pObj) {
...
bool
CTCmdMoveDialogIcon2Ground::Exec(CTObject* pObj) {
    assert(pObj);
    if (pObj == NULL)
        return true;
...
bool
CTCmdMoveDialogIcon2GroundFromMenu::Exec(CTObject* pObj) {
    assert(pObj);
    if (pObj == NULL)
        return true;
...
bool
CTCmdRemoveDialogIcon::Exec(CTObject* pObj) {
    g_itMGR.DelDialogIcon(m_iDialogType);
    return true;
}

bool
CTCmdRegistDialogIcon2QuickBar::Exec(CTObject* pObj) {
    if (pObj == NULL) {
...
bool
CTCmdAddItem2WishList::Exec(CTObject* pObj) {
...
bool
CTCmdChangeStateUpgradeDlg::Exec(CTObject* pObj) {
...
bool
CTCmdEndRepair::Exec(CTObject* pObj) {
    CGame::GetInstance().EndRepair();
    return true;
}

bool
CTCmdEndAppraisal::Exec(CTObject* pObj) {
...
bool
CTCmdSendAppraisalReq::Exec(CTObject* pObj) {
    g_pNet->Send_cli_APPRAISAL_REQ(m_iIndex);
    return true;
}

CTCmdQuerySellItem2PrivateStore::CTCmdQuerySellItem2PrivateStore(WORD wSellerSvrIdx,
    int iItemCount,
    tagSELL_ITEM& Item) {
...
bool
CTCmdQuerySellItem2PrivateStore::Exec(CTObject* pObj) {
    g_pNet->Send_cli_P_STORE_SELL_REQ(m_wSellerSvrIdx, m_iItemCount, &m_SellItem);
    return true;
}

CTCmdQueryBuyItemFromPrivateStore::CTCmdQueryBuyItemFromPrivateStore(WORD wSellerSvrIdx,
    int iItemCount,
...
bool
CTCmdQueryBuyItemFromPrivateStore::Exec(CTObject* pObj) {
...
bool
CTCmdAcceptReqJoinClan::Exec(CTObject* pObj) {
...
bool
CTCmdRejectReqJoinClan::Exec(CTObject* pObj) {
    assert(m_strMasterName.c_str());
    if (m_strMasterName.c_str())
...
bool
CTCmdClanCommand::Exec(CTObject* pObj) {
    if (m_strMsg.empty())
...
bool
CTCmdCancelWaitDisconnect::Exec(CTObject* pObj) {
...
/*--------------------------------------------------------------------------------------------------------------*/
void
CTCmdInputName::SetName(const char* pszName) {
...
bool
CTCmdInputName::Exec(CTObject* pObj) {
    return true;
}
...
bool
CTCmdRegisterClanMark::Exec(CTObject* pObj) {
    if (m_filename.empty())
        return true;
...
bool
CTCmdChangeStateInterface::Exec(CTObject* pObj) {
