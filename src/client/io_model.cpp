...
bool
CPointPART::Load(CFileSystem* pFileSystem) {
    BYTE btTAG, btLen;
...
//-------------------------------------------------------------------------------------------------
void
CBasicPART::UnloadVisible(HNODE hVIS) {
...
bool
CBasicPART::Load(CFileSystem* pFileSystem, short nLinkBoneNo) {
    BYTE btTAG, btLen;
...
//-------------------------------------------------------------------------------------------------
bool
CCharPART::Load(CFileSystem* pFileSystem, short nLinkBoneNo, short nLinkDummyNo) {
    BYTE btTAG, btLen;
...
//-------------------------------------------------------------------------------------------------
bool
CCharPART::Is_LinkedMODEL(void) {
    if (m_pMeshAniFILE || m_nLinkBoneIDX >= 0 || m_nLinkDummyIDX >= 0)
        return true;
...
//-------------------------------------------------------------------------------------------------
HNODE
CCharPART::Load_ZMODEL(char* szName, HNODE hModelNODE) {
    ...
    if (!hMesh || !hMat) {
        char* szStr = CStr::Printf("Mesh %s and Texture %s not found \n",
            g_MeshFILE.Get_FileNAME(m_uiMeshKEY),
            g_MatFILE.Get_FileNAME(m_uiMatKEY));
...
//-------------------------------------------------------------------------------------------------
void
CCharPART::Free_ZMODEL(HNODE hNODE) {
    if (m_pMeshAniFILE) {
        for (short nI = 0; nI < MAX_AVT_ANI; nI++)
    ...
    while (btTAG) {
        ...
        switch (btTAG) {
            ...
            case SWITCH_ROTAXIS:
                ...
                pFileSystem->ReadFloat4(&m_RotAXIS.w,
                    &m_RotAXIS.x,
                    &m_RotAXIS.y,
                    &m_RotAXIS.z); // degree
                break;
...
CFixedPART::LoadVisible(char* szName, D3DVECTOR BasePOS, HNODE hParent) {
    ...
    if (!hMesh) {
        LogString(LOG_DEBUG_,
            "ERROR:: LoadVisible[ %s ] hMesh: %d, hMat: %d \n",
            szName,
            hMesh,
            hMat);
        _ASSERT(0);
    ...
    if (hVis) {
        ...
        if (hParent) {
            ...
            ::setPosition(hVis,
                BasePOS.x + m_Transform.x,
                BasePOS.y + m_Transform.y,
                BasePOS.z + m_Transform.z);
...
void
CFixedPART::UnloadVisible(HNODE hNODE) {
