...
/// 현재 위치를 구하고.. 각 맴버세팅..
void
CBulletDIRECTION::Cal_MoveVEC(CObjCHAR* pDest) {
    // 바라볼 모델의 위치 얻기 -> pos
...
//-------------------------------------------------------------------------------------------------
bool
CBulletDIRECTION::Init(CObjCHAR* pSour,
    CObjCHAR* pDest,
    int iEffectIDX,
    bool bLinkToPoint,
    int iPointNO,
...
CBulletDIRECTION::Proc() {
    ...
    if (m_nDestOBJ) {
        ...
        if (this->IsIntersect(pDestOBJ->GetZMODEL())) {
            ...
            /// 맞으면 총알을 없앤다.. 이게.. 부활시 다운되는거랑 관계있다.. 왜냐.. 이놈이 꽂혀
            /// 있는데 아바타가 워프하면 대략 낭패..
            return 0;
        }
...
/// 현재 위치를 구하고.. 각 맴버세팅..
void
CBulletToPosition::Cal_MoveVEC(D3DXVECTOR3 TargetPos) {
...
//-------------------------------------------------------------------------------------------------
bool
CBulletToPosition::Init(CObjCHAR* pSour,
...
CBulletToPosition::Proc() {
    ...
    int iDistance1 =
        CD3DUtil::distance((int)m_PosCUR.x, (int)m_PosCUR.y, (int)m_PosFROM.x, (int)m_PosFROM.y);
...
//----------------------------------------------------------------------------------------------------
void
CBulletPARABOLA::Cal_MoveVEC(CObjCHAR* pDest) {
    //----------------------------------------------------------------------------------------------------
    /// 바라볼 모델의 위치 얻기 -> pos
    ...
    m_fDistance = (float)sqrt(
        (m_DisplacementVec.x * m_DisplacementVec.x) + (m_DisplacementVec.y * m_DisplacementVec.y));
    ...
    if (m_fDistance < 0.0001f) {
        ...
        this->m_MoveVEC =
            (m_DisplacementVec * m_fSpeed) / (m_fDistance * 1000.0f); // 1/1000초당 이동량.
...
//----------------------------------------------------------------------------------------------------
bool
CBulletPARABOLA::Init(CObjCHAR* pSour,
    CObjCHAR* pDest,
    int iEffectIDX,
    bool bLinkToPoint,
    int iPointNO,
...
CBulletPARABOLA::Proc() {
    ...
    float fAdjustHeight =
        m_DisplacementVec.z * (((float)m_dwElapsedTime / 1000.0f) / m_fArrivedTime);
    ...
    /// LogString( LOG_NORMAL, "PARA POS[ %f, %f, %f ], m_MoveVEC[ %f, %f, %f ][ Z1:%f ][
    /// PassTime:%d]\n", m_PosCUR.x, m_PosCUR.y, m_PosCUR.z, m_MoveVEC.x, m_MoveVEC.y, m_MoveVEC.z,
    /// Z1, dwPassTime );
...
//-------------------------------------------------------------------------------------------------
CBullet*
CBulletMANAGER::Add_BULLET(CObjCHAR* pSour,
    CObjCHAR* pDest,
    int iEffectIDX,
    bool bLinkToPoint,
    int iPointNO,
    ...
    switch (EFFECT_BULLET_MOVETYPE(iEffectIDX)) {
        ...
        case BULLET_MOVE_PARABOLA: {
            ...
            /// if bSkillEffect is true, link to right hand dummy
            if (!pBullet
                     ->Init(pSour, pDest, iEffectIDX, bLinkToPoint, iPointNO, bDummy, iSkillIDX)) {
                SAFE_DELETE(pBullet);
                return NULL;
            }
            break;
