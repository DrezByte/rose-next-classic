...
bool
CMatLIST::Load(char* szSTBFile, char* szNameTemplate) {
    CGameSTB fSTB;
        ...
        for (short nI = 0; nI < m_nDataCNT; nI++) {
            ...
            if (szFileName) {
                ...
                switch (fSTB.GetInteger(1, nI)) { // IsLinkBone
                    case 1:
                        pMat->m_hShader = g_GameDATA.m_hShader_nolit_skin;
                        break;
                    case 2:
                        pMat->m_hShader = g_GameDATA.m_hShader_terrain;
                        break;
                    case 0:
                        pMat->m_hShader = g_GameDATA.m_hShader_nolit;
                        break;
                }
...
CMatLIST::Load_FILE(tagFileDATA<tagMATERIAL*>* pData) {
    ...
    if (pData->m_DATA->m_hMAT) {
        ...
        ::setMaterialGlow(pData->m_DATA->m_hMAT,
            pData->m_DATA->m_nGlowType,
            pData->m_DATA->m_fGlowColor[0],
            pData->m_DATA->m_fGlowColor[1],
            pData->m_DATA->m_fGlowColor[2]);
...
void
CMatLIST::Free_FILE(tagFileDATA<tagMATERIAL*>* pData) {
...
void
CMatLIST::Mem_FREE(tagFileDATA<tagMATERIAL*>* pData) {
...
t_HASHKEY
CMatLIST::Add_MATERIAL(char* szFileName,
    short nIsSkin,
    short nIsAlpha,
    short nIs2Side,
    bool bTerrainMAT,
    char* szMatNameUseLightMap,
    short nAlphaTest,
    short nAlphaRef,
    short nZTest,
    short nZWrite,
    ...
    if (szMatNameUseLightMap) {
        ...
        if (pfGlowColor && nGlowType) {
            szKeyName = CStr::Printf("%s_%d%d%d%d%4.3f%d<%4.3f,%4.3f,%4.3f>",
                szFileName,
                nIsSkin,
                nIsAlpha,
                nIs2Side,
            ...
            szKeyName = CStr::Printf("%s_%d%d%d%d%4.3f",
                szFileName,
                nIsSkin,
                nIsAlpha,
                nIs2Side,
        ...
        if (bTerrainMAT) {
            ...
            if (nSpecular) {
                hShader =
                    nIsSkin ? g_GameDATA.m_hShader_specular_skin : g_GameDATA.m_hShader_specular;
            } else {
...
//-------------------------------------------------------------------------------------------------
HNODE
CMatLIST::IDX_HNODE(short nIndex) {
    tagMATERIAL* pMat;
...
HNODE
CMatLIST::KEY_HNODE(t_HASHKEY HashKEY) {
    tagMATERIAL* pMat;
