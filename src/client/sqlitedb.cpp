...
int CSqliteDB::AddMemo( const char* pszName, const char* pszSender, const char* pszContent, DWORD
time )
{
    assert( m_db );
    if( m_db == NULL )
    ...
    char* zErrMsg;
    int rc = sqlite_exec_printf( m_db, sqlFormat, NULL, 0, &zErrMsg, pszName, pszSender, pszContent,
time ); if( rc != SQLITE_OK )
    {
        sqlite_freemem( zErrMsg );
        return 1;
    }
