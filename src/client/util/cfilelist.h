...
public:
    ...
    t_HASHKEY Add_FILE(char* szFileName,
        short nIndex,
        dType DATA,
        bool bCheckFileExist = false,
        bool bImmediateLoad = false);
    virtual t_HASHKEY Add_FILE(char* szFileName, short nIndex = -1, bool bImmediateLoad = false) {
        return this->Add_FILE(szFileName, nIndex, NULL, false, bImmediateLoad);
    }
    ...
    tagFileDATA<dType>* IDX_Find_DATA(short nIDX) {
        return ((nIDX >= 0 && nIDX < m_nDataCNT) ? &m_pDATAs[nIDX] : NULL);
    }
...
template<class dType>
dType
CFileLIST<dType>::Get_DATAUseKEY(unsigned int uiKEY) {
...
template<class dType>
dType
CFileLIST<dType>::Get_DATAUseIDX(short nIndex) {
...
template<class dType>
CFileLIST<dType>::CFileLIST(char* szNameTAG, short nMaxLoadCnt, short nHashEnetryCount):
    m_HASH(nHashEnetryCount) {
    m_pDATAs = NULL;
    m_nDataCNT = 0;
    m_nLoadCNT = 0;
    m_nTotalFileCNT = 0;
...
template<class dType>
bool
CFileLIST<dType>::Alloc_FILES(short nFilesCNT) {
...
template<class dType>
t_HASHKEY
CFileLIST<dType>::Add_FILE(char* szFileName,
    short nIndex,
    dType DATA,
    ...
    if (bCheckFileExist && ((CVFSManager::GetSingleton()).IsExistFile(szFileName) == false)) {
        char* szMSG =
            CStr::Printf("ERROR File [%s] not found in %s ... \n", szFileName, m_NameTAG.Get());
    ...
    /// 파일 리스트 내부에서 관리되는 데이터 배열의 인덱스..( 리스트를 인덱스로 유지 관리하는 데이터
    /// 들의 경우??)
...
template<class dType>
bool
CFileLIST<dType>::Del_FILE(unsigned int uiHashKey) {
    tagHASH<tagFileDATA<dType>*>* pHashNode;
...
template<class dType>
bool
CFileLIST<dType>::Del_FILE(char* szFileName) {
...
template<class dType>
void
CFileLIST<dType>::Delete_LIST(void) {
...
template<class dType>
void
CFileLIST<dType>::Delete_DATA(tagFileDATA<dType>* pDATA) {
    if (pDATA->m_bLoad) {
...
template<class dType>
dType
CFileLIST<dType>::Get_DATA(tagFileDATA<dType>* pDATA) {
    if (pDATA == NULL) {
...
template<class dType>
bool
CFileLIST<dType>::Sub_DATA(tagFileDATA<dType>* pDATA, bool bImmediateFree) {
    if (pDATA == NULL) {
        // not exist file in m_pDATAs[] ..
        return false;
    }
...
template<class dType>
bool
CFileLIST<dType>::Load(char* szSTBFile, short nFileNameColNO) {
    CGameSTB fSTB;
