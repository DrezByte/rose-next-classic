...
LANGID
CLocalizing::GetCurrentLanguageID() {
...
int
CLocalizing::GetCharsetFromLang(LANGID langid) {
...
int
CLocalizing::GetCodePageFromCharset(int charset) {
    switch (charset) {
...
int
CLocalizing::GetCodePageFromLang(LANGID langid) {
...
int
CLocalizing::GetCurrentCodePage() {
...
CLocalizing::UTF8ToMBCS(LPSTR inBuf, int iInBufSize) {
    ...
    /// Unicode -> MBCS
    return WideCharToMultiByte(CP_ACP, 0, pOutBuf, -1, inBuf, iInBufSize, 0, 0);
}

int
CLocalizing::MBCSToUTF8(LPSTR inBuf, int iInBufSize) {
    WCHAR outBuf[1024];
    ...
    /// Unicode -> UTF8
    return WideCharToMultiByte(CP_UTF8, 0, pOutBuf, -1, inBuf, iInBufSize, 0, 0);
}

int
CLocalizing::MBCSToUnicode(LPSTR inBuf, int iInBufSize, WCHAR* pOutBuf, int iOutBufSize) {
    /// UTF8 -> Unicode
...
CLocalizing::UnicodeToMBCS(WCHAR* pInBuf, int iInBufSize, LPSTR OutBuf, int iOutBufSize) {
    /// Unicode -> MBCS
    return WideCharToMultiByte(CP_ACP, 0, pInBuf, -1, OutBuf, iOutBufSize, 0, 0);
}
