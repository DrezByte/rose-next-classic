...
//-------------------------------------------------------------------------------------------------
bool
classLUA::GetGlobalValue(const char* VarName, double& dbNumber) {
...
//-------------------------------------------------------------------------------------------------
bool
classLUA::GetGlobalValue(const char* VarName, int& iNumber) {
    this->GetGlobal(VarName); // 	lua_getglobal (m_pState, VarName)
    if (Is_Number(-1)) { //	lua_isstring  (m_pState, -1)
        iNumber = To_Number(-1);
        return true;
...
//-------------------------------------------------------------------------------------------------
bool
classLUA::GetGlobalValue(const char* VarName, char* pString) {
    this->GetGlobal(VarName); // 	lua_getglobal (m_pState, VarName)
...
//-------------------------------------------------------------------------------------------------
void
classLUA::SetGolbalValue(const char* VarName, double dbValue) {
    this->Stack_PushNumber(dbValue);
    this->SetGlobal(VarName);
}
//-------------------------------------------------------------------------------------------------
void
classLUA::SetGolbalValue(const char* VarName, int iValue) {
    this->Stack_PushNumber(iValue);
    this->SetGlobal(VarName);
}
//-------------------------------------------------------------------------------------------------
void
classLUA::SetGolbalValue(const char* VarName, char* szValue) {
...
//-------------------------------------------------------------------------------------------------
int
classLUA::LoadToBuffer(const char* szFileName) {
    // Check File exist
