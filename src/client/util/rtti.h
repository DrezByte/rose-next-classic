...
inline rtti::rtti(const char* pTypeName): pType(pTypeName), Next(*this) {}

inline rtti::rtti(const char* pTypeName, const rtti& RTTI): pType(pTypeName), Next(RTTI) {}

inline bool
rtti::IsKindOf(const rtti& RTTI) const {
    const rtti* p = this;
    do {
        if (p == &RTTI)
            return true;
...
#define PRIVATE_RTTI_FUNCTION_SET(TYPE, BASE_CLASS)                                               \
    virtual inline const rtti& GetObjectRTTI(void) const { return TYPE::GetRTTI(); }              \
    inline const bool IsKindOf(const rtti& RTTI) const { return GetObjectRTTI().IsKindOf(RTTI); } \
