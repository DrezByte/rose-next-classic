...
//-------------------------------------------------------------------------------------------------
HRESULT
classMMPLAYER::Init(HWND argHwnd, UINT uiEventMsg) {
    HRESULT hr;
    // Initialize COM
    ...
    if (FAILED(m_pGraphBuilder->QueryInterface(IID_IMediaEvent,
            reinterpret_cast<void**>(&m_pMediaEvent))))
        return hr;
...
classMMPLAYER::HandleEvent() {
    ...
    while (SUCCEEDED(
        m_pMediaEvent->GetEvent(&evCode, (LONG_PTR*)&evParam1, (LONG_PTR*)&evParam2, 0))) {
...
//-------------------------------------------------------------------------------------------------
HRESULT
classMMPLAYER::Play(LPSTR argName) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr = m_pGraphBuilder->EnumFilters(&pFilterEnum))) {
            ...
            IBaseFilter** ppFilters =
                reinterpret_cast<IBaseFilter**>(_alloca(sizeof(IBaseFilter*) * iFiltCount));
            pFilterEnum->Reset();
    ...
    if (SUCCEEDED(hr)) {
        LONGLONG IIPos = 0;
        hr = m_pMediaSeeking->SetPositions(&IIPos,
            AM_SEEKING_AbsolutePositioning,
            &IIPos,
            AM_SEEKING_NoPositioning);
    }
