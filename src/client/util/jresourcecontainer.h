...
public:
    CResObject(const char* strName) { SetName(strName); }
...
template<class T>
void
CResourceContainer<T>::InitObjectPool(int iSize) {
...
template<class T>
T*
CResourceContainer<T>::GetObjectInstanceFromPool() {
    T* pT = NULL;
...
template<typename T>
void
CResourceContainer<T>::ReturnObjectToPool(T* o) {
...
template<class T>
void
CResourceContainer<T>::ClearPool() {
    std::for_each(m_pObjectPool.begin(),
        m_pObjectPool.end(),
        CResourceContainer<T>::DeletePoolObject);
...
template<class T>
void
CResourceContainer<T>::DeletePoolObject(T* pT) {
    if (pT) {
        delete pT;
        pT = NULL;
    }
...
template<typename T>
bool
CResourceContainer<T>::AddObject(const char* strName, T* obj) {
...
template<class T>
T*
CResourceContainer<T>::GetObjectByName(const char* strName) {
    CResObject keyObj(strName);
    auto itor = m_ObjectMap.find(keyObj);
    if (itor != m_ObjectMap.end()) {
        return itor->second;
