...
protected:
    virtual t_sounddata*
    CreateSoundData(CWaveFile* pCWave, DWORD dwCreationFlags, GUID guid3DAlgorithm);
    virtual t_sounddata*
    CreateSoundData(t_wavedata* pWave, DWORD dwCreationFlags, GUID guid3DAlgorithm);
    t_sounddata* DuplicateSoundData(LPDIRECTSOUNDBUFFER lpDSBSour);
    bool
    SetPrimaryBufferFormat(DWORD dwPrimaryChannels, DWORD dwPrimaryFreq, DWORD dwPrimaryBitRate);
...
public:
    ...
    t_sounddata* LoadSoundData(LPSTR szFileName,
        short nMaxMixCnt,
        DWORD dwCreationFlags = DSBCAPS_STATIC | DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLPAN,
