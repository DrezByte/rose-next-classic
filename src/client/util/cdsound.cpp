...
//
static t_wavedata*
WaveData_Load(LPSTR szFileName, long lFilePtr) {
    FILE* fpWAV;
    ULONG lFPos;
    DWORD dwLength;
    t_wavedata* pWave;
...
//-------------------------------------------------------------------------------------------------
static void
WaveData_Free(t_wavedata* pWave) {
    if (pWave->m_pWaveData != NULL)
...
//-------------------------------------------------------------------------------------------------
bool
CDSOUND::SetPrimaryBufferFormat(DWORD dwPrimaryChannels,
    DWORD dwPrimaryFreq,
    DWORD dwPrimaryBitRate) {
...
//-------------------------------------------------------------------------------------------------
t_sounddata*
CDSOUND::CreateSoundData(t_wavedata* pWave, DWORD dwCreationFlags, GUID guid3DAlgorithm) {
    if (m_pDS == NULL)
        return NULL;
...
//-------------------------------------------------------------------------------------------------
t_sounddata*
CDSOUND::CreateSoundData(CWaveFile* pCWave, DWORD dwCreationFlags, GUID guid3DAlgorithm) {
    if (m_pDS == NULL)
        return NULL;
    ...
    if (m_hR == DS_OK) {
        ...
        if (dwWavDataRead == 0) {
            ...
            ::FillMemory((BYTE*)pMem1,
                dwSize1,
                (BYTE)(pCWave->m_pwfx->wBitsPerSample == 8 ? 128 : 0));
        } else if (dwSize2 != 0) {
            // ::CopyMemory (pMem2, (pWave->m_pWaveData+dwSize1), dwSize2);
...
//-------------------------------------------------------------------------------------------------
t_sounddata*
CDSOUND::DuplicateSoundData(LPDIRECTSOUNDBUFFER lpDSBSour) {
...
t_sounddata*
CDSOUND::LoadSoundData(LPSTR szFileName,
    short nMaxMixCnt,
...
//-------------------------------------------------------------------------------------------------
void
CDSOUND::FreeSoundData(t_sounddata* pData) {
    t_sounddata* pNext;
...
//-------------------------------------------------------------------------------------------------
DWORD
CDSOUND::GetStatus(t_sounddata* pData) {
    DWORD dwStatus;
...
//-------------------------------------------------------------------------------------------------
void
CDSOUND::SetMaxMixCount(t_sounddata* pData, short nMaxMinCnt) {
    if (pData) {
...
//-------------------------------------------------------------------------------------------------
short
CDSOUND::GetMaxMixCount(t_sounddata* pData) {
    if (pData) {
...
//-------------------------------------------------------------------------------------------------
void
CDSOUND::PlaySound(t_sounddata* pData, long lVolume, long lPan, DWORD dwFlags) {
    ...
    if (pData == NULL || pData->m_pDSB == NULL)
        return;
...
//-------------------------------------------------------------------------------------------------
void
CDSOUND::StopSound(t_sounddata* pData) {
    t_sounddata* pNext;
...
//-------------------------------------------------------------------------------------------------
HRESULT
CD3DSOUND::Get3DListenerInterface(LPDIRECTSOUND3DLISTENER* ppDSListener) {
    ...
    *ppDSListener = NULL;
    ...
    if (FAILED(
            m_hR = pDSBPrimary->QueryInterface(IID_IDirectSound3DListener, (VOID**)ppDSListener))) {
        SAFE_RELEASE(pDSBPrimary);
...
CD3DSOUND::_Init(HWND hWnd,
    ...
    // if (FAILED(m_pDSListener->SetRolloffFactor(fGlobal3DVolume * (DS3D_MAXROLLOFFFACTOR -
    // DS3D_MINROLLOFFFACTOR), DS3D_DEFERRED))) { 	return false;
    //}
    //
    return true;
}
...
//-------------------------------------------------------------------------------------------------
t_sounddata*
CD3DSOUND::CreateSoundData(t_wavedata* pWave, DWORD dwCreationFlags, GUID guid3DAlgorithm) {
    t_sounddata* pSound;

    pSound = CDSOUND::CreateSoundData(pWave, dwCreationFlags, guid3DAlgorithm);
    if (pSound) {
        m_hR = pSound->m_pDSB->QueryInterface(IID_IDirectSound3DBuffer,
            (VOID**)&pSound->m_pDS3DBuffer);
        if (FAILED(m_hR)) {
...
t_sounddata*
CD3DSOUND::LoadSoundData(LPSTR szFileName, short nMaxMixCnt, GUID guid3DAlgorithm, long lFilePtr) {
    DWORD dwCreationFlags = DSBCAPS_CTRL3D | DSBCAPS_CTRLVOLUME;

    return CDSOUND::LoadSoundData(szFileName,
        nMaxMixCnt,
        dwCreationFlags,
        guid3DAlgorithm,
        lFilePtr);
...
//-------------------------------------------------------------------------------------------------
void
CD3DSOUND::Set3DBufferParam(t_sounddata* pSound, DS3DBUFFER& dsBufferParam) {
    pSound->m_pDS3DBuffer->SetAllParameters(&dsBufferParam, DS3D_DEFERRED);
}

//-------------------------------------------------------------------------------------------------
void
CD3DSOUND::SetListenerParam(void) {
    /*
    ...
    */
    ...
    // DS3D_DEFERRED	: 애플리케이션이 IDirectSound3DListener8::CommitDeferredSettings 메서드를
    // 호출할 때까지, 설정은 적용되지 않는다. 복수의 설정을 변경한 후, 1 회의 재계산으로 그것들을
    // 적용할 수 있다.
...
//-------------------------------------------------------------------------------------------------
void
CD3DSOUND::ConvertToSoundSpace(D3DXVECTOR3& posSoundSpace, const D3DXVECTOR3& posWorldSpace) {
...
//-------------------------------------------------------------------------------------------------
bool
CD3DSOUND::SetListenerPosition(const D3DXVECTOR3& PosWorld) {
    if (!m_pDSListener)
        return false;
...
//-------------------------------------------------------------------------------------------------
bool
CD3DSOUND::SetListenerOrientation(const D3DXVECTOR3& PosFront, const D3DXVECTOR3& PosTop) {
    if (!m_pDSListener)
        return false;

    if (FAILED(m_pDSListener->SetOrientation(PosFront.x,
            PosFront.z,
            PosFront.y,
            PosTop.x,
            PosTop.z,
...
bool
CD3DSOUND::SetListenerRolloffFactor(float fRolloffFactor) {
    if (!m_pDSListener)
        return false;
...
//-------------------------------------------------------------------------------------------------
bool
CD3DSOUND::UpdateListener(const CCamera* pCamera) {
    if (!m_pDSListener)
        return false;
    ...
    if (0 == getCameraEye(hCamera, posEye))
        return false;
    if (0 == getCameraDir(hCamera, dirFront))
        return false;
...
bool
CD3DSOUND::Set3D(t_sounddata* pData, bool b3DMode) {
    if (!pData || !pData->m_pDS3DBuffer)
        return false;
...
bool
CD3DSOUND::SetPosition(t_sounddata* pData, const D3DXVECTOR3& posWorld) {
    if (!pData || !pData->m_pDS3DBuffer)
        return false;
    ...
    return !FAILED(
        pData->m_pDS3DBuffer->SetPosition(posSound.x, posSound.y, posSound.z, DS3D_DEFERRED));
}

bool
CD3DSOUND::SetVelocity(t_sounddata* pData, const D3DXVECTOR3& posVelocity) {
    if (!pData || !pData->m_pDS3DBuffer)
        return false;
    ...
    return !FAILED(
        pData->m_pDS3DBuffer->SetVelocity(posSound.x, posSound.y, posSound.z, DS3D_DEFERRED));
}

bool
CD3DSOUND::SetMinDistance(t_sounddata* pData, float fMinDistance) {
    if (!pData || !pData->m_pDS3DBuffer)
        return false;
...
bool
CD3DSOUND::SetMaxDistance(t_sounddata* pData, float fMaxDistance) {
    if (!pData || !pData->m_pDS3DBuffer)
        return false;
