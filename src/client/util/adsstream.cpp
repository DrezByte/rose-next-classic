...
ADirectSoundSTREAM::ADirectSoundSTREAM() {
    ...
    m_hNotifyThread = (HANDLE)
        _beginthreadex(NULL, 0, ThreadDSStream, this, CREATE_SUSPENDED, &m_dwNotifyThreadID);
}

void
ADirectSoundSTREAM::CloseStreamThread() {
    if (m_hNotifyThread) {
...
//-------------------------------------------------------------------------------------------------
void
ADirectSoundSTREAM::_Free(void) {
    SAFE_RELEASE(m_pDSBuffer);
}

//-------------------------------------------------------------------------------------------------
void
ADirectSoundSTREAM::SetVolume(long lVolume) {
    HRESULT hR;
...
//-------------------------------------------------------------------------------------------------
HRESULT
ADirectSoundSTREAM::CreateStreamBuffer(LPDIRECTSOUND pDirectSound,
    WAVEFORMATEX* pWFX,
    int iBufferingSec,
    short nNumPlayNotifications) {
    if (!pDirectSound)
        return E_FAIL;
...
//-------------------------------------------------------------------------------------------------
HRESULT
ADirectSoundSTREAM::PlayStreamBuffer(bool bLooped, long lVolume) {
    HRESULT hResult;
    VOID* pbBuffer = NULL;
...
//-------------------------------------------------------------------------------------------------
void
ADirectSoundSTREAM::StopStreamBuffer(void) {
    if (NULL == m_pDSBuffer)
        return;
    ...
    if (FAILED(hr = m_pDSBuffer
                        ->Lock(0, m_dwTotalBuffSize, &pbBuffer, &dwBufferLength, NULL, NULL, 0L)))
        return;
...
//-------------------------------------------------------------------------------------------------
HRESULT
ADirectSoundSTREAM::RestoreBuffers(void) {
    HRESULT hResult;
...
//-------------------------------------------------------------------------------------------------
HRESULT
ADirectSoundSTREAM::FillBuffer(void) {
    HRESULT hr;
    void* pbBuffer = NULL;
    DWORD dwBufferLength;
...
//-------------------------------------------------------------------------------------------------
HRESULT
ADirectSoundSTREAM::WriteToBuffer(void* pbBuffer, DWORD dwBufferLength) {
    HRESULT hr;
...
//-------------------------------------------------------------------------------------------------
bool
ADirectSoundSTREAM::IsCurrentBlockPassed() {
...
WORD
ADirectSoundSTREAM::GetBlockIndex(DWORD dwBuffPos) {
    for (WORD wI = 0; wI < m_wDataBlockCount; wI++)
        if (dwBuffPos >= m_pDataBlock[wI].m_dwFROM && dwBuffPos <= m_pDataBlock[wI].m_dwTO)
            return wI;
...
//-------------------------------------------------------------------------------------------------
HRESULT
ADirectSoundSTREAM::UpdateProgress(void) {
    HRESULT hr;
    ...
    ODS("		CurBLOCK: %d / PlayingBLOCK: %d / %d\n",
        m_wCurrentBlockIDX,
        this->GetBlockIndex(dwPlayPos),
        this->GetBlockIndex(dwWritePos));
...
//-------------------------------------------------------------------------------------------------
HRESULT
ADirectSoundSTREAM::HandleNotification(void) {
    HRESULT hr;
    VOID* pbBuffer = NULL;
    DWORD dwBufferLength;
...
//-------------------------------------------------------------------------------------------------
bool
AOggSTREAM::OpenFile(char* szOGGFile, int iBufferingSec, int iBufferBlock) {
    this->CloseFile();
...
//-------------------------------------------------------------------------------------------------
bool
AOggSTREAM ::ReadData(UINT uiSizeToRead, void* pBuffer, UINT* puiActualBytesWritten) {
    /*
    long ov_read(OggVorbis_File *vf, char *buffer, int length, int bigendianp, int word, int sgned,
    int *bitstream); Parameters ::
        vf			A pointer to the OggVorbis_File structure--this is used for ALL the externally
    visible libvorbisfile functions. buffer		A pointer to an output buffer. The decoded output is
    inserted into this buffer.
        length		Number of bytes to be read into the buffer. Should be the same size as the
    buffer. A typical value is 4096. bigendianp	Specifies big or little endian byte packing. 0 for
    little endian, 1 for b ig endian. Typical value is 0. word		Specifies word size. Possible
    arguments are 1 for 8-bit samples, or 2 or 16-bit samples. Typical value is 2.
    ...
    Return Values ::
        OV_HOLE		indicates there was an interruption in the data.
                    (one of: garbage between pages, loss of sync followed by recapture, or a corrupt
    page) OV_EBADLINK indicates that an invalid stream section was supplied to libvorbisfile, or the
    requested link is corrupt. 0			indicates EOF n			indicates actual number of bytes
    read. ov_read() will decode at most one vorbis packet per invocation, so the value returned will
    generally be less than length.
    */
    long lReadBytes;
    int iBitStream;
    ...
    *puiActualBytesWritten = 0;
    ...
    do {
        lReadBytes = ::ov_read(m_pOVF,
            (char*)pBuffer + *puiActualBytesWritten,
            uiSizeToRead,
            0,
            2,
            1,
...
//-------------------------------------------------------------------------------------------------
bool
AOggSTREAM ::OpenSoundStreamFile(char* szOggFile,
