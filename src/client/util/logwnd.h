...
inline void
_LogOut(int nType, LPCTSTR pFrom, LPCTSTR pWhere, LPCTSTR pMessage, LPCTSTR pDump) {
    COPYDATASTRUCT cd;
...
inline void
LogOut(int iType, void* pFilename, unsigned int lineno, LPCTSTR pszMessage, LPCTSTR pDump = NULL) {
    TCHAR where[1024];
...
inline DWORD
LogOutLastError(void* pFilename, unsigned int lineno, LPCTSTR pszMessage) {
    if (::GetLastError() == 0)
        return 0;
...
public:
    ...
    CLogSystem(int nType, void* pFrom, int nLineno, int nLastError):
        m_pFrom(pFrom), m_nLineno(nLineno), m_nType(nType), m_pDump(NULL),
...
protected:
    ...
    LPCTSTR m_pDump;
        ...
        // LOGOUT_LASTERROR
        #define LOGOUT_LASTERROR \
            CLogSystem(LIT_ERROR, __FILE__, __LINE__, GetLastError()).TraceLastError()
