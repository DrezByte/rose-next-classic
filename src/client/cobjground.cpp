...
/// static member
bool
CObjITEM::SetMotion(const char* strMotion) {
    int ZZ_INTERP_SQUAD = 3, ZZ_INTERP_CATMULLROM = 4;
    m_hMotion =
        ::loadMotion("ItemMotion", strMotion, 1, ZZ_INTERP_CATMULLROM, ZZ_INTERP_SQUAD, 1, 1);
...
CObjITEM::CreateAnimatable() {
    ...
    if (m_hAnimatable == NULL)
        m_hAnimatable = ::loadAnimatable(CStr::Printf("ItemAnimatable_%d", this->Get_INDEX()),
            NULL,
            CObjITEM::m_hMotion,
            NULL,
