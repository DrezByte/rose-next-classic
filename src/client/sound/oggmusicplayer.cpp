...
bool
COggMusicPlayer::Init() {
    return true;
}

bool
COggMusicPlayer::Play(const char* fName) {
    Stop();
    ...
    if (m_pOggStream->OpenSoundStreamFile((char*)fName, g_pSoundLIST->m_SOUND.m_pDS, 24, 4)
        == false) {
        assert(0 && "m_pOggStream->OpenSoundStreamFile failed");
        Stop();
        return false;
    }
    if (FAILED(m_pOggStream->PlayStreamBuffer(true,
...
void
COggMusicPlayer::SetVolume(long lVolume) {
    if (m_pOggStream) {
