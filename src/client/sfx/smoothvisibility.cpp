...
void
CSmoothVisibility::Start(DWORD dwTransitionTime, bool bToDisappear) {
...
void
CSmoothVisibilityCombo::ResetTime() {
    m_fCurrentVisibility = 1.0f;
...
///
void
CSmoothVisibilityCombo::StartDisappearCombo(DWORD dwToDisappear,
    DWORD dwDuration,
    DWORD dwToAppear) {
    ResetTime();
...
///
void
CSmoothVisibilityCombo::StartAppearCombo(DWORD dwToAppear, DWORD dwDuration, DWORD dwToDisappear) {
    ResetTime();
...
/// flash effect
void
CSmoothVisibilityCombo::StartFlash(DWORD dwDurationTime, DWORD dwFlashTime) {
    ResetTime();
...
/// update visibility
void
CSmoothVisibilityCombo::UpdateVisibility() {
...
///
void
CSmoothVisibilityCombo::UpdateDisappearCombo() {
    DWORD dwCurrentTime = g_GameDATA.GetGameTime();
    ...
    /// Expire
    if (dwElapsedTime
        > (m_dwToFirstTransitionTime + m_dwDurationTime + m_dwToSecondTransitionTime)) {
        m_dwToSecondTransitionTime = 0;
        m_bTransitionActivated = false;
    ...
    if (m_dwToFirstTransitionTime > 0) {
        ...
        DWORD dwTransitionTime = m_dwToSecondTransitionTime
            - (dwElapsedTime - m_dwToFirstTransitionTime + m_dwDurationTime);
        m_fCurrentVisibility = (float)dwTransitionTime / (float)m_dwToFirstTransitionTime;
    }
}
...
///
void
CSmoothVisibilityCombo::UpdateAppearCombo() {
    DWORD dwCurrentTime = g_GameDATA.GetGameTime();
    DWORD dwElapsedTime = dwCurrentTime - m_dwStartTime;
...
///
void
CSmoothVisibilityCombo::UpdateFlashCombo() {
    DWORD dwCurrentTime = g_GameDATA.GetGameTime();
    DWORD dwElapsedTime = dwCurrentTime - m_dwStartTime;
