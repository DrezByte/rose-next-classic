...
///
int
CSystemProcScript::DoScript(const char* strScriptName) {
    int iResult = 0;
...
///
void
CSystemProcScript::PrepareSystemFunction(lua_State* L) {
...
///
int
CSystemProcScript::CallLuaFunction(const char* strFuncName, ...) {
    va_list va;
...
int
CSystemProcScript::CallLuaFunction(int& iReturn, const char* strFuncName, ...) {
    va_list va;
    va_start(va, strFuncName);
    int iResultCnt = lua_CallFUNC(m_pLUA->m_pState, strFuncName, va);
    va_end(va);
...
int
CSystemProcScript::CallLuaFunction(char** pChar, const char* strFuncName, ...) {
    va_list va;
    va_start(va, strFuncName);
    int iResultCnt = lua_CallFUNC(m_pLUA->m_pState, strFuncName, va);
    va_end(va);
...
int
CSystemProcScript::CallLuaFunction(float& fResult, const char* strFuncName, ...) {
    va_list va;
    va_start(va, strFuncName);
    int iResultCnt = lua_CallFUNC(m_pLUA->m_pState, strFuncName, va);
    va_end(va);
...
void
CSystemProcScript::set_initial_globals() {
...
void
CSystemProcScript::set_global(const char* variable_name, int value) {
    lua_pushnumber(m_pLUA->m_pState, value);
    lua_setglobal(m_pLUA->m_pState, variable_name);
}

void
CSystemProcScript::set_global(const char* variable_name, float value) {
    lua_pushnumber(m_pLUA->m_pState, value);
    lua_setglobal(m_pLUA->m_pState, variable_name);
}

void
CSystemProcScript::set_global(const char* variable_name, const char* value) {
