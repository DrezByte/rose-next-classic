...
int
CGameStateSelectAvatar::Update(bool bLostFocus) {
...
int
CGameStateSelectAvatar::Enter(int iPrevStateID) {
...
int
CGameStateSelectAvatar::Leave(int iNextStateID) {
    CSystemProcScript::GetSingleton().CallLuaFunction("LeaveSelectAvatar", ZZ_PARAM_END);
    return 0;
}

int
CGameStateSelectAvatar::ProcMouseInput(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (getAnimatableState(g_pCamera->GetZHANDLE()) != 0)
        return 0;
    ...
    switch (uiMsg) {
        case WM_LBUTTONDOWN: {
            const char* pszName =
                CGameDataCreateAvatar::GetInstance().Pick_AVATAR(LOWORD(lParam), HIWORD(lParam));
            if (pszName) {
                const char* pszSelectedName =
                    CGameDataCreateAvatar::GetInstance().GetSelectedAvatarName();

                ///이전 선택되어진 아바타와 같은 아바타라면 무시
                if (pszSelectedName && strcmp(pszName, pszSelectedName) == 0)
                    break;

                CJustModelAVT* pPrevAvt =
                    CGameDataCreateAvatar::GetInstance().FindAvatar(pszSelectedName);
                ...
                CSystemProcScript::GetSingleton().CallLuaFunction("OnSelectAvatar",
                    ZZ_PARAM_INT,
                    CGameDataCreateAvatar::GetInstance().GetSelectedAvatarIndex(),
                    ZZ_PARAM_STRING,
                    pszName,
                    ZZ_PARAM_INT,
                    iSex,
                    ...
                    iPrevAvtDeleteState,
                    ZZ_PARAM_INT,
                    (pNowAvt->GetDeleteRemainSEC()) ? 1 : 0,
                    ZZ_PARAM_END);
...
int
CGameStateSelectAvatar::ProcKeyboardInput(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (getAnimatableState(g_pCamera->GetZHANDLE()) != 0)
        return 0;
...
void
CGameStateSelectAvatar::ServerDisconnected() {
