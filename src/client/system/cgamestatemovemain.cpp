...
void
CGameStateMoveMain::ServerDisconnected() {
    g_pCApp->SetExitGame();
}

int
CGameStateMoveMain::ProcWndMsgInstant(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    return 0;
}

int
CGameStateMoveMain::ProcMouseInput(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    return 0;
}

int
CGameStateMoveMain::ProcKeyboardInput(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    return 0;
}

int
CGameStateMoveMain::Update(bool bLostFocus) {
    g_pCamera->Update();
...
int
CGameStateMoveMain::Enter(int iPrevStateID) {
    CSystemProcScript::GetSingleton().CallLuaFunction("EnterMoveMain", ZZ_PARAM_END);
    return 0;
}

int
CGameStateMoveMain::Leave(int iNextStateID) {
