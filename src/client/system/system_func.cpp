...
void
SC_SetAvatarMotionByIndex(ZSTRING strName, int iMotionIDX) {
    CJustModelAVT* pAvt = CGameDataCreateAvatar::GetInstance().FindAvatar(strName);
    if (pAvt)
...
void
SC_SetVisible(HNODE hAVT, int iIsVisible) {
    CJustModelAVT* pAvt = reinterpret_cast<CJustModelAVT*>(hAVT);
...
void
SC_SetPosition(HNODE hNODE, float x, float y, float z) {
    ::setPosition(hNODE, x, y, z);
}

HNODE
SC_LoadModel(ZSTRING pModelName, HNODE hSkeleton, HNODE hMotion, float ScaleInLoad) {
...
HNODE
SC_LoadSkeleton(ZSTRING pSkeletonName, ZSTRING pSkeletonPath) {
...
HNODE
SC_LoadMotion(ZSTRING pMotionName,
...
void
SC_AttachMotion(HNODE hNode, HNODE hMotion) {
    if (hNode == 0) {
...
int
SC_GetTotalFrame(HNODE hNode) {
    return ::getMotionTotalFrame(hNode);
}

void
SC_ControlAnimation(HNODE hNode, int iControl) {
    ::controlAnimatable(hNode, iControl);
}

HNODE
SC_LoadCamera(ZSTRING pCameraName, ZSTRING pCameraPath, HNODE hCameraMotion) {
...
HNODE
SC_SetCameraDefault(HNODE hCamera) {
    return ::setCameraDefault(hCamera);
}

void
SC_UnloadNode(HNODE hNode) {
    if (hNode == 0) {
...
int
SC_SetRepeatCount(HNODE hAnimatable, int iRepeatCount) {
    if (hAnimatable == 0) {
...
HNODE
SC_GetMotion(HNODE hAnimatable) {
    if (hAnimatable == 0) {
...
void
SC_SetModelPosition(HNODE hModel, float x, float y, float z) {
    if (hModel == 0)
...
void
SC_SetModelDirection(HNODE hModel, float fAngleDegree) {
    if (hModel == 0) {
...
void
SC_SetScale(HNODE hVisible, float x, float y, float z) {
    if (hVisible == 0) {
...
HNODE
SC_AddCreateAVT(ZSTRING pszName,
    int iRace,
    int iFace,
    int iHair,
    int iHelmet,
...
void
SC_RemoveCreateAVT(ZSTRING pszName) {
    CGameDataCreateAvatar::GetInstance().RemoveCreateAvatar(pszName);
}

void
SC_AddSelectItem4CreateAvatar(int iType, ZSTRING pszItem, ZSTRING pszValue) {
    CCreateAvata* pDialog = (CCreateAvata*)g_EUILobby.GetEUI(EUI_CREATE_AVATA);
    pDialog->AddSelectItem(iType, pszItem, pszValue);
}

void
SC_ClearSelectItem4CreateAvatar(int iType) {
    CCreateAvata* pDialog = (CCreateAvata*)g_EUILobby.GetEUI(EUI_CREATE_AVATA);
    pDialog->ClearSelectItem(iType);
}

void
SC_SelectItem4CreateAvatar(int iType, int iSubscript) {
    CCreateAvata* pDialog = (CCreateAvata*)g_EUILobby.GetEUI(EUI_CREATE_AVATA);
    pDialog->SelectItem(iType, iSubscript);
}

void
SC_SetAvatarFace(ZSTRING pszName, int iFace) {
...
void
SC_SetAvatarHair(ZSTRING pszName, int iHair) {
    CJustModelAVT* pAvt = CGameDataCreateAvatar::GetInstance().FindAvatar(pszName);
    if (pAvt) {
...
int
SC_GetEventObjectIndex(int iEventID) {
    CObjFixedEvent* pObj = g_pObjMGR->GetEventObject(iEventID);
    if (pObj == NULL)
        return 0;
...
void
SC_RunEvent(int iOwnerIndex, ZSTRING szQuestFILE, int iEventIDX) {
    if (g_itMGR.IsDlgOpened(DLG_TYPE_DIALOG))
        return;

    if (g_pEventLIST->Run_EVENT(iOwnerIndex, (char*)szQuestFILE, iEventIDX, EVENT_DLG_NPC)
        == false) {
        ;
    }
}
...
void
SC_RunEventFieldWarp(int iOwnerIndex, ZSTRING szQuestFILE, int iEventIDX) {
    if (g_itMGR.IsDlgOpened(DLG_TYPE_SELECTEVENT))
        return;

    if (g_pEventLIST->Run_EVENT(iOwnerIndex, (char*)szQuestFILE, iEventIDX, EVENT_DLG_WARP)
        == false) {
        ;
    }
}
...
void
SC_RunEventObjectEvent(int iOwnerIndex, ZSTRING szQuestFILE, int iEventIDX) {

    if (g_itMGR.IsDlgOpened(DLG_TYPE_EVENTDIALOG))
        return;

    if (g_pEventLIST->Run_EVENT(iOwnerIndex, (char*)szQuestFILE, iEventIDX, EVENT_DLG_EVENT_OBJECT)
        == false) {
        ;
    }
}
...
///
void
SC_LogString(ZSTRING szMessage) {
...
int
SC_ScreenFadeInStart(float fade_in_t,
    float fade_m_t,
    float fade_out_t,
    int color_r,
    int color_g,
...
int
SC_GetEffectUseIndex(int iEffectIndex) {
...
void
SC_CreateEventButton(int iButtonIndex) {
    CTutorialEventUtility::GetSingleton().CreateNotifyEventButton(iButtonIndex);
}

SYSTEM_SCRIPT
void
SC_AddNpcIndicator(int npcno, int auto_remove) {
...
SYSTEM_SCRIPT
void
SC_AddCoordinatesIndicator(int index, int zoneno, float x, float y) {
    if (CTDialog* pDlg = g_itMGR.FindDlg(DLG_TYPE_MINIMAP)) {
        CMinimapDLG* p = (CMinimapDLG*)pDlg;
...
SYSTEM_SCRIPT
void
SC_RemoveNpcIndicator(int npcno) {
    if (CTDialog* pDlg = g_itMGR.FindDlg(DLG_TYPE_MINIMAP)) {
        CMinimapDLG* p = (CMinimapDLG*)pDlg;
...
SYSTEM_SCRIPT
void
SC_RemoveCoordinatesIndicator(int index) {
    if (CTDialog* pDlg = g_itMGR.FindDlg(DLG_TYPE_MINIMAP)) {
        CMinimapDLG* p = (CMinimapDLG*)pDlg;
...
SC_ShowTutorialImage(ZSTRING filename,
    ...
    // CTutorialEventManager::GetSingleton().RegistImage( filename, x, y, fadein_time,fadeout_time,
    // max_time, append_or_renewal );
    CTutorialEventManager::GetSingleton().RegistImage(filename,
        x,
        y,
        fadein_endtime,
        fadeout_starttime,
...
SYSTEM_SCRIPT
int
SC_GF_getVariable(int iVarTYPE) {
    return GF_getVariable(iVarTYPE);
}

SYSTEM_SCRIPT
int
SC_QF_getNpcQuestZeroVal(int iNpcNO) {
    return QF_getNpcQuestZeroVal(iNpcNO);
}

SYSTEM_SCRIPT
int
SC_QF_findQuest(int iQuestID) {
    return QF_findQuest(iQuestID);
}

SYSTEM_SCRIPT
int
SC_QF_getEpisodeVAR(int iVarNO) {
    return QF_getEpisodeVAR(iVarNO);
}

SYSTEM_SCRIPT
int
SC_QF_getJobVAR(int iVarNO) {
    return QF_getJobVAR(iVarNO);
}

SYSTEM_SCRIPT
int
SC_QF_getPlanetVAR(int iVarNO) {
    return QF_getPlanetVAR(iVarNO);
}

SYSTEM_SCRIPT
int
SC_QF_getUnionVAR(int iVarNO) {
    return QF_getUnionVAR(iVarNO);
}

SYSTEM_SCRIPT
int
SC_QF_getQuestVar(int hQUEST, int iVarNO) {
    return QF_getQuestVar(hQUEST, iVarNO);
}

SYSTEM_SCRIPT
int
SC_QF_getQuestSwitch(int hQUEST, int iSwitchNO) {
    return QF_getQuestSwitch(hQUEST, iSwitchNO);
}

SYSTEM_SCRIPT
int
SC_QF_getQuestItemQuantity(int iQuestID, int iItemNo /*5자리:Type+No*/) {
    return QF_getQuestItemQuantity(iQuestID, iItemNo);
}

SYSTEM_SCRIPT
int
SC_QF_getUserSwitch(int iSwitchNO) {
    return QF_getUserSwitch(iSwitchNO);
}

int
SC_QF_GetNpcQuestStatus(int npcindex) {
    int iReturn;
    CSystemProcScript::GetSingleton().CallLuaFunction(iReturn,
        "SC_QF_GetNpcQuestStatus",
        ZZ_PARAM_INT,
        npcindex,
        ZZ_PARAM_END);
    return iReturn;
