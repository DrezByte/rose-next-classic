...
int
CGameStatePrepareSelectAvatar::Update(bool bLostFocus) {
...
int
CGameStatePrepareSelectAvatar::Enter(int iPrevStateID) {
...
CGameStatePrepareSelectAvatar::Leave(int iNextStateID) {
    ...
    ::setDeltaTime(0); // 데이터 로딩 때문에, 프레임간 시간차가 너무 많이 나서, 카메라 애니메이션이
                       // 종료될 수 있다. 따라서, 프레임간 시간차를 0으로 함.
...
void
CGameStatePrepareSelectAvatar::Draw() {
