...
int
CGameStateCreateAvatar::Update(bool bLostFocus) {
    g_EUILobby.Update();
...
int
CGameStateCreateAvatar::Enter(int iPrevStateID) {
...
int
CGameStateCreateAvatar::Leave(int iNextStateID) {
    CSystemProcScript::GetSingleton().CallLuaFunction("LeaveCreateAvatar", ZZ_PARAM_END);
    return 0;
}

int
CGameStateCreateAvatar::ProcMouseInput(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
...
int
CGameStateCreateAvatar::ProcKeyboardInput(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (g_EUILobby.MsgProc(uiMsg, wParam, lParam))
        return 1;
