...
public:
    ...
    CMODEL() {
        m_pParts = NULL, m_szName = NULL, m_nRootPART = -1, m_nDummyPointCNT = 0,
        m_pDummyPoints = NULL;
...
template<class CModelPart>
bool
CMODEL<CModelPart>::Load(CFileSystem* pFileSystem,
    t_HASHKEY* pMeshKEY,
    t_HASHKEY* pMatKEY,
    t_HASHKEY* pEftKEY,
    short nLinkBoneNo,
...
public:
    CModelDATA() {
        m_pMODELS = NULL, m_nModelCNT = 0, m_nDefaultLinkBoneNo = -1, m_nDefaultLinkDummyNo = -1;
    }
...
CModelDATA<CModelPart>::Load(char* szFileName, short nBoneIdx, short nDummyIdx) {
    ...
    for (nI = 0; nI < nMatFileCNT; nI++) {
        ...
        LogString(LOG_DEBUG_,
            "Model MAT ::[ %s ], skin:%d, alpha:%d, 2side:%d, alphatest:%d, alpharef:%d, ztest:%d, "
            "zwrite:%d, blendtype:%d, specular:%d, alphavalue:%f, glowtype:%d, glowcolor:%f,%f,%f "
            "\n",
            pStr,
            nIsSkin,
            nIsAlpha,
            ...
            fGlowColor[2]);
#endif
        pMatKEY[nI] = g_MatFILE.Add_MATERIAL(pStr,
            nIsSkin,
            nIsAlpha,
            nIs2Side,
            false, /* bTerrainMAT */
    ...
    for (short nM = 0; nM < m_nModelCNT; nM++) {
        m_pMODELS[nM].Load(pFileSystem,
            pMeshKEY,
            pMatKEY,
            pEftKEY,
            m_nDefaultLinkBoneNo,
            m_nDefaultLinkDummyNo);
    }
...
template<class CModelPart>
void
CModelDATA<CModelPart>::Free(void) {
    SAFE_DELETE_ARRAY(m_pMODELS);
}
template<class CModelPart>
CMODEL<CModelPart>*
CModelDATA<CModelPart>::GetMODEL(short nIndex) {
    if (nIndex < 0 || nIndex >= m_nModelCNT) {
        LogString(LOG_DEBUG_,
            "ERROR:: Invalid model data index[ %d / %d ] in %s \n",
            nIndex,
            m_nModelCNT,
            m_DataFileName.Get());
        return NULL;
