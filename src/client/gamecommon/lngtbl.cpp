...
void
tagStrTblDATA::Set(const WCHAR* szStr, short nLen) {
    if (m_pStr) {
        delete[] m_pStr;
        m_pStr = NULL;
        m_nStrLEN = 0;
...
bool
AStringTable::Open(const char* szFileName) {
    if (m_pFS) {
        m_pFS->CloseFile();
...
short
AStringTable::GetLength(WORD wColIDX, WORD wRowIDX) {
...
bool
AStringTable::Load(const char* szFileName) {
...
WCHAR*
AStringTable::GetWString(WORD wColIDX, WORD wRowIDX) {
    if (wRowIDX >= m_iRowCNT || wColIDX >= m_iColCNT)
        return NULL;
...
char*
AStringTable::GetMbcsString(WORD wColIDX, WORD wRowIDX) {
    ...
    if (szWStr) {
        ...
        int iWrtCNT =
            WideCharToMultiByte(CP_ACP, 0, szWStr, -1, m_szMbcBuffer, m_nMbcsTmpStrLen, 0, 0);
...
int
AStringTable::GetRowCNT(void) {
    return m_iRowCNT;
}

int
AStringTable::GetClmnCNT(void) {
    return m_iColCNT;
}

int
AStringTable::FindEmptyClmn(int iClmnIDX) {
...
int
AStringTable::FindEmptyRow(void) {
    for (int i = 0; i < m_iRowCNT; i++) {
...
// CStringManager::GetSingleton().GetFontNameByCharSet(
...
const char*
CLtbManager::GetEventString(int StrID) {
    if (m_pEvent)
