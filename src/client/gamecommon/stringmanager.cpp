...
const char* _ItemTypeTable[MAX_ITEM_TYPE_TB] = {"3DData\\STB\\LIST_ARMS_s.stl",
...
const int _ItemTypeToItemTable[ITEM_TYPE_RIDE_PART + 1] = {
    ...
    LIST_JEWEL_S_TB, // ITEM_TYPE_JEWEL,				// 7	LIST_JEWEL.stb		장신구 : 목걸이
                     // 반지
...
int CALLBACK
EnumFontFamExProc(ENUMLOGFONTEX* lpelfe, NEWTEXTMETRICEX* lpntme, int FontType, LPARAM lParam) {
...
LANGUAGE_TYPE
CStringManager::GetLanguageIDInGame(int iCharSet) {
    switch (iCharSet) {
...
char*
CStringManager::GetFontNameByCharSet(int iCharSet) {
...
//-----------------------------------------------------------------------------------------------
void
CStringManager::InitStringTables() {
...
void
CStringManager::ClearStringTables() {
    int i = 0;
...
//------------------------------------------------------------------------------------------
bool
CStringManager::LoadTables(int iCharSet) {
...
//------------------------------------------------------------------------------------------
bool
CStringManager::LoadTypeTable(LANGUAGE_TYPE languageType,
    ...
    for (i = 0; i < iKeyCount; i++) {
        ...
        switch (iTableType) {
            case ITEM_TYPE_ST: {
                stItemTypeTable* itTable =
                    m_ItemTypeTables[iStringType].GetObjectInstanceFromPool();
            ...
            case NORMAL_TYPE_ST: {
                stNormalTypeTable* itTable =
                    m_NormalTypeTables[iStringType].GetObjectInstanceFromPool();
            ...
            case QUEST_TYPE_ST: {
                stQuestTypeTable* itTable =
                    m_QuestTypeTables[iStringType].GetObjectInstanceFromPool();
    ...
    for (i = 0; i < iKeyCount; i++) {
        switch (iTableType) {
            case ITEM_TYPE_ST: {
                ...
                /// stItemTypeTable* pItemTypeTable = m_ItemTypeTables[ iStringType
                /// ].GetObjectByName( m_IndexToKey[ i ].c_str() );
            ...
            case NORMAL_TYPE_ST: {
                pFileSystem->ReadPascalString(m_TempBuf, sizeof(m_TempBuf));
                (CLocalizing::GetSingleton()).UTF8ToMBCS(m_TempBuf, sizeof(m_TempBuf));
                /// stNormalTypeTable* pNormalTypeTable = m_NormalTypeTables[ iStringType
                /// ].GetObjectByName( m_IndexToKey[ i ].c_str() );
            ...
            case QUEST_TYPE_ST: {
                ...
                pFileSystem->ReadPascalString(m_TempBuf, sizeof(m_TempBuf));
                (CLocalizing::GetSingleton()).UTF8ToMBCS(m_TempBuf, sizeof(m_TempBuf));
                /// stQuestTypeTable* pQuestTypeTable = m_QuestTypeTables[ iStringType
                /// ].GetObjectByName( m_IndexToKey[ i ].c_str() );
...
//
//	if( m_ItemTypeTables[  _ItemTypeToItemTable[ iType ] ].find( strKey ) == m_ItemTypeTables[
//_ItemTypeToItemTable[ iType ] ].end() )
//	{
...
stItemTypeTable*
CStringManager::GetItemStringData(int iType, int iItemNo) {
    if (iType < 1 || iType > ITEM_TYPE_RIDE_PART)
        return NULL;
...
//---------------------------------------------------------------------------------------
const char*
CStringManager::GetItemName(int iType, int iItemNo) {
...
//---------------------------------------------------------------------------------------
const char*
CStringManager::GetItemDesc(int iType, int iItemNo) {
...
const char*
CStringManager::GetSkillName(int iSkillNo) {
...
const char*
CStringManager::GetSkillDesc(int iSkillNo) {
...
const char*
CStringManager::GetStatusName(int iStatusNo) {
...
const char*
CStringManager::GetStatusStartMsg(int iStatusNo) {
    stQuestTypeTable* pQuestTypeTable = GetStatusStringData(iStatusNo);
    if (pQuestTypeTable == NULL)
        return m_strNull.c_str();
...
const char*
CStringManager::GetStatusEndMsg(int iStatusNo) {
    stQuestTypeTable* pQuestTypeTable = GetStatusStringData(iStatusNo);
    if (pQuestTypeTable == NULL)
        return m_strNull.c_str();
...
/// Quest name
const char*
CStringManager::GetQuestName(int iQuestNo) {
...
/// Quest start msg
const char*
CStringManager::GetQuestStartMsg(int iQuestNo) {
    stQuestTypeTable* pQuestTypeTable = GetQuestStringData(iQuestNo);
    if (pQuestTypeTable == NULL) {
        return m_strNull.c_str();
    }
...
/// Quest end msg
const char*
CStringManager::GetQuestEndMsg(int iQuestNo) {
    stQuestTypeTable* pQuestTypeTable = GetQuestStringData(iQuestNo);
    if (pQuestTypeTable == NULL) {
        return m_strNull.c_str();
    }
...
/// Quest desc
const char*
CStringManager::GetQuestDesc(int iQuestNo) {
    stQuestTypeTable* pQuestTypeTable = GetQuestStringData(iQuestNo);
    if (pQuestTypeTable == NULL) {
        return m_strNull.c_str();
    }
...
//-----------------------------------------------------------------------------------------------
const char*
CStringManager::GetNpcName(int iIndex) {
...
const char*
CStringManager::GetNpcDesc(int iIndex) {
    return m_strNull.c_str();
}
...
//-----------------------------------------------------------------------------------------------
char*
CStringManager::GetNormalTypeTableString(int iType, std::string& strKey) {
...
const std::string&
CStringManager::GetItemPrefix(int iIndex) {
    std::string strKey(CStr::Printf("%d", iIndex));

    stNormalTypeTable* pNormalTypeTable =
        m_NormalTypeTables[STR_ITEMPREFIX].GetObjectByName(strKey.c_str());
    if (pNormalTypeTable == NULL)
        return m_strNull;
...
//-----------------------------------------------------------------------------------------------
stItemTypeTable*
CStringManager::GetZoneStringData(int iZoneNO) {
    ...
    stItemTypeTable* pItemTypeTable =
        m_ItemTypeTables[LIST_ZONE_S_TB].GetObjectByName(strKey.c_str());
    if (pItemTypeTable == NULL) {
        return NULL;
    }
...
const char*
CStringManager::GetZoneName(int iZoneNO) {
...
const char*
CStringManager::GetZoneDesc(int iZoneNO) {
    stItemTypeTable* pItemTypeTable = GetZoneStringData(iZoneNO);
    if (pItemTypeTable == NULL) {
        return m_strNull.c_str();
    }
...
//-----------------------------------------------------------------------------------------------
const char*
CStringManager::GetNotifyMessageString(int iIndex) {
    std::string strKey(CStr::Printf("%d", iIndex));
