...
enum enumSKILL_ACTION_TYPE {
    ...
    SKILL_ACTION_ENFORCE_BULLET =
        5, ///< 강화총알 변경 발사. ( 아이스 애로우.. 실제 화살이 변하는.. )
    ...
    SKILL_ACTION_SELF_DAMAGE =
        17, ///< 자신주위로 데미지를 주는 스킬..( 이건 셀프로 안되나? ) 물론 범위..
...
protected:
    ...
    short m_nClanSkillIndex; ///클랜스킬일경우 SkillIndex, 메인 스킬일경우에는 CSkillSlot에서
                             ///가져온다.
    int m_iSkillLevel;
...
public:
    ...
    virtual int SendSkillActionReq(int iSkillSlot,
        int iTarget,
        D3DXVECTOR3& PosTO = D3DXVECTOR3(0.0f, 0.0f, 0.0f)) = 0 {
        *(int*)0 = 10;
    };
