...
void
SubAbilities(tagBaseITEM* item, int* curr_abilities, const int ability_count) {
    assert(item);
    assert(curr_abilities);
...
void
AddAbilities(tagBaseITEM* item, int* curr_abilities, const int ability_count) {
    assert(item);
    assert(curr_abilities);
...
CTCmdItemEquipInInventory::Exec(CTObject* pObj) {
    ...
    if (g_pAVATAR->Check_EquipCondition(Item)) {
        ...
        if (Item.IsTwoHands() && pInven->GetEmptyInventory(0) == -1 && g_pAVATAR->Get_L_WEAPON()
            && g_pAVATAR->Get_R_WEAPON())
            g_itMGR.AppendChatMsg(STR_NOT_ENOUGH_EQUIP_SPACE, IT_MGR::CHAT_TYPE_SYSTEM);
        else if (!equiped_item.IsEmpty() && equiped_item.IsTwoHands()
            && Item.GetTYPE() == ITEM_TYPE_SUBWPN)
            ...
            if (!Item.IsEmpty()) {
                ...
                std::list<int> removeitem_indexlist; ///서버에 장착해제 요청할 장착장비들의 인벤토리
                                                     ///인덱스 리스트
                ...
                if (g_pAVATAR->m_Inventory.GetEmptyInvenSlotCount(INV_WEAPON)
                    < (int)removeitem_indexlist.size()) {
                    g_itMGR.OpenMsgBox(STR_NOT_ENOUGH_INVENTORY_SPACE);
                } else {
                    g_pNet->Send_cli_EQUIP_ITEM(nEquipIDX, pItem->GetIndex());

                    std::list<int>::iterator iter;
                    for (iter = removeitem_indexlist.begin(); iter != removeitem_indexlist.end();
                         ++iter)
...
bool
CTCmdItemUseInInventory::Exec(CTObject* pObj) {
    if (pObj == NULL) {
    ...
    switch (ITEM_TYPE(Item.GetTYPE(), Item.GetItemNO())) {
        ...
        case USE_ITEM_EVENT_ITEM: {
            g_pEventLIST->Run_EVENT(g_pAVATAR->Get_INDEX(),
                USEITEM_CONFILE_IDX(Item.GetItemNO()),
                -1,
                true);
        } break;
    }
    ...
    if (g_pNet->Send_cli_USE_ITEM(pItem->GetIndex(),
            bTargetSkill,
...
bool
CTCmdItemEtcInInventory::Exec(CTObject* pObj) {
...
CTCmdItemEquiped::Exec(CTObject* pObj) {
    ...
    if (!Item.IsEmpty()) {
        ...
        tagITEM equiped_items[MAX_EQUIP_IDX]; ///현재 아바타의 장착장비 임지 저장 변수
        std::list<int>
            removeitem_indexlist; ///서버에 장착해제 요청할 장착장비들의 인벤토리 인덱스 리스트
...
bool
CTCmdItemBullet::Exec(CTObject* pObj) {
    if (pObj == NULL) {
