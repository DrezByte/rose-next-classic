...
CObservable::~CObservable(void) {}
void
CObservable::AddObserver(IObserver* pObserver) {
...
void
CObservable::DeleteObserver(IObserver* pObserver) {
    std::list<IObserver*>::iterator iter;
    for (iter = m_Observers.begin(); iter != m_Observers.end(); ++iter) {
        if (*iter == pObserver) {
...
void
CObservable::DeleteObservers() {
    m_Observers.clear();
}

int
CObservable::CountObservers() {
    return m_Observers.size();
}

void
CObservable::NotifyObservers(CTObject* pObj) {
    std::list<IObserver*>::iterator iter;
    for (iter = m_Observers.begin(); iter != m_Observers.end(); ++iter)
