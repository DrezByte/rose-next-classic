...
void
DeleteDropItemInfo(CDropItemInfo* pInfo) {
    if (pInfo) {
        delete pInfo;
        pInfo = NULL;
    }
...
void
CDropItemInfo::SetDropItemInfo(CObjITEM* pItem) {
    Reset();

    m_pDropItem = pItem;

    m_Info.AddString(m_pDropItem->Get_NAME(),
        CItem::GetItemNameColor(pItem->m_ITEM.GetTYPE(), pItem->m_ITEM.GetItemNO()));
...
void
CItemInfoView::ReserverDropItemPool(int iPoolSize) {
...
void
CItemInfoView::Reset() {
    m_DropItemInfoPool.insert(m_DropItemInfoPool.begin(),
...
bool
CItemInfoView::PushDropItemInfo(CObjITEM* pItem) {
    if (pItem == NULL)
        return false;
...
void
CItemInfoView::Draw() {
    std::for_each(m_ViewedItemList.begin(), m_ViewedItemList.end(), [](auto v) { v->Draw(); });
}
