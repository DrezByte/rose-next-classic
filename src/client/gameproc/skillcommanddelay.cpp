...
bool
CSkillCommandDelay::Init() {
    if (m_hCoverTexture) {
        ::unloadTexture(m_hCoverTexture);
        m_hCoverTexture = NULL;
    }

    m_hCoverTexture = ::loadTexture("3DData\\Control\\Res\\skilldelay.dds",
        "3DData\\Control\\Res\\skilldelay.dds",
        1,
        0);
...
void
CSkillCommandDelay::StartSkill() {
    m_dwCastingStartTime = g_GameDATA.GetGameTime();
    m_iAnimationFrame = 0;
}

bool
CSkillCommandDelay::CanCastSkill() {
    return m_bCanCastSkill;
}

void
CSkillCommandDelay::Proc() {
...
void
CSkillCommandDelay::Draw(int iX, int iY) {
    if (m_bCanCastSkill)
        return;
