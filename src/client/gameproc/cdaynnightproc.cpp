...
void
CDayNNightProc::ResetDayEnviTick() {
    m_iDAYENVIR_TICK = DAYENVIR_TICK;
}
...
int
CDayNNightProc::GetHeartsDay() {
    int iDay =
        ((GetWorldTime() - (GetHeartsYear() * 103680) - (8640 * (GetHeartsMonth() - 1)))) / 160 + 1;
...
void
CDayNNightProc::SetWorldTime(int iZoneNO, DWORD dwWorldTime) {
    m_iZoneNO = iZoneNO;
    ...
    switch (m_iCurrentTimeState) {
        case TIME_DN_MORNING: {
            ...
            Start(ZONE_BG_IMAGE(m_iZoneNO),
                m_iCurrentState,
                ZONE_DAY_TIME(m_iZoneNO) - ZONE_MORNING_TIME(m_iZoneNO));
        } break;
        case TIME_DN_DAY: {
            m_iCurrentState = DN_DAY;
        ...
        case TIME_DN_EVENING: {
            ...
            Start(ZONE_BG_IMAGE(m_iZoneNO),
                m_iCurrentState,
                ZONE_NIGHT_TIME(m_iZoneNO) - ZONE_EVENING_TIME(m_iZoneNO));
        } break;
        case TIME_DN_NIGHT: {
            m_iCurrentState = DN_NIGHT;
            m_dwAddiditionalTime = 0;
...
bool
CDayNNightProc::CheckUpdateEnvironment() {
    ...
    if (bChangeState) {
        ...
        // CBGMManager::GetSingleton().ChangeBGMMusicWithDayAndNight( m_iZoneNO, ( m_iCurrentState
        // == DN_NIGHT ) );
...
void
CDayNNightProc::SetGlobalIllumination(float fBlendRatio) {
    /// Background color
    ...
    float fFogR =
        ((FogPreColor.r * (1.0f - fBlendRatio)) + ((FogCurrentColor.r * fBlendRatio))) / 255.0f;
    float fFogG =
        ((FogPreColor.g * (1.0f - fBlendRatio)) + ((FogCurrentColor.g * fBlendRatio))) / 255.0f;
    float fFogB =
        ((FogPreColor.b * (1.0f - fBlendRatio)) + ((FogCurrentColor.b * fBlendRatio))) / 255.0f;
...
D3DXCOLOR
CDayNNightProc::GetColorStringFromSTB(int iType, int iState) {
...
bool
CDayNNightProc::PushEffect(CEffect* pEffect) {
    if (pEffect == NULL)
        return false;
...
void
CDayNNightProc::DeleteEffectFromList(CEffect* pEffect) {
...
void
CDayNNightProc::ShowAllEffect(bool bShow) {
    if (bShow) {
        for_each(m_EffectList.begin(), m_EffectList.end(), ShowDayNNight);
    } else {
        for_each(m_EffectList.begin(), m_EffectList.end(), HideDayNNight);
