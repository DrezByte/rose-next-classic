...
D3DXCOLOR
CGameUtil::GetRGBFromString(unsigned int iColor) {
...
WORD
CGameUtil::GetPrimaryLanguage() {
    return PRIMARYLANGID(LOWORD(GetKeyboardLayout(0)));
}
WORD
CGameUtil::GetSubLanguage() {
    return SUBLANGID(LOWORD(GetKeyboardLayout(0)));
}

void
CGameUtil::ConvertMoney2String(__int64 money, char* buffer, int buffer_size) {
    char temp_buf[128];
