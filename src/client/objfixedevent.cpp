...
//-------------------------------------------------------------------------------------------------
void
CObjFixedEvent::UnlinkVisibleWorld(void) {
...
//-------------------------------------------------------------------------------------------------
void
CObjFixedEvent::LinkToModel(HNODE hModel) {
...
//-------------------------------------------------------------------------------------------------
void
CObjFixedEvent::InsertToScene(void) {
...
//-------------------------------------------------------------------------------------------------
void
CObjFixedEvent::RemoveFromScene(bool bIncludeEFFECT) {
...
//------------------------------------------------------------------------------------------
void
CObjFixedEvent::SetPOSITION(D3DVECTOR& Position) {
...
//------------------------------------------------------------------------------------------
bool
CObjFixedEvent::Create(CMODEL<CFixedPART>* pMODEL, D3DVECTOR& Position, bool bCreateEffect) {
    /// 더미 포지션이 있기때문에 오브젝트 자체는 원점에서 생성하고
    /// 실제 좌표이동은 더미포지션 오브젝트에 적용한다. ( SetPOSITION 참고 현재 위치( m_PosCUR ) 을
    /// 그쪽에서 등록 )
...
//------------------------------------------------------------------------------------------------
void
CObjFixedEvent::ExecEventScript(int iCurrentState,
    bool bJustEnd,
    bool bCollisionEvent,
    bool bInsertToScene) {
    /// 충돌에 의한 스크립트 실행이라면
    if (bCollisionEvent) {
        /// 상태가 변경되지 않았다면 그냥 리턴..
