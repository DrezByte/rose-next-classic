...
void
CSendPACKET::Send_gsv_ADD_CHAR(int iObjectIndex, short nCharIdx, tPOINTF& PosSET) {
...
void
CSendPACKET::Send_gsv_ADD_USER(CObjAVT* pAvtCHAR) {
    /*
    ...
    ::CopyMemory( m_pSendPacket->m_gsv_ADD_USER.m_nPartItemIDX , pAvtCHAR->m_nPartItemIDX,
    sizeof(short)*MAX_BODY_PART ); Packet_AppendString (m_pSendPacket, pAvtCHAR->Get_NAME() );

    this->Send_PACKET( m_pSendPacket );
    */
}
void
CSendPACKET::Send_gsv_ADD_FIELDITEM(int iObjectIndex, tagITEM& sITEM) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_HEADER(WORD wType, bool bSendToWorld) {
#ifdef __VIRTUAL_SERVER
    ;
    return;
    ;
...
*/
void
CSendPACKET::Send_cli_JOIN_SERVER_REQ(DWORD dwLSVID, bool bWorldServer) {
#ifdef __VIRTUAL_SERVER
    ;
    _ASSERT(0);
    ;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_LOGOUT_REQ() {
#ifdef __VIRTUAL_SERVER
    ;
    //_ASSERT( 0 );
    ;
...
void
CSendPACKET::Send_cli_LOGOUT_CANCEL() {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CHAR_LIST() {
#ifdef __VIRTUAL_SERVER
    ;
    _ASSERT(0);
    ;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_DELETE_CHAR(BYTE btCharNO, bool bDeleteOrRevive, char* szCharName) {
#ifdef __VIRTUAL_SERVER
    ;
    _ASSERT(0);
    ;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_SELECT_CHAR(BYTE btCharNO, char* szCharName) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_JOIN_ZONE(BYTE btWeightRate) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_REVIVE_REQ(BYTE btReviveTYPE) {
#ifdef __VIRTUAL_SERVER
    return;
...
void
CSendPACKET::Send_cli_SET_REVIVE_POS() {
#ifdef __VIRTUAL_SERVER
    return;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_SET_VAR_REQ(BYTE btVarTYPE, int iValue) {
#ifdef __VIRTUAL_SERVER
    return;
#else
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_SET_MOTION(short nMotionNO, BYTE btStopCMD, WORD wETC) {
#ifdef __VIRTUAL_SERVER
    m_pSendPacket->m_HEADER.m_wType = GSV_SET_MOTION;
    m_pSendPacket->m_HEADER.m_nSize = sizeof(gsv_SET_MOTION);
    m_pSendPacket->m_gsv_SET_MOTION.m_wObjectIDX =
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_PARTY_CHAT(char* szMsg) {
...
void
CSendPACKET::Send_cli_CHAT(char* szMsg) {
#ifdef __VIRTUAL_SERVER
...
void
CSendPACKET::Send_cli_ALLIED_CHAT(char* pszMsg) {
    assert(pszMsg);
...
void
CSendPACKET::Send_cli_ALLIED_SHOUT(char* pszMsg) {
    assert(pszMsg);
    if (pszMsg == NULL || g_pNet->m_bWarping)
        return;
...
void
CSendPACKET::Send_cli_WHISPER(char* szTargetAccount, char* szMsg) {
#ifdef __VIRTUAL_SERVER
...
void
CSendPACKET::Send_cli_SHOUT(char* szMsg) {
#ifdef __VIRTUAL_SERVER
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_STOP(D3DVECTOR& PosCUR) {
#ifdef __VIRTUAL_SERVER
...
void
CSendPACKET::Send_cli_MOUSECMD(int iClientTarget, D3DVECTOR& PosTO) {
#ifdef __VIRTUAL_SERVER
    m_pSendPacket->m_HEADER.m_wType = GSV_MOUSECMD;
    m_pSendPacket->m_HEADER.m_nSize = sizeof(gsv_MOUSECMD);
    m_pSendPacket->m_gsv_MOUSECMD.m_wSourObjIDX =
        g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->m_nIndex);
    ...
    m_pSendPacket->m_cli_MOUSECMD.m_wTargetObjectIDX =
...
CSendPACKET::Send_cli_ATTACK(int iClientTarget) {
    ...
    m_pSendPacket->m_gsv_ATTACK.m_wAtkObjIDX =
        g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->m_nIndex);
    ...
    m_pSendPacket->m_cli_ATTACK.m_wTargetObjectIndex =
        g_pObjMGR->Get_ServerObjectIndex(iClientTarget);
#endif
    ...
    switch (pDefOBJ->Get_TYPE()) {
        ...
        case OBJ_MOB:
            if (pDefOBJ->Get_HP() - nDamage <= 0) {
                if (pAtkOBJ->IsUSER()) {
                    tagITEM sITEM;
                    if (CCal::Get_DropITEM(pAtkOBJ->Get_LEVEL() - pDefOBJ->Get_LEVEL(),
                            (CObjMOB*)pDefOBJ,
                            sITEM)) {
                        // Create field item ...
                    ...
                    // 죽였을때..
                    g_AI_LIST.AI_Kill(NPC_AI_TYPE(pAtkOBJ->Get_CharNO()),
                        pAtkOBJ,
                        pDefOBJ,
                        nDamage);
                }
...
void
CSendPACKET::Send_cli_STORE_TRADE_REQ(WORD wNPCObjIDX,
    char cBuyCNT,
    char cSellCNT,
...
void
CSendPACKET::Send_cli_HP_REQ(int iClientTarget) {
...
extern bool IsTWAIN();
bool
CSendPACKET::Send_cli_USE_ITEM(short nInventoryIndex, bool bTargetSkill, int iClientTarget) {
    ...
    m_pSendPacket->m_gsv_USE_ITEM.m_wObjectIDX =
        g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->m_nIndex);
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CHANGE_SKIN(BYTE btBodyIDX, short nItemNO) {
#ifdef __VIRTUAL_SERVER
    m_pSendPacket->m_HEADER.m_wType = GSV_CHANGE_SKIN;
    m_pSendPacket->m_HEADER.m_nSize = sizeof(gsv_CHANGE_SKIN);

    m_pSendPacket->m_gsv_CHANGE_SKIN.m_wObjectIDX =
        g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->m_nIndex);
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_EQUIP_ITEM(short nEquipInvIDX, short nWeaponInvIDX) {
    ...
    if (0 == nWeaponInvIDX) {
        ...
        m_pSendPacket->m_gsv_SET_INV_ONLY.m_sInvITEM[0].m_ITEM =
            g_pAVATAR->m_Inventory.m_ItemLIST[nEquipInvIDX];

        m_pSendPacket->m_gsv_SET_INV_ONLY.m_sInvITEM[1].m_btInvIDX = nEquipInvIDX;
        m_pSendPacket->m_gsv_SET_INV_ONLY.m_sInvITEM[1].m_ITEM =
            g_pAVATAR->m_Inventory.m_ItemLIST[nWeaponInvIDX];
    }
    ...
    if (m_pSendPacket->m_gsv_SET_INV_ONLY.m_btItemCNT) {
        ...
        m_pSendPacket->m_gsv_EQUIP_ITEM.m_wObjectIDX =
            g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->m_nIndex);
        m_pSendPacket->m_gsv_EQUIP_ITEM.m_nEquipIndex = nEquipInvIDX;
        m_pSendPacket->m_gsv_EQUIP_ITEM.m_nItemNO =
            g_pAVATAR->m_Inventory.m_ItemLIST[nWeaponInvIDX].GetItemNO();

    } else
        return;
#else
    if (nWeaponInvIDX && EQUIP_IDX_WEAPON_L == nEquipInvIDX
        && g_pAVATAR->m_Inventory.m_ItemLIST[EQUIP_IDX_WEAPON_R].IsTwoHands()) {
    ...
    if (0 == nWeaponInvIDX) {
        ...
        _ASSERT(
            nWeaponInvIDX >= MAX_EQUIP_IDX && nWeaponInvIDX < MAX_EQUIP_IDX + INVENTORY_PAGE_SIZE);
...
void
CSendPACKET::Send_cli_DROP_ITEM(short nInventoryIndex, int iQuantity) {
#ifdef __VIRTUAL_SERVER
    m_pSendPacket->m_HEADER.m_wType = GSV_ADD_FIELDITEM;
    m_pSendPacket->m_HEADER.m_nSize = sizeof(gsv_ADD_FIELDITEM);
    m_pSendPacket->m_gsv_ADD_FIELDITEM.m_wOwnerObjIDX =
        g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->m_nIndex);
    ;
...
//-------------------------------------------------------------------------------------------------
bool
CSendPACKET::Send_cli_GET_FIELDITEM_REQ(CGameOBJ* pUSER, int iServerObject) {
    if (!pUSER)
        return false;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_TELEPORT_REQ(CGameOBJ* pUSER, short nWarpIDX) {
    if (!pUSER->IsA(OBJ_USER))
        return;
    ...
    m_pSendPacket->m_gsv_TELEPORT_REPLY.m_wObjectIDX =
        g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->m_nIndex);
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_SET_HOTICON(BYTE btListIDX, tagHotICON HotICON) {
#ifdef __VIRTUAL_SERVER
    ;
    return;
    ;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_USE_BPOINT_REQ(BYTE btAbilityTYPE) {
#ifdef __VIRTUAL_SERVER
    return;
#else
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_SKILL_LEVELUP_REQ(BYTE btSkillSLOT, short nNextLevelSkillIDX) {
    ...
    m_pSendPacket->m_cli_SKILL_LEVELUP_REQ.m_btSkillSLOT =
        btSkillSLOT; /// 현재 레벨업을 할 스킬이 있는 슬롯
    m_pSendPacket->m_cli_SKILL_LEVELUP_REQ.m_nNextLevelSkillIDX =
        nNextLevelSkillIDX; /// 레벨업 하려는 스킬번호.
#endif
    this->Send_PACKET(m_pSendPacket);
}
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_P_STORE_OPEN(BYTE btSellItemCount,
    BYTE btBuyItemCount,
    tagPS_ITEM* SellItems,
    tagPS_ITEM* BuyItems,
    char* pszTitle) {
    m_pSendPacket->m_HEADER.m_wType = CLI_P_STORE_OPEN;
    m_pSendPacket->m_HEADER.m_nSize = sizeof(cli_P_STORE_OPEN)
        + btSellItemCount * sizeof(tagPS_ITEM) + btBuyItemCount * sizeof(tagPS_ITEM);
    ...
    if (btSellItemCount > 0 || btBuyItemCount > 0) {
        for (int i = 0; i < btSellItemCount; ++i)
            memcpy(&m_pSendPacket->m_cli_P_STORE_OPEN.m_ITEMs[i],
                &SellItems[i],
                sizeof(tagPS_ITEM));

        for (int i = btSellItemCount; i < btSellItemCount + btBuyItemCount; ++i)
            memcpy(&m_pSendPacket->m_cli_P_STORE_OPEN.m_ITEMs[i],
                &BuyItems[i - btSellItemCount],
                sizeof(tagPS_ITEM));
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_P_STORE_CLOSE(void) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_P_STORE_LIST_REQ(WORD wStoreObjectIDX) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_P_STORE_SELL_REQ(WORD wStoreObjectIDX,
    int iItemCount,
    tagSELL_ITEM* pSellItems) {
    m_pSendPacket->m_HEADER.m_wType = CLI_P_STORE_SELL_REQ;
    m_pSendPacket->m_HEADER.m_nSize =
        sizeof(cli_P_STORE_SELL_REQ) + iItemCount * sizeof(tagSELL_ITEM);
    ...
    if (iItemCount > 0) {
        for (int i = 0; i < iItemCount; ++i)
            memcpy(&m_pSendPacket->m_cli_P_STORE_SELL_REQ.m_SellITEMs[i],
                &pSellItems[i],
                sizeof(tagSELL_ITEM));
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_P_STORE_BUY_REQ(WORD wStoreObjectIDX,
    int iItemCount,
    tagPS_SLOT_ITEM* pBuyItems) {
    m_pSendPacket->m_HEADER.m_wType = CLI_P_STORE_BUY_REQ;
    m_pSendPacket->m_HEADER.m_nSize =
        sizeof(cli_P_STORE_BUY_REQ) + iItemCount * sizeof(tagPS_SLOT_ITEM);
    ...
    if (iItemCount > 0) {
        for (int i = 0; i < iItemCount; ++i)
            memcpy(&m_pSendPacket->m_cli_P_STORE_BUY_REQ.m_BuyITEMs[i],
                &pBuyItems[i],
                sizeof(tagPS_SLOT_ITEM));
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_SELF_SKILL(BYTE btSkillSLOT) {
    /*
    ...
    */
    ...
    m_pSendPacket->m_gsv_SELF_SKILL.m_wSourObjIDX =
        g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->m_nIndex);
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_TARGET_SKILL(int iClientTarget, BYTE btSkillSLOT) {
#ifdef __VIRTUAL_SERVER
    m_pSendPacket->m_HEADER.m_wType = GSV_TARGET_SKILL;
    m_pSendPacket->m_HEADER.m_nSize = sizeof(gsv_TARGET_SKILL);

    m_pSendPacket->m_gsv_TARGET_SKILL.m_wSourObjIDX =
        g_pObjMGR->Get_ServerObjectIndex(g_pAVATAR->m_nIndex);
    m_pSendPacket->m_gsv_TARGET_SKILL.m_wDestObjIDX =
        g_pObjMGR->Get_ServerObjectIndex(iClientTarget);
    ...
    /// 스킬 연타 방지..
    //--------------------------------------------------------------------------------------
    if (CPreventDuplicatedCommand::GetSingleton().CanSendTargetSkillCommand(iClientTarget,
            btSkillSLOT)) {
        CPreventDuplicatedCommand::GetSingleton().PushTargetSkillCommand(iClientTarget,
            btSkillSLOT);
    } else
        return;
    ...
    m_pSendPacket->m_cli_TARGET_SKILL.m_wDestObjIDX =
        g_pObjMGR->Get_ServerObjectIndex(iClientTarget);
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_POSITION_SKILL(D3DVECTOR& PosTO, BYTE btSkillSLOT) {
#ifdef __VIRTUAL_SERVER
    return;
#else
    if (g_pNet->m_bWarping)
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_QUEST_REQ(BYTE btReqTYPE,
    BYTE btQuestSLOT,
    int iQuestID,
    char* szQuestTriggerName) {
#ifdef __VIRTUAL_SERVER
    return;
#else
...
bool
CSendPACKET::Send_cli_TRADE_P2P(WORD wServerIdx, BYTE btResult, char cSlotIdx) {
...
void
CSendPACKET::Send_cli_TRADE_P2P_ITEM(char cTradeSLOT, short nInventoryIndex, DWORD iQuantity) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_PARTY_REQ(BYTE btRequest, DWORD dwDestIDXorTAG) {
...
void
CSendPACKET::Send_cli_PARTY_REPLY(BYTE btRequest, DWORD dwDestIDXorTAG) {
...
///제조 관련
void
CSendPACKET::Send_cli_CREATE_ITEM_REQ(BYTE btSkillSLOT,
    ...
    memcpy(m_pSendPacket->m_cli_CREATE_ITEM_REQ.m_nUseItemINV,
        pnUseItemINV,
        sizeof(short) * CREATE_ITEM_STEP);
    this->Send_PACKET(m_pSendPacket);
}
...
void
CSendPACKET::Send_cli_ITEM_RESULT_REPORT(BYTE btREPORT, BYTE btItemType, short nItemNo) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_MOVE_ITEM(BYTE btMoveTYPE,
    BYTE btFromIDX,
    tagITEM& MoveITEM,
    bool bPlatinum) {
...
void
CSendPACKET::Send_cli_BANK_LIST_REQ(BYTE btREQ, char* pszPassword) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_SET_BULLET(BYTE btShotType, short nInvenIdx) {
    /// 탈착시 nInvenIdx == 0;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_ASSEMBLE_RIDE_ITEM(short nPartIdx, short nInvenIdx) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_SET_WEIGHT_RATE(BYTE btWeightRate) {
    if (g_pNet->m_bWarping)
        return;
...
void
CSendPACKET::Send_cli_SET_WISHITEM(int iIndex, tagITEM& Item) {
...
void
CSendPACKET::Send_cli_CHANNEL_LIST_REQ(int iServerID) {
...
// 친구 추가 요청
void
CSendPACKET::Send_cli_MCMD_APPEND_REQ(char* szName) {
...
// 친구 추가 요청에 대한 응답
void
CSendPACKET::Send_cli_MCMD_APPEND_REPLY(BYTE btCMD, WORD wUserIDX, char* pszName) {
    m_pSendPacket->m_HEADER.m_wType = CLI_MESSENGER;
...
// 대상을 삭제, 수신거부 상태로 변경.
void
CSendPACKET::Send_cli_MCMD_TAG(BYTE btCMD, DWORD dwUserTAG) {
    m_pSendPacket->m_HEADER.m_wType = CLI_MESSENGER;
...
// 나의 상태 변경.
void
CSendPACKET::Send_cli_MCMD_STATUS_REQ(BYTE btStatus) {
    m_pSendPacket->m_HEADER.m_wType = CLI_MESSENGER;
...
// CHAT_REQ_LEFT
void
CSendPACKET::Send_tag_CHAT_HEADER(BYTE btRoomCMD) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CHAT_ROOM_MAKE(BYTE btRoomTYPE,
    BYTE btMaxUSER,
    char* szRoomTitle,
    char* szPasswd) {
    m_pSendPacket->m_HEADER.m_wType = CLI_CHATROOM;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CHAT_ROOM_JOIN(BYTE btRoomTYPE, WORD wRoomID, char* szPasswd) {
    m_pSendPacket->m_HEADER.m_wType = CLI_CHATROOM;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CHAT_ROOM_KICK(t_HASHKEY HashUSER) {
    m_pSendPacket->m_HEADER.m_wType = CLI_CHATROOM;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CHAT_ROOM_LIST(BYTE btRoomTYPE, WORD wFromRoomID) {
    m_pSendPacket->m_HEADER.m_wType = CLI_CHATROOM;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CHATROOM_MSG(char* pszMsg) {
    assert(pszMsg);
    if (pszMsg == NULL)
        return;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_MESSENGER_CHAT(DWORD dwUserTag, char* pszMsg) {
    assert(pszMsg);
    if (pszMsg == NULL)
        return;
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_MEMO(char* pszName, char* pszMemo) {
...
//-------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_MEMO_CNT_REQ() {
    m_pSendPacket->m_HEADER.m_wType = CLI_MEMO;
    m_pSendPacket->m_HEADER.m_nSize = sizeof(cli_MEMO);
...
//----------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CRAFT_GEMMING_REQ(BYTE btEquipInvIDX, BYTE btGemInvIDX) {
...
//----------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CRAFT_BREAKUP_REQ(BYTE btType,
    short nSkillSLOTorNpcIDX,
    BYTE btTargetInvIDX) {
    m_pSendPacket->m_HEADER.m_wType = CLI_CRAFT_ITEM_REQ;
...
//----------------------------------------------------------------------------------------------------
void
CSendPACKET::Send_cli_CRAFT_UPGRADE_REQ(BYTE btType,
    short nSkillSLOTorNpcIDX,
    BYTE btTargetInvIDX,
...
void
CSendPACKET::Send_cli_USE_ITEM_TO_REPAIR(short nUsedItemInvenIdx, short nTargetItemInvenIdx) {
...
void
CSendPACKET::Send_cli_REPAIR_FROM_NPC(WORD wNpcSvrIdx, short nTargetItemInvenIdx) {
...
void
CSendPACKET::Send_cli_PARTY_RULE(BYTE btRule) {
...
void
CSendPACKET::Send_cli_CHAR_CHANGE() {
...
void
CSendPACKET::Send_cli_APPRAISAL_REQ(int iInventoryIndex) {
...
void
CSendPACKET::Send_cli_CLAN_COMMAND(BYTE btCmd, char* pszMsg) {
...
void
CSendPACKET::Send_cli_CLAN_CREATE(WORD wMarkBack,
    WORD wMarkCenter,
    char* pszName,
    char* pszSlogan) {
    assert(pszName);
...
void
CSendPACKET::Send_cli_CLAN_CHAT(char* szMsg) {
    assert(szMsg);
...
void
CSendPACKET::Send_cli_CLANMARK_REQ(int iClanID) {
...
void
CSendPACKET::Send_cli_CLANMARK_SET(unsigned char* pDATA, int iSize, WORD crc16) {
...
void
CSendPACKET::Send_cli_CLAN_MEMBER_JOBnLEV(short nLev, short nJob) {
    m_pSendPacket->m_HEADER.m_wType = CLI_CLAN_COMMAND;
...
void
CSendPACKET::Send_cli_CLANMARK_REG_TIME() {
...
void
CSendPACKET::Send_cli_MOVE_ZULY_INV2BANK(__int64 i64MoveZuly) {
...
void
CSendPACKET::Send_cli_MOVE_ZULY_BANK2INV(__int64 i64MoveZuly) {
    m_pSendPacket->m_HEADER.m_wType = CLI_MOVE_ZULY;
    m_pSendPacket->m_HEADER.m_nSize = sizeof(cli_MOVE_ZULY);
...
// 2인승 카트.
void
CSendPACKET::Send_cli_CART_RIDE(BYTE bType, WORD wOwnerObjIDX_, WORD wGuestObjIDX_) {
...
void
CSendPACKET::Send_cli_SCREEN_SHOT_TIME() {
...
void
CSendPACKET::Send_cli_UPDATE_NAME(char* szName) {
...
void
CSendPACKET::Send_cli_SET_RIGHTS(DWORD dwRight) {
