 ...
 * - 연관class		CTIme
 *
 * @Warning		다국어 지원 때문에 복잡도가 높아졌다. 수정시 모든 언어( 일어, 중국어에 대한 테스트가
 * 필요하다 )
 *
 * @Author		최종진
 *
...
public:
    ...
    enum { TES_ALIGN_LEFT, TES_ALIGN_CENTER, TES_ALIGN_RIGHT };
...
protected:
    ...
    bool m_bMultiline; // 멀티라인 입력?( 실제적으로 Draw시에 줄넘김 기능만이 있어. 프롬프트의
                       // 위치가 이상하다 )
