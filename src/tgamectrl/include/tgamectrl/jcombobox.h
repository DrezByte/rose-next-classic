...
public:
    ...
    virtual void
    OwnerDraw(); /// 특정한 상황에 제일 마지막에 드랍박스를 그리기 위하여 Draw부분이 구분되어있다
    ...
    void SetImage(CTImage* pTop,
        CTImage* pMiddle,
        CTImage* pBottom); /// 드랍박스를 그릴 이미지(3부분)를 Set
...
protected:
    ITFont* m_pFontMgr; /// TControlMgr에서 얻어온 Client가 구현한 ITFont의 Implemented-Class의
                        /// 포인터 임시보관
