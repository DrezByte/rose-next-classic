 ...
 *	- TDialog를 상속 받아서 모든 TDialog의 기능을 가지고 있으며, Ok,Cancel버튼, 3개로 나누어진
 *이미지 부분이 추가되어 있다.
 *
 * @Author	최종진
 *
...
public:
    ...
    enum { BT_NONE = 0x0000, BT_OK = 0x0001, BT_CANCEL = 0x0002 };
...
protected:
    ...
    CTImage* m_pImageMiddle; // 배경이미지중 가운데( 메세지 박스에 표시될 스트링의 길이에 따라서
                             // 자동으로 반복되어 그려진다 )
