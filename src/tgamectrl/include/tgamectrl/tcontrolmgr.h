 ...
 *
 * @Warning			ScrollBar는 생성후 PrevButton,NextButton(MakeButton),ScrollBox(MakeScrollBox)를
 *만들어서 넣어주어야한다
 *
 * @Author			최종진
 *
...
public:
    ...
    CTButton* MakeButton(int ID,
        ...
        int iDownGraphicID,
        int iModuleID);
    CTPushButton* MakePushButton(int ID,
        int iScrX,
        int iScrY,
        int iWidth,
        int iHeight,
    ...
    CTImage*
    MakeImage(int ID, int iScrX, int iScrY, int iWidth, int iHeight, int iGraphicID, int iModuleID);
    CTListBox* MakeListBox(int ID,
        int iScrX,
        int iScrY,
        int iWidth,
        int iHeight,
...
private:
    ...
    std::string GetNodeStringValueFromNamedNodeMapByName(MSXML::IXMLDOMNamedNodeMap* pNodeMap,
        char* name);
