...
ITreeNode*
CTDefaultTreeModel::getChild(ITreeNode* pParent, int& index) {
...
void
CTDefaultTreeModel::getChildCount(ITreeNode* pParent, int& iCount) {
    if (pParent == NULL)
        return;
...
int
CTDefaultTreeModel::getIndexOfChild(CTObject* pParent, CTObject* pChild) {
    return -1;
}

ITreeNode*
CTDefaultTreeModel::getRoot() {
    return m_pRootNode;
}

bool
CTDefaultTreeModel::isLeaf(CTObject* pNode) {
    return false;
}

int
CTDefaultTreeModel::getSelectRow() {
    return m_iSelectRow;
}

void
CTDefaultTreeModel::setSelectRow(int iRow) {
    m_iSelectRow = iRow;
}
