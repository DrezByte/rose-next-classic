...
void
CTAnimation::Update(POINT ptMouse) {
    if (m_curr_frame <= 0)
        return;
...
void
CTAnimation::add_frame(CTFrame* frame) {
    m_frames.push_back(frame);
}

void
CTAnimation::set_loop(bool loop) {
    m_loop = loop;
}

void
CTAnimation::SetPosition(int x, int y) {
...
void
CTAnimation::set_curr_frame(unsigned i) {
