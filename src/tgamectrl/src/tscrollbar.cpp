...
bool
CTScrollBar::Create(int iScrX, int iScrY, int iWidth, int iHeight, int iType) {
    switch (iType) {
...
unsigned int
CTScrollBar::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CTScrollBar::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
void
CTScrollBar::SetPrevButton(CTButton* pButton) {
    m_pPrevButton = pButton;
}

void
CTScrollBar::SetNextButton(CTButton* pButton) {
    m_pNextButton = pButton;
}

void
CTScrollBar::SetScrollBox(CTScrollBox* pThumb) {
    m_pScrollBox = pThumb;
    m_pScrollBox->SetScrollBar(this);
}

void
CTScrollBar::SetOffset(POINT pt) {
    ...
    if (m_pPrevButton) {
        ...
        if (m_pScrollBarType)
            ptNewOffset =
                m_pScrollBarType->GetPrevButtonOffset(GetOffset(), sizeScrollBar, sizePrevButton);
    ...
    if (m_pNextButton) {
        ...
        if (m_pScrollBarType)
            ptNewOffset =
                m_pScrollBarType->GetNextButtonOffset(GetOffset(), sizeScrollBar, sizeNextButton);
    ...
    if (m_pScrollBox) {
        ...
        if (m_pScrollBarType)
            ptNewOffset = m_pScrollBarType->GetScrollBoxOffset(GetOffset(),
                sizeScrollBar,
                sizePrevButton,
                sizeScrollBox);
...
void
CTScrollBar::MoveWindow(POINT pt) {
    ...
    if (m_pScrollBox) {
        ...
        if (m_pScrollModel)
            m_pScrollBox->MoveBoxPosition(m_pScrollModel->GetValue(),
                m_pScrollModel->GetMaximum(),
                m_pScrollModel->GetExtent());
    }
}

RECT
CTScrollBar::GetThumbMoveRange() {
    RECT rc = {0, 0, 0, 0};
    ...
    rc = m_pScrollBarType->GetThumbMoveRange(m_sPosition,
        pt,
        sizeScrollBar,
        sizePrevButton,
        sizeScrollBox);
...
void
CTScrollBar::SetValue(int iValue) {
    if (m_pScrollModel)
        m_pScrollModel->SetValue(iValue);
    if (m_pScrollBox)
        m_pScrollBox->MoveBoxPosition(iValue,
            m_pScrollModel->GetMaximum(),
            m_pScrollModel->GetExtent());
}

void
CTScrollBar::SetModelValue(int i) {
    if (m_pScrollModel != NULL)
        m_pScrollModel->SetValue(i);
}

void
CTScrollBar::SetModel(IScrollModel* pModel) {
...
void
CTScrollBar::MoveHome() {
    SetValue(0);
}

std::vector<CWinCtrl*>
CTScrollBar::GetChildren() {
