...
int
CTIme::Initialize(SIZE sizeScreen) {
    ...
    iRet =
        WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)Underline, 1, szBuf, 3, " ", &bChangedDefaultChar);
    _ASSERT(iRet);
    m_strPrompt = szBuf;
...
int
CTIme::Process(HWND hWnd, unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    switch (uiMsg) {
        case WM_IME_CHAR:
                ...
                case IMN_CHANGECANDIDATE: {
                    if (HIMC hIMC = ImmGetContext(hWnd)) {
                        ...
                        if (dwBufLen > 0) {
                            ...
                            char pszBuffer[16];
                            int iStart = (pCandidateList->dwSelection / pCandidateList->dwPageSize)
                                * pCandidateList->dwPageSize;

                            std::string strItem;
                            int iCount = 1;
                            for (int i = iStart; i < (iStart + pCandidateList->dwPageSize)
                                 && (i < pCandidateList->dwCount);
                                 ++i, ++iCount) {
                            ...
                            m_pCandidateWindow->SetSelection(
                                pCandidateList->dwSelection % pCandidateList->dwPageSize);
...
std::string&
CTIme::GetCompsitionString() {
    return m_strComposition;
}

void
CTIme::SetCandidateWindowPosition(POINT pt) {
...
void
CTIme::ChangeScreenSize(SIZE sizeScreen) {
    m_sizeScreen = sizeScreen;
    switch (CTControlMgr::GetInstance()->GetCodePage()) {
        case 949: //??
...
std::vector<DWORD>&
CTIme::GetCompClauseInfo() {
    return m_CompClauseInfo;
}
std::vector<DWORD>&
CTIme::GetCompReadClauseInfo() {
    return m_CompReadClauseInfo;
}

std::string&
CTIme::GetStr_ATTR_TARGET_CONVERTED() {
    return m_strMark_ATTR_TARGET_CONVERTED;
}
std::string&
CTIme::GetStr_ATTR_CONVERTED() {
    return m_strATTR_CONVERTED;
}
std::string&
CTIme::GetStr_ATTR_INPUT() {
...
bool
CTIme::IsVisibleCandidateWindow() {
    if (m_pCandidateWindow)
