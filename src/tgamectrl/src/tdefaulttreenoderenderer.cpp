...
CWinCtrl*
CTDefaultTreeNodeRenderer::getTreeCellRendererComponent(CTree* tree,
    CTObject* value,
    bool selected,
    bool expanded,
    bool leaf,
...
void
CTDefaultTreeNodeRenderer::Draw(POINT ptDraw) {
...
void
CTDefaultTreeNodeRenderer::setParentImage(CTImage* pImage) {
    m_pParentImage = pImage;
    m_pParentImage->Show();
}

void
CTDefaultTreeNodeRenderer::setLeafImage(CTImage* pImage) {
    m_pLeafImage = pImage;
    m_pLeafImage->Show();
}

void
CTDefaultTreeNodeRenderer::setOpenImage(CTImage* pImage) {
    _ASSERT(pImage);
    if (pImage) {
...
void
CTDefaultTreeNodeRenderer::setCloseImage(CTImage* pImage) {
    _ASSERT(pImage);
    if (pImage) {
        m_sizeImage.cx = pImage->GetWidth();
        m_sizeImage.cy = pImage->GetHeight();
