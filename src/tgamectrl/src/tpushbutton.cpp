...
unsigned int
CTPushButton::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CTPushButton::Update(POINT ptMouse) {
    if (!IsVision() || !IsEnable())
        return;
...
void
CTPushButton::SetText(char* szText) {
    if (szText == NULL)
        m_stText.clear();
    else
        m_stText = szText;
