...
CJListBox::Draw() {
    ...
    CTControlMgr::GetInstance()->GetFontMgr()->SetTransformSprite((float)m_sPosition.x,
        (float)m_sPosition.y);
    ...
    int iCount = 0;
    for (citer = ItemList.begin() + m_iValue - 1; citer != ItemList.end() && iCount < m_iExtent;
         ++citer, ++iCount) {
        pObject = *citer;
...
unsigned int
CJListBox::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
    assert(m_pContainer);
...
/// ---------------------------------------------------------------------------
bool
CJListBox::Create(int iScrX, int iScrY, int iWidth, int iHeight, int iItemHeight) {
...
void
CJListBox::SetCellRenderer(IListCellRenderer* pRenderer) {
    assert(pRenderer);
    m_pListCellRenderer = pRenderer;
}

void
CJListBox::SetContainer(CTContainer* pContainer) {
    assert(pContainer);
...
CWinCtrl*
CJListBox::GetRendererComponent(const CTObject* pObject, bool bSelect, bool bMouseOver) {
    assert(m_pListCellRenderer);
    return m_pListCellRenderer->GetListCellRendererComponent(pObject, bSelect, bMouseOver);
}

void
CJListBox::SelectItem(unsigned subscript) {
    assert(m_pContainer);
...
void
CJListBox::AddItem(CTObject* pObject) {
    assert(m_pContainer);
    assert(pObject);
...
unsigned
CJListBox::GetSelectedItemIndex() {
    assert(m_pContainer);
    return m_pContainer->GetSelectedItemIndex();
}

void
CJListBox::SetCellHeight(int iHeight) {
...
int
CJListBox::GetMaximum() {
    _ASSERT(m_pContainer);
    return m_pContainer->GetSize();
}

int
CJListBox::GetMinimum() {
    return 0;
}

void
CJListBox::SetValue(int iValue) {
    m_iValue = iValue + 1;
}

void
CJListBox::SetExtent(int iExtent) {
    m_iExtent = iExtent;
}

void
CJListBox::SetMaximum(int iMaximum) {}

void
CJListBox::SetMinimum(int iMinimum) {}

RECT
CJListBox::GetWindowRect() {
