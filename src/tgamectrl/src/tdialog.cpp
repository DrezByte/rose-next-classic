...
bool
CTDialog::Create(int iScrX, int iScrY, int iWidth, int iHeight) {
...
bool
CTDialog::Create(const char* IDD) {
    return CTControlMgr::GetInstance()->MakeDialogByXML(IDD, this);
}

void
CTDialog::ChangeCaptionRect(RECT rc) {
    if (m_pCaption) {
...
unsigned int
CTDialog::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
            ...
            if (uiMsg == WM_LBUTTONDOWN) {
                if (pCtrl->GetControlType() != CTRL_TABBEDPANE
                    && pCtrl->GetControlType()
                        != CTRL_PANE) ///컨테이너는 기본적으로 위로 올라오면 안되요..
    ...
    if (IsInside(ptMouse.x, ptMouse.y) && uiMsg != WM_KEYDOWN && uiMsg != WM_CHAR
        && uiMsg != WM_SYSKEYDOWN) {
        if (uiMsg == WM_LBUTTONDOWN)
            ProcessLButtonDown();
...
void
CTDialog::SetStatusBar(CTStatusBar* pStatusBar) {
    m_pStatusBar = pStatusBar;
}

void
CTDialog::SetCaption(CTCaption* pCaption) {
...
CWinCtrl*
CTDialog::Find(const char* strName) {
    WINCTRL_LIST_ITOR iter;
    CWinCtrl* pCtrl = NULL;
    for (iter = m_listChild.begin(); iter != m_listChild.end(); ++iter) {
        pCtrl = *iter;
...
bool
CTDialog::ShowChild(unsigned int iID) {
    WINCTRL_LIST_ITOR iter;
    CWinCtrl* pCtrl = NULL;
    for (iter = m_listChild.begin(); iter != m_listChild.end(); ++iter) {
        pCtrl = *iter;
...
bool
CTDialog::ShowChild(const char* strName) {
    WINCTRL_LIST_ITOR iter;
    CWinCtrl* pCtrl = NULL;
    for (iter = m_listChild.begin(); iter != m_listChild.end(); ++iter) {
        pCtrl = *iter;
...
bool
CTDialog::HideChild(unsigned int iID) {
    WINCTRL_LIST_ITOR iter;
    CWinCtrl* pCtrl = NULL;
    for (iter = m_listChild.begin(); iter != m_listChild.end(); ++iter) {
        pCtrl = *iter;
...
bool
CTDialog::HideChild(const char* strName) {
    WINCTRL_LIST_ITOR iter;
    CWinCtrl* pCtrl = NULL;
    for (iter = m_listChild.begin(); iter != m_listChild.end(); ++iter) {
        pCtrl = *iter;
...
bool
CTDialog::SetEnableChild(unsigned int iID, bool bEnable) {
...
void
CTDialog::MoveWindow(int x, int y) {
    POINT pt = {x, y};
    MoveWindow(pt);
}

long
CTDialog::GetChildCount() {
    return (long)m_listChild.size();
}

void
CTDialog::Push_Back_Cmd(CTCommand* pCmd) {
    m_CommandQ.Push_Back(pCmd);
}

void
CTDialog::Push_Front_Cmd(CTCommand* pCmd) {
    m_CommandQ.Push_Front(pCmd);
}

void
CTDialog::ClearCommandQ() {
    m_CommandQ.Clear();
}
CTCommand*
CTDialog::GetTCommand() {
    return m_CommandQ.GetCommand();
}

void
CTDialog::MoveCtrl2ListEnd(int iID) {
    WINCTRL_LIST_ITOR iter;
    CWinCtrl* pCtrl = NULL;
    for (iter = m_listChild.begin(); iter != m_listChild.end(); ++iter) {
        /// REMARK : For Debuging
...
void
CTDialog::SetExtent(int i) {
    m_iExtent = i;
}
void
CTDialog::SetImage(CTImage* pImage) {
    m_pImage = pImage;
}
