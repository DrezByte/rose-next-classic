...
BOOL
CImeView::IsProcessEvent() {
    if (m_nState & IME_IN_CHOSECAND || (m_nState & IME_IN_COMPOSITION && GetCompCursorPos())) {
        return FALSE;
    } else
        return TRUE;
}
...
void
CImeView::SetCompWndPos(POINT& pt) {
...
BOOL
CImeView::GetCompString(LONG flag) {
...
LONG
CImeView::OnInputLangChange(WPARAM dwCommand, LPARAM dwData) {
...
LONG
CImeView::OnIMESetContext(WPARAM dwCommand, LPARAM dwData) {
    if (m_property & IME_PROP_AT_CARET) {
...
LONG
CImeView::OnIMEStartComposition(WPARAM dwCommand, LPARAM dwData) {
...
LONG
CImeView::OnIMEComposition(WPARAM dwCommand, LPARAM dwData) {
    if (Check()) { // !IME_PROP_SPECIAL_UI && IME_PROP_AT_CARET
...
LONG
CImeView::OnIMEEndComposition(WPARAM dwCommand, LPARAM dwData) {
    if (Check()) { // !IME_PROP_SPECIAL_UI && IME_PROP_AT_CARET
        m_nCompLen = 0;
        m_nState &= ~IME_IN_COMPOSITION;
...
LONG
CImeView::OnIMECompositionFull(WPARAM dwCommand, LPARAM dwData) {
...
LONG
CImeView::OnIMENotify(WPARAM dwCommand, LPARAM dwData) {
...
LONG
CImeView::OnIMEControl(WPARAM dwCommand, LPARAM dwData) {
...
// IMN_OPENCANDIDATE:
BOOL
CImeView::OpenCandidate(LONG lParam) {
    ...
    for (int index = 0; index < MAX_LISTCAND; index++) {
        if (lParam & (1 << index)) {
            ...
            dwPreferNumPerPage =
                (!lpCandList->dwPageSize) ? DEFAULT_CAND_NUM_PER_PAGE : lpCandList->dwPageSize;
            ...
            m_hwndCand[index] = CreateWindow("HanjaCandidate",
                "CandWindow",
...
// IMN_CLOSECANDIDATE:
BOOL
CImeView::CloseCandidate(LONG CandList) {
    for (int i = 0; i < MAX_LISTCAND; i++) {
...
// IMN_CHANGECANDIDATE:
BOOL
CImeView::ChangeCandidate(LONG CandList) {
    ...
    ::SetWindowPos(m_hwndCand[dwIndex],
        m_hWnd,
        rect.left,
        rect.top,
        (max_width + 3) * m_charWidth + 4,
...
BOOL
CImeView::DisplayCandStrings(HWND hwnd, LPCANDIDATELIST lpCandList) {
    ...
    // Draw Selection
    rect.top = rect.top + (lpCandList->dwSelection - dwStartIndex) * m_charHeight;
...
void
CImeView::ProcessResultString(LPSTR str) {
    //	PutString(str);
}

void
CImeView::ProcessCompString(LPSTR str, LPSTR strAttr) {
...
void
CImeView::IMEStartComposition(LONG dwData) {}

void
CImeView::IMEEndComposition(LONG dwData) {}

/////////////////////////////////////////////////////////////////////////////
// CImeView message handlers

bool
CImeView::IsYouMessage(UINT msg, WPARAM wparam, LPARAM lparam) {
    if (!m_hWnd)
        return FALSE;
    switch (msg) {

        case WM_IME_STARTCOMPOSITION: // 글자 조합의 시작. WM_IME_COMPOSITION 메시지를           //
                                      // 받을 준비를 한다.
...
void
CImeView::Darw(HDC dc, POINT* ppo) {
...
BOOL
CImeView::ChkKorean2(LPSTR str, int nCnt) {
    if (str[nCnt] & 0x80)
        return TRUE;
    return FALSE;
}
