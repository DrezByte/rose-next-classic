...
unsigned
CTPane::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CTPane::AddChild(CWinCtrl* pCtrl) {
    m_Children.Add(pCtrl);
}

CWinCtrl*
CTPane::FindChild(unsigned iID) {
    return m_Children.Find(iID);
}

CWinCtrl*
CTPane::Find(const char* szName) {
