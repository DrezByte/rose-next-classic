...
void
CJContainer::Update(POINT ptMouse) {
    for_each(m_Children.begin(), m_Children.end(), bind2nd(mem_fun(&CWinCtrl::Update), ptMouse));
}

unsigned
CJContainer::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    unsigned uiProcID = 0;
...
void
CJContainer::MoveCtrl2ListEnd(int iID) {
    std::list<CWinCtrl*>::iterator iter;
    for (iter = m_Children.begin(); iter != m_Children.end(); ++iter) {
...
void
CJContainer::MoveWindow(POINT pt) {
    std::list<CWinCtrl*>::iterator iter;
    for (iter = m_Children.begin(); iter != m_Children.end(); ++iter)
        (*iter)->MoveWindow(pt);
}
...
CWinCtrl*
CJContainer::Find(unsigned index) {
    std::list<CWinCtrl*>::iterator iter;
    //	CWinCtrl*	pCtrl = NULL;
    for (iter = m_Children.begin(); iter != m_Children.end(); ++iter) {
...
CWinCtrl*
CJContainer::Find(const char* szName) {
    std::list<CWinCtrl*>::iterator iter;
...
void
CJContainer::SetChildEnable(unsigned iID, bool bEnable) {
...
void
CJContainer::Add(CWinCtrl* pCtrl) {
    _ASSERT(pCtrl);
    m_Children.push_back(pCtrl);
}

void
CJContainer::Remove(unsigned index) {
    std::list<CWinCtrl*>::iterator iter;
    CWinCtrl* pCtrl = NULL;
    for (iter = m_Children.begin(); iter != m_Children.end(); ++iter) {
        if ((*iter)->GetControlID() == index) {
