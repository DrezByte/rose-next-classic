...
unsigned int
CJComboBox::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
...
void
CJComboBox::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
CJListBox*
CJComboBox::GetListBoxPtr() {
    return m_pListBox;
}

bool
CJComboBox::Create(int iScrX, int iScrY, int iWidth, int iHeight) {
...
void
CJComboBox::SetButton(CTButton* pButton) {
    m_pDropButton = pButton;
}

void
CJComboBox::SetImage(CTImage* pTop, CTImage* pMiddle, CTImage* pBottom) {
...
void
CJComboBox::SetListBox(CJListBox* pListBox) {
    m_pListBox = pListBox;
}

void
CJComboBox::MoveWindow(POINT pt) {
...
void
CJComboBox::AddItem(CTObject* pItem) {
    assert(m_pListBox);
    assert(pItem);
...
unsigned
CJComboBox::GetSelectedItemIndex() {
    assert(m_pListBox);
    if (m_pListBox)
...
void
CJComboBox::SelectItem(unsigned iSubscript) {
    assert(m_pListBox);
    if (m_pListBox)
