...
bool
CTImage::Create(int iScrX, int iScrY, int iWidth, int iHeight, int iGraphicID, int iModuleID) {
...
unsigned int
CTImage::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    return 0;
}
...
CTImage::Draw() {
    ...
    if (m_pDraw) {
        ...
        if (GetSizeFit() && m_btAlphaValue == 0) {
            m_pDraw->DrawFit((int)m_sPosition.x,
                (int)m_sPosition.y,
                module_id,
                graphic_id,
                GetWidth(),
...
void
CTImage::SetImage(int iGraphicID, int iModuleID) {
    m_iModuleID = iModuleID;
    m_iGraphicID = iGraphicID;
}

void
CTImage::SetBlinkImage(int iGraphicID, int iModuleID) {
...
void
CTImage::SetBlinkSwapTime(DWORD swap_time) {
