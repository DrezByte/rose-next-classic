...
bool
CTRadioButton::Create(int iScrX,
    int iScrY,
    int iWidth,
    int iHeight,
...
unsigned int
CTRadioButton::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (m_bGhost)
        return 0;
    if (!IsVision())
        return 0;
...
void
CTRadioButton::Update(POINT ptMouse) {
    if (m_bGhost)
        return;
    if (!IsVision())
        return;
...
void
CTRadioButton::SetText(char* szText) {
    if (m_bGhost)
        return;
...
void
CTRadioButton::SetButtonState(int iState) {
    if (m_bGhost)
        return;
...
CSinglelineString&
CTRadioButton::GetText() {
    return m_text;
}

void
CTRadioButton::SetSoundDisableID(int i) {
    m_iDisableSoundID = i;
}

void
CTRadioButton::SetGhost() {
    m_bGhost = true;
}