...
void
CSinglelineString::set_string(const char* msg, const RECT& rect, int ifont) {
    _ASSERT(msg);
    if (msg == NULL)
        return;
...
void
CSinglelineString::set_rect(const RECT& rect) {
    m_rect = rect;
    ...
    if (m_end_ellipsis != end_ellipsis) {
        ...
        if (m_end_ellipsis) {
            ...
            while (curr != next) {
                ...
                if (size_temp_string.cx + size_ellipsis_msg.cx + size_ellipsis.cx
...
CSinglelineString::draw() {
    if (m_end_ellipsis && !m_ellipsis_msg.empty())
        CTControlMgr::GetInstance()->GetFontMgr()->Draw(m_font,
            true,
            &m_rect,
            m_color,
            0,
            m_ellipsis_msg.c_str());
    else
        CTControlMgr::GetInstance()->GetFontMgr()->Draw(m_font,
            true,
            &m_rect,
...
bool
CSinglelineString::is_ellipsis() {
    return m_end_ellipsis;
}

const char*
CSinglelineString::get_string() {
    return m_msg.c_str();
}

void
CSinglelineString::set_color(DWORD color) {
    m_color = color;
}

bool
CSinglelineString::empty() {
    return m_msg.empty();
}

void
CSinglelineString::clear() {
    m_msg.clear();
    m_ellipsis_msg.clear();
...
void
CSinglelineString::set_format(DWORD format) {
    m_format = format;
}