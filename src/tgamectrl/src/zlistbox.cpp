...
int
CZListBox::GetMaximum() {
    return m_Items.size();
}

int
CZListBox::GetMinimum() {
    return 0;
}

void
CZListBox::SetValue(int iValue) {
    if ((int)m_Items.size() >= iValue) {
        ...
        for (iter = m_Items.begin() + m_iValue; iter != m_Items.end() && iCount < m_iExtent;
             ++iCount, ++iter) {
...
void
CZListBox::SetExtent(int iExtent) {
    m_iExtent = iExtent;
}

void
CZListBox::SetMaximum(int iMaximum) {}
void
CZListBox::SetMinimum(int iMinimium) {}

RECT
CZListBox::GetWindowRect() {
...
void
CZListBox::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
unsigned
CZListBox::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CZListBox::SetSelected(int iIndex) {
...
int
CZListBox::GetSelectedItemIndex() {
    std::deque<CWinCtrl*>::iterator iter;
    int iIndex = 0;
...
bool
CZListBox::DelItemByControlID(int iID) {
    std::deque<CWinCtrl*>::iterator iter;
    std::deque<CWinCtrl*> tempContainer;
    int iCount = 0;
    for (iter = m_Items.begin(); iter != m_Items.end(); ++iter, ++iCount) {
...
void
CZListBox::InsertItem(int iIndex, CWinCtrl* pCtrl) {
    _ASSERT(pCtrl && iIndex >= 0 && iIndex < (int)m_Items.size());
    if (pCtrl == NULL || iIndex < 0 || iIndex >= (int)m_Items.size())
        return;
