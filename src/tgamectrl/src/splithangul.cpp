...
bool
CSplitHangul::DoSplit(char* pStr, int iLength, unsigned uiCodePage) {
    if (pStr == NULL)
        return false;
...
// 한글을 주어진 크기로 자른다..
bool
CSplitHangul::SplitHangul(char* pStr, int iLength, unsigned uiCodePage) {
    if (pStr == NULL)
        return false;
...
int
CSplitHangul::GetLineCount() {
    return (int)m_StringList.size();
}

const char*
CSplitHangul::GetString(int iIndex) {
