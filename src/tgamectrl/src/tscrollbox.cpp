...
bool
CTScrollBox::Create(int iWidth, int iHeight, int iGraphicID, int iModuleID, int iType) {
...
unsigned int
CTScrollBox::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CTScrollBox::Update(POINT ptMouse) {
    if (m_pBlinkImage)
...
void
CTScrollBox::MoveBoxPosition(int iValue, int iMax, int iExtent) {
...
void
CTScrollBox::MoveBoxPositionByScreenPoint(POINT pt) {
    ...
    POINT ptNew = m_pScrollBoxType->GetMoveBoxPosByScreenPoint(sizeScrollBox,
        rcMoveRange,
        pt,
        iMax,
        iExtent,
...
void
CTScrollBox::SetBlink() {
    m_bBlink = true;
    if (m_pBlinkImage)
        m_pBlinkImage->SetBlink();
}
void
CTScrollBox::SetUnblink() {
    m_bBlink = false;
    if (m_pBlinkImage)
        m_pBlinkImage->SetUnblink();
}
void
CTScrollBox::SetBlinkImage(CTImage* pImage) {
    m_pBlinkImage = pImage;
}

std::vector<CWinCtrl*>
CTScrollBox::GetChildren() {
    if (m_pBlinkImage) {
