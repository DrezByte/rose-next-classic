...
unsigned
CJTable::Process(unsigned uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
    unsigned uiProcID;
    std::vector<CWinCtrl*>::iterator iter;
...
CJTable::SetValue(int iValue) {
    ...
    std::vector<CWinCtrl*>::iterator iter;
    POINT ptOffset = {0, 0};
    for (iter = m_Items.begin() + m_iValue * m_iColumnCount;
         iter != m_Items.end() && iCount < iMaxView;
         ++iter, ++iCount) {
...
void
CJTable::SetExtent(int iExtent) {
    m_iExtent = iExtent;
}

void
CJTable::SetMaximum(int iMaximum) {}

void
CJTable::SetMinimum(int iMinimum) {}

RECT
CJTable::GetWindowRect() {
    RECT rc = {m_sPosition.x, m_sPosition.y, m_sPosition.x + m_iWidth, m_sPosition.y + m_iHeight};
    return rc;
}

void
CJTable::SetColumnCount(int iCount) {
    _ASSERT(iCount > 0);
    if (iCount > 0) {
...
void
CJTable::SetCellWidth(int iWidth) {
    m_iCellWidth = iWidth;
}

void
CJTable::SetCellHeight(int iHeight) {
    m_iCellHeight = iHeight;
}

void
CJTable::SetRowMargin(int iMargin) {
    m_iRowMargin = iMargin;
}

void
CJTable::SetColMargin(int iMargin) {
    m_iColMargin = iMargin;
}

void
CJTable::Add(CWinCtrl* pCtrl) {
    m_Items.push_back(pCtrl);
    SetValue((int)m_Items.size() - 1);
}

void
CJTable::MoveWindow(POINT pt) {
