...
CTComboBox::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (m_pDropButton && m_pDropButton->Process(uiMsg, wParam, lParam)) {
        ...
        // if( uiMsg == WM_LBUTTONDOWN )///다른 열려있는 콤보들을 닫기위하여 Dialog내에서 메세지
        // 전달 	return 0;
...
void
CTComboBox::Update(POINT ptMouse) {
    if (!IsVision())
        return;
...
bool
CTComboBox::Create(int iScrX, int iScrY, int iWidth, int iHeight) {
...
void
CTComboBox::SetButton(CTButton* pButton) {
    m_pDropButton = pButton;
}

void
CTComboBox::SetImage(CTImage* pTop, CTImage* pMiddle, CTImage* pBottom) {
...
void
CTComboBox::SetListBox(CTListBox* pListBox) {
    m_pListBox = pListBox;
}
void
CTComboBox::MoveWindow(POINT pt) {
...
void
CTComboBox::AddItem(char* szItem) {
    if (m_pListBox) {
...
const char*
CTComboBox::GetSelectedItem() {
    if (m_pListBox)
        return m_pListBox->GetSelectedItem();
    return NULL;
}
void
CTComboBox::SetSelectedItem(short i) {
    if (m_pListBox)
        m_pListBox->SetSelectedItem(i);
}
bool
CTComboBox::IsShowDropBox() {
...
void
CTComboBox::ClearItem() {
    m_pListBox->ClearText();
}

std::vector<CWinCtrl*>
CTComboBox::GetChildren() {
