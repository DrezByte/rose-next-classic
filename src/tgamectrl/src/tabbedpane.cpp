...
unsigned int
CTabbedPane::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (m_Tabs.empty())
        return 0;
    if (!IsVision())
        return 0;
...
void
CTabbedPane::MoveWindow(POINT pt) {
...
void
CTabbedPane::Update(POINT ptMouse) {
    if (m_Tabs.empty())
        return;
    if (!IsVision())
        return;
...
unsigned
CTabbedPane::AddTab(CTRadioButton* pButton, CJContainer* pContainer) {
    _ASSERT(pButton);
    _ASSERT(pContainer);
...
CJContainer*
CTabbedPane::GetTabContainer(int iIndex) {
    std::list<S_TabbedPane>::iterator iter;
    for (iter = m_Tabs.begin(); iter != m_Tabs.end(); ++iter) {
...
bool
CTabbedPane::SetSelectedIndex(unsigned index) {
...
void
CTabbedPane::SetMinimize(bool bMinimize) {
...
void
CTabbedPane::SetMinimizeRect(const RECT& rc) {
    m_rcMinimize = rc;
}

bool
CTabbedPane::IsMinimize() {
    return m_bMinimize;
}
...
void
CTabbedPane::SetMinimizeButton(CTButton* pButton) {
