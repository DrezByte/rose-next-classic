...
void
CToolTip::DrawTopImage(POINT pt) {
    if (m_pImageTop) {
...
void
CToolTip::DrawText(int iFont, POINT pt, D3DCOLOR color, const char* szText) {
    if (m_pImageMiddle) {
...
void
CToolTip::DrawBottomImage(POINT pt) {
    if (m_pImageBottom) {
...
void
CToolTip::SetImageTop(CTImage* pImage) {
    m_pImageTop = pImage;
    pImage->Show();
}
void
CToolTip::SetImageMiddle(CTImage* pImage) {
    m_pImageMiddle = pImage;
    pImage->Show();
}
void
CToolTip::SetImageBottom(CTImage* pImage) {
    m_pImageBottom = pImage;
    pImage->Show();
}

void
CToolTip::SetText(char* szText) {
    if (szText == NULL) {
        m_stMsg.clear();
        m_splitMsg.Clear();
    } else {
