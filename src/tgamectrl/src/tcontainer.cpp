...
void
CTContainer::AddItem(CTObject* item) {
    if (item == NULL) {
...
const CTObject*
CTContainer::GetItem(unsigned iSubscript) {
...
const CTObject*
CTContainer::GetSelectedItem() {
    if (m_uSelectedItemIndex <= 0 || m_uSelectedItemIndex > m_listItem.size()) {
        _RPT1(_CRT_WARN,
            "Invalid SelectedItemIndex(%d) @TContainer::GetSelectedItem() \n",
...
unsigned
CTContainer::GetSelectedItemIndex() {
    if (m_uSelectedItemIndex > m_listItem.size()) {
        _RPT0(_CRT_WARN,
            "Invalid SelectedItemIndex ^ SelectedItemIndex = 0 @TContainer::GetSelectedItemIndex "
            "\n");
        return 0;
    }
...
void
CTContainer::SelectItem(unsigned iSubscript) {
    if (iSubscript <= 0 || iSubscript > m_listItem.size()) {
