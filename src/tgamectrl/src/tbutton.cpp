...
unsigned int
CTButton::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision() || !IsEnable())
        return 0;
...
void
CTButton::SetBlink(int iBlink, int iBlinkTime) {
...
void
CTButton::SetBlinkGid(int iGid) {
    m_iBlinkGraphicID = iGid;
}

void
CTButton::SetTextColor(DWORD color) {
    m_textcolor = color;
}
