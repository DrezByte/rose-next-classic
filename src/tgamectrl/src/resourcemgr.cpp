...
int
CResourceMgr::GetImageNID(int iModuleID, const char* szSID) {
...
bool
CResourceMgr::AddResource(char* szFileName, int iModuleID) {
    TABLE_MODULE_ITER iterModule;
    iterModule = m_ResourceTable.find(iModuleID);
    if (iterModule != m_ResourceTable.end()) {
        char Buff[256];
...
void
CResourceMgr::UnLoadResource(int iModuleID) {
    TABLE_MODULE_ITER iterModule;
    iterModule = m_ResourceTable.find(iModuleID);
    if (iterModule == m_ResourceTable.end()) {
        //		char Buff[256];
...
void
CResourceMgr::LoadResourceFile(char* szFileName, int iModuleID) {
