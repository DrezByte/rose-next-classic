...
int
CImageIDMgr::GetImageNID(int iModuleID, const char* szSID) {
...
bool
CImageIDMgr::AddResource(char* szFileName, int iModuleID) {
    TABLE_MODULE_ITER iterModule;
    iterModule = m_ResourceTable.find(iModuleID);
    if (iterModule != m_ResourceTable.end()) {
        char Buff[256];
...
void
CImageIDMgr::LoadResourceFile(char* szFileName, int iModuleID) {
