...
unsigned int
CTMsgBox::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
...
void
CTMsgBox::SetString(const char* szString) {
...
void
CTMsgBox::SetButtons(CTButton* pBtnOk, CTButton* pBtnCancel) {
    m_pButtonOk = pBtnOk;
...
void
CTMsgBox::SetImages(CTImage* pImageTop, CTImage* pImageMiddle, CTImage* pImageBottom) {
...
bool
CTMsgBox::Create(char* szIDD) {
    return CTControlMgr::GetInstance()->MakeMsgBoxByXML(szIDD, this);
}

void
CTMsgBox::AddExtraHeight(int iHeight) {
    _ASSERT(m_iHeight == 0);
...
int
CTMsgBox::GetExtraHeightStartPoint() {
