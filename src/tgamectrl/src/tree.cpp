...
void
CTree::reset() {
    m_bRootVisible = true; /// True if the root node is displayed, false if its children are the
                           /// highest visible nodes.
...
*/
void
CTree::SetTreeModel(ITreeModel* pTreeModel) {
    reset();
    m_pTreeModel = pTreeModel;
}

long
CTree::DrawTree(ITreeNode* pNode, POINT ptDraw, int& iSelectRow, int& iExtent, int& iStart) {
    ...
    if (!pNode->isLeaf() && pNode->isOpen()) {
        for (int i = 0; i < pNode->getChildCount(); ++i)
            ptChildDraw.y =
                DrawTree(pNode->getChildAt(i), ptChildDraw, iSelectRow, iExtent, iStart);
    }
...
void
CTree::DrawNode(ITreeNode* pNode, POINT ptDraw, bool bSelect) {
    if (pNode == NULL)
        return;

    CWinCtrl* pCtrl;
    pCtrl = m_pTreeNodeRenderer->getTreeCellRendererComponent(this,
        pNode->getUserObject(),
        bSelect,
        pNode->isOpen(),
        pNode->isLeaf(),
...
void
CTree::SetNodeRenderer(ITreeNodeRenderer* pRenderer) {
    m_pTreeNodeRenderer = pRenderer;
}
///*----------------------------------------------------------------------------------------
/// CWinCtrl
unsigned int
CTree::Process(UINT uiMsg, WPARAM wParam, LPARAM lParam) {
    if (!IsVision())
        return 0;
    ...
    switch (uiMsg) {
        case WM_LBUTTONDOWN: {
            for (; rc.bottom < m_iHeight;
                 rc.top += m_iRowHeight, rc.bottom += m_iRowHeight, ++iRow) {
...
void
CTree::OnLButtonDblClk(int iRow) {
    ITreeNode* pNode = NULL;
    ITreeNode* pRootNode = m_pTreeModel->getRoot();
    int iSelectRow = iRow + m_iValue;
...
void
CTree::SetExtent(int i) {
    m_iExtent = i;
}

void
CTree::SetMaximum(int i) {
    m_iMaximum = i;
}

void
CTree::SetMinimum(int i) {
    m_iMinimum = i;
}
